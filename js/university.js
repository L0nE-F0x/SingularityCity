            ...
            if (g.phase === 'walking' && elapsed > 120) {
                ...
                for (let c = 0; c < 40; c++) {
                    this._confetti.push({
                        ...
                        vy: -2 - Math.random() * 3,
                        color: [0xff4444, 0x44ff44, 0x4488ff, 0xffdd44, 0xff44ff, 0x44ffff][
...
const UniversityEnv = {
    ...
    buildBuilding(g, bld, h) {
        ...
        if (bld.id === 'uni_main') {
            ...
            g.moveTo(bw / 2, -27);
            g.lineTo(bw / 2 + Math.cos(hrAngle) * 4, -27 + Math.sin(hrAngle) * 4);
            g.moveTo(bw / 2, -27);
            g.lineTo(bw / 2 + Math.cos(mnAngle) * 5.5, -27 + Math.sin(mnAngle) * 5.5);
            g.lineStyle(0);
            // Arched entrance
...
const UniversityInterior = {
    ...
    build(bld, layer) {
        ...
        const badgeMap = {
            uni_main: 'AI ACADEMY',
            uni_library: 'DATA LIBRARY',
            uni_dorm: 'MODEL DORMITORY',
            uni_lab: 'RESEARCH LAB',
        };
        ...
        const bW = 220,
            bH = 34,
            bX = startX + usableW / 2 - bW / 2 + 10,
            bY = roofH - bH - 10;
        const sg = new PIXI.Graphics();
        sg.beginFill(0x111111);
        sg.lineStyle(2, accentCol, 0.8);
        sg.drawRect(bX, bY, bW, bH);
        sg.endFill();
        sg.lineStyle(0);
        sg.beginFill(0x333333);
        sg.drawRect(bX + 15, bY + bH, 6, 10);
        sg.drawRect(bX + bW - 21, bY + bH, 6, 10);
        sg.endFill();
        rc.addChild(sg);
        const lt = new PIXI.Text(bld.name.toUpperCase(), {
        ...
        lt.x = bX + bW / 2;
        lt.y = bY + bH / 2;
        if (lt.width > bW - 8) lt.scale.set((bW - 8) / lt.width);
        rc.addChild(lt);
        const bdg = new PIXI.Text(badgeMap[bld.id] || 'UNIVERSITY', {
            fontFamily: 'JetBrains Mono',
            fontSize: 8,
            fill: 0x94a3b8,
            letterSpacing: 2,
        });
        bdg.anchor.set(0.5, 0.5);
        bdg.x = bX + bW / 2;
        bdg.y = bY - 8;
        rc.addChild(bdg);
        const rl = new PIXI.Graphics();
        rl.beginFill(accentCol, 0.3);
        rl.drawRect(startX, roofH - 4, usableW + shaftW + 20, 4);
        rl.endFill();
        rl.beginFill(accentCol, 0.1);
        rl.drawRect(startX, roofH - 8, usableW + shaftW + 20, 4);
        rl.endFill();
        rc.addChild(rl);
        this.scene.addChild(rc);

        // ─── FLOORS ───
        const windowX = startX + 50,
            windowW = usableW - 100;
        const floorConts = {};
        for (let f = -1; f < numFloors; f++) {
            ...
            rg.drawRect(startX - 8, fy, 8, floorH);
            rg.endFill();
            // Right wall (before shaft)
            rg.beginFill(0x0a0e14);
            rg.drawRect(shaftX - 2, fy, 8, floorH);
            rg.endFill();
            // Shaft enclosure (solid wall behind elevator — keeps it inside the building)
            rg.beginFill(0x0a0e14);
            rg.drawRect(shaftX + 6, fy, shaftW - 6, floorH);
            rg.endFill();

            if (!isB) {
                // Above-ground: draw wall with window holes
                rg.beginFill(0x0a0e14);
                rg.drawRect(startX, fy, usableW, 22);
                rg.endFill();
                rg.beginFill(0x0a0e14);
                rg.drawRect(startX, fy + 54, usableW, floorH - 54);
                rg.endFill();
                let wx = windowX;
                rg.beginFill(0x0a0e14);
                ...
                while (wx + 40 <= windowX + windowW) {
                    wx += 40;
                    if (wx + 20 <= windowX + windowW) {
                        rg.drawRect(wx, fy + 22, 20, 32);
                    }
                    wx += 20;
                }
                ...
                while (cwx + 40 <= windowX + windowW) {
                    ...
                    wfr.moveTo(cwx + 20, fy + 22);
                    wfr.lineTo(cwx + 20, fy + 54);
                    wfr.moveTo(cwx, fy + 38);
                    wfr.lineTo(cwx + 40, fy + 38);
                    wfr.lineStyle(0);
                    cwx += 60;
                }
                ...
                rg.beginFill(0x0a0e14);
                rg.drawRect(startX, fy, usableW, floorH);
                rg.endFill();
            }

            // Floor surface
            rg.beginFill(0x060a10);
            rg.drawRect(startX, fy + floorH - 8, usableW, 8);
            rg.endFill();
            // Ceiling trim
            rg.beginFill(0x111822);
            rg.drawRect(startX, fy, usableW, 4);
            rg.endFill();
            // Floor separator line
            rg.beginFill(0x2a2a42);
            rg.drawRect(startX - 8, fy + floorH - 4, usableW + shaftW + 28, 4);
            rg.endFill();
            ...
            for (let li = 1; li <= 4; li++) {
                ...
                const lg2 = new PIXI.Graphics();
                lg2.beginFill(lc, 0.4);
                lg2.drawRect(lx - 10, fy, 20, 2);
                lg2.endFill();
                const bm = new PIXI.Graphics();
                bm.beginFill(lc, 0.04);
                bm.moveTo(lx - 10, fy + 2);
                bm.lineTo(lx + 10, fy + 2);
                bm.lineTo(lx + 30, fy + floorH - 8);
                bm.lineTo(lx - 30, fy + floorH - 8);
                bm.closePath();
                bm.endFill();
            ...
            dr.beginFill(0x1a2030);
            dr.lineStyle(1, 0x111822);
            dr.drawRect(shaftX + 15, fy + floorH - 44, 30, 40);
            dr.moveTo(shaftX + 30, fy + floorH - 44);
            dr.lineTo(shaftX + 30, fy + floorH - 4);
            dr.endFill();
            dr.beginFill(0x111822);
            dr.drawRect(shaftX + 5, fy + floorH - 25, 4, 8);
            dr.beginFill(isB ? 0xef4444 : 0x4ade80);
            dr.drawCircle(shaftX + 7, fy + floorH - 23, 1.5);
            dr.endFill();
        ...
        earth.beginFill(0x2a2218);
        earth.drawRect(0, groundY, startX - 8, floorH);
        earth.endFill();
        earth.beginFill(0x3a3020);
        earth.drawRect(0, groundY, startX - 8, 6);
        earth.endFill();
        earth.beginFill(0x2a2218);
        earth.drawRect(shaftX + shaftW, groundY, G.vpW - shaftX - shaftW, floorH);
        earth.endFill();
        earth.beginFill(0x3a3020);
        earth.drawRect(shaftX + shaftW, groundY, G.vpW - shaftX - shaftW, 6);
        earth.endFill();
        earth.beginFill(0x4a4a5a);
        earth.drawRect(0, groundY - 2, startX - 8, 6);
        earth.endFill();
        earth.beginFill(0x4a4a5a);
        earth.drawRect(shaftX + shaftW, groundY - 2, G.vpW - shaftX - shaftW, 6);
        earth.endFill();
        earth.beginFill(0x2d5a3f);
        earth.drawRect(0, groundY - 4, startX - 8, 4);
        earth.endFill();
        earth.beginFill(0x2d5a3f);
        earth.drawRect(shaftX + shaftW, groundY - 4, G.vpW - shaftX - shaftW, 4);
        earth.endFill();
        ...
        if (G.models && G.charRefs) {
            const students = G.models.filter((m) => {
            ...
            const dormBedXs = [];
            if (bld.id === 'uni_dorm') {
                for (let bx = startX + 30; bx < startX + usableW - 80; bx += 90) dormBedXs.push(bx + 16);
            }
            const floors = Object.keys(floorConts)
                .map(Number)
                .sort((a, b) => b - a);
            if (floors.length > 0 && students.length > 0) {
                ...
                students.forEach((m, idx) => {
                    ...
                    if (isNight && bld.id === 'uni_dorm' && fi >= 2 && dormBedXs.length > 0) {
                        ...
                        const sg = new PIXI.Graphics();
                        sg.beginFill(0xddd8c8, 0.7);
                        sg.drawRoundedRect(bedX - 11, bt - 8, 12, 6, 3);
                        sg.endFill();
                        sg.beginFill(0xfdd8b5);
                        sg.drawCircle(bedX - 5, bt - 5, 3.5);
                        sg.endFill();
                        sg.beginFill(0x2c1810, 0.7);
                        sg.drawEllipse(bedX - 5, bt - 9, 3, 1.2);
                        sg.endFill();
                        sg.beginFill(0x2c1810, 0.5);
                        sg.drawRect(bedX - 7.5, bt - 5, 1.5, 0.7);
                        sg.drawRect(bedX - 3.5, bt - 5, 1.5, 0.7);
                        sg.endFill();
                        sg.beginFill(0x2c1810, 0.2);
                        sg.drawRect(bedX - 6, bt - 2.5, 2, 0.5);
                        sg.endFill();
                        sg.beginFill(col, 0.5);
                        sg.moveTo(bedX + 1, bt + 2);
                        sg.lineTo(bedX + 1, bt - 1);
                        sg.quadraticCurveTo(bedX + 8, bt - 10, bedX + 18, bt);
                        sg.lineTo(bedX + 18, bt + 2);
                        sg.closePath();
                        sg.endFill();
                        sg.beginFill(col, 0.25);
                        sg.moveTo(bedX + 2, bt + 1);
                        sg.lineTo(bedX + 2, bt);
                        sg.quadraticCurveTo(bedX + 8, bt - 8, bedX + 17, bt + 1);
                        sg.lineTo(bedX + 17, bt + 1);
                        sg.closePath();
                        sg.endFill();
                        ...
                        sc.on('pointerover', (e) => {
                            const stg =
                                typeof getStage === 'function' ? getStage(m.rel, m.ret, m.phase) : 'baby';
        ...
        // Position & scroll
        const bp = 56,
            initY = G.vpH - bp - this.totalH + floorH;
        this.scene.y = initY;
        ...
        this._onMove = (e) => {
            if (
                !UniversityInterior.isDragging ||
                !UniversityInterior.scene ||
                UniversityInterior.scene.destroyed
            )
                return;
            let ny = UniversityInterior._startSceneY + (e.clientY - UniversityInterior._startY);
            ny = Math.max(UniversityInterior.minY, Math.min(ny, UniversityInterior.maxY));
            UniversityInterior.scene.y = ny;
        };
        this._onUp = () => {
            UniversityInterior.isDragging = false;
            if (UniversityInterior.layer) UniversityInterior.layer.cursor = 'grab';
        };
    ...
    _drawBasement(c, id, sx, uw, pY, fy, fh, col) {
        if (id === 'uni_main') {
            this._lbl(c, sx + uw / 2, pY - fh + 14, 'ARCHIVES', 0xfbbf24);
            for (let x = sx + 40; x < sx + uw - 100; x += 90) this._fileCabinet(c, x, pY);
            for (let x = sx + 60; x < sx + uw - 100; x += 120) this._docBox(c, x, pY);
        } else if (id === 'uni_library') {
            this._lbl(c, sx + uw / 2, pY - fh + 14, 'RARE BOOKS VAULT', 0x4ade80);
            for (let x = sx + 40; x < sx + uw - 100; x += 100) this._secureShelf(c, x, pY);
            for (let x = sx + 80; x < sx + uw - 120; x += 140) this._glassCase(c, x, pY);
        } else if (id === 'uni_dorm') {
            this._lbl(c, sx + uw / 2, pY - fh + 14, 'LAUNDRY', 0x60a5fa);
            for (let x = sx + 40; x < sx + uw - 80; x += 70) this._washer(c, x, pY);
        } else if (id === 'uni_lab') {
            this._lbl(c, sx + uw / 2, pY - fh + 14, 'EQUIPMENT STORAGE', 0x22d3ee);
            for (let x = sx + 40; x < sx + uw - 100; x += 90) this._crate(c, x, pY);
            for (let x = sx + 70; x < sx + uw - 100; x += 130) this._spareParts(c, x, pY);
        }
    },
    ...
    _npc(c, x, y, name, col, bld) {
        ...
        const npcModel = {
            ...
            role: name,
            phase: 'released',
            lab: bld ? bld.lab : 'other',
            desc: 'Facility staff — keeping systems operational.',
        };
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Facility Staff');
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
        });
    ...
    _chalkboard(c, x, y, w) {
        ...
        g.drawRoundedRect(x, y, w, 40, 3);
        g.endFill();
        g.lineStyle(2, 0x8b6e4e);
        g.drawRoundedRect(x, y, w, 40, 3);
        g.lineStyle(0);
        c.addChild(g);
        const eqs = ['L = -Sum y log(y)', 'grad J(t)', 'f(x)=max(0,Wx+b)'];
        eqs.forEach((eq, i) => {
            const t = new PIXI.Text(eq, { fontFamily: 'JetBrains Mono', fontSize: 6, fill: 0xccddcc });
            t.x = x + 8;
            t.y = y + 4 + i * 12;
            c.addChild(t);
        });
    ...
    _podium(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x5a3a1e);
        g.drawRect(x - 14, y - 22, 28, 22);
        g.endFill();
        g.beginFill(0x6a4a2e);
        g.drawRect(x - 16, y - 24, 32, 4);
        g.endFill();
        c.addChild(g);
    },
    _whiteboard(c, x, y) {
        ...
        g.drawRect(x, y - 50, 60, 30);
        g.endFill();
        g.lineStyle(1, 0xaaaaaa);
        g.drawRect(x, y - 50, 60, 30);
        g.lineStyle(0);
        g.beginFill(0xef4444);
        g.drawRect(x + 4, y - 20, 5, 2);
        g.endFill();
        g.beginFill(0x2255ee);
        g.drawRect(x + 12, y - 20, 5, 2);
        g.endFill();
        g.beginFill(0x22aa44);
        g.drawRect(x + 20, y - 20, 5, 2);
        g.endFill();
        c.addChild(g);
    },
    _studyTable(c, x, y) {
        ...
        g.drawRect(x, y - 16, 55, 16);
        g.endFill();
        g.beginFill(0x3a2a1a);
        g.drawRect(x + 5, y, 3, 8);
        g.drawRect(x + 47, y, 3, 8);
        g.endFill();
        c.addChild(g);
    },
    _workstation(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x4a3a2a);
        g.drawRect(x, y - 16, 50, 16);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 10, y - 32, 24, 16);
        g.endFill();
        const gl = new PIXI.Graphics();
        gl.eventMode = 'none';
        gl.beginFill(col, 0.15);
        gl.drawRect(x + 12, y - 30, 20, 12);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, gl);
        this.indoorLights.push({ g: gl, maxA: 0.2, type: 'screen' });
    },
    _frontDesk(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x, y - 18, 80, 18);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawRect(x, y - 20, 80, 3);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 20, y - 35, 30, 14);
        g.endFill();
        g.beginFill(0x06b6d4, 0.3);
        g.drawRect(x + 22, y - 33, 26, 10);
        g.endFill();
        c.addChild(g);
    },
    _noticeBoard(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x5a4030);
        g.drawRect(x, y - 48, 60, 35);
        g.endFill();
        g.beginFill(0xfbbf24, 0.6);
        g.drawRect(x + 5, y - 43, 12, 10);
        g.endFill();
        g.beginFill(0x4ade80, 0.6);
        g.drawRect(x + 22, y - 43, 12, 10);
        g.endFill();
        g.beginFill(0x60a5fa, 0.6);
        g.drawRect(x + 39, y - 43, 12, 10);
        g.endFill();
        g.beginFill(0xef4444, 0.5);
        g.drawRect(x + 10, y - 28, 18, 8);
        g.endFill();
        g.beginFill(0xa855f7, 0.5);
        g.drawRect(x + 32, y - 28, 18, 8);
        g.endFill();
        c.addChild(g);
    },

    // ── uni_library props ──
    _bookshelf(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x5a4030);
        g.drawRect(x, y - 50, 50, 50);
        g.endFill();
        const bookCols = [0xcc4444, 0x4488cc, 0x44aa44, 0xddaa33, 0x884488];
        for (let bi = 0; bi < 6; bi++) {
            const bx = x + 4 + bi * 7;
            const bh = 18 + Math.floor(Math.random() * 20);
            g.beginFill(bookCols[bi % bookCols.length]);
            g.drawRect(bx, y - 4 - bh, 6, bh);
            g.endFill();
        }
        g.beginFill(0x6a5040);
        g.drawRect(x, y - 4, 50, 4);
        g.endFill();
        g.beginFill(0x6a5040);
        g.drawRect(x, y - 26, 50, 3);
        g.endFill();
        c.addChild(g);
    },
    _readingDesk(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x4a3a2a);
        g.drawRect(x, y - 16, 60, 16);
        g.endFill();
        g.beginFill(0x3a2a1a);
        g.drawRect(x + 5, y, 3, 8);
        g.drawRect(x + 52, y, 3, 8);
        g.endFill();
        g.beginFill(0x44aa44, 0.5);
        g.drawCircle(x + 50, y - 22, 5);
        g.endFill();
        g.beginFill(0x333333);
        g.drawRect(x + 49, y - 16, 2, 10);
        g.endFill();
        c.addChild(g);
    },
    _checkoutDesk(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x, y - 18, 90, 18);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawRect(x, y - 20, 90, 3);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 25, y - 35, 28, 14);
        g.endFill();
        g.beginFill(0x4ade80, 0.3);
        g.drawRect(x + 27, y - 33, 24, 10);
        g.endFill();
        c.addChild(g);
    },
    ...
    _dormRoom(c, x, y, col) {
        ...
        g.drawRect(x - 2, y - 14, 4, 14);
        g.endFill(); // headboard
        g.beginFill(0x334155);
        g.drawRect(x + 2, y - 6, 34, 6);
        g.endFill(); // base
        g.beginFill(0xf1f5f9);
        g.drawRect(x + 4, y - 10, 30, 4);
        g.endFill(); // mattress
        g.beginFill(0x60a5fa);
        g.drawRect(x + 16, y - 11, 16, 6);
        g.endFill(); // duvet
        g.beginFill(0xffffff);
        g.drawRect(x + 5, y - 12, 5, 3);
        g.drawRect(x + 11, y - 12, 5, 3);
        g.endFill(); // pillows
        // Desk + monitor
        g.beginFill(0x4a3a2a);
        g.drawRect(x + 42, y - 12, 28, 12);
        g.endFill();
        g.beginFill(0x2a2a3a);
        g.drawRect(x + 44, y - 26, 10, 14);
        g.endFill();
        c.addChild(g);
    },
    _couch(c, x, y) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x664422);
        g.drawRoundedRect(x, y - 18, 60, 18, 4);
        g.endFill();
        g.beginFill(0x553311);
        g.drawRect(x, y - 24, 8, 24);
        g.drawRect(x + 52, y - 24, 8, 24);
        g.endFill();
        c.addChild(g);
    },
    _vendingMachine(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x334155);
        g.drawRect(x, y - 48, 35, 48);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 3, y - 44, 29, 28);
        g.endFill();
        for (let r = 0; r < 3; r++) {
            for (let ci = 0; ci < 3; ci++) {
                g.beginFill([0xef4444, 0x4ade80, 0x60a5fa][ci]);
                g.drawRect(x + 5 + ci * 9, y - 42 + r * 9, 7, 7);
                g.endFill();
            }
        }
        g.beginFill(col, 0.4);
        g.drawRect(x + 3, y - 12, 29, 8);
        g.endFill();
        c.addChild(g);
    },
    _tv(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x111120);
        g.drawRect(x, y - 40, 40, 26);
        g.endFill();
        const gl = new PIXI.Graphics();
        gl.eventMode = 'none';
        gl.beginFill(0x3388ff, 0.15);
        gl.drawRect(x + 2, y - 38, 36, 22);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        g.beginFill(0x333333);
        g.drawRect(x + 17, y - 14, 6, 8);
        g.endFill();
        g.beginFill(0x444444);
        g.drawRect(x + 10, y - 6, 20, 3);
        g.endFill();
        c.addChild(g, gl);
        this.indoorLights.push({ g: gl, maxA: 0.2, type: 'screen' });
    },
    ...
    _labBench(c, x, y, col) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x2a3a4a);
        g.drawRect(x, y - 16, 70, 16);
        g.endFill();
        g.beginFill(0x3a4a5a);
        g.drawRect(x + 5, y, 3, 8);
        g.drawRect(x + 62, y, 3, 8);
        g.endFill();
        const liqCols = [0x44ff88, 0xff4488, 0x4488ff];
        for (let bi = 0; bi < 3; bi++) {
            const bx = x + 8 + bi * 20;
            g.beginFill(liqCols[bi], 0.4);
            g.drawCircle(bx + 5, y - 22, 5);
            g.endFill();
            g.beginFill(0x88ccff, 0.2);
            g.drawRect(bx, y - 28, 10, 12);
            g.endFill();
        }
        c.addChild(g);
    },
    _oscilloscope(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x1a1a2e);
        g.drawRect(x, y - 40, 50, 40);
        g.endFill();
        g.beginFill(0x0a0a18);
        g.drawRect(x + 5, y - 35, 30, 22);
        g.endFill();
        const gl = new PIXI.Graphics();
        gl.eventMode = 'none';
        gl.beginFill(col, 0.12);
        gl.drawRect(x + 6, y - 34, 28, 20);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        g.beginFill(0x444444);
        g.drawCircle(x + 42, y - 30, 3);
        g.endFill();
        g.beginFill(0x444444);
        g.drawCircle(x + 42, y - 20, 3);
        g.endFill();
        c.addChild(g, gl);
    ...
    _rack(c, x, y, col) {
        ...
        g.drawRect(x, y - 50, 36, 50);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 3, y - 47, 30, 44);
        g.endFill();
        for (let s = y - 45; s < y - 5; s += 8) {
            g.beginFill(0x1a1a30);
            g.drawRect(x + 5, s, 26, 6);
            g.endFill();
            g.beginFill(0x4ade80);
            g.drawCircle(x + 9, s + 3, 1);
            g.endFill();
            g.beginFill(col, 0.15);
            g.drawRect(x + 13, s + 1, 15, 4);
            g.endFill();
        }
        const gl = new PIXI.Graphics();
        gl.beginFill(col, 0.04);
        gl.drawRect(x - 2, y - 52, 40, 54);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, gl);
    ...
    _pipes(c, x, y, w) {
        ...
        g.drawRect(x, y + 5, w, 2);
        g.endFill();
        for (let d = x + 50; d < x + w; d += 80) {
            g.beginFill(0x22d3ee, 0.3);
            g.drawCircle(d, y + 10, 2);
            g.endFill();
        }
        c.addChild(g);
    },
    _deskWithMonitor(c, x, y, col) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x, y - 18, 70, 18);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawRect(x, y - 20, 70, 3);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 18, y - 35, 28, 14);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawRect(x + 20, y - 33, 24, 10);
        g.endFill();
        c.addChild(g);
    },
    ...
    _fileCabinet(c, x, y) {
        ...
        g.drawRect(x, y - 45, 28, 45);
        g.endFill();
        for (let d = 0; d < 3; d++) {
            g.beginFill(0x2a2a3a);
            g.drawRect(x + 3, y - 42 + d * 14, 22, 11);
            g.endFill();
            g.beginFill(0x666677);
            g.drawRect(x + 12, y - 38 + d * 14, 4, 3);
            g.endFill();
        }
        c.addChild(g);
    },
    _docBox(c, x, y) {
        ...
        g.drawRect(x, y - 20, 30, 20);
        g.endFill();
        g.beginFill(0x6a5a4a);
        g.drawRect(x + 2, y - 18, 26, 2);
        g.drawRect(x + 2, y - 12, 26, 2);
        g.drawRect(x + 2, y - 6, 26, 2);
        g.endFill();
        c.addChild(g);
    },
    _secureShelf(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x3a3a4a);
        g.drawRect(x, y - 48, 50, 48);
        g.endFill();
        g.beginFill(0x4a4a5a);
        g.drawRect(x + 2, y - 46, 46, 20);
        g.drawRect(x + 2, y - 22, 46, 18);
        g.endFill();
        const bookCols = [0x884422, 0xaa6633, 0x664411];
        for (let bi = 0; bi < 5; bi++) {
            g.beginFill(bookCols[bi % bookCols.length]);
            g.drawRect(x + 4 + bi * 8, y - 44, 6, 16);
            g.endFill();
        }
        c.addChild(g);
    },
    _glassCase(c, x, y) {
        ...
        g.drawRect(x, y - 40, 45, 40);
        g.endFill();
        g.lineStyle(1, 0x88ccff, 0.3);
        g.drawRect(x + 3, y - 37, 39, 34);
        g.lineStyle(0);
        g.beginFill(0x884422);
        g.drawRect(x + 10, y - 28, 10, 16);
        g.endFill();
        g.beginFill(0xaa6633);
        g.drawRect(x + 25, y - 25, 10, 13);
        g.endFill();
        c.addChild(g);
    },
    _washer(c, x, y) {
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(0xb0b8c0);
        g.drawRect(x, y - 40, 35, 40);
        g.endFill();
        g.beginFill(0xd4d8dd);
        g.drawRect(x + 3, y - 37, 29, 14);
        g.endFill();
        g.beginFill(0x0a0a18);
        g.drawCircle(x + 17, y - 12, 8);
        g.endFill();
        g.beginFill(0x60a5fa, 0.3);
        g.drawCircle(x + 17, y - 12, 6);
        g.endFill();
        g.beginFill(0x4ade80);
        g.drawCircle(x + 28, y - 34, 2);
        g.endFill();
        c.addChild(g);
    },
    _crate(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x5a4a3a);
        g.drawRect(x, y - 30, 35, 30);
        g.endFill();
        g.beginFill(0x6a5a4a);
        g.drawRect(x, y - 30, 35, 3);
        g.drawRect(x, y - 15, 35, 3);
        g.endFill();
    ...
    _spareParts(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x334155);
        g.drawRect(x, y - 24, 50, 24);
        g.endFill();
        g.beginFill(0x444466);
        g.drawCircle(x + 12, y - 12, 6);
        g.endFill();
        g.beginFill(0x555577);
        g.drawCircle(x + 12, y - 12, 3);
        g.endFill();
        g.beginFill(0x666688);
        g.drawRect(x + 28, y - 18, 14, 10);
        g.endFill();
        g.beginFill(0x22d3ee, 0.3);
        g.drawRect(x + 30, y - 16, 10, 6);
        g.endFill();
        c.addChild(g);
    },
    ...
    _student(c, x, y, model) {
        ...
        cont.hitArea = new PIXI.Rectangle(-bw, -h - 10, bw * 2, h + 14);
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined') UI.selectModel(model);
        });
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined')
                UI.showTooltip(
                    e,
                    model.name,
                    stg === 'baby' ? 'Pre-Training' : stg === 'kid' ? 'Training' : 'Rumored'
                );
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
    ...
    update() {
        ...
        this.indoorLights.forEach((l) => {
            ...
            if (l.type === 'blink')
                l.g.alpha = l.maxA * (0.5 + Math.sin(G.tick * 0.05 + Math.random() * 0.1) * 0.5);
        ...
        this.avatars.forEach((av) => {
            ...
            if (av._isZzz) {
                const t = G.tick * 0.04 + av._phase;
                av._z1.y = Math.sin(t) * 3;
                av._z1.alpha = 0.5 + Math.sin(t) * 0.3;
                av._z2.y = -7 + Math.sin(t + 1) * 3;
                av._z2.alpha = 0.3 + Math.sin(t + 1) * 0.25;
                av._z3.y = -15 + Math.sin(t + 2) * 3;
                av._z3.alpha = 0.15 + Math.sin(t + 2) * 0.2;
                return;
            }
            av._walkTimer = (av._walkTimer || 0) - 1;
            if (av._walkTimer <= 0) {
                av._walkDir = Math.random() > 0.5 ? 1 : -1;
                av._walkTimer = 60 + Math.random() * 120;
            }
            ...
            const ah = av._h || 32;
            if (av.head) {
                av.head.y = -ah + Math.sin(G.tick * 0.15 + av._phase) * 1.5;
            }
            if (av.body) {
                av.body.y = -ah + Math.round(ah * 0.35) + Math.abs(Math.sin(G.tick * 0.15 + av._phase)) * 1.5;
            }
            if (av.legL) {
                av.legL.y = Math.sin(G.tick * 0.2 + av._phase) * 3;
            }
            if (av.legR) {
                av.legR.y = -Math.sin(G.tick * 0.2 + av._phase) * 3;
            }
        });
    },
};
