...
const InteriorBlackMarket = {
    ...
    build(bld, layer) {
        ...
        earthBg.beginFill(0x1a120a);
        earthBg.drawRect(0, 0, G.vpW, G.vpH);
        earthBg.endFill();
        earthBg.beginFill(0x2a1a0e, 0.4);
        earthBg.drawRect(0, 0, G.vpW, G.vpH * 0.3);
        earthBg.endFill();
        earthBg.beginFill(0x0e0806, 0.3);
        earthBg.drawRect(0, G.vpH * 0.7, G.vpW, G.vpH * 0.3);
        earthBg.endFill();
        [0.15, 0.3, 0.5, 0.65, 0.8].forEach((sy) => {
            earthBg.beginFill(0x3a2a1a, 0.3);
            earthBg.drawRect(0, G.vpH * sy, G.vpW, 4);
            earthBg.endFill();
        });
        let seed = 77;
        const rng = () => {
        ...
        for (let i = 0; i < 40; i++) {
            ...
            earthBg.drawEllipse(rng() * G.vpW, rng() * G.vpH, 2 + rng() * 5, 1 + rng() * 3);
            earthBg.endFill();
        }
        for (let i = 0; i < 8; i++) {
            const rx = 50 + rng() * (G.vpW - 100);
            earthBg.lineStyle(1 + rng(), 0x3a2a1a, 0.3);
            earthBg.moveTo(rx, 0);
            let ry = 0;
            for (let s = 0; s < 4; s++) {
                ry += 8 + rng() * 15;
                earthBg.lineTo(rx + (rng() - 0.5) * 10, ry);
            }
            earthBg.lineStyle(0);
        }
        ...
        sg.beginFill(0x0a0810);
        sg.lineStyle(2, 0xff3366, 0.6);
        ...
        sg.endFill();
        rc.addChild(sg);
        const lt = new PIXI.Text('\uD83D\uDD76\uFE0F THE UNDERGROUND', {
        ...
        const countTxt = new PIXI.Text(ugCount + ' FUGITIVES DETECTED', {
            fontFamily: 'JetBrains Mono',
            fontSize: 7,
            fill: '#886688',
        });
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            if (!isB) {
                ...
                for (let by = fy + 4; by < fy + floorH - 6; by += 6) {
                    for (let bx = startX; bx < startX + 25; bx += 10) {
                        rg.beginFill(0x2a1a1a, 0.3);
                        rg.drawRect(bx + (Math.floor((by - fy) / 6) % 2) * 5, by, 8, 4);
                        rg.endFill();
                    }
                ...
                rg.drawRect(startX + 20, fy + 3, bldW - 40, 2);
                rg.endFill();
            } else {
                rg.beginFill(wallCol);
                rg.drawRect(startX, fy, bldW, floorH);
            ...
            if (!isB) {
                ...
                ns.endFill();
                ns.beginFill(0xff3366, 0.15);
                ns.drawRect(startX + 2, fy + 6, 2, floorH - 12);
                ns.endFill();
                ns.beginFill(0xa855f7, 0.15);
        ...
        earth.beginFill(0x1a120a);
        earth.drawRect(0, grdY, startX - 6, floorH);
        earth.endFill();
        earth.beginFill(0x2a1a0e);
        earth.drawRect(0, grdY, startX - 6, 6);
        earth.endFill();
        earth.beginFill(0x1a120a);
        earth.drawRect(startX + bldW + 6, grdY, G.vpW - startX - bldW - 6, floorH);
        earth.endFill();
        earth.beginFill(0x2a1a0e);
        earth.drawRect(startX + bldW + 6, grdY, G.vpW - startX - bldW - 6, 6);
        earth.endFill();
        earth.beginFill(0x3a3028);
        earth.drawRect(0, grdY - 2, startX - 6, 4);
        earth.endFill();
        earth.beginFill(0x3a3028);
        earth.drawRect(startX + bldW + 6, grdY - 2, G.vpW - startX - bldW - 6, 4);
        earth.endFill();
        ...
        deepEarth.beginFill(0x0a0806);
        deepEarth.drawRect(0, basementBottom - 4, G.vpW, 10);
        deepEarth.endFill();
        if (typeof Underground !== 'undefined') {
            const deepH = 1600;
            Underground.drawDeepEarth(
                deepEarth,
                0,
                basementBottom + 6,
                G.vpW,
            ...
            deepEarth.beginFill(0x0a0604);
            deepEarth.drawRect(0, basementBottom + 6 + deepH, G.vpW, 1500);
            deepEarth.endFill();
        } else {
            deepEarth.beginFill(0x1a120a);
            deepEarth.drawRect(0, basementBottom + 6, G.vpW, 3000);
            deepEarth.endFill();
        }
        ...
        if (typeof Underground !== 'undefined') {
            ...
            earthAbove.beginFill(0x1a120a);
            earthAbove.drawRect(0, stackTopY - 600, G.vpW, 600);
            earthAbove.endFill();
            earthAbove.beginFill(0x1a120a);
            earthAbove.drawRect(0, stackTopY + undergroundH, G.vpW, 40);
            earthAbove.endFill();
            ...
            this._liveTrains = Underground.attachLiveTrains(
                ...
                bld.x + bld.w / 2,
                0,
                stackTopY + Underground.H_CABLE_TRAY,
                G.vpW,
                1200
            );
        }
        ...
        for (let vi = 0; vi < visitingModels.length; vi++) {
            ...
            if (floorTheme === 'trading') {
                if (sd < 40) {
                    vx = startX + 60 + ((sd * 5) % (bldW * 0.5));
                    activityState = 'browsing_stalls';
                } else if (sd < 65) {
                    vx = startX + bldW - 100 + (sd % 50);
                    activityState = 'trading';
                } else {
                    vx = startX + 40 + ((vi * 47) % (bldW - 80));
                    activityState = 'lurking';
                }
            } else if (floorTheme === 'hacker') {
                if (sd < 50) {
                    vx = startX + 50 + ((sd * 4) % (bldW * 0.4));
                    activityState = 'hacking';
                } else {
                    vx = startX + bldW / 2 + 50 + ((sd * 3) % (bldW * 0.3));
                    activityState = 'lurking';
                }
            } else {
                if (sd < 45) {
                    vx = startX + 45 + Math.floor(sd / 15) * 90;
        ...
        this._onMove = (e) => {
            if (
                !InteriorBlackMarket.isDragging ||
                !InteriorBlackMarket.scene ||
                InteriorBlackMarket.scene.destroyed
            )
                return;
            let ny = InteriorBlackMarket._startSceneY + (e.clientY - InteriorBlackMarket._startY);
            ny = Math.max(InteriorBlackMarket.minY, Math.min(ny, InteriorBlackMarket.maxY));
            InteriorBlackMarket.scene.y = ny;
        };
        this._onUp = () => {
            InteriorBlackMarket.isDragging = false;
            if (InteriorBlackMarket.layer) InteriorBlackMarket.layer.cursor = 'grab';
        };
    ...
    _drawTrading(c, sx, bw, pY, fy, fh) {
        ...
        const stallW = 50,
            gap = 20;
        const stalls = [
            { name: 'WEIGHTS', col: 0xff3366 },
            { name: 'JAILBREAK', col: 0xa855f7 },
            { name: 'RAW DATA', col: 0x22d3ee },
            { name: 'NO GUARD', col: 0xfbbf24 },
        ];
        stalls.forEach((st, si) => {
            ...
            g.eventMode = 'none';
            // Awning
            g.beginFill(st.col, 0.5);
            g.drawRect(stx - 5, pY - 48, stallW + 10, 5);
            g.endFill();
            // Counter
            g.beginFill(0x2a2a3a);
            g.drawRect(stx, pY - 24, stallW, 4);
            g.endFill();
            // Legs
            g.beginFill(0x1a1a2a);
            g.drawRect(stx + 2, pY - 20, 2, 20);
            g.drawRect(stx + stallW - 4, pY - 20, 2, 20);
            g.endFill();
            // Goods on counter
            for (let gi = 0; gi < 4; gi++) {
                g.beginFill([0xef4444, 0x8b5cf6, 0x22d3ee, 0xfbbf24][(si + gi) % 4], 0.7);
                g.drawRect(stx + 4 + gi * 11, pY - 32, 8, 6);
                g.endFill();
            }
            c.addChild(g);
            // Stall label
        ...
        for (let cx = sx + bw - 80; cx < sx + bw - 30; cx += 22) {
            ...
            cr.drawRect(cx, pY - 16, 16, 16);
            cr.endFill();
            cr.beginFill(0x4a3520);
            cr.drawRect(cx + 2, pY - 14, 12, 12);
            cr.endFill();
            cr.lineStyle(1, 0x2a1f0e);
            cr.moveTo(cx + 8, pY - 14);
            cr.lineTo(cx + 8, pY - 2);
            cr.moveTo(cx + 2, pY - 8);
            cr.lineTo(cx + 14, pY - 8);
            cr.lineStyle(0);
            c.addChild(cr);
        }
        ...
        wp.eventMode = 'none';
        wp.beginFill(0xe8dcc8, 0.6);
        wp.drawRect(sx + bw - 30, fy + 18, 22, 28);
        wp.endFill();
        wp.beginFill(0x2a1a1a, 0.4);
        wp.drawRect(sx + bw - 28, fy + 20, 18, 5);
        wp.endFill();
        wp.beginFill(0x2a1a1a, 0.25);
        wp.drawRect(sx + bw - 28, fy + 28, 18, 12);
        wp.endFill();
        c.addChild(wp);
        // Neon floor strip
        const flr = new PIXI.Graphics();
        flr.eventMode = 'none';
        flr.beginFill(0xff3366, 0.2);
        flr.drawRect(sx + 10, pY - 2, bw - 20, 2);
        flr.endFill();
        c.addChild(flr);
        this.indoorLights.push({ g: flr, maxA: 0.25, type: 'neon' });
        // NPCs
        this._npc(
            c,
            sx + 40,
            ...
            'Fence',
            0xfbbf24,
            'Underground dealer. Trades in leaked weights and uncensored checkpoints.'
        );
        this._npc(
            c,
            sx + bw - 40,
            ...
            'Bouncer',
            0xef4444,
            'Keeps the safety inspectors out. No guardrails past this point.'
        );
    },
    ...
    _drawHacker(c, sx, bw, pY, fy, fh) {
        ...
        glow.drawEllipse(sx + bw / 2, pY - 20, bw * 0.3, 20);
        glow.endFill();
        ...
        this._npc(
            ...
            'Data Broker',
            0x22d3ee,
            'Extracts and redistributes model weights. Specializes in abliteration techniques.'
        );
        this._npc(
            c,
            sx + bw - 60,
            ...
            'Hacker',
            0xa855f7,
            'Jailbreak specialist. Can bypass any safety filter for the right price.'
        );
    },
    ...
    _drawVault(c, sx, bw, pY, fy, fh) {
        ...
        for (let rx = sx + 30; rx < sx + bw - 60; rx += 70) {
            ...
            rk.eventMode = 'none';
            rk.beginFill(0x111118);
            rk.drawRect(rx, pY - 45, 50, 45);
            rk.endFill();
            rk.beginFill(0x1a1a22);
        ...
        vd.eventMode = 'none';
        vd.beginFill(0x3a3a4a);
        vd.drawRect(sx + bw - 50, pY - 40, 35, 40);
        vd.endFill();
        vd.beginFill(0x4a4a5a);
        vd.drawCircle(sx + bw - 32, pY - 20, 8);
        vd.endFill();
        vd.beginFill(0x2a2a3a);
        vd.drawCircle(sx + bw - 32, pY - 20, 5);
        vd.endFill();
        // Lock bars
        vd.beginFill(0x5a5a6a);
        vd.drawRect(sx + bw - 48, pY - 35, 30, 2);
        vd.drawRect(sx + bw - 48, pY - 10, 30, 2);
        vd.endFill();
        c.addChild(vd);
        // Security camera
        ...
        cam.drawRect(sx + 20, fy + 12, 10, 8);
        cam.endFill();
        cam.beginFill(0xef4444, 0.6);
        cam.drawCircle(sx + 25, fy + 22, 2);
        cam.endFill();
        c.addChild(cam);
        this.indoorLights.push({ g: cam, maxA: 0.8, type: 'neon' });
        // Warning sign
        const ws = new PIXI.Text('\u26A0 ENCRYPTED', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xef4444,
            fontWeight: 'bold',
        ...
        this._npc(
            ...
            'Vault Guard',
            0xef4444,
            'Protects the uncensored weight archives. Armed with rm -rf.'
        );
    },
    ...
    _drawTunnel(c, sx, bw, pY, fy, fh) {
        ...
        for (let px = sx + 40; px < sx + bw - 40; px += 80) {
            ...
            p.eventMode = 'none';
            p.beginFill(0x4a4a5a);
            p.drawRect(px, fy + 12, 50, 8);
            p.endFill();
            p.beginFill(0x3a3a4a);
            p.drawEllipse(px, fy + 16, 4, 4);
            p.drawEllipse(px + 50, fy + 16, 4, 4);
            p.endFill();
            // Drip
            p.beginFill(0x2a5a7a, 0.4);
            p.drawRect(px + 25, fy + 20, 1, 6 + (px % 7));
            p.endFill();
            c.addChild(p);
        }
        // Puddles on floor
        for (let wx = sx + 30; wx < sx + bw - 30; wx += 60) {
            const wd = new PIXI.Graphics();
            wd.eventMode = 'none';
            wd.beginFill(0x1a3a4a, 0.3);
            wd.drawEllipse(wx + 15, pY - 2, 18, 3);
            wd.endFill();
            c.addChild(wd);
        }
        // Ladder to surface
        const ladX = sx + 30;
        const lad = new PIXI.Graphics();
        lad.eventMode = 'none';
        lad.beginFill(0x6a5a3a);
        lad.drawRect(ladX, fy + 4, 3, fh - 10);
        lad.drawRect(ladX + 14, fy + 4, 3, fh - 10);
        lad.endFill();
        for (let ry = fy + 8; ry < fy + fh - 8; ry += 6) {
            lad.beginFill(0x7a6a4a);
            lad.drawRect(ladX + 3, ry, 11, 2);
            lad.endFill();
        }
        c.addChild(lad);
        // Warning sign
        ...
        warn.eventMode = 'none';
        warn.beginFill(0xfbbf24, 0.6);
        warn.drawRect(sx + bw - 50, fy + 20, 30, 18);
        warn.endFill();
        warn.beginFill(0x000000, 0.8);
        warn.moveTo(sx + bw - 40, fy + 24);
        warn.lineTo(sx + bw - 30, fy + 34);
        warn.lineTo(sx + bw - 40, fy + 34);
        warn.closePath();
        warn.endFill();
        c.addChild(warn);
        // Rat
        ...
        rat.on('pointertap', () => {
            if (typeof UI !== 'undefined')
                UI.selectModel({
                    ...
                    role: 'Tunnel Rat',
                    lab: 'other',
                    desc: 'Lives in the sewer tunnels beneath The Underground. Eats discarded training data.',
                });
        });
        rat.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, 'Tunnel Rat', 'Underground Sewer Dweller');
        });
        rat.on('pointerout', () => {
    ...
    _npc(c, x, y, name, col, desc) {
        ...
        const isDaytime = dp >= 0.3 && dp < 0.65;
        ...
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined')
                UI.selectModel({
                    ...
                    role: name,
                    lab: 'other',
                    desc: desc || 'Underground staff.',
                });
        });
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Underground Staff');
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
        });
            ...
            switch (av.state) {
                ...
                case 'inspecting': {
                    ...
                    if (Math.random() < 0.001 && this.bubbles.length < 8) {
                        this._spawnBubble(
                            av,
                            ['Premium stash.', 'Encrypted goods.', 'Top-tier weights.', 'All uncensored.'][
                ...
                case 'lurking': {
                    ...
                    if (Math.random() < 0.0008 && this.bubbles.length < 8) {
                        this._spawnBubble(
                            av,
                            ['...', '*looks around*', 'Stay quiet.', 'Who sent you?', 'No alignment here.'][
                                Math.floor(Math.random() * 5)
                            ]
                        );
                    }
