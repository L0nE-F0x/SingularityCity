...
const CityPark = {
    BLDS: [
        {
            ...
            emoji: '🌳',
            type: 'park',
            desc: 'A sprawling green oasis in the heart of the city. Benches, fountains, jogging paths, and ancient oaks invite weary models to recharge.',
        },
    ],
    ...
    drawPark(gfx, container, b, h) {
        ...
        gfx.drawRect(0, h - 14, w, 14);
        gfx.endFill();
        // Lighter grass stripe
        gfx.beginFill(0x3a7d5e);
        gfx.drawRect(0, h - 14, w, 5);
        gfx.endFill();
        // Grass texture dots
        ...
        const pathY = h - 8;
        gfx.beginFill(0x8b8678);
        gfx.drawRect(20, pathY, w - 40, 5);
        gfx.endFill();
        for (let px = 22; px < w - 42; px += 6) {
            gfx.beginFill(0x9a9588);
            gfx.drawRect(px, pathY + 1, 4, 3);
            gfx.endFill();
        }
        ...
        for (let step = 0; step < 12; step++) {
            ...
            gfx.beginFill(0x7a7668, 0.6);
            gfx.drawRect(jx, jy, 5, 3);
            gfx.endFill();
        }
        ...
        gfx.beginFill(0x2a4a6a, 0.7);
        gfx.drawEllipse(fx, fy, 32, 8);
        gfx.endFill();
        gfx.beginFill(0x3a6a8a, 0.5);
        gfx.drawEllipse(fx, fy, 28, 6);
        gfx.endFill();
        // Water shimmer
        gfx.beginFill(0x60a0c8, 0.3);
        gfx.drawEllipse(fx - 5, fy - 1, 12, 3);
        gfx.endFill();
        // Pedestal
        gfx.beginFill(0x6a6a7a);
        gfx.drawRect(fx - 4, fy - 20, 8, 14);
        gfx.endFill();
        gfx.beginFill(0x7a7a8a);
        gfx.drawRect(fx - 6, fy - 22, 12, 4);
        gfx.endFill();
        // Fountain top bowl
        gfx.beginFill(0x7a7a8a);
        gfx.drawEllipse(fx, fy - 22, 10, 3);
        gfx.endFill();

        // Animated fountain spray (container for update loop)
        const sprayContainer = new PIXI.Container();
        sprayContainer.x = fx;
        sprayContainer.y = fy - 24;
        ...
        benchPositions.forEach((bx) => {
            ...
            gfx.drawRect(bx - 8, h - 14, 2, 6);
            gfx.endFill();
            gfx.beginFill(0x5c4033);
            gfx.drawRect(bx + 6, h - 14, 2, 6);
            gfx.endFill();
            // Bench seat
            gfx.beginFill(0x8b6b4a);
            gfx.drawRect(bx - 10, h - 16, 20, 3);
            gfx.endFill();
            // Bench back
            gfx.beginFill(0x7a5c3d);
            gfx.drawRect(bx - 9, h - 20, 18, 2);
            gfx.endFill();
        });
        ...
        for (let fi = 0; fi < 18; fi++) {
            ...
            gfx.drawCircle(ffx, ffy, 2);
            gfx.endFill();
            gfx.beginFill(0x2d8a4f, 0.5);
            gfx.drawRect(ffx - 0.5, ffy + 1, 1, 3);
            gfx.endFill();
        }
        ...
        const px = w - 55;
        const py = h - 12;
        gfx.beginFill(0x1a3a5a, 0.6);
        gfx.drawEllipse(px, py, 22, 6);
        gfx.endFill();
        gfx.beginFill(0x2a5a7a, 0.4);
        gfx.drawEllipse(px - 3, py - 1, 14, 4);
        gfx.endFill();
        // Cattails
        gfx.beginFill(0x4a5a3a);
        gfx.drawRect(px + 14, py - 16, 1, 14);
        gfx.endFill();
        gfx.beginFill(0x6a5a3a);
        gfx.drawEllipse(px + 14.5, py - 17, 2, 4);
        gfx.endFill();
        gfx.beginFill(0x4a5a3a);
        gfx.drawRect(px + 18, py - 12, 1, 10);
        gfx.endFill();
        gfx.beginFill(0x6a5a3a);
        gfx.drawEllipse(px + 18.5, py - 13, 2, 3);
        gfx.endFill();
        ...
        lampPositions.forEach((lx) => {
            ...
            gfx.drawRect(lx - 1, h - 38, 2, 26);
            gfx.endFill();
            // Lamp bracket
            gfx.beginFill(0x4a4a5a);
            gfx.drawRect(lx - 3, h - 40, 6, 3);
            gfx.endFill();
        ...
        for (let fenceX = 0; fenceX < 16; fenceX += 4) {
            gfx.beginFill(0xd4c9a8, 0.5);
            gfx.drawRect(fenceX, h - 16, 2, 6);
            gfx.endFill();
        }
        gfx.beginFill(0xc4b998, 0.4);
        gfx.drawRect(0, h - 14, 18, 1);
        gfx.endFill();
        // Right entrance gap
    ...
    _drawTree(gfx, x, groundH, type) {
        if (type === 'oak') {
            // Thick trunk
            gfx.beginFill(0x5c3a1e);
            gfx.drawRect(x - 3, groundH - 45, 6, 33);
            gfx.endFill();
            // Branch stubs
            gfx.beginFill(0x4a2e16);
            gfx.drawRect(x - 7, groundH - 38, 5, 2);
            gfx.endFill();
            gfx.beginFill(0x4a2e16);
            gfx.drawRect(x + 3, groundH - 42, 5, 2);
            gfx.endFill();
            // Canopy (large, round)
            gfx.beginFill(0x1b6a35, 0.85);
            gfx.drawEllipse(x, groundH - 50, 18, 14);
            gfx.endFill();
            gfx.beginFill(0x238a42, 0.6);
            gfx.drawEllipse(x - 4, groundH - 52, 10, 8);
            gfx.endFill();
            gfx.beginFill(0x166a30, 0.5);
            gfx.drawEllipse(x + 5, groundH - 48, 8, 6);
            gfx.endFill();
        } else {
            // Maple — thinner trunk, wider crown
            gfx.beginFill(0x6a4422);
            gfx.drawRect(x - 2, groundH - 42, 4, 30);
            gfx.endFill();
            // Crown layers
            gfx.beginFill(0x1a7a38, 0.8);
            gfx.drawEllipse(x, groundH - 48, 16, 12);
            gfx.endFill();
            gfx.beginFill(0x22994a, 0.5);
            gfx.drawEllipse(x - 3, groundH - 52, 8, 7);
            gfx.endFill();
            gfx.beginFill(0x2aaa52, 0.4);
            gfx.drawEllipse(x + 4, groundH - 46, 7, 5);
            gfx.endFill();
        }
    },
