...
const InteriorEmbassy = {
    ...
    COUNTRIES: {
        embassy_us: {
            ...
            policyMsgs: [
                'Voluntary commitments signed.',
                'Chip export review pending.',
                'Frontier Model Forum memo.',
                'AISI joint eval scheduled.',
                'State patchwork tracked.',
                'SB 1047 status: vetoed.',
                '14110 implementation update.',
                'DoD procurement brief.',
            ],
        },
        embassy_cn: {
            ...
            warmAccent: 0xffde00,
            policyMsgs: [
                'DeepSeek weights released.',
                'Qwen3 benchmark update.',
                'Export control workaround.',
                'Domestic compute survey.',
                'Generative AI filing approved.',
                'Content audit passed.',
                'H20 shipment delayed.',
                'MiC 2025 progress brief.',
            ],
        },
        embassy_eu: {
            ...
            warmAccent: 0xffcc00,
            policyMsgs: [
                'AI Act phase-in review.',
                'Conformity assessment filed.',
                'GPAI code of practice draft.',
                'Standards body coordination.',
                'National DPA consultation.',
                'Mistral compliance ping.',
                'Data Act cross-reference.',
                'Copyright triage update.',
            ],
        },
        embassy_uk: {
            full: 'UNITED KINGDOM',
            ...
            warmAccent: 0xc8102e,
            policyMsgs: [
                'Frontier model eval booked.',
                'DeepMind lab visit scheduled.',
                'Bletchley follow-up meeting.',
                'Principles white paper v2.',
                'AISI capability probe run.',
                'DSIT briefing finalised.',
                'Inspect framework update.',
        ...
        embassy_in: {
            ...
            warmAccent: 0x138808,
            policyMsgs: [
                'Sarvam benchmark posted.',
                'GPU allocation request.',
                'Krutrim partnership memo.',
                'Bhashini language expansion.',
                'MeitY consultation draft.',
                'Compute portal onboarding.',
                'IndiaAI grant awarded.',
                'UN digital public goods note.',
            ],
        },
        embassy_ae: {
            ...
            warmAccent: 0xce1126,
            policyMsgs: [
                'Falcon 3 release coordinated.',
                'MBZUAI MoU signed.',
                'G42 / Microsoft checkpoint.',
                'Stargate UAE site survey.',
                'Sovereign data pact ping.',
                'Arabic benchmark milestone.',
                'Talent visa onboarding.',
                'Crescent Fund placement.',
            ],
        },
    },
    ...
    build(bld, layer) {
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            if (!isBasement) {
                ...
                rg.endFill();
                rg.beginFill(accent, 0.18);
                rg.drawRect(startX, fy + 3, bldW, 1);
                rg.endFill();
            } else {
                // Basement: stone texture, exposed brick band
                for (let bx = startX; bx < startX + bldW; bx += 20) {
                    rg.beginFill(0x1d180e, 0.35);
                    rg.drawRect(bx + 1, fy + 12, 18, 2);
                    rg.endFill();
                    rg.beginFill(0x1d180e, 0.35);
                    rg.drawRect(bx + 11, fy + 22, 18, 2);
                    rg.endFill();
                }
            }

            // Floor slab divider
            rg.beginFill(0xbab396);
        ...
        this._onMove = (e) => {
            if (!InteriorEmbassy.isDragging || !InteriorEmbassy.scene || InteriorEmbassy.scene.destroyed)
                return;
    ...
    _drawPediment(c, sx, bw, roofH, flagCols, accent, warm, embName) {
        ...
        pole.drawRect(-1.5, -60, 3, 60);
        pole.endFill();
        pole.beginFill(0xfbbf24);
        pole.drawCircle(0, -60, 3);
        pole.endFill();
        ...
        const country =
            ...
            (this.bld && this.bld.id ? this.bld.id.replace('embassy_', '') : 'us');
    ...
    _drawArchedWindow(g, x, y, w, h, accent) {
        ...
        g.drawRect(x, y + h * 0.15, w, h - h * 0.15);
        g.endFill();
        g.beginFill(0x0a0a14);
        ...
        g.drawRect(x + 3, y + h * 0.15, w - 6, h - h * 0.15 - 3);
        g.endFill();
        g.beginFill(0xffe9a8, 0.55);
        g.drawEllipse(x + w / 2, y + h * 0.15, w / 2 - 3, h * 0.15 - 1);
        g.endFill();
        ...
        for (let i = 1; i < paneCount; i++) {
            ...
            g.moveTo(mx, y + h * 0.15);
            g.lineTo(mx, y + h - 4);
        }
        g.moveTo(x + 3, y + h * 0.55);
        g.lineTo(x + w - 3, y + h * 0.55);
        g.lineStyle(0);
        // Stone sill
        g.beginFill(0xc6bfa2);
        g.drawRect(x - 2, y + h - 4, w + 4, 4);
        g.endFill();
    ...
    _drawFloorProps(c, sx, bw, pY, fy, fh, key, accent, warm, country, flagCols) {
        ...
        if (key === 'reception') {
            ...
            for (let ix = sx + 20; ix < sx + bw - 20; ix += 36) {
                ...
                g.drawRect(ix, pY - 4, 18, 4);
                g.endFill();
            }
            // Accent inlay strip along centre
            g.beginFill(accent, 0.12);
            g.drawRect(sx + 10, pY - 2, bw - 20, 1);
            g.endFill();
            // Grand reception desk — semi-circular, marble top
            const dw = bw * 0.34;
            const dx = sx + bw / 2 - dw / 2;
            g.beginFill(0x7a5a2a);
            g.drawRoundedRect(dx, pY - 22, dw, 22, 4);
            g.endFill();
            g.beginFill(0xe9e4d2);
            g.drawRect(dx + 2, pY - 22, dw - 4, 3);
            g.endFill();
            // Country seal on desk front
            g.beginFill(accent);
            g.drawCircle(dx + dw / 2, pY - 11, 7);
            g.endFill();
            g.beginFill(warm, 0.55);
            g.drawCircle(dx + dw / 2, pY - 11, 4);
            g.endFill();
            g.beginFill(0xfbbf24);
            g.drawCircle(dx + dw / 2, pY - 11, 2);
            g.endFill();
            c.addChild(g);
            // Phone + ledger on desk
            const ph = new PIXI.Graphics();
            ph.eventMode = 'none';
            ph.beginFill(0x1a1a28);
            ph.drawRect(dx + 18, pY - 22, 10, 6);
            ph.endFill();
            ph.beginFill(0x3b3b58);
            ph.drawRect(dx + 19, pY - 21, 8, 2);
            ph.endFill();
            ph.beginFill(0xf4efdc);
            ph.drawRect(dx + dw - 34, pY - 22, 16, 4);
            ph.endFill(); // ledger
            ph.beginFill(accent, 0.4);
            ph.drawRect(dx + dw - 32, pY - 21, 12, 1);
            ph.endFill();
            c.addChild(ph);
            ...
            for (const off of [sx + 30, sx + bw - 110]) {
                ...
                sf.eventMode = 'none';
                sf.beginFill(0x6b3d26);
                sf.drawRoundedRect(off, pY - 15, 70, 12, 3);
                sf.endFill();
                sf.beginFill(0x7c4a30);
                sf.drawRoundedRect(off, pY - 20, 70, 6, 3);
                sf.endFill();
                // Tufted button detail
                for (let bi = 0; bi < 3; bi++) {
                    sf.beginFill(0x4a2715);
                    sf.drawCircle(off + 14 + bi * 22, pY - 12, 0.9);
                    sf.endFill();
                }
                // Arms
                sf.beginFill(0x4a2715);
                sf.drawRoundedRect(off - 4, pY - 20, 6, 20, 2);
                sf.drawRoundedRect(off + 68, pY - 20, 6, 20, 2);
                sf.endFill();
                c.addChild(sf);
                // Coffee table
                const ct = new PIXI.Graphics();
                ct.eventMode = 'none';
                ct.beginFill(0x4a2e18);
                ct.drawRect(off + 14, pY - 6, 42, 5);
                ct.endFill();
                ct.beginFill(0xa8a288, 0.5);
                ct.drawRect(off + 14, pY - 6, 42, 1);
                ct.endFill();
                // Magazines/newspaper
                ct.beginFill(0xf4efdc);
                ct.drawRect(off + 20, pY - 7, 10, 5);
                ct.endFill();
                ct.beginFill(accent, 0.4);
                ct.drawRect(off + 22, pY - 6, 6, 1);
                ct.endFill();
                c.addChild(ct);
            }
            // Country silhouette on back wall (country-specific motif)
            this._drawCountrySilhouette(
                c,
                sx + bw / 2,
                fy + fh * 0.45,
                110,
            ...
            for (let sx2 of [sx + bw * 0.35, sx + bw * 0.65]) {
                rg.beginFill(0xfbbf24);
                rg.drawCircle(sx2, pY - 10, 2);
                rg.endFill();
                rg.beginFill(0x3a2a18);
                rg.drawRect(sx2 - 1, pY - 10, 2, 10);
                rg.endFill();
            }
            rg.lineStyle(1.5, warm, 0.75);
            rg.moveTo(sx + bw * 0.35, pY - 10);
            rg.lineTo(sx + bw * 0.65, pY - 10);
            rg.lineStyle(0);
            c.addChild(rg);
        } else if (key === 'consular') {
            // Wood floor
            g.beginFill(0x6b4a2c);
            g.drawRect(sx, pY - 4, bw, 4);
            g.endFill();
            for (let ix = sx; ix < sx + bw; ix += 30) {
                g.beginFill(0x5a3d24, 0.45);
            ...
            for (let bi = 0; bi < booths; bi++) {
                ...
                g.drawRect(bx + 6, pY - 22, boothW - 12, 22);
                g.endFill();
                g.beginFill(0xa8a288);
                g.drawRect(bx + 6, pY - 22, boothW - 12, 3);
                g.endFill();
                // Glass partition with small opening
                g.beginFill(0xcce6ff, 0.18);
                g.drawRect(bx + 6, pY - 42, boothW - 12, 20);
                g.endFill();
                g.lineStyle(1, 0x7a7a7a, 0.6);
                g.drawRect(bx + 6, pY - 42, boothW - 12, 20);
                g.lineStyle(0);
                // Speaker grille / transaction opening
                g.beginFill(0x1a1a28);
                g.drawRect(bx + boothW / 2 - 8, pY - 30, 16, 5);
                g.endFill();
                // Window number sign
                g.beginFill(accent);
                g.drawRect(bx + boothW / 2 - 8, pY - 48, 16, 4);
                g.endFill();
                // Stamp tray + papers
                g.beginFill(warm, 0.65);
                g.drawRect(bx + 12, pY - 19, 10, 3);
                g.endFill();
                g.beginFill(0xf4efdc);
                g.drawRect(bx + 28, pY - 20, 14, 5);
                g.endFill();
            }
            c.addChild(g);
            ...
            for (let bi = 0; bi < booths; bi++) {
                const bx = sx + 30 + bi * boothW;
                const t = new PIXI.Text(String(bi + 1), {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 8,
                    fill: 0xfffbe8,
                    fontWeight: 'bold',
            ...
            for (const s of stanchions) {
                rg.beginFill(0x7a7a7a);
                rg.drawRect(s.x - 0.8, s.y, 1.6, 8);
                rg.endFill();
                rg.beginFill(0xfbbf24);
                rg.drawCircle(s.x, s.y, 1.6);
                rg.endFill();
            }
            c.addChild(rg);
            ...
            bb.eventMode = 'none';
            bb.beginFill(0x5a3d24);
            bb.drawRect(bbX, bbY, bbW, 26);
            bb.endFill();
            bb.beginFill(0x3a2a18);
            bb.drawRect(bbX + 2, bbY + 2, bbW - 4, 22);
            bb.endFill();
            ...
            country.stamps.forEach((stamp, si) => {
                ...
                bb.beginFill(0xfffbe8);
                bb.drawRect(pX, pY2, (bbW - 14) / 2, 8);
                bb.endFill();
                bb.beginFill(warm, 0.55);
                bb.drawRect(pX + 1, pY2 + 1, (bbW - 14) / 2 - 2, 2);
                bb.endFill();
            });
            c.addChild(bb);
        } else if (key === 'attache') {
            // Office carpet
            g.beginFill(0x2a4a6e);
            g.drawRect(sx, pY - 4, bw, 4);
            g.endFill();
            g.beginFill(0x1a3a5e);
            g.drawRect(sx + 60, pY - 4, bw - 120, 4);
            g.endFill();
            // Policy framework whiteboard — country-specific framework lines
            const wbW = Math.min(180, bw * 0.44),
                wbX = sx + 30,
                wbY = fy + 16;
            g.beginFill(0xd8d1b2);
            g.drawRect(wbX - 3, wbY - 3, wbW + 6, 44);
            g.endFill();
            g.beginFill(0xfffbe8);
            g.drawRect(wbX, wbY, wbW, 38);
            g.endFill();
            // Whiteboard title band
            g.beginFill(accent, 0.92);
            g.drawRect(wbX, wbY, wbW, 10);
            g.endFill();
            c.addChild(g);
            ...
            country.frameworkLines.forEach((line, li) => {
                ...
                lineTxt.anchor.set(0, 0);
                lineTxt.x = wbX + 5;
                lineTxt.y = wbY + 14 + li * 5;
            ...
            for (let di = 0; di < deskCount; di++) {
                ...
                dg.eventMode = 'none';
                // Desk
                dg.beginFill(0x5a3d24);
                dg.drawRect(dx, pY - 14, deskW, 14);
                dg.endFill();
                dg.beginFill(0x7c4a30);
                dg.drawRect(dx, pY - 14, deskW, 2);
                dg.endFill();
                // Monitor
                dg.beginFill(0x1a1a28);
                dg.drawRect(dx + 8, pY - 30, 24, 16);
                dg.endFill();
                dg.beginFill(accent, 0.25);
                dg.drawRect(dx + 10, pY - 28, 20, 12);
                dg.endFill();
                // Monitor stand
                dg.beginFill(0x333344);
                dg.drawRect(dx + 18, pY - 14, 4, 2);
                dg.endFill();
                // Keyboard + mouse
                dg.beginFill(0x222230);
                dg.drawRect(dx + 10, pY - 12, 20, 3);
                dg.endFill();
                dg.beginFill(0x333344);
                dg.drawCircle(dx + 36, pY - 10, 1.5);
                dg.endFill();
                // Papers
                dg.beginFill(0xfffbe8);
                dg.drawRect(dx + 38, pY - 12, 14, 10);
                dg.endFill();
                dg.beginFill(0x1a1a28, 0.25);
                dg.drawRect(dx + 40, pY - 10, 10, 1);
                dg.endFill();
                dg.beginFill(0x1a1a28, 0.25);
                dg.drawRect(dx + 40, pY - 8, 10, 1);
                dg.endFill();
                // Desk light
                dg.beginFill(0x333344);
                dg.drawRect(dx + 2, pY - 18, 2, 4);
                dg.endFill();
                dg.beginFill(0xfbbf24, 0.8);
                dg.drawCircle(dx + 3, pY - 20, 2.5);
                dg.endFill();
            ...
            pr.eventMode = 'none';
            pr.beginFill(0x222230);
            pr.drawRect(sx + bw - 40, pY - 18, 26, 18);
            pr.endFill();
            pr.beginFill(0x333344);
            pr.drawRect(sx + bw - 38, pY - 22, 22, 4);
            pr.endFill();
            pr.beginFill(0xfbbf24, 0.45);
            pr.drawRect(sx + bw - 36, pY - 14, 4, 2);
            pr.endFill();
            pr.beginFill(0x4ade80, 0.65);
            pr.drawRect(sx + bw - 30, pY - 14, 2, 2);
            pr.endFill();
            c.addChild(pr);
        } else if (key === 'ambassador') {
            // Hardwood + grand rug
            g.beginFill(0x5a3d24);
            g.drawRect(sx, pY - 4, bw, 4);
            g.endFill();
            const rugW = bw * 0.72,
                rugX = sx + bw / 2 - rugW / 2;
            ...
            g.drawRect(dx, pY - 20, dw, 20);
            g.endFill();
            g.beginFill(0x4a2e18);
            g.drawRect(dx, pY - 20, dw, 3);
            g.endFill();
            g.beginFill(0x1a0a04, 0.7);
            g.drawRect(dx + 10, pY - 20, dw - 20, 2);
            g.endFill();
            // Leather blotter
            g.beginFill(0x2a4a6e, 0.9);
            g.drawRect(dx + 12, pY - 20, dw - 24, 4);
            g.endFill();
            // Pen holder + papers
            g.beginFill(0x1a0a04);
            g.drawCircle(dx + dw - 18, pY - 20, 2);
            g.endFill();
            g.beginFill(0xfffbe8);
            g.drawRect(dx + 14, pY - 19, 16, 4);
            g.endFill();
            g.beginFill(accent, 0.5);
            g.drawRect(dx + 15, pY - 18, 14, 1);
            g.endFill();
            // Desk name plaque
            g.beginFill(0x1a1a28);
            g.drawRect(dx + dw / 2 - 22, pY - 20, 44, 4);
            g.endFill();
            g.beginFill(0xfbbf24, 0.75);
            g.drawRect(dx + dw / 2 - 20, pY - 19, 40, 2);
            g.endFill();
            c.addChild(g);
            ...
            for (let fi = 0; fi < 2; fi++) {
                ...
                fg.eventMode = 'none';
                fg.beginFill(0x7a7a7a);
                fg.drawRect(fx, pY - 42, 1.6, 42);
                fg.endFill();
                fg.beginFill(0xfbbf24);
                fg.drawCircle(fx + 0.8, pY - 42, 1.5);
                fg.endFill();
                if (fi === 0) {
                    ...
                    const fw = 14,
                        fh = 9;
                    if (typeof EmbassyRow !== 'undefined' && EmbassyRow.drawCountryFlag) {
                        const flagSub = new PIXI.Graphics();
                        flagSub.eventMode = 'none';
                    ...
                    fg.beginFill(0x0a0a14);
                    fg.drawRect(fx - 14, pY - 38, 14, 9);
                    fg.endFill();
                    fg.beginFill(0x22d3ee);
                    fg.drawCircle(fx - 7, pY - 33, 2.5);
                    fg.endFill();
                    fg.lineStyle(0.4, 0x22d3ee, 0.7);
                    fg.drawCircle(fx - 7, pY - 33, 4);
                    fg.lineStyle(0);
                    fg.beginFill(0xfbbf24);
                    fg.drawCircle(fx - 7, pY - 29, 0.6);
                    fg.endFill();
                }
                c.addChild(fg);
            }
            ...
            ch.drawRoundedRect(dx + dw / 2 - 10, pY - 32, 20, 32, 3);
            ch.endFill();
            ch.beginFill(0x4a2e18);
            ch.drawRect(dx + dw / 2 - 9, pY - 32, 18, 4);
            ch.endFill();
            // Chair base
            ch.beginFill(0x1a0a04);
            ch.drawRect(dx + dw / 2 - 2, pY - 4, 4, 4);
            ch.endFill();
            c.addChild(ch);
            ...
            const ct = new PIXI.Graphics();
            ct.eventMode = 'none';
            ct.beginFill(0x3a1f10);
            ct.drawRoundedRect(ctX, ctY, ctW, 12, 2);
            ct.endFill();
            ct.beginFill(0x4a2e18);
            ct.drawRect(ctX, ctY, ctW, 2);
            ct.endFill();
            // Four chairs around
            for (let ci = 0; ci < 4; ci++) {
                const cOff = ctX + 8 + ci * (ctW / 4);
                ct.beginFill(0x2a4a6e);
                ct.drawRect(cOff, ctY + 14, 8, 6);
                ct.endFill();
                ct.beginFill(0x1a3a5e);
                ct.drawRect(cOff, ctY + 14, 8, 2);
                ct.endFill();
            }
            // Teacups on table
            for (let cp = 0; cp < 3; cp++) {
                ct.beginFill(0xfffbe8);
                ct.drawCircle(ctX + 12 + cp * 20, ctY + 4, 2);
                ct.endFill();
                ct.beginFill(0x6b3d26);
                ct.drawCircle(ctX + 12 + cp * 20, ctY + 4, 1);
                ct.endFill();
            }
            c.addChild(ct);
            ...
            const ptX = sx + bw - 110,
                ptY = fy + 14;
            pt.beginFill(0x4a2e18);
            pt.drawRect(ptX, ptY, 60, 40);
            pt.endFill();
            pt.beginFill(0xfbbf24, 0.25);
            pt.drawRect(ptX + 2, ptY + 2, 56, 36);
            pt.endFill();
            pt.beginFill(accent, 0.35);
            pt.drawRect(ptX + 4, ptY + 4, 52, 32);
            pt.endFill();
            // Silhouette head in portrait
            pt.beginFill(0x1a1a28);
            pt.drawEllipse(ptX + 30, ptY + 22, 8, 10);
            pt.endFill();
            pt.beginFill(0x1a1a28);
            pt.drawRect(ptX + 22, ptY + 28, 16, 8);
            pt.endFill();
            c.addChild(pt);
            // Portrait caption
            ...
            for (let row = 0; row < 2; row++) {
                ...
                for (let col = 0; col < 12; col++) {
                    ...
                    bs.beginFill(bkCol);
                    bs.drawRect(bkX, rowY + (12 - bkH), 5, bkH);
                    bs.endFill();
                    bs.beginFill(0xfbbf24, 0.3);
                    bs.drawRect(bkX, rowY + (12 - bkH), 5, 1);
                    bs.endFill();
                }
            }
            c.addChild(bs);
            ...
            ch2.drawRect(sx + bw / 2 - 0.5, fy + 8, 1, 14);
            ch2.endFill();
            ch2.beginFill(0xfbbf24);
            ch2.drawCircle(sx + bw / 2, fy + 24, 6);
            ch2.endFill();
            ch2.beginFill(0xffe9a8, 0.7);
            ch2.drawCircle(sx + bw / 2, fy + 24, 4);
            ch2.endFill();
            for (let arm = 0; arm < 6; arm++) {
                ...
                ch2.beginFill(0xfbbf24);
                ch2.drawCircle(ax, ay, 1.5);
                ch2.endFill();
                ch2.beginFill(0xffe9a8, 0.6);
                ch2.drawCircle(ax, ay, 1);
                ch2.endFill();
            }
            c.addChild(ch2);
            ...
            for (let ix = sx; ix < sx + bw; ix += 40) {
                g.beginFill(0x2a1f12, 0.65);
                g.drawRect(ix, pY - 4, 38, 1);
                g.endFill();
            }
            // Locked filing cabinets along wall
            const cabW = 22,
                cabCount = Math.floor((bw - 60) / (cabW + 4));
            for (let ci = 0; ci < cabCount; ci++) {
                const cx = sx + 30 + ci * (cabW + 4);
                g.beginFill(0x2a2a38);
                g.drawRect(cx, pY - 50, cabW, 50);
                g.endFill();
                g.beginFill(0x3a3a48);
                g.drawRect(cx, pY - 50, cabW, 2);
                g.endFill();
                // 4 drawers
                for (let di = 0; di < 4; di++) {
                    g.beginFill(0x1a1a28);
                    g.drawRect(cx + 1, pY - 48 + di * 12, cabW - 2, 10);
                    g.endFill();
                    // Lock / handle
                    g.beginFill(0xfbbf24, 0.65);
                    g.drawCircle(cx + cabW / 2, pY - 43 + di * 12, 0.8);
                    g.endFill();
                    g.beginFill(0x7a7a7a);
                    g.drawRect(cx + cabW / 2 - 3, pY - 45 + di * 12, 6, 1.5);
                    g.endFill();
                }
                // Label tab
                g.beginFill(warm, 0.55);
                g.drawRect(cx + 3, pY - 48, 6, 2);
                g.endFill();
            }
            // Reading desk in centre
            const rdX = sx + bw / 2 - 40,
                rdY = pY - 14;
            g.beginFill(0x3a1f10);
            g.drawRect(rdX, rdY, 80, 14);
            g.endFill();
            g.beginFill(0x4a2e18);
            g.drawRect(rdX, rdY, 80, 2);
            g.endFill();
            // Green banker's lamp
            g.beginFill(0x333344);
            g.drawRect(rdX + 38, rdY - 12, 4, 12);
            g.endFill();
            g.beginFill(0x14532d);
            g.drawRect(rdX + 30, rdY - 18, 20, 8);
            g.endFill();
            g.beginFill(0x22c55e, 0.6);
            g.drawRect(rdX + 32, rdY - 12, 16, 2);
            g.endFill();
            // Papers + open folder
            g.beginFill(0xfffbe8);
            g.drawRect(rdX + 4, rdY - 4, 20, 8);
            g.endFill();
            g.beginFill(warm, 0.5);
            g.drawRect(rdX + 6, rdY - 3, 16, 1);
            g.endFill();
            // CLASSIFIED stamp on paper
            g.beginFill(warm, 0.8);
            g.drawRect(rdX + 8, rdY - 1, 12, 2);
            g.endFill();
            c.addChild(g);
            // Warning sign on wall
            const ws = new PIXI.Graphics();
            ws.eventMode = 'none';
            ws.beginFill(0x1a1a28);
            ws.drawRect(sx + 20, fy + 14, 80, 18);
            ws.endFill();
            ws.beginFill(warm, 0.85);
            ws.drawRect(sx + 20, fy + 14, 80, 4);
            ws.endFill();
            c.addChild(ws);
            ...
            bu.drawRect(sx + bw / 2 - 0.5, fy + 8, 1, 10);
            bu.endFill();
            bu.beginFill(0xfbbf24);
            bu.drawCircle(sx + bw / 2, fy + 22, 4);
            bu.endFill();
            bu.beginFill(0xffe9a8, 0.7);
    ...
    _drawCountrySilhouette(c, cx, cy, w, h, accent, warm, key) {
        ...
        g.eventMode = 'none';
        // Glow backing
        g.beginFill(accent, 0.06);
        g.drawRoundedRect(cx - w / 2 - 6, cy - h / 2 - 4, w + 12, h + 8, 4);
        g.endFill();
        g.beginFill(accent, 0.12);
        ...
        if (key === 'eagle') {
            ...
            // Head
            g.beginFill(accent, 0.78);
            g.drawCircle(cx, by + h * 0.25, h * 0.12);
            g.endFill();
            // Beak
            g.beginFill(warm);
            g.drawPolygon([cx, by + h * 0.28, cx + h * 0.08, by + h * 0.32, cx, by + h * 0.36]);
            g.endFill();
            ...
            for (let mi = 0; mi < 8; mi++) {
                ...
                g.beginFill(accent, 0.78);
                g.drawRect(mx, by + h * 0.62, w * 0.04, h * 0.08);
                g.endFill();
            }
            ...
            for (let si = 0; si < 12; si++) {
                ...
                g.drawStar ? g.drawStar(sx2, sy2, 5, h * 0.07, h * 0.03) : g.drawCircle(sx2, sy2, h * 0.05);
                g.endFill();
            }
            // Centre "EU" monogram
            g.beginFill(accent, 0.5);
            g.drawCircle(cx, cy, rr * 0.45);
            g.endFill();
        } else if (key === 'big_ben') {
            // Big Ben clock tower
            g.beginFill(accent, 0.78);
            ...
            g.endFill();
            // Clock face
            g.beginFill(warm, 0.7);
            g.drawCircle(cx, by + h * 0.32, h * 0.1);
            g.endFill();
            g.beginFill(0x1a1a28);
            g.drawCircle(cx, by + h * 0.32, h * 0.02);
            g.endFill();
            // Clock hands
            g.lineStyle(1, 0x1a1a28, 0.9);
            g.moveTo(cx, by + h * 0.32);
            g.lineTo(cx, by + h * 0.26);
            g.moveTo(cx, by + h * 0.32);
            g.lineTo(cx + h * 0.06, by + h * 0.32);
            g.lineStyle(0);
            // Spire
            g.beginFill(accent, 0.78);
            ...
            // Central dome
            g.beginFill(accent, 0.78);
            g.drawEllipse(cx, by + h * 0.5, w * 0.16, h * 0.2);
            g.endFill();
            g.beginFill(accent, 0.78);
            g.drawRect(cx - w * 0.18, by + h * 0.5, w * 0.36, h * 0.4);
            g.endFill();
            // Minaret spires (4)
            for (const mx of [bx + w * 0.1, bx + w * 0.28, bx + w * 0.72, bx + w * 0.9]) {
                g.beginFill(accent, 0.78);
                g.drawRect(mx - w * 0.02, by + h * 0.3, w * 0.04, h * 0.6);
                g.endFill();
                g.beginFill(warm, 0.65);
                g.drawEllipse(mx, by + h * 0.28, w * 0.025, h * 0.05);
                g.endFill();
            }
            ...
            g.drawCircle(bx + w * 0.82, by + h * 0.22, h * 0.12);
            g.endFill();
            g.beginFill(accent, 0.08);
            g.drawCircle(bx + w * 0.86, by + h * 0.2, h * 0.11);
            g.endFill();
            // Palm tree
            g.beginFill(0x3a2a18);
            g.drawRect(bx + w * 0.22, by + h * 0.4, w * 0.02, h * 0.25);
            g.endFill();
            for (let fr = 0; fr < 5; fr++) {
                const ang = (fr / 5) * Math.PI - Math.PI / 2;
                g.beginFill(0x166534, 0.85);
                g.drawEllipse(
                    bx + w * 0.23 + Math.cos(ang) * w * 0.05,
                    by + h * 0.38 + Math.sin(ang) * h * 0.08,
                    w * 0.05,
                    h * 0.025
                );
                g.endFill();
    ...
    _getNPCsForFloor(key, bw, accent, warm, country) {
        ...
        if (key === 'reception') {
            ...
            return [{ role: 'Archivist', col: 0x64748b, xOff: bw * 0.5, nightShift: false }];
        }
        return [];
    },
    ...
    drawNPC(c, x, y, role, col, country) {
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined' && UI.showTooltip)
                UI.showTooltip(e, role, country ? country.full : 'Embassy');
        });
        cont.on('pointerout', () => {
                ...
                case 'working': {
                    ...
                    if (av.timer <= 0) {
                        ...
                        if (r < 0.25) {
                            ...
                            const msgs =
                                av._country && av._country.policyMsgs
                                    ? av._country.policyMsgs.concat(GENERIC_MSGS)
                                    : GENERIC_MSGS;
