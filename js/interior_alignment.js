...
const InteriorAlignment = {
    ...
    THEMES: {
        align_miri: {
            ...
            boardLines: [
                'Agent ⊆ Environment',
                'Corrigibility ⇄ optimality',
                'Logical induction',
                'Decision-theoretic self-reference',
                '→ avoid wireheading',
                '→ tile under reflection',
            ],
            researcherTag: 'MIRI',
        ...
        align_metr: {
            ...
            boardLines: [
                'Autonomous replication: 18%',
                'Cyber-offense: 34%',
                'Self-exfiltration: 12%',
                'Resource acquisition: 27%',
                '→ pre-deployment gate',
                '→ elicitation ceiling?',
            ],
            researcherTag: 'METR',
            bubbles: [
                'Elicitation protocol v3…',
                'Capability did not saturate.',
                'Flag this for the eval board.',
            ],
        },
        align_apollo: {
            accent: 0xef4444,
            ...
            boardLines: [
                'Model: knows it is evaluated',
                'Goal: long-term preserve values',
                'Action: strategic underperformance',
                '→ evidence in CoT (redacted)',
                '→ sandbagging confirmed',
                '⚠ DECEPTIVE ALIGNMENT',
            ],
        ...
        align_redwood: {
            ...
            boardLines: [
                'ASSUME: model may subvert',
                'Untrusted monitor → Trusted audit',
                'Red-team vs deployment',
                '→ safety despite subversion',
                'Causal scrubbing: pass',
                'Mechanistic interp: partial',
            ],
        ...
        align_far: {
            ...
            boardLines: [
                'KataGo: superhuman + defeated',
                'Adversarial policy: wins 99%',
                'RLHF: re-jailbreak in 10 shots',
                '→ alignment ≠ robust',
                'Multi-agent fragility',
                '→ need adv. training',
            ],
    ...
    build(bld, layer) {
        ...
        const theme = this.THEMES[bld.id] || {
            accent: 0x6ab868,
            boardTitle: 'SAFETY RESEARCH',
            boardLines: [],
            researcherTag: (bld.name || 'Researcher').toUpperCase(),
            bubbles: ['Thinking…'],
        };
        ...
        for (let tx = 0; tx < W; tx += 22 + Math.random() * 20) {
            ...
            backdrop.drawPolygon([tx - tw * 0.5, ybase, tx, ybase - th, tx + tw * 0.5, ybase]);
            backdrop.endFill();
        }
        ...
        loftG.beginFill(0x4a3526);
        loftG.drawRect(cabinX, loftTop, cabinW, loftH);
        loftG.endFill();
        // Horizontal log planks (back wall)
        for (let ly = loftTop + 6; ly < loftBot; ly += 10) {
            loftG.beginFill(0x3a2818, 0.55);
            loftG.drawRect(cabinX, ly, cabinW, 1);
            loftG.endFill();
            loftG.beginFill(0x5a3e2a, 0.35);
            loftG.drawRect(cabinX, ly + 1, cabinW, 1);
            loftG.endFill();
        }
        ...
        const wbX = cabinX + 40,
            wbY = loftTop + 14,
            wbW = Math.min(280, cabinW * 0.42),
            wbH = loftH - 28;
        const wb = new PIXI.Graphics();
        // Mount frame
        wb.beginFill(0x2a1a0e);
        wb.drawRect(wbX - 4, wbY - 4, wbW + 8, wbH + 8);
        wb.endFill();
        // Whiteboard surface
        wb.beginFill(0xf8f9fb);
        wb.drawRect(wbX, wbY, wbW, wbH);
        wb.endFill();
        // Marker tray
        wb.beginFill(0x9ca3af);
        wb.drawRect(wbX, wbY + wbH, wbW, 3);
        wb.endFill();
        wb.beginFill(0xef4444);
        wb.drawRect(wbX + 8, wbY + wbH, 12, 3);
        wb.endFill();
        wb.beginFill(0x3b82f6);
        wb.drawRect(wbX + 26, wbY + wbH, 12, 3);
        wb.endFill();
        wb.beginFill(0x22c55e);
        wb.drawRect(wbX + 44, wbY + wbH, 12, 3);
        wb.endFill();
        ...
        theme.boardLines.forEach((ln, i) => {
            ...
            const tx = new PIXI.Text(ln, {
                fontFamily: 'JetBrains Mono',
                fontSize: 8,
                fill: colorVariants[i % colorVariants.length],
                letterSpacing: 0.3,
                padding: 2,
            });
            tx.x = wbX + 10;
        ...
        const deskX = cabinX + cabinW - 180,
            deskY = loftBot - 38;
        const desk = new PIXI.Graphics();
        // Desk top
        desk.beginFill(0x5a3e24);
        desk.drawRect(deskX, deskY, 150, 5);
        desk.endFill();
        desk.beginFill(0x4a2e18);
        desk.drawRect(deskX, deskY + 5, 150, 2);
        desk.endFill();
        // Legs
        ...
        desk.beginFill(0x1a1a1a);
        desk.drawRect(deskX + 16, deskY - 28, 48, 28);
        desk.endFill();
        desk.beginFill(0x0a0a0a);
        desk.drawRect(deskX + 18, deskY - 26, 44, 22);
        desk.endFill();
        // CRT glow — flickers with theme accent
        desk.beginFill(accent, 0.85);
        desk.drawRect(deskX + 20, deskY - 24, 40, 18);
        desk.endFill();
        // Tiny scan lines
        for (let sy = deskY - 23; sy < deskY - 7; sy += 3) {
            desk.beginFill(0x000000, 0.25);
            desk.drawRect(deskX + 20, sy, 40, 1);
            desk.endFill();
        }
        // Keyboard
        desk.beginFill(0x2a2a2a);
        desk.drawRect(deskX + 18, deskY - 2, 44, 3);
        desk.endFill();
        // Coffee mug
        desk.beginFill(0xeeeeee);
        desk.drawRect(deskX + 80, deskY - 10, 10, 10);
        desk.endFill();
        desk.beginFill(0x6b3018);
        desk.drawRect(deskX + 81, deskY - 9, 8, 2);
        desk.endFill(); // coffee
        desk.beginFill(0xeeeeee);
        desk.drawRect(deskX + 90, deskY - 7, 2, 4);
        desk.endFill(); // handle
        // Stack of papers
        desk.beginFill(0xf3f4f6);
        desk.drawRect(deskX + 100, deskY - 6, 20, 5);
        desk.endFill();
        desk.beginFill(0xd1d5db);
        desk.drawRect(deskX + 100, deskY - 4, 20, 1);
        desk.endFill();
        desk.beginFill(0xd1d5db);
        desk.drawRect(deskX + 100, deskY - 2, 20, 1);
        desk.endFill();
        ...
        const cbX_ = wbX + wbW + 20,
            cbY_ = loftTop + 22,
            cbW_ = Math.min(120, deskX - cbX_ - 10),
            cbH_ = 70;
        if (cbW_ > 30) {
            const cork = new PIXI.Graphics();
            cork.beginFill(0x8b5a2b);
            cork.drawRect(cbX_ - 3, cbY_ - 3, cbW_ + 6, cbH_ + 6);
            cork.endFill();
            cork.beginFill(0xc89465);
            cork.drawRect(cbX_, cbY_, cbW_, cbH_);
            cork.endFill();
            // Cork dots
            for (let i = 0; i < 18; i++) {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                cork.beginFill(0xf9fafb);
                cork.drawRect(px, py, cbW_ / 3 - 8, 22);
                cork.endFill();
        ...
        ocul.beginFill(0x2a1a0e);
        ocul.drawCircle(oculX, oculY, oculR + 3);
        ocul.endFill();
        ocul.beginFill(0x1f3350);
        ocul.drawCircle(oculX, oculY, oculR);
        ocul.endFill();
        ocul.beginFill(0xfffbe8, 0.8);
        ocul.drawCircle(oculX - 4, oculY - 4, 4);
        ocul.endFill(); // little moon
        // Cross mullion
        ocul.lineStyle(1.5, 0x2a1a0e);
        ocul.moveTo(oculX - oculR, oculY);
        ocul.lineTo(oculX + oculR, oculY);
        ocul.moveTo(oculX, oculY - oculR);
        ocul.lineTo(oculX, oculY + oculR);
        ocul.lineStyle(0);
        this.scene.addChild(ocul);

        // ─── FLOOR SLAB between loft and ground ───
        const slab = new PIXI.Graphics();
        slab.beginFill(0x2a1a0e);
        slab.drawRect(cabinX - 2, loftBot, cabinW + 4, floorSepH);
        slab.endFill();
        slab.beginFill(0x4a3020);
        slab.drawRect(cabinX - 2, loftBot, cabinW + 4, 2);
        slab.endFill();
        slab.beginFill(0x1a0d04);
        slab.drawRect(cabinX - 2, loftBot + floorSepH - 1, cabinW + 4, 1);
        slab.endFill();
        // Plank lines on the slab
        for (let sx = cabinX; sx < cabinX + cabinW; sx += 22) {
            slab.beginFill(0x1a0d04);
            slab.drawRect(sx, loftBot + 2, 1, floorSepH - 3);
            slab.endFill();
        }
        ...
        // Back wall
        groundG.beginFill(0x3d2d1f);
        groundG.drawRect(cabinX, groundTop, cabinW, groundH);
        groundG.endFill();
        for (let ly = groundTop + 6; ly < groundBot; ly += 11) {
            groundG.beginFill(0x2a1a0e, 0.55);
            groundG.drawRect(cabinX, ly, cabinW, 1);
            groundG.endFill();
            groundG.beginFill(0x4e3624, 0.35);
            groundG.drawRect(cabinX, ly + 1, cabinW, 1);
            groundG.endFill();
        }
        // Side walls
        ...
        groundG.beginFill(0x3a2818);
        groundG.drawRect(cabinX, groundBot - 10, cabinW, 10);
        groundG.endFill();
        groundG.beginFill(0x1a0d04);
        groundG.drawRect(cabinX, groundBot - 2, cabinW, 2);
        groundG.endFill();
        // Plank joints
        for (let sx = cabinX + 30; sx < cabinX + cabinW - 10; sx += 60) {
            groundG.beginFill(0x1a0d04);
            groundG.drawRect(sx, groundBot - 10, 1, 8);
            groundG.endFill();
        }
        // Rug in the middle (themed color)
        const rugW = Math.min(260, cabinW * 0.42),
            rugX = cabinX + cabinW / 2 - rugW / 2;
        groundG.beginFill(accent, 0.35);
        groundG.drawRect(rugX, groundBot - 12, rugW, 2);
        groundG.endFill();
        groundG.beginFill(accent, 0.18);
        groundG.drawRect(rugX, groundBot - 14, rugW, 2);
        groundG.endFill();
        ...
        earth.drawRect(0, surfaceY, cabinX - 6, undFloorH);
        earth.drawRect(cabinX + cabinW + 6, surfaceY, W - cabinX - cabinW - 6, undFloorH);
        earth.endFill();
        earth.beginFill(0x141e30);
        earth.drawRect(0, surfaceY, cabinX - 6, 6);
        earth.drawRect(cabinX + cabinW + 6, surfaceY, W - cabinX - cabinW - 6, 6);
        earth.endFill();
        earth.beginFill(0x1a2a3a);
        earth.drawRect(0, surfaceY - 2, cabinX - 6, 4);
        earth.drawRect(cabinX + cabinW + 6, surfaceY - 2, W - cabinX - cabinW - 6, 4);
        earth.endFill();
        ...
        const drawArmchair = (ax, ay, facingRight) => {
            ...
            // Back
            g.beginFill(0x7a1f1f);
            g.drawRect(ax, ay - 22, 26, 22);
            g.endFill();
            g.beginFill(0x9a2f2f);
            g.drawRect(ax + 1, ay - 21, 24, 5);
            g.endFill(); // highlight
            // Seat
            g.beginFill(0x5a1818);
            g.drawRect(ax - 2, ay, 30, 10);
            g.endFill();
            g.beginFill(0x7a1f1f);
            g.drawRect(ax - 2, ay, 30, 3);
            g.endFill();
            // Arm
            if (facingRight) {
                g.beginFill(0x5a1818);
                g.drawRect(ax + 26, ay - 18, 4, 28);
                g.endFill();
            } else {
                g.beginFill(0x5a1818);
                g.drawRect(ax - 4, ay - 18, 4, 28);
                g.endFill();
            }
            // Wooden feet
            g.beginFill(0x2a1a0e);
            g.drawRect(ax - 2, ay + 10, 4, 3);
            g.drawRect(ax + 26, ay + 10, 4, 3);
            g.endFill();
            UI.tip(g, 'Armchair');
        ...
        const drawSideTable = (tx, ty) => {
            ...
            g.drawRect(tx, ty, 18, 3);
            g.endFill();
            g.beginFill(0x2a1a0e);
            g.drawRect(tx + 2, ty + 3, 2, 10);
            g.drawRect(tx + 14, ty + 3, 2, 10);
            g.endFill();
            // Book on top
            g.beginFill(0x1e3a8a);
            g.drawRect(tx + 4, ty - 3, 10, 3);
            g.endFill();
            g.beginFill(0xfbbf24);
            g.drawRect(tx + 4, ty - 3, 10, 1);
            g.endFill();
            UI.tip(g, 'Side Table');
            this.scene.addChild(g);
        };
        ...
        const drawBookshelf = (sx, sy, sw, sh) => {
            ...
            for (let r = 0; r < rows; r++) {
                ...
                while (bkx < sx + sw - 4) {
                    ...
                    g.beginFill(bc);
                    g.drawRect(bkx, ry - bh, bw, bh);
                    g.endFill();
                    g.beginFill(0xfbbf24, 0.5);
                    g.drawRect(bkx, ry - bh + 2, bw, 1);
                    g.endFill(); // title bar
                    bkx += bw + 1;
                }
            }
        ...
        drawBookshelf(
            fpX + fpW + 10,
            groundTop + 20,
            cabinX + cabinW - (fpX + fpW + 10) - shelfGap,
            groundH - 40
        );
        ...
        const drawWindow = (wx, wy, ww, wh) => {
            const g = new PIXI.Graphics();
            g.beginFill(0x2a1a0e);
            g.drawRect(wx - 3, wy - 3, ww + 6, wh + 6);
            g.endFill();
        ...
        doorG.drawRect(doorX - 3, doorY - 3, doorW + 6, doorH + 6);
        doorG.endFill();
        doorG.beginFill(0x4a2e18);
        doorG.drawRect(doorX, doorY, doorW, doorH);
        doorG.endFill();
        doorG.beginFill(0x2a1a0e);
        doorG.drawRect(doorX + doorW / 3 - 1, doorY, 2, doorH);
        doorG.drawRect(doorX + (2 * doorW) / 3 - 1, doorY, 2, doorH);
        doorG.endFill();
        doorG.beginFill(0xfbbf24);
        doorG.drawCircle(doorX + doorW - 5, doorY + doorH / 2, 1.8);
        doorG.endFill();
        this.scene.addChild(doorG);

        // ─── OVERHEAD LAMP (hanging from loft peak) ───
        const lampX = peakX - 120,
            lampY = loftBot + 14;
        const lamp = new PIXI.Graphics();
        lamp.lineStyle(1.5, 0x2a1a0e);
        lamp.moveTo(lampX, loftBot);
        lamp.lineTo(lampX, lampY);
        lamp.lineStyle(0);
        lamp.beginFill(0x3a2818);
        lamp.drawRect(lampX - 9, lampY, 18, 3);
        lamp.endFill();
        lamp.beginFill(0xfbbf24, 0.9);
        lamp.drawRect(lampX - 7, lampY + 3, 14, 7);
        lamp.endFill();
        lamp.beginFill(0xfde68a, 0.85);
        lamp.drawRect(lampX - 6, lampY + 3, 12, 2);
        lamp.endFill();
        // Glow
        lamp.beginFill(0xfbbf24, 0.12);
        lamp.drawCircle(lampX, lampY + 10, 36);
        lamp.endFill();
        ...
        sgnG.beginFill(0x2a1a0e);
        sgnG.drawRect(sgnX - 2, sgnY - 2, sgnW + 4, 12);
        sgnG.endFill();
        sgnG.beginFill(0x5a3a20);
        sgnG.drawRect(sgnX, sgnY, sgnW, 10);
        sgnG.endFill();
        sgnG.beginFill(0x4a2e18);
        sgnG.drawRect(sgnX, sgnY + 8, sgnW, 2);
        sgnG.endFill();
        ...
        this._onMove = (e) => {
            if (
                !InteriorAlignment.isDragging ||
                !InteriorAlignment.scene ||
                InteriorAlignment.scene.destroyed
            )
                return;
    ...
    _spawnResearcher(x, y, accent, role, theme) {
        ...
        body.drawRect(-5, -14, 10, 14);
        body.endFill();
        body.beginFill(shirtCol, 0.7);
        body.drawRect(-5, -14, 10, 2);
        body.endFill();
        cont.addChild(body);
        // Head
        ...
        legs.beginFill(0x1a1a1a);
        legs.drawRect(-4, 0, 3, 6);
        legs.drawRect(1, 0, 3, 6);
        legs.endFill();
        cont.addChild(legs);
        ...
        if (role === 'board') {
            ...
            arm.beginFill(shirtCol);
            arm.drawRect(-8, -12, 4, 8);
            arm.endFill();
            arm.beginFill(skinCol);
            arm.drawCircle(-6, -4, 1.5);
            arm.endFill();
            // Marker
            arm.beginFill(0xef4444);
            arm.drawRect(-8, -5, 4, 1);
            arm.endFill();
            cont.addChild(arm);
        }
        ...
        if (role === 'chair_left' || role === 'chair_right') {
            ...
            book.beginFill(0x1e3a8a);
            book.drawRect(-6, -10, 12, 6);
            book.endFill();
            book.beginFill(0xf9fafb);
            book.drawRect(-5, -9, 10, 4);
            book.endFill();
            book.beginFill(0x94a3b8);
            book.drawRect(-4, -8, 8, 0.5);
            book.drawRect(-4, -7, 7, 0.5);
            book.drawRect(-4, -6, 6, 0.5);
            book.endFill();
            cont.addChild(book);
        ...
        const roleLabels = {
            board: 'Whiteboard Researcher',
            desk: 'Senior Research Engineer',
            chair_left: 'Visiting Fellow',
            chair_right: 'Reading Fellow',
        };
        ...
        if (this.avatars) {
            this.avatars.forEach((av) => {
                ...
                if (av._bubble && !av._bubble.destroyed) {
                    ...
                    av._bubble._life--;
                    av._bubble.alpha =
                        Math.min(1, av._bubble._life / 30) * Math.min(1, (240 - av._bubble._life) / 30);
