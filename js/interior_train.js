...
const InteriorTrain = {
    ...
    _buildCutaway() {
        ...
        const L = this._BODY_L + 3,
            R = this._BODY_R - 3,
            W = R - L; // -177..177
        const T = this._BODY_T + 3,
            B = this._BODY_B - 2; // -32..28
        const seatY = B - 16,
            floorY = B - 7;

        // ── Walls — light slate so the open car reads as an interior, not a void ──
        g.beginFill(0x55677d);
        g.drawRect(L, T, W, B - T);
        g.endFill();
        g.beginFill(0x647a91);
        g.drawRect(L, T, W, 13);
        g.endFill(); // brighter near ceiling
        g.beginFill(0x3d4d60);
        g.drawRect(L, seatY - 4, W, 4);
        g.endFill(); // dado shadow line

        // ── Ceiling: dark rail + warm recessed lights ──
        g.beginFill(0x1e293b);
        g.drawRect(L, T, W, 6);
        g.endFill();
        g.beginFill(0xfde68a, 0.3);
        g.drawRect(L + 10, T + 3, W - 20, 2);
        g.endFill();
        ...
        const mapY = T + 8;
        g.beginFill(0x0ea5e9, 0.65);
        g.drawRect(L + 30, mapY, W - 60, 1.5);
        g.endFill();
        ...
        for (let wx = L + 12; wx + ww < R - 8; wx += pitch) {
            g.beginFill(0x0b111b);
            g.drawRect(wx, winY, ww, wh);
            g.endFill();
            g.beginFill(0x9fd8f5, 0.12);
            g.drawRect(wx + 2, winY + 2, ww - 4, 4);
            g.endFill();
            g.lineStyle(1.5, 0xaeb9c7, 0.9);
            g.drawRect(wx, winY, ww, wh);
            g.lineStyle(0);
