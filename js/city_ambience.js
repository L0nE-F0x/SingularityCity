...
const CityAmbience = {
    ...
    _nightness(dp) {
        if (dp >= 0.86 || dp < 0.2) return 1;
        if (dp >= 0.8) return (dp - 0.8) / 0.06; // dusk ramp-up
        if (dp < 0.28) return 1 - (dp - 0.2) / 0.08; // dawn ramp-down
        return 0;
    },
    ...
    GRADE_KEYS: [
        // [dayPhase, r, g, b]
        [0.0, 0.8, 0.88, 1.1], // deep night — cool blue
        [0.2, 0.82, 0.88, 1.08], // late night
        [0.26, 1.1, 0.94, 0.88], // sunrise — warm gold
        [0.34, 1.02, 1.0, 0.98], // morning
        [0.55, 1.0, 1.0, 1.0], // noon — neutral
        [0.72, 1.04, 0.99, 0.94], // afternoon
        [0.79, 1.16, 0.94, 0.84], // golden hour — amber
        [0.86, 0.86, 0.88, 1.06], // dusk fades to cool
        [1.0, 0.8, 0.88, 1.1], // wraps to night
    ],
    ...
    _applyGrading(dp) {
        ...
        const a = keys[i],
            b2 = keys[Math.min(i + 1, keys.length - 1)];
    ...
    _buildFurniture() {
        ...
        if (!this.furnGfx) {
            ...
            this.trafficA = new PIXI.Graphics();
            this.trafficA.zIndex = 2;
            this.trafficB = new PIXI.Graphics();
            this.trafficB.zIndex = 2;
        ...
        if (pads.length)
            excl.push([
                Math.min(...pads.map((b) => b.x)) - 400,
                Math.max(...pads.map((b) => b.x + b.w)) + 400,
            ]);
        if (typeof PortZone !== 'undefined' && PortZone.oceanStartX !== undefined)
            excl.push([PortZone.oceanStartX - 400, PortZone.coastlineX + 40]);
        ...
        for (let x = 120; x < G.cityW - 80; x += 90) {
            ...
            if (kind < 4) {
                // ── Streetlamp ──
                fg.beginFill(0x2b3444);
                fg.drawRect(px - 1.5, sw - 34, 3, 34);
                fg.endFill();
                fg.beginFill(0x2b3444);
                fg.drawRect(px - 1.5, sw - 34, 9, 2);
                fg.endFill();
                fg.beginFill(0x3d4a5f);
                fg.drawRect(px - 3, sw - 2, 6, 2);
                fg.endFill();
                fg.beginFill(0x475569);
                fg.drawRoundedRect(px + 4, sw - 37, 7, 4, 1.5);
                fg.endFill();
                // Night bulb + ground pool (additive, driven by container alpha)
                const glow = new PIXI.Graphics();
                glow.beginFill(0xffd98a, 0.85);
                glow.drawRoundedRect(px + 4.5, sw - 36.5, 6, 3, 1.5);
                glow.endFill();
                glow.beginFill(0xffcf70, 0.16);
                glow.drawCircle(px + 7.5, sw - 34, 9);
                glow.endFill();
                glow.beginFill(0xffc860, 0.1);
                glow.drawPolygon([px + 2, sw - 33, px + 13, sw - 33, px + 20, sw, px - 5, sw]);
                glow.endFill();
                glow.beginFill(0xffc860, 0.13);
                glow.drawEllipse(px + 7.5, sw + 1, 15, 4);
                glow.endFill();
                glow.blendMode = PIXI.BLEND_MODES.ADD;
                this.glowLayer.addChild(glow);
            } else if (kind < 6) {
                // ── Bench ──
                fg.beginFill(0x6b4a2f);
                fg.drawRect(px - 8, sw - 7, 16, 2.5);
                fg.endFill();
                fg.beginFill(0x5a3d26);
                fg.drawRect(px - 8, sw - 12, 16, 2);
                fg.endFill();
                fg.beginFill(0x2b3444);
                fg.drawRect(px - 7, sw - 5, 2, 5);
                fg.drawRect(px + 5, sw - 5, 2, 5);
                fg.endFill();
            } else if (kind < 7) {
                // ── Hydrant ──
                fg.beginFill(0xb91c1c);
                fg.drawRoundedRect(px - 2.5, sw - 8, 5, 8, 2);
                fg.endFill();
                fg.beginFill(0xdc2626);
                fg.drawCircle(px, sw - 9, 2);
                fg.endFill();
                fg.beginFill(0x991b1b);
                fg.drawRect(px - 4, sw - 6, 8, 1.5);
                fg.endFill();
            } else if (kind < 8 && placed % 3 === 0) {
                // ── Traffic light (animated via A/B overlays) ──
                fg.beginFill(0x1f2733);
                fg.drawRect(px - 1.5, sw - 30, 3, 30);
                fg.endFill();
                fg.beginFill(0x141a24);
                fg.drawRoundedRect(px - 4, sw - 40, 8, 12, 2);
                fg.endFill();
                this.trafficB.beginFill(0xef4444, 0.95);
                this.trafficB.drawCircle(px, sw - 36.5, 1.8);
                this.trafficB.endFill();
                this.trafficA.beginFill(0x4ade80, 0.95);
                this.trafficA.drawCircle(px, sw - 31.5, 1.8);
                this.trafficA.endFill();
            }
            placed++;
        }
        ...
        for (let x = 600; x < G.cityW - 200; x += 1400) {
            ...
            fg.beginFill(0x11161f);
            fg.drawEllipse(px, gy + 10, 7, 2.6);
            fg.endFill();
            fg.beginFill(0x3a4456, 0.8);
            fg.drawEllipse(px, gy + 9.4, 5.5, 1.8);
            fg.endFill();
            fg.beginFill(0x11161f, 0.6);
            fg.drawRect(px - 3.5, gy + 8.6, 7, 0.6);
            fg.drawRect(px - 3.5, gy + 9.8, 7, 0.6);
            fg.endFill();
        ...
        this.steamVents.slice(0, 14).forEach((v, i) => {
            for (let k = 0; k < 2; k++) {
                ...
                g.beginFill(0xcfd8e3, 0.16);
                g.drawCircle(0, 0, 2.5 + this._noise(i * 9 + k) * 2.5);
                g.endFill();
                g.x = v.x;
                g.y = v.y;
                g.alpha = 0;
                ...
                this.steamParts.push({
                    g,
                    vent: v,
                    life: this._noise(i * 3 + k * 7) * 80,
                    maxLife: 90 + k * 30,
                });
            }
        });
    ...
    _buildVignettes() {
        ...
        plat.beginFill(0x8a94a3);
        plat.drawRect(-14, 0, 28, 3);
        plat.endFill();
        plat.beginFill(0x64748b);
        plat.drawRect(-14, -2, 2, 2);
        plat.drawRect(12, -2, 2, 2);
        plat.endFill();
        // two tiny washers
        plat.beginFill(0x2563eb);
        plat.drawRect(-8, -6, 4, 6);
        plat.endFill();
        plat.beginFill(0xf59e0b);
        plat.drawRect(4, -6, 4, 6);
        plat.endFill();
        plat.beginFill(0xfdd8b5);
        plat.drawRect(-7.5, -8, 3, 2.5);
        plat.drawRect(4.5, -8, 3, 2.5);
        plat.endFill();
        ...
        this.sweeps = [];
        [
            ['port_authority', -46, 26, 0x22d3ee],
            ['backbone_ground', 4, 20, 0xa855f7],
        ].forEach(([id, dy, len, col]) => {
            const g = new PIXI.Graphics();
        ...
        for (let i = 0; i < 10; i++) {
            const g = new PIXI.Graphics();
            g.beginFill(0xd9f99d, 0.9);
            g.drawCircle(0, 0, 1.1);
            g.endFill();
            g.blendMode = PIXI.BLEND_MODES.ADD;
            g.zIndex = 4;
            g.alpha = 0;
        ...
        for (let i = 0; i < 4; i++) {
            ...
            wing.beginFill(col, 0.9);
            wing.drawEllipse(-1.5, 0, 1.8, 1.2);
            wing.drawEllipse(1.5, 0, 1.8, 1.2);
            wing.endFill();
            cont.addChild(wing);
            cont.zIndex = 4;
    ...
    _refreshBioRanges() {
        this._bioRanges = BLDS.filter(
            (b) => b.id === 'forest_0' || b.id === 'forest_1' || b.id === 'city_park'
    ...
    update(dp, night) {
        ...
        if (nightness > 0.05) {
            ...
            this.steamParts.forEach((p) => {
                if (p.g && !p.g.destroyed) p.g.alpha = 0;
            });
        }
    ...
    _updateCleaner(dp) {
        ...
        if (!cl.bld || !BLDS.includes(cl.bld)) {
            cl.state = 'idle';
            cl.cont.visible = false;
            return;
        }
