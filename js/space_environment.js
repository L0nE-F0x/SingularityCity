...
const SpaceEnvironment = {
    ...
    buildSpaceBuildings(bldLayer, b, gy) {
        ...
        if (b.type === 'launchpad') {
            ...
            const vehName =
                typeof SpaceRockets !== 'undefined' && b.org ? SpaceRockets.vehicleName(b.org) : null;
            const signLabel = org ? (vehName ? `${org.name} · ${vehName}` : org.name) : b.name;
            const sign = new PIXI.Text(signLabel, {
            ...
            const sign = new PIXI.Text('MISSION CONTROL', {
                fontFamily: 'Silkscreen',
                fontSize: 7,
                fill: 0x0ea5e9,
                align: 'center',
            ...
            const sign = new PIXI.Text('VEHICLE ASSEMBLY', {
                fontFamily: 'Silkscreen',
                fontSize: 7,
                fill: 0x94a3b8,
                align: 'center',
            ...
            gfx.beginFill(0x64748b);
            gfx.drawRect(b.w / 2 - 1.5, 0, 3, 14);
            gfx.endFill(); // fixed mast
            gfx.beginFill(0x475569);
            gfx.drawCircle(b.w / 2, 0, 2.5);
            gfx.endFill(); // yoke hub
            const scanDish = new PIXI.Container();
            const dg = new PIXI.Graphics();
            dg.beginFill(0xf1f5f9);
            dg.drawPolygon([-14, 0, 0, -16, 14, 0]);
            dg.endFill(); // dish bowl
            dg.beginFill(0xcbd5e1);
            dg.drawPolygon([-9, -1, 0, -12, 9, -1]);
            dg.endFill(); // inner
            dg.beginFill(0x475569);
            dg.drawRect(-1, -11, 2, 8);
            dg.endFill(); // feed strut
            dg.beginFill(0xef4444);
            dg.drawCircle(0, -11, 1.6);
            dg.endFill(); // feed horn tip
            scanDish.addChild(dg);
            scanDish.x = b.w / 2;
            scanDish.y = 0; // pivot at the mast top
            ...
            const sign = new PIXI.Text('ORBITAL TRACKING', {
                fontFamily: 'Silkscreen',
                fontSize: 7,
                fill: 0x22d3ee,
                align: 'center',
