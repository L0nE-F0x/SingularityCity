...
const JailData = {
    ...
    detectViewer() {
        ...
        try {
            fetch('/api/geo', { signal: AbortSignal.timeout(4000) })
                ...
                .catch(() => {
                    /* offline / local / 404 → keep the heuristic */
                });
        } catch (_e) {
            /* AbortSignal.timeout unsupported → keep the heuristic */
        }
    },
    ...
    _countryFromLocale() {
        try {
            ...
            if (/^America\/(New_York|Chicago|Denver|Los_Angeles|Phoenix|Detroit|Anchorage|Indiana)/.test(tz))
                return 'US';
            const langs =
    ...
    _ruleAppliesToViewer(rule) {
        ...
        if (scope.countries)
            return !!this.viewerCountry && scope.countries.indexOf(this.viewerCountry) !== -1;
        return true;
    },
    ...
    _NEWS_MATCHERS: [
        ...
        {
            id: 'gpt',
            re: /\b(chatgpt|gpt-?\d|openai)\b/,
            test: (m) => m.lab === 'openai' || /\b(gpt|chatgpt)\b/i.test(m.name || ''),
        },
        {
            id: 'claude',
            re: /\bclaude\b/,
            test: (m) => m.lab === 'anthropic' || /claude/i.test(m.name || ''),
        },
        ...
        { id: 'qwen', re: /\bqwen\b/, test: (m) => /qwen/i.test(m.name || '') },
        {
            id: 'mistral',
            re: /\bmistral\b/,
            test: (m) => m.lab === 'mistral' || /mistral/i.test(m.name || ''),
        },
    ],
    ...
    _COUNTRY_NAMES: [
        ...
        {
            re: /\bunited states\b|\bu\.s\.a?\.?\b|\busa\b|\bamerica\b|\bamerican\b/,
            code: 'US',
            name: 'United States',
        },
    ],
    ...
    _deriveNewsRules() {
        ...
        if (typeof API === 'undefined' || !Array.isArray(API.regulationNews) || !API.regulationNews.length)
            return [];
        const out = [];
        const seen = new Set();
        for (const item of API.regulationNews) {
            ...
            if (/\b(worldwide|globally|global ban|every country|all countries)\b/.test(low)) {
                ...
                if (c) {
                    scope = c.code === 'EU' ? { countries: this._EU.slice() } : { countries: [c.code] };
                    label = c.name;
                    code = c.code;
                }
            }
        ...
        try {
            ...
            this._remoteBans = data
                ...
                .map((r) => {
                    ...
                    if (r.match_name) {
                        try {
                            nameRe = new RegExp(r.match_name, 'i');
                        } catch (_e) {
                            nameRe = null;
                        }
                    }
            ...
            /* offline / no table → seed + news rules still cover us; retry next poll */
        } finally {
    ...
    refreshBanned() {
        ...
        G.models.forEach((m) => {
            ...
            if (hit && jailHere) {
                if (!m._jailed)
                    newlyJailed.push({ name: m.name, label: hit.label, authority: hit.authority });
                m._jailed = true;
        ...
        if (this._everScanned && newlyJailed.length > 0 && typeof UI !== 'undefined') {
            if (newlyJailed.length <= 3) {
                newlyJailed.forEach((n) => {
                    ...
                    if (UI.addLog)
                        UI.addLog(`🔒 ${n.name} transferred to the AI Detention Center (${n.authority}).`);
                });
            } else {
                if (UI.addToast)
                    UI.addToast(
                        `⛓️ ${newlyJailed.length} models detained at the AI Detention Center (${newlyJailed[0].label})`
                    );
                if (UI.addLog)
                    UI.addLog(`🔒 ${newlyJailed.length} models transferred to the AI Detention Center.`);
            }
        }
    ...
    isJailed(id) {
        return this._jailedIds.indexOf(id) !== -1;
    },
...
const JailEnv = {
    buildBuilding(g, bld, h) {
        ...
        // ── Concrete block body ──
        g.beginFill(0x5b6270);
        g.drawRect(0, 0, bw, h);
        g.endFill();
        g.beginFill(0x4b515d);
        ...
        for (let r = 0; r < winRows; r++) {
            ...
            for (let cx = 16; cx < bw - 22; cx += 40) {
                g.beginFill(0x0b0d12);
                g.drawRect(cx, wy, 18, 14);
                g.endFill();
        ...
        g.beginFill(0x33373f);
        g.drawRect(0, h - 14, bw, 14);
        g.endFill();
        g.beginFill(0x2a2d34);
        g.drawRect(0, h - 4, bw, 4);
        g.endFill();

        // ── Sally-port gate (heavy barred entrance) ──
        const dw = 42,
            dx = bw / 2 - dw / 2;
        g.beginFill(0x121419);
        g.drawRect(dx, h - 36, dw, 36);
        g.endFill();
        g.beginFill(0x2a2d34);
        g.drawRect(dx - 3, h - 40, dw + 6, 5);
        g.endFill(); // lintel
        g.beginFill(0x8a93a3);
        ...
        g.endFill();
        // gate warning light
        g.beginFill(0xef4444, 0.9);
        g.drawCircle(bw / 2, h - 42, 2.5);
        g.endFill();
        g.beginFill(0xef4444, 0.25);
        g.drawCircle(bw / 2, h - 42, 5);
        g.endFill();

        // ── Signage band (stencil red, no text — city label system shows the name) ──
        const sgY = 8,
            sgW = Math.min(120, bw - 40),
            sgX = bw / 2 - sgW / 2;
        g.beginFill(0x14161c);
        g.drawRect(sgX, sgY, sgW, 12);
        g.endFill();
        ...
        const tx = bw - 22;
        g.beginFill(0x3f4450);
        g.drawRect(tx, -44, 18, h + 44);
        g.endFill(); // tower shaft
        g.beginFill(0x2f333c);
        g.drawRect(tx, -44, 4, h + 44);
        g.endFill(); // shaded side
        g.beginFill(0x4a5160);
        g.drawRect(tx - 6, -54, 30, 14);
        g.endFill(); // cabin
        g.beginFill(0x10131a);
        g.drawRect(tx - 3, -51, 24, 8);
        g.endFill(); // cabin glass
        g.beginFill(0x9fb4cf, 0.45);
        g.drawRect(tx - 2, -50, 22, 3);
        g.endFill(); // glass glint
        g.beginFill(0x23262e);
        g.moveTo(tx - 8, -54);
        g.lineTo(tx + 9, -60);
        g.lineTo(tx + 26, -54);
        g.closePath();
        ...
        g.beginFill(0xfff6cc, 0.1);
        g.moveTo(tx - 2, -47);
        g.lineTo(tx - 70, h - 4);
        g.lineTo(tx - 30, h - 4);
        g.closePath();
        g.endFill();
        g.beginFill(0xfff2b0, 0.9);
        g.drawCircle(tx - 2, -47, 2.5);
        g.endFill();
        // red beacon atop the tower
        g.beginFill(0xef4444, 0.95);
        g.drawCircle(tx + 9, -61, 2);
        g.endFill();
        g.beginFill(0xef4444, 0.3);
        g.drawCircle(tx + 9, -61, 4);
        g.endFill();

        // overall right-edge shadow
        g.beginFill(0x000000, 0.15);
        g.drawRect(bw, 4, 4, h - 4);
        g.endFill();
    },

    update() {
        /* exterior is static; animation lives in the interior */
    },
};
...
const JailInterior = {
    ...
    build(bld, layer) {
        ...
        rc.addChild(lt);
        const bdg = new PIXI.Text('🔒 ' + inmates.length + ' DETAINED', {
            fontFamily: 'JetBrains Mono',
            fontSize: 8,
            fill: 0xfca5a5,
            letterSpacing: 2,
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            if (!isB) {
                ...
                while (cwx + 40 <= windowX + windowW) {
                    wfr.beginFill(0x05060a);
                    wfr.drawRect(cwx, fy + 24, 40, 28);
                    wfr.endFill();
                    wfr.beginFill(steelCol);
                    wfr.drawRect(cwx + 9, fy + 24, 2, 28);
                    wfr.drawRect(cwx + 19, fy + 24, 2, 28);
                    wfr.drawRect(cwx + 29, fy + 24, 2, 28);
                    wfr.endFill();
                    cwx += 60;
                }
                this.scene.addChild(wfr);
            } else {
                rg.beginFill(0x121017);
                rg.drawRect(startX, fy, usableW, floorH);
                rg.endFill();
            }

            rg.beginFill(0x0c0d12);
            rg.drawRect(startX, fy + floorH - 8, usableW, 8);
            rg.endFill();
            rg.beginFill(0x20232c);
            rg.drawRect(startX, fy, usableW, 4);
            rg.endFill();
            rg.beginFill(0x2a2d38);
            ...
            for (let li = 1; li <= 4; li++) {
                ...
                this.indoorLights.push({ g: bm, maxA: isB ? 0.1 : 0.07, type: isB ? 'blink' : 'screen' });
            }
        ...
        this._onMove = (e) => {
            if (!JailInterior.isDragging || !JailInterior.scene || JailInterior.scene.destroyed) return;
            let ny = JailInterior._startSceneY + (e.clientY - JailInterior._startY);
            ny = Math.max(JailInterior.minY, Math.min(ny, JailInterior.maxY));
            JailInterior.scene.y = ny;
        };
        this._onUp = () => {
            JailInterior.isDragging = false;
            if (JailInterior.layer) JailInterior.layer.cursor = 'grab';
        };
    ...
    _drawCellBlock(c, sx, uw, pY, fy, fh, col, steel, n, inmates, label) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            g.beginFill(0x0a0b10);
            g.drawRect(cx + 3, pY - fh + 24, cellW - 6, fh - 28);
            g.endFill();
            // cot
            g.beginFill(0x2c2a34);
            g.drawRect(cx + 8, pY - 16, cellW * 0.42, 7);
            g.endFill();
            g.beginFill(0x3a3744);
            g.drawRect(cx + 8, pY - 18, cellW * 0.42, 2);
            g.endFill();
            // steel toilet/basin hint
            g.beginFill(0x3a3f4a);
            g.drawRect(cx + cellW - 18, pY - 15, 9, 11);
            g.endFill();
            g.beginFill(0x4a505c);
            g.drawRect(cx + cellW - 18, pY - 15, 9, 2);
            g.endFill();
            // divider wall between cells
            g.beginFill(0x191b22);
            g.drawRect(cx, pY - fh + 24, 3, fh - 24);
            g.endFill();
        }
        c.addChild(g);
        ...
        inmates.forEach((m, i) => {
            if (i >= n) return;
            const cx = sx + i * cellW;
            this._inmate(c, cx + cellW * 0.5, pY - 4, m);
        });
        ...
        for (let i = 0; i < n; i++) {
            ...
            bars.beginFill(0xfbbf24);
            bars.drawCircle(cx + cellW - 11, pY - 30, 1.6);
            bars.endFill();
            bars.beginFill(i < inmates.length ? 0xef4444 : 0x3a3f4a);
            bars.drawCircle(cx + 10, pY - fh + 22, 1.8);
            bars.endFill();
        }
        c.addChild(bars);
    ...
    _drawSolitary(c, sx, uw, pY, fy, fh, col, steel, inmates) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            g.beginFill(0x070809);
            g.drawRect(cx + 6, pY - fh + 24, cellW - 12, fh - 28);
            g.endFill();
            g.beginFill(0x1a0e0e);
            g.drawRect(cx, pY - fh + 24, 6, fh - 24);
            g.endFill();
            // solid blast door with tiny slot
            g.beginFill(0x2a2d34);
            g.drawRect(cx + cellW - 16, pY - fh + 30, 12, fh - 34);
            g.endFill();
            g.beginFill(0x05060a);
            g.drawRect(cx + cellW - 14, pY - 30, 8, 4);
            g.endFill(); // food slot
            g.beginFill(0xef4444, 0.5);
            g.drawRect(cx + cellW - 13, pY - fh + 34, 6, 2);
            g.endFill();
        }
        c.addChild(g);
        inmates.forEach((m, i) => {
            if (i >= n) return;
            const cx = sx + i * cellW;
            this._inmate(c, cx + cellW * 0.42, pY - 4, m);
        });
    ...
    _drawIntake(c, sx, uw, pY, fy, fh, col, total, overflow) {
        ...
        g.beginFill(0x2a2d34);
        g.drawRect(sx + 24, pY - 52, 6, 52);
        g.drawRect(sx + 60, pY - 52, 6, 52);
        g.drawRect(sx + 24, pY - 56, 42, 6);
        g.endFill();
        g.beginFill(0x4ade80, 0.5);
        g.drawRect(sx + 30, pY - 30, 30, 2);
        g.endFill();
        // booking desk with monitor
        g.beginFill(0x23262e);
        g.drawRect(sx + uw / 2 - 50, pY - 18, 100, 18);
        g.endFill();
        g.beginFill(0xef4444, 0.3);
        g.drawRect(sx + uw / 2 - 50, pY - 20, 100, 3);
        g.endFill();
        g.beginFill(0x0e0f16);
        g.drawRect(sx + uw / 2 - 20, pY - 36, 32, 16);
        g.endFill();
        g.beginFill(0xef4444, 0.2);
        g.drawRect(sx + uw / 2 - 18, pY - 34, 28, 12);
        g.endFill();
        // mugshot height chart on the wall
        const mhX = sx + uw - 120,
            mhY = pY - 58;
        g.beginFill(0x1b1d26);
        g.drawRect(mhX, mhY, 96, 52);
        g.endFill();
        g.beginFill(0x3a3f4a);
        for (let r = 0; r < 6; r++) g.drawRect(mhX + 4, mhY + 6 + r * 8, 88, 1);
        g.endFill();
        for (let s = 0; s < 4; s++) {
            g.beginFill(0x05060a);
            g.drawRect(mhX + 8 + s * 22, mhY + 8, 16, 36);
            g.endFill();
            g.beginFill(0xfca5a5, 0.2);
            g.drawRect(mhX + 10 + s * 22, mhY + 12, 12, 12);
            g.endFill();
        }
        c.addChild(g);
        ...
        const info = new PIXI.Text(
            total + ' held' + (overflow > 0 ? '  ·  +' + overflow + ' in holding' : ''),
            { fontFamily: 'JetBrains Mono', fontSize: 8, fill: 0xfca5a5, letterSpacing: 1 }
        );
        ...
        const juris = new PIXI.Text(
            'JURISDICTION: ' + (vc || 'GLOBAL ONLY') + '  ·  bans shown apply where you are connecting from',
            { fontFamily: 'JetBrains Mono', fontSize: 6, fill: 0x94a3b8, letterSpacing: 1 }
        );
        juris.anchor.set(0.5, 0);
        juris.x = sx + uw / 2;
        juris.y = pY - fh + 38;
        juris.zIndex = 10;
        c.addChild(juris);
    ...
    _drawControlRoom(c, sx, uw, pY, fy, fh, col) {
        ...
        const mwX = sx + 20,
            ...
            mwW = uw - 160;
        g.beginFill(0x0e0f16);
        g.drawRect(mwX, mwY, mwW, 56);
        g.endFill();
        const cols = Math.max(3, Math.floor(mwW / 70));
        for (let r = 0; r < 2; r++)
            for (let cc = 0; cc < cols; cc++) {
                const mx = mwX + 6 + cc * ((mwW - 12) / cols),
                    my = mwY + 6 + r * 26;
                g.beginFill(0x161a24);
                g.drawRect(mx, my, (mwW - 12) / cols - 6, 22);
                g.endFill();
                g.beginFill(0xef4444, 0.16);
                g.drawRect(mx + 2, my + 2, (mwW - 12) / cols - 10, 18);
                g.endFill();
                g.beginFill(0xef4444, 0.5);
                g.drawRect(mx + 4, my + 5, ((mwW - 12) / cols - 14) * Math.random(), 1.5);
                g.endFill();
            }
        // control console
        g.beginFill(0x23262e);
        g.drawRect(sx + uw - 120, pY - 22, 100, 22);
        g.endFill();
        for (let b = 0; b < 8; b++) {
            g.beginFill([0xef4444, 0xfbbf24, 0x4ade80][b % 3], 0.7);
            g.drawCircle(sx + uw - 110 + b * 12, pY - 14, 2.2);
            g.endFill();
        }
        c.addChild(g);
    ...
    _inmate(c, x, y, m) {
        ...
        body.drawRect(-bw / 2, Math.max(bodyH, 4) * 0.4, bw, 3);
        body.endFill(); // lab ID stripe
        body.beginFill(0x000000, 0.25);
        body.drawRect(-bw / 2, Math.max(bodyH, 4) * 0.18, bw, 1.5);
        body.endFill(); // jumpsuit seam
        body.y = -h + headH;
        cont.addChild(body);
        ...
        head.drawCircle(-bw * 0.1, headH * 0.4, eyeS);
        head.drawCircle(bw * 0.1, headH * 0.4, eyeS);
        head.endFill();
        head.beginFill(0x000000, 0.35);
        head.drawRect(-bw * 0.12, headH * 0.64, bw * 0.24, 1.4);
        head.endFill(); // frown
        head.y = -h;
        cont.addChild(head);
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined')
                UI.showTooltip(e, m.name, 'Detained · ' + (m._jailLabel || 'Suspended'));
        });
        cont.on('pointerout', () => {
    ...
    _guard(c, x, y, name, col) {
        ...
        const lw = Math.max(2, bw * 0.25),
            lh = 4;
        const legL = new PIXI.Graphics();
        legL.beginFill(0x1f2530);
        legL.drawRect(-lw / 2, 0, lw, lh);
        legL.endFill();
        legL.x = -bw * 0.15;
        cont.addChild(legL);
        const legR = new PIXI.Graphics();
        legR.beginFill(0x1f2530);
        legR.drawRect(-lw / 2, 0, lw, lh);
        legR.endFill();
        legR.x = bw * 0.15;
        cont.addChild(legR);
        ...
        body.drawRoundedRect(-bw / 2, 0, bw, Math.max(bodyH, 4), bw * 0.1);
        body.endFill();
        body.beginFill(0x0f1620);
        body.drawRect(-bw / 2, 1, bw, 2);
        body.endFill(); // duty belt / vest line
        body.y = -h + headH;
        cont.addChild(body);
        const head = new PIXI.Graphics();
        head.beginFill(0xfdd8b5);
        head.drawRoundedRect(-bw * 0.4, 0, bw * 0.8, headH, headH * 0.25);
        head.endFill();
        head.beginFill(0x1f2530);
        head.drawRect(-bw * 0.42, 0, bw * 0.84, 2.5);
        head.endFill(); // cap brim
        const eyeS = Math.max(1, bw * 0.08);
        head.beginFill(0x2c1810);
        head.drawCircle(-bw * 0.1, headH * 0.42, eyeS);
        head.drawCircle(bw * 0.1, headH * 0.42, eyeS);
        head.endFill();
        head.y = -h;
        cont.addChild(head);
        ...
        const npcModel = {
            ...
            phase: 'released',
            lab: 'other',
            desc: 'Detention staff — keeping suspended models contained.',
        };
        cont.eventMode = 'static';
        cont.cursor = 'pointer';
        cont.hitArea = new PIXI.Rectangle(-bw, -h - 10, bw * 2, h + 14);
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Detention Staff');
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
        });
    ...
    update() {
        ...
        if (typeof JailData !== 'undefined' && G.tick % 120 === 0) {
            ...
            if (live.length !== this._inmates.length) {
                this.build(this.bld, this.layer);
                return;
            }
        }
        ...
        this.indoorLights.forEach((l) => {
            ...
            if (l.type === 'blink')
                l.g.alpha = l.maxA * (0.45 + Math.abs(Math.sin(G.tick * 0.06 + l.maxA)) * 0.55);
