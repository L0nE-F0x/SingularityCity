...
const Environment = {
    ...
    isRainy() {
        return this._WEATHER_INFO[this.weather]?.category === 'rainy';
    },
    isSnowing() {
        return this.weather === 'snow';
    },
    isCloudy() {
        const c = this._WEATHER_INFO[this.weather]?.category;
        return c === 'cloudy' || c === 'rainy' || c === 'snowy';
    },
    isWet() {
        return this.isRainy();
    }, // alias — for reflection/trail decisions
    hasPrecipitation() {
        return !!this._WEATHER_INFO[this.weather]?.hasParticles;
    },
    ...
    init(layers) {
        ...
        this.starsLayer = layers.starsLayer;
        this.celestialGfx = layers.celestialGfx;
        this.cloudLayer = layers.cloudLayer;
        this.bldLayer = layers.bldLayer;
        this.groundGfx = layers.groundGfx;
        this.reflectionLayer = layers.reflectionLayer;
        this.staticLightsGfx = layers.staticLightsGfx;
        this.lightLayer = layers.lightLayer;
        this.fxGfx = layers.fxGfx;
        this.buildStars();
        this.buildGround();
        this.buildDataPulses();
        this.buildClouds();
        this.buildBuildings();
        ...
        setTimeout(() => {
            ...
            UI.addToast(
                `${icons[this.climate] || '🌍'} Climate: ${labels[this.climate] || this.climate} (${this.hemisphere === 'S' ? 'Southern' : 'Northern'} Hemisphere)`
            );
        }, 1500);
    },
    ...
    buildStars() {
        ...
        for (let i = 0; i < 100; i++) {
            this._starData.push({
                ...
                y: this._labNoise(i * 5 + 2) * G.vpH * 0.5,
                sz: 0.5 + this._labNoise(i * 5 + 3) * 1.5,
                phase: this._labNoise(i * 5 + 4) * Math.PI * 2,
                baseAlpha: 0.15 + this._labNoise(i * 5 + 5) * 0.5,
            });
        }
    ...
    _drawStars(tick) {
        ...
        for (let i = 0; i < stars.length; i++) {
            const s = stars[i];
            const a = t > 0 ? 0.15 + Math.abs(Math.sin(t * 0.03 + s.phase)) * 0.5 : s.baseAlpha;
    ...
    buildGround() {
        ...
        g.drawRect(-4000, gy + 170, G.cityW + 8000, 3000);
        g.endFill();
        ...
        if (hasPortZone) {
            portStartX = Math.max(0, portStartX - 80);
            portEndX += 40;
        }

        // ─── Determine power zone X range ───
        let powerStartX = Infinity,
            powerEndX = 0;
        BLDS.forEach((b) => {
        ...
        const drawCityTerrain = (startX, endX) => {
            ...
            g.drawRect(startX, gy - 24, w, 24);
            g.endFill();
            g.beginFill(0x33334a);
            g.drawRect(startX, gy - 24, w, 12);
            g.endFill();
            g.beginFill(0x44445a);
            g.drawRect(startX, gy - 24, w, 2);
            g.endFill();
            for (let x = startX; x < endX; x += 30) {
                g.beginFill(0x3a3a50, 0.3);
                g.drawRect(x, gy - 22, 1, 20);
                g.endFill();
            }
            g.beginFill(0x1e1e32);
            g.drawRect(startX, gy, w, 32);
            g.endFill();
            g.beginFill(0x22223a);
            g.drawRect(startX, gy, w, 16);
            g.endFill();
            g.beginFill(0x2a2a3e);
            g.drawRect(startX, gy, w, 2);
            g.endFill();
            for (let x = startX; x < endX; x += 40) {
                g.beginFill(0x50506a);
                g.drawRect(x, gy + 14, 20, 3);
                g.endFill();
            }
            g.beginFill(0x50506a, 0.3);
            g.drawRect(startX, gy + 2, w, 1);
            g.endFill();
            g.beginFill(0x50506a, 0.3);
            g.drawRect(startX, gy + 29, w, 1);
            g.endFill();
        };
        ...
        if (hasPortZone) {
            ...
            g.drawRect(psx, gy - 24, pw, 24);
            g.endFill();
            g.beginFill(0x0c1e3a);
            g.drawRect(psx, gy - 18, pw, 18);
            g.endFill();
            g.beginFill(0x1a5a8a, 0.25);
            g.drawRect(psx, gy - 6, pw, 4);
            g.endFill();
            // Wooden dock planks
            g.beginFill(0x5a4a3a);
            g.drawRect(portStartX, gy - 3, portEndX - portStartX, 5);
            g.endFill();
            for (let dx = portStartX; dx < portEndX; dx += 12) {
                g.beginFill(0x4a3a2a);
                g.drawRect(dx, gy - 3, 1, 5);
                g.endFill();
            }
            // Dock posts + bollards
            for (let pp = portStartX + 30; pp < portEndX; pp += 60) {
                g.beginFill(0x3a2a1a);
                g.drawRect(pp, gy + 2, 5, 14);
                g.endFill();
            }
            for (let bb = portStartX + 50; bb < portEndX - 40; bb += 80) {
                g.beginFill(0xfbbf24);
                g.drawCircle(bb, gy - 5, 3);
                g.endFill();
            }
            // Water SOLID FILL covering ALL underground
            g.beginFill(0x061220);
            g.drawRect(psx, gy, pw, 600);
            g.endFill();
            g.beginFill(0x0e2240, 0.5);
            g.drawRect(psx, gy, pw, 8);
            g.endFill();
            for (let wx = psx; wx < psx + pw; wx += 18) {
                g.beginFill(0x2a7aaa, 0.12);
                g.drawRect(wx, gy + 2, 10, 1);
                g.endFill();
            }
            // ─── COASTLINE (port → desert) ───
            const coastX = portEndX - 10;
            g.beginFill(0x8a7a5a);
            g.drawRect(coastX, gy - 6, 20, 22);
            g.endFill();
            g.beginFill(0xa8956a);
            g.drawRect(coastX + 20, gy - 4, 25, 20);
            g.endFill();
            g.beginFill(0xc4a872);
            g.drawRect(coastX + 45, gy - 3, 30, 18);
            g.endFill();
            g.beginFill(0xd4b882, 0.5);
            g.drawRect(coastX + 75, gy - 2, 25, 16);
            g.endFill();
            g.beginFill(0xffffff, 0.12);
            g.drawRect(coastX - 2, gy - 2, 5, 1);
            g.endFill();
            // Palm trees (proper polygon fronds)
            for (let ptx = coastX + 18; ptx < coastX + 65; ptx += 25) {
                g.beginFill(0x6b4226);
                g.moveTo(ptx + 4, gy - 4);
                g.lineTo(ptx + 1, gy - 28);
                g.lineTo(ptx - 2, gy - 50);
                g.lineTo(ptx + 2, gy - 50);
                g.lineTo(ptx + 5, gy - 28);
                g.lineTo(ptx + 8, gy - 4);
                g.closePath();
                g.endFill();
                g.beginFill(0x7a5030, 0.4);
                for (let ty = gy - 46; ty < gy - 8; ty += 6) {
                    g.drawRect(ptx - 1, ty, 6, 2);
                }
                g.endFill();
                ...
                g.drawCircle(ptx + 1, gy - 47, 2);
                g.drawCircle(ptx + 4, gy - 46, 2);
                g.endFill();
            }
        }
        ...
        if (hasPowerZone) {
            ...
            g.beginFill(0x2a2a42);
            g.drawRect(powerStartX, gy - 24, pw, 24);
            g.endFill();
            g.beginFill(0x33334a);
            g.drawRect(powerStartX, gy - 24, pw, 12);
            g.endFill();
            g.beginFill(0x44445a);
            g.drawRect(powerStartX, gy - 24, pw, 2);
            g.endFill();
            // Sidewalk cracks
            for (let x = powerStartX; x < powerStartX + pw; x += 30) {
                g.beginFill(0x3a3a50, 0.3);
                g.drawRect(x, gy - 22, 1, 20);
                g.endFill();
            }
            // Road
            g.beginFill(0x1e1e32);
            g.drawRect(powerStartX, gy, pw, 32);
            g.endFill();
            g.beginFill(0x22223a);
            g.drawRect(powerStartX, gy, pw, 16);
            g.endFill();
            g.beginFill(0x2a2a3e);
            g.drawRect(powerStartX, gy, pw, 2);
            g.endFill();
            // Road dashes
            for (let x = powerStartX; x < powerStartX + pw; x += 40) {
                g.beginFill(0x50506a);
                g.drawRect(x, gy + 14, 20, 3);
                g.endFill();
            }
            g.beginFill(0x50506a, 0.3);
            g.drawRect(powerStartX, gy + 2, pw, 1);
            g.endFill();
            g.beginFill(0x50506a, 0.3);
            g.drawRect(powerStartX, gy + 29, pw, 1);
            g.endFill();
            ...
            for (let sx = powerStartX + 110; sx < powerEndX + 100; sx += 25 + this._labNoise(sx) * 20) {
                if (!powerBlds.some((b) => sx > b.x - 5 && sx < b.x + b.w + 5)) {
                    g.beginFill(0x5a5530, 0.4);
                    g.drawRect(sx, gy - 8, 1, 4 + this._labNoise(sx + 1) * 4);
                    g.drawRect(sx + 3, gy - 6, 1, 3 + this._labNoise(sx + 2) * 3);
                    g.endFill();
                }
            }
            // Chain link fence at zone entrance
            g.beginFill(0x6b7280, 0.3);
            g.drawRect(powerStartX + 100, gy - 22, 2, 18);
            g.drawRect(powerStartX + 100, gy - 20, 20, 1);
            g.endFill();
        }
        ...
        if (hasBackboneZone) {
            ...
            g.beginFill(0x0c1525);
            g.drawRect(backboneStartX, gy - 24, bkw, 24);
            g.endFill();
            g.beginFill(0x111e30);
            g.drawRect(backboneStartX, gy - 24, bkw, 12);
            g.endFill();
            g.beginFill(0x1a2a40);
            g.drawRect(backboneStartX, gy - 24, bkw, 2);
            g.endFill();
            // Fiber glow lines embedded in sidewalk
            for (let fx = backboneStartX; fx < backboneEndX; fx += 50) {
                g.beginFill(0x22d3ee, 0.08);
                g.drawRect(fx, gy - 20, 25, 1);
                g.endFill();
                g.beginFill(0x4ade80, 0.06);
                g.drawRect(fx + 30, gy - 16, 15, 1);
                g.endFill();
            }
            // Dark road
            g.beginFill(0x0a1220);
            g.drawRect(backboneStartX, gy, bkw, 32);
            g.endFill();
            g.beginFill(0x0e1828);
            g.drawRect(backboneStartX, gy, bkw, 16);
            g.endFill();
            // Cyan fiber center line (data running under road)
            g.beginFill(0x22d3ee, 0.12);
            g.drawRect(backboneStartX, gy + 14, bkw, 3);
            g.endFill();
            g.beginFill(0x4ade80, 0.08);
            g.drawRect(backboneStartX, gy + 12, bkw, 1);
            g.endFill();
            // Road dashes
            for (let x = backboneStartX; x < backboneEndX; x += 40) {
                g.beginFill(0x1a3050, 0.5);
                g.drawRect(x, gy + 14, 20, 3);
                g.endFill();
            }
            // Edge glow accents
            g.beginFill(0x22d3ee, 0.06);
            g.drawRect(backboneStartX, gy + 2, bkw, 1);
            g.endFill();
            g.beginFill(0x22d3ee, 0.06);
            g.drawRect(backboneStartX, gy + 29, bkw, 1);
            g.endFill();
        }
        ...
        if (hasAgentsZone) {
            ...
            g.beginFill(0x0e0a1a);
            g.drawRect(agentsStartX, gy - 24, agw, 24);
            g.endFill();
            g.beginFill(0x14101e);
            g.drawRect(agentsStartX, gy - 24, agw, 12);
            g.endFill();
            g.beginFill(0x1e1530);
            g.drawRect(agentsStartX, gy - 24, agw, 2);
            g.endFill();
            // Data flow lines in sidewalk (rose/amber)
            for (let fx = agentsStartX; fx < agentsEndX; fx += 50) {
                g.beginFill(0xf43f5e, 0.06);
                g.drawRect(fx, gy - 20, 25, 1);
                g.endFill();
                g.beginFill(0xfbbf24, 0.05);
                g.drawRect(fx + 30, gy - 16, 15, 1);
                g.endFill();
            }
            // Dark road
            g.beginFill(0x0a0818);
            g.drawRect(agentsStartX, gy, agw, 32);
            g.endFill();
            g.beginFill(0x0e0c1e);
            g.drawRect(agentsStartX, gy, agw, 16);
            g.endFill();
            // Rose-tinted center line
            g.beginFill(0xf43f5e, 0.1);
            g.drawRect(agentsStartX, gy + 14, agw, 3);
            g.endFill();
            g.beginFill(0xa855f7, 0.06);
            g.drawRect(agentsStartX, gy + 12, agw, 1);
            g.endFill();
            // Road dashes
            for (let x = agentsStartX; x < agentsEndX; x += 40) {
                g.beginFill(0x1e1535, 0.5);
                g.drawRect(x, gy + 14, 20, 3);
                g.endFill();
            }
            // Edge accents
            g.beginFill(0xf43f5e, 0.05);
            g.drawRect(agentsStartX, gy + 2, agw, 1);
            g.endFill();
            g.beginFill(0xf43f5e, 0.05);
            g.drawRect(agentsStartX, gy + 29, agw, 1);
            g.endFill();
        }
        ...
        if (hasSpaceZone || hasPortZone) {
            ...
            let cursor = -2000;
            if (hasPortZone) {
                segments.push([cursor, portStartX]);
                cursor = portEndX;
            }
            if (hasSpaceZone) {
                segments.push([cursor, spaceStartX]);
                cursor = spaceEndX;
            }
            ...
            segments.forEach(([s, e]) => {
                if (e > s) {
                    g.beginFill(0x0a0a0f);
                    g.drawRect(s, gy + 32, e - s, 38);
                    g.endFill();
                }
            });
        } else {
            g.beginFill(0x0a0a0f);
            g.drawRect(-2000, gy + 32, G.cityW + 4000, 38);
            g.endFill();
        }
        ...
        const drawCityH = (y, h, col, alpha) => {
            ...
            zones.forEach(([zs, ze]) => {
                if (cursor < zs) {
                    g.beginFill(col, a);
                    g.drawRect(cursor, y, zs - cursor, h);
                    g.endFill();
                }
                cursor = ze;
            });
            g.beginFill(col, a);
            g.drawRect(cursor, y, G.cityW + 4000 - cursor, h);
            g.endFill();
        };
        // Helper: is X in a non-city zone?
        const inSpecialZone = (x) =>
            (hasSpaceZone && x >= spaceStartX && x <= spaceEndX) ||
            (hasPortZone && x >= portStartX && x <= portEndX) ||
            (hasPowerZone && x >= powerStartX && x <= powerEndX) ||
            (hasBackboneZone && x >= backboneStartX && x <= backboneEndX) ||
            (hasAgentsZone && x >= agentsStartX && x <= agentsEndX);
        ...
        g.beginFill(0x060a14);
        g.drawRect(cableStartX, gy + 32, cableEndX - cableStartX, 38);
        g.endFill();
        ...
        const cityBlds = BLDS.filter(
            (b) =>
                !b.id.startsWith('backbone_') &&
                !b.id.startsWith('port_') &&
                !b.id.startsWith('power_') &&
                !b.id.startsWith('space_') &&
                !b.id.startsWith('agents_') &&
                !inSpecialZone(b.x + b.w / 2) &&
                b.x > cableStartX &&
                b.x < cableEndX
        );
        cityBlds.forEach((bb) => {
            ...
            g.drawRect(cx - 4, gy + 32, 8, 20);
            g.endFill();
            g.beginFill(0x22d3ee, 0.2);
            g.drawRect(cx - 2, gy + 34, 4, 16);
            g.endFill();
            g.beginFill(0x1a2a40);
            g.drawRect(cx - 6, gy + 30, 12, 5);
            g.endFill();
            g.beginFill(0x22d3ee, 0.4);
            g.drawCircle(cx - 2, gy + 32, 1.2);
            g.endFill();
            g.beginFill(0x4ade80, 0.4);
            g.drawCircle(cx + 2, gy + 32, 1.2);
            g.endFill();
        });
        ...
        let tunnelEndX = mLong ? mLong.x + mLong.w / 2 + 160 : mEast ? mEast.x + mEast.w / 2 + 160 : G.cityW;
        ...
        let rockSeed = 99;
        const rRand = () => {
            rockSeed = (rockSeed * 16807) % 2147483647;
            return (rockSeed - 1) / 2147483646;
        };
        ...
        for (let px = -500; px < G.cityW + 500; px += 200) {
            ...
            g.drawRect(px, gy + 175, 15, 40);
            g.endFill();
            g.beginFill(0x0ea5e9);
            g.drawRect(px + 50, gy + 218, 10, 12);
            g.endFill();
            g.beginFill(0xf59e0b);
            g.drawRect(px + 100, gy + 233, 10, 16);
            g.endFill();
        }
        ...
        if (hasPortZone) {
            ...
            g.beginFill(0x061220);
            g.drawRect(portStartX - 20, gy + 32, pw + 40, 250);
            g.endFill();
            // Water gradient layers
            g.beginFill(0x081830, 0.8);
            g.drawRect(portStartX - 20, gy + 32, pw + 40, 40);
            g.endFill();
            g.beginFill(0x0a2040, 0.5);
            g.drawRect(portStartX - 20, gy + 70, pw + 40, 30);
            g.endFill();
            // Sandy ocean floor
            g.beginFill(0x2a2218);
            g.drawRect(portStartX - 20, gy + 230, pw + 40, 20);
            g.endFill();
            g.beginFill(0x3a3228, 0.5);
            g.drawRect(portStartX - 20, gy + 225, pw + 40, 8);
            g.endFill();
            // Coral reef patches
            let coralSeed = 77;
            const cr = () => {
                coralSeed = (coralSeed * 16807) % 2147483647;
                return (coralSeed - 1) / 2147483646;
            };
            const coralCols = [
                0xff6b6b, 0xff9a76, 0xffd166, 0xa8e6cf, 0xf4845f, 0xf78ca0, 0x7ec8e3, 0xc5a3ff,
            ];
        ...
        if (hasBackboneZone) {
            ...
            g.beginFill(0x060a14);
            g.drawRect(backboneStartX - 2, gy + 32, bkw + 4, 38);
            g.endFill();
            ...
            g.drawRect(backboneStartX, gy + 170, bkw, 10);
            g.endFill();
            ...
            for (let di = 0; di < 3; di++) {
                ...
                g.drawRect(backboneStartX, dy, bkw, 1);
                g.endFill();
            }
            ...
            for (let jx = backboneStartX + 40; jx < backboneEndX; jx += 180) {
                g.beginFill(0x334155);
                g.drawRect(jx, gy + 175, 12, 40);
                g.endFill();
                g.beginFill(0x22d3ee, 0.5);
                g.drawCircle(jx + 6, gy + 178, 1.5);
                g.endFill();
                g.beginFill(0x0ea5e9);
                g.drawRect(jx + 50, gy + 218, 8, 12);
                g.endFill();
                g.beginFill(0xf59e0b);
                g.drawRect(jx + 100, gy + 233, 8, 16);
                g.endFill();
            }
        }
        ...
        if (hasAgentsZone) {
            ...
            g.beginFill(0x06040e);
            g.drawRect(agentsStartX - 2, gy + 32, agw + 4, 38);
            g.endFill();
            ...
            for (let fi = 0; fi < 10; fi++) {
                ...
                g.beginFill(col, 0.3 + this._labNoise(fi + 901) * 0.2);
            ...
            agBlds.forEach((ab) => {
                ...
                g.beginFill(0x1a1530);
                g.drawRect(cx - 4, gy + 32, 8, 20);
                g.endFill();
                g.beginFill(0xf43f5e, 0.15);
                g.drawRect(cx - 2, gy + 34, 4, 16);
                g.endFill();
                g.beginFill(0x1a1530);
                g.drawRect(cx - 6, gy + 30, 12, 5);
                g.endFill();
                g.beginFill(0xf43f5e, 0.4);
                g.drawCircle(cx - 2, gy + 32, 1.2);
                g.endFill();
                g.beginFill(0xa855f7, 0.4);
                g.drawCircle(cx + 2, gy + 32, 1.2);
                g.endFill();
            });
            g.beginFill(0x06040e);
            g.drawRect(agentsStartX, gy + 170, agw, 10);
            g.endFill();
            for (let di = 0; di < 3; di++) {
                const dy = gy + 192 + di * 8;
                const col = agentCols[di % agentCols.length];
                g.beginFill(col, 0.12);
                g.drawRect(agentsStartX, dy, agw, 1);
                g.endFill();
            }
        }
        ...
        if (hasPowerZone) {
            ...
            g.drawRect(termX, gy + 33, termW, termH);
            g.endFill();
            g.beginFill(0x252540);
            g.drawRect(termX + 1, gy + 34, termW - 2, 2);
            g.endFill(); // top edge highlight
            g.beginFill(0x101020);
            g.drawRect(termX + 1, gy + 33 + termH - 2, termW - 2, 2);
            g.endFill(); // bottom edge
            // Vertical divider lines (panel look)
            g.beginFill(0x0a0a18);
            g.drawRect(termX + 5, gy + 36, 1, termH - 6);
            g.endFill();
            g.beginFill(0x0a0a18);
            g.drawRect(termX + 10, gy + 36, 1, termH - 6);
            g.endFill();
            // Status LEDs on terminal
            const ledCols = [0x22d3ee, 0x4ade80, 0xf43f5e, 0xfacc15, 0x8b5cf6, 0x3b82f6];
            for (let li = 0; li < 6; li++) {
                g.beginFill(ledCols[li], 0.8);
                g.drawCircle(termX + 3 + (li % 3) * 5, gy + 40 + Math.floor(li / 3) * 8, 1.2);
                g.endFill();
            }
            ...
            for (let si = 0; si < 5; si++) {
                ...
                g.beginFill(sc, 0.4);
                g.drawRect(termX - 6, gy + 37 + si * 6, 8, 2);
                g.endFill();
            }
            // Mounting bolts
            g.beginFill(0x444460);
            g.drawCircle(termX + 2, gy + 34, 1.5);
            g.endFill();
            g.beginFill(0x444460);
            g.drawCircle(termX + termW - 2, gy + 34, 1.5);
            g.endFill();
            g.beginFill(0x444460);
            g.drawCircle(termX + 2, gy + 33 + termH - 1, 1.5);
            g.endFill();
            g.beginFill(0x444460);
            g.drawCircle(termX + termW - 2, gy + 33 + termH - 1, 1.5);
            g.endFill();
            // "JUNCTION" label (tiny)
            // Ground-level terminal cap (visible on the road surface)
            g.beginFill(0x2a2a3a);
            g.drawRect(termX - 2, gy + 30, termW + 4, 4);
            g.endFill();
            g.beginFill(0xfbbf24, 0.3);
            g.drawRect(termX, gy + 30, termW, 1);
            g.endFill(); // caution stripe

            // Solid fill covers metro tunnel, data cables, everything
            g.beginFill(0x0a0a0f);
            g.drawRect(powerStartX, gy + 32, ppw, 38);
            g.endFill();
            g.beginFill(0x2d1a11);
            g.drawRect(powerStartX, gy + 70, ppw, 200);
            g.endFill();
            // Rock texture
            let prs = 77;
            const prr = () => {
                prs = (prs * 16807) % 2147483647;
                return (prs - 1) / 2147483646;
            };
            for (let rx = powerStartX; rx < powerStartX + ppw; rx += 14) {
                for (let ry = gy + 70; ry < gy + 270; ry += 14) {
                    if (prr() > 0.5) {
                        g.beginFill(prr() > 0.5 ? 0x3d261a : 0x1f100a, 0.7);
                        g.drawRect(rx + prr() * 8, ry + prr() * 8, 2 + prr() * 3, 2 + prr() * 2);
                        g.endFill();
                    }
                }
            }
            ...
            g.drawRect(powerStartX, gy + 220, ppw, 8);
            g.endFill();
            g.beginFill(0x0284c7);
            g.drawRect(powerStartX, gy + 222, ppw, 4);
            g.endFill();
            // Power trunk (orange at gy+235 — matches city)
            g.beginFill(0xb45309);
            g.drawRect(powerStartX, gy + 235, ppw, 12);
            g.endFill();
            g.beginFill(0xd97706);
            g.drawRect(powerStartX, gy + 237, ppw, 8);
            g.endFill();
            // Vertical risers
            ...
            pwrBlds.forEach((pb) => {
                ...
                g.beginFill(0x0284c7, 0.4);
                g.drawRect(cx - 2, gy + 32, 3, 188);
                g.endFill();
                g.beginFill(0x0369a1, 0.6);
                g.drawRect(cx - 4, gy + 218, 7, 5);
                g.endFill();
                // Power riser (orange, from gy+32 down to power trunk at gy+235)
                g.beginFill(0xd97706, 0.5);
                g.drawRect(cx + 10, gy + 32, 4, 203);
                g.endFill();
                g.beginFill(0xb45309, 0.7);
                g.drawRect(cx + 8, gy + 233, 8, 6);
                g.endFill();
                g.beginFill(0xfbbf24, 0.5);
                g.drawCircle(cx + 12, gy + 236, 3);
                g.endFill();
            });
        }
        ...
        if (window.BLDS) {
            ...
            window.BLDS.forEach((b) => {
                const isSpace =
                    ...
                    b.id === 'mission_control' ||
                    b.id === 'space_assembly' ||
                    b.id === 'tracking_station' ||
                    b.id === 'forest_space';
                if (
        ...
        if (window.BLDS) {
            window.BLDS.forEach((b) => {
                blocked.push({ l: b.x + 5, r: b.x + b.w - 5 });
            });
        }
        ...
        for (let tx = 20; tx < G.cityW; tx += 45) {
            ...
            g.drawEllipse(tx, gy - 18, sz + 4, 3);
            g.endFill();

            if (isWilderness && sr() > 0.2) {
                g.beginFill(0x4a2e1a);
                g.drawRect(tx - 2, gy - 24 - sz * 1.5, 4, sz * 1.5 + 6);
                g.endFill();
                ...
                if (tx < 450) {
                    ...
                    g.drawRect(tx - 2, cy, 4, sz * 1.5);
                    g.endFill();
                    g.beginFill(0xffb7c5);
                    g.drawCircle(tx, cy - sz, sz * 1.2);
                    g.drawCircle(tx - sz * 0.8, cy - sz * 0.5, sz);
                    g.drawCircle(tx + sz * 0.8, cy - sz * 0.5, sz);
                    g.endFill();
                    g.beginFill(0xff99a8, 0.5);
                    g.drawCircle(tx, cy - sz, sz * 0.8);
                    g.endFill();
                } else {
                    g.beginFill(0x3d2914);
                    g.drawRect(tx - 2, cy, 4, sz * 1.5);
                    g.endFill();
                    g.beginFill(0x2d6a4f);
                    g.drawCircle(tx, cy - sz, sz * 1.3);
                    g.endFill();
                    g.beginFill(0x1b4332);
                    g.drawCircle(tx - sz * 0.5, cy - sz * 0.2, sz * 0.8);
                    g.endFill();
                }
            } else {
                g.beginFill(0x4a2e1a);
                ...
                g.beginFill(0x1b4332);
                g.drawEllipse(tx - 3, cy + 2, sz + 1, sz - 1);
                g.endFill();
                g.beginFill(0x2d6a4f);
                g.drawEllipse(tx + 2, cy - 1, sz, sz + 1);
                g.endFill();
                g.beginFill(0x3d8a5f);
                g.drawEllipse(tx - 1, cy - 2, sz - 1, sz - 2);
                g.endFill();
                g.beginFill(0x4ade80, 0.12);
                g.drawCircle(tx - 2, cy - 3, 1.5);
                g.drawCircle(tx + 3, cy - 1, 1);
                g.endFill();
            }
        }
        ...
        for (let lx = 45; lx < G.cityW; lx += 95) {
            ...
            g.drawRect(lx - 3, gy - 24, 6, 3);
            g.endFill();
            g.beginFill(0x444444);
            g.drawRect(lx - 1.5, gy - 58, 3, 34);
            g.endFill();
            g.beginFill(0x505050);
            g.drawRect(lx - 1, gy - 58, 2, 34);
            g.endFill();
            if (isRight) {
                g.beginFill(0x555555);
                g.drawRect(lx - 1, gy - 60, 8, 2);
                g.endFill();
                g.beginFill(0x606060);
                g.drawRect(lx + 4, gy - 62, 6, 4);
                g.endFill();
                g.beginFill(0xffeaa7, 0.9);
                g.drawRect(lx + 5, gy - 59, 4, 3);
                g.endFill();
            } else {
                g.beginFill(0x555555);
                g.drawRect(lx - 7, gy - 60, 8, 2);
                g.endFill();
                g.beginFill(0x606060);
                g.drawRect(lx - 10, gy - 62, 6, 4);
                g.endFill();
                g.beginFill(0xffeaa7, 0.9);
                g.drawRect(lx - 9, gy - 59, 4, 3);
                g.endFill();
            }
        }
        ...
        if (window.BLDS)
            BLDS.forEach((b) => {
                poleBlocked.push({ l: b.x - 35, r: b.x + b.w + 35 });
            });
        const forestBlds = BLDS
            ? BLDS.filter((b) => b.id === 'forest_0' || b.id === 'forest_1' || b.id === 'forest_space')
            : [];
        forestBlds.forEach((f) => {
            poleBlocked.push({ l: f.x - 40, r: f.x + f.w + 40 });
        });
        ...
        for (let px = poleStart; px < poleEnd; px += 120) {
            ...
            g.beginFill(0x5a4a3a);
            g.drawRect(px - 2, gy - 58, 4, 34);
            g.endFill();
            g.beginFill(0x6a5a4a);
            g.drawRect(px - 1, gy - 58, 2, 34);
            g.endFill();
            g.beginFill(0x5a4a3a);
            g.drawRect(px - 10, gy - 56, 20, 2);
            g.endFill();
            g.beginFill(0xd1d5db);
            g.drawRect(px - 9, gy - 58, 2, 3);
            g.drawRect(px + 7, gy - 58, 2, 3);
            g.endFill();
            g.beginFill(0x5a4a3a);
            g.drawRect(px - 7, gy - 48, 14, 2);
            g.endFill();
            g.beginFill(0xd1d5db);
            g.drawRect(px - 6, gy - 50, 2, 3);
            g.drawRect(px + 4, gy - 50, 2, 3);
            g.endFill();
        }
        for (let i = 0; i < polePositions.length - 1; i++) {
            const x1 = polePositions[i],
                x2 = polePositions[i + 1];
            if (x2 - x1 > 400) continue;
            g.lineStyle(1, 0x4b5563, 0.35);
            const sagT = Math.min(6, (x2 - x1) * 0.02);
            g.moveTo(x1 + 8, gy - 55);
            g.quadraticCurveTo((x1 + x2) / 2, gy - 55 + sagT, x2 - 8, gy - 55);
            g.moveTo(x1 - 8, gy - 55);
            g.quadraticCurveTo((x1 + x2) / 2, gy - 54 + sagT, x2 + 8, gy - 55);
            const sagB = Math.min(5, (x2 - x1) * 0.018);
            g.moveTo(x1 + 5, gy - 47);
            g.quadraticCurveTo((x1 + x2) / 2, gy - 47 + sagB, x2 - 5, gy - 47);
            g.moveTo(x1 - 5, gy - 47);
    ...
    buildDataPulses() {
        ...
        if (hasSpaceZone)
            startX = Math.max(startX, SpaceEnvironment.zoneEndX || SpaceEnvironment.zoneStartX + 800);
    ...
    buildClouds() {
        ...
        for (let i = 0; i < numClouds; i++) {
            ...
            c.alpha = 0.1 + this._labNoise(i * 3 + 3) * 0.06;
            c._i = i;
    ...
    _drawVCEmblem(gfx, b, cx, cy, bc) {
        switch (b.id) {
            case 'vcrow_horizon': // Sequoia — a redwood tree
                gfx.beginFill(0x5a3a22);
                gfx.drawRect(cx - 1.5, cy - 2, 3, 10);
                gfx.endFill();
                gfx.beginFill(bc);
            ...
            case 'vcrow_titan': // SoftBank — the twin silver bars ("=")
                gfx.beginFill(bc);
                gfx.drawRoundedRect(cx - 12, cy - 4, 24, 3, 1.5);
                gfx.drawRoundedRect(cx - 12, cy + 2, 24, 3, 1.5);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.4);
                gfx.drawRect(cx - 12, cy - 4, 24, 1);
                gfx.endFill();
                break;
            case 'vcrow_mgx': // MGX — a gold diamond
                gfx.beginFill(bc);
                gfx.drawPolygon([cx, cy - 10, cx + 8, cy, cx, cy + 10, cx - 8, cy]);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.35);
                gfx.drawPolygon([cx, cy - 10, cx + 8, cy, cx, cy]);
                gfx.endFill();
                break;
            case 'vcrow_thrive': // Thrive — an upward growth chevron
                gfx.lineStyle(3, bc, 1);
                gfx.moveTo(cx - 9, cy + 5);
                gfx.lineTo(cx - 2, cy - 4);
                gfx.lineTo(cx + 3, cy + 1);
                gfx.lineTo(cx + 10, cy - 7);
                gfx.lineStyle(0);
                gfx.beginFill(bc);
                gfx.drawPolygon([cx + 10, cy - 7, cx + 4, cy - 7, cx + 10, cy - 1]);
                gfx.endFill();
                break;
            case 'vcrow_foundersfund': // Founders Fund — a rising rocket arrow
                ...
                gfx.endFill();
                gfx.beginFill(0xef4444, 0.9);
                gfx.drawPolygon([cx, cy + 1, cx + 2, cy + 7, cx - 2, cy + 7]);
                gfx.endFill();
                break;
            case 'vcrow_launchpad': // Y Combinator — the orange square with a Y
                gfx.beginFill(bc);
                gfx.drawRoundedRect(cx - 9, cy - 9, 18, 18, 3);
                gfx.endFill();
                gfx.lineStyle(2, 0xffffff, 0.95);
                gfx.moveTo(cx - 4, cy - 4);
                gfx.lineTo(cx, cy + 1);
                gfx.moveTo(cx + 4, cy - 4);
                gfx.lineTo(cx, cy + 1);
                gfx.moveTo(cx, cy + 1);
            ...
            case 'vcrow_apex': // a16z — bold monogram bar
                gfx.beginFill(bc);
                gfx.drawRoundedRect(cx - 12, cy - 6, 24, 12, 2);
                gfx.endFill();
                gfx.beginFill(0x0c1420);
                gfx.drawRect(cx - 8, cy - 2, 3, 5);
                gfx.drawRect(cx - 1, cy - 2, 3, 5);
                gfx.drawRect(cx + 6, cy - 2, 3, 5);
                gfx.endFill();
                break;
    ...
    _labStyleFor(labId, lab) {
        ...
        if (region === 'eu' || region === 'uk' || region === 'fr')
            return ['euro', 'setback', 'monolith'][hsh % 3];
    ...
    _drawLabEmblem(gfx, labId, cx, cy, colHex, s = 1) {
        switch (labId) {
            ...
            case 'anthropic': // minimal geometric "A"
                ...
                gfx.endFill();
                gfx.beginFill(0xd97757);
                gfx.drawRect(cx - 2.6 * s, cy + 0.5 * s, 5.2 * s, 2.2 * s);
                gfx.endFill();
                break;
            case 'google': // four dots
                [
                    ...
                    gfx.beginFill(c);
                    gfx.drawCircle(cx + ox * s, cy, 1.9 * s);
                    gfx.endFill();
                });
                break;
            ...
            case 'microsoft': // 2×2 squares
                gfx.beginFill(0xf25022);
                gfx.drawRect(cx - 6 * s, cy - 6 * s, 5.4 * s, 5.4 * s);
                gfx.endFill();
                gfx.beginFill(0x7fba00);
                gfx.drawRect(cx + 0.6 * s, cy - 6 * s, 5.4 * s, 5.4 * s);
                gfx.endFill();
                gfx.beginFill(0x00a4ef);
                gfx.drawRect(cx - 6 * s, cy + 0.6 * s, 5.4 * s, 5.4 * s);
                gfx.endFill();
                gfx.beginFill(0xffb900);
                gfx.drawRect(cx + 0.6 * s, cy + 0.6 * s, 5.4 * s, 5.4 * s);
                gfx.endFill();
                break;
            case 'deepseek': // whale swoosh
                ...
                gfx.endFill();
                gfx.beginFill(0x0a1420);
                gfx.drawEllipse(cx - 1 * s, cy + 2.2 * s, 4.5 * s, 2 * s);
                gfx.endFill();
                break;
            case 'mistral': // stepped pixel-M in orange gradient
                ...
                gfx.beginFill(0xfa500f);
                gfx.drawRect(cx - 7 * s, cy + 6 * s, 14 * s, 2 * s);
                gfx.endFill();
                break;
            case 'nvidia': {
                // eye swoosh
                ...
                gfx.lineStyle(0);
                gfx.beginFill(0x76b900);
                gfx.drawCircle(cx + 1 * s, cy, 2.2 * s);
                gfx.endFill();
                break;
            }
            case 'amazon': {
            ...
            case 'ibm': // horizontal bars
                ...
                for (let r = 0; r < 4; r++)
                    gfx.drawRect(cx - 6.5 * s, cy + (-6 + r * 3.4) * s, 13 * s, 1.8 * s);
                gfx.endFill();
                break;
            default:
                // No emblem for unknown labs — the plaque shows the lab color chip instead.
                gfx.beginFill(colHex, 0.9);
                gfx.drawCircle(cx, cy, 3 * s);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.35);
                gfx.drawCircle(cx - 1 * s, cy - 1 * s, 1.2 * s);
                gfx.endFill();
        }
    },
        ...
        switch (b.id) {
            case 'agents_orchestrator': // conductor node — hub with spokes
                ...
                for (let i = 0; i < 6; i++) {
                    const a = (i * Math.PI) / 3;
                    gfx.moveTo(cx, cy);
                    gfx.lineTo(cx + Math.cos(a) * 9, cy + Math.sin(a) * 9);
                }
                gfx.lineStyle(0);
                gfx.beginFill(ac);
                gfx.drawCircle(cx, cy, 3.5);
                gfx.endFill();
                for (let i = 0; i < 6; i++) {
                    const a = (i * Math.PI) / 3;
                    gfx.beginFill(ac, 0.8);
                    gfx.drawCircle(cx + Math.cos(a) * 9, cy + Math.sin(a) * 9, 1.8);
                    gfx.endFill();
                }
                break;
            case 'agents_toolshop': // puzzle piece
                gfx.beginFill(ac);
                gfx.drawRoundedRect(cx - 8, cy - 6, 16, 12, 2);
                gfx.endFill();
                gfx.beginFill(ac);
                gfx.drawCircle(cx + 8, cy, 3);
                gfx.endFill();
                gfx.beginFill(0x0c0a18);
                gfx.drawCircle(cx - 3, cy, 3);
                gfx.endFill();
                break;
            case 'agents_sandbox': // trophy / benchmark bars
                gfx.beginFill(ac);
                gfx.drawRect(cx - 9, cy + 4, 5, -6);
                gfx.drawRect(cx - 2, cy + 4, 5, -11);
                gfx.drawRect(cx + 5, cy + 4, 5, -8);
                gfx.endFill();
                gfx.beginFill(0xfbbf24);
                gfx.drawCircle(cx + 7.5, cy - 8, 1.6);
                gfx.endFill();
                break;
            case 'agents_deploy': // rocket
                gfx.beginFill(ac);
                gfx.drawPolygon([cx, cy - 9, cx + 4, cy + 3, cx, cy, cx - 4, cy + 3]);
                gfx.endFill();
                gfx.beginFill(0xef4444, 0.9);
                gfx.drawPolygon([cx, cy + 1, cx + 2, cy + 7, cx - 2, cy + 7]);
                gfx.endFill();
            ...
            case 'agents_memory': // brain / memory rings
                gfx.lineStyle(1.5, ac, 0.9);
                gfx.drawCircle(cx, cy, 8);
                gfx.drawCircle(cx, cy, 4.5);
                gfx.lineStyle(0);
                gfx.beginFill(ac);
                gfx.drawCircle(cx, cy, 2);
                gfx.endFill();
                break;
            default:
                gfx.beginFill(ac);
    ...
    _drawLongevityMotif(gfx, b, ac, h) {
        ...
        gfx.beginFill(0x060d14, 0.82);
        gfx.drawRoundedRect(cx - 22, cy - 20, 44, 40, 5);
        gfx.endFill();
        gfx.lineStyle(1, ac, 0.5);
        gfx.drawRoundedRect(cx - 22, cy - 20, 44, 40, 5);
        gfx.lineStyle(0);
        switch (b.id) {
            case 'longevity_protein': {
                // AlphaFold ribbon — folded chain of nodes
                gfx.lineStyle(2, ac, 0.9);
                for (let i = 0; i < 7; i++) {
                    const nx = cx - 15 + i * 5,
                        ny = cy + Math.sin(i * 1.1) * 11;
                    if (i === 0) gfx.moveTo(nx, ny);
                    else gfx.lineTo(nx, ny);
                }
                gfx.lineStyle(0);
                for (let i = 0; i < 7; i++) {
                    const nx = cx - 15 + i * 5,
                        ny = cy + Math.sin(i * 1.1) * 11;
                    gfx.beginFill(i % 2 ? 0x22d3ee : ac);
                    gfx.drawCircle(nx, ny, 2);
                    gfx.endFill();
                }
                break;
            }
            case 'longevity_discovery': {
                ...
                for (const [x, y] of pts) {
                    gfx.beginFill(ac);
                    gfx.drawCircle(x, y, 2.2);
                    gfx.endFill();
                }
                gfx.beginFill(0x22d3ee, 0.6);
                gfx.drawCircle(cx, cy, 3);
                gfx.endFill();
                break;
            }
            case 'longevity_trials': {
                // Medical cross + heartbeat trace
                gfx.beginFill(ac);
                gfx.drawRect(cx - 4, cy - 12, 8, 24);
                gfx.drawRect(cx - 12, cy - 4, 24, 8);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.25);
                gfx.drawRect(cx - 4, cy - 12, 8, 3);
                gfx.endFill();
                gfx.lineStyle(1.5, 0x4ade80, 0.9);
                gfx.moveTo(cx - 18, cy + 16);
                gfx.lineTo(cx - 8, cy + 16);
                gfx.lineTo(cx - 4, cy + 9);
                gfx.lineTo(cx, cy + 22);
                gfx.lineTo(cx + 5, cy + 16);
                gfx.lineTo(cx + 18, cy + 16);
                gfx.lineStyle(0);
                break;
            }
            case 'longevity_genomics': {
                // DNA double helix
                ...
                for (let s = 0; s < 2; s++) {
                    ...
                    for (let i = 0; i <= 16; i++) {
                        const yy = cy - 15 + i * 1.9,
                            xx = cx + Math.sin(i * 0.5 + off) * 10;
                        if (i === 0) gfx.moveTo(xx, yy);
                        else gfx.lineTo(xx, yy);
                    }
                }
                gfx.lineStyle(0);
                ...
                for (let i = 2; i < 15; i += 3) {
                    const yy = cy - 15 + i * 1.9;
                    const x1 = cx + Math.sin(i * 0.5) * 10,
                        x2 = cx + Math.sin(i * 0.5 + Math.PI) * 10;
                    gfx.lineStyle(1.2, rungCols[(i / 3) % 4], 0.8);
                    gfx.moveTo(x1, yy);
                    gfx.lineTo(x2, yy);
                    gfx.lineStyle(0);
                }
            ...
            case 'longevity_cryo': {
                // Frost crystal + vault ring
                gfx.lineStyle(1, ac, 0.5);
                gfx.drawCircle(cx, cy, 16);
                gfx.lineStyle(0);
                gfx.lineStyle(2, ac, 0.9);
                for (let i = 0; i < 6; i++) {
                    const a = (i * Math.PI) / 3;
                    gfx.moveTo(cx, cy);
                    gfx.lineTo(cx + Math.cos(a) * 14, cy + Math.sin(a) * 14);
                    // barbs
                    const bx = cx + Math.cos(a) * 8,
                        by = cy + Math.sin(a) * 8;
                    gfx.moveTo(bx, by);
                    gfx.lineTo(bx + Math.cos(a + 0.6) * 4, by + Math.sin(a + 0.6) * 4);
                    gfx.moveTo(bx, by);
                    gfx.lineTo(bx + Math.cos(a - 0.6) * 4, by + Math.sin(a - 0.6) * 4);
                }
                gfx.lineStyle(0);
                gfx.beginFill(0xffffff, 0.7);
                gfx.drawCircle(cx, cy, 2.5);
                gfx.endFill();
                break;
            }
        }
    ...
    _drawSocialFacade(gfx, container, b, h, floors) {
        ...
        gfx.beginFill(0x000000, 0.14);
        gfx.drawRect(b.w, 4, 6, h - 4);
        gfx.endFill();
    ...
    _drawCafeExt(gfx, b, h, floors, n) {
        const AC = 0xf59e0b,
            CREAM = 0xf5e6c8;
        // Warm brick body
        gfx.beginFill(0x241410);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(AC, 0.07);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.04);
        gfx.drawRect(0, 8, 3, h - 8);
        gfx.endFill();
        gfx.beginFill(0x000000, 0.16);
        gfx.drawRect(b.w - 4, 8, 4, h - 8);
        gfx.endFill();
        ...
        gfx.beginFill(CREAM, 0.9);
        gfx.drawRect(0, 0, b.w, 6);
        gfx.endFill();
        gfx.beginFill(AC, 0.9);
        gfx.drawRect(0, 6, b.w, 2.5);
        gfx.endFill();
        gfx.beginFill(0x000000, 0.2);
        gfx.drawRect(0, 8.5, b.w, 2);
        gfx.endFill();
        ...
        gfx.endFill();
        gfx.beginFill(0xa9825a);
        gfx.drawRect(0, -7, b.w, 1.5);
        gfx.endFill();
        [6, b.w - 24].forEach((px) => {
            gfx.beginFill(0x7c2d12);
            gfx.drawRect(px, -4, 18, 4);
            gfx.endFill();
            gfx.beginFill(0x2d6a4f);
            gfx.drawCircle(px + 4, -6, 3);
            gfx.drawCircle(px + 9, -7, 3.5);
            gfx.drawCircle(px + 14, -6, 3);
            gfx.endFill();
        });
        ...
        for (let f = 0; f < floors - 1; f++) {
            ...
            for (let c = 0; c < cols; c++) {
                ...
                gfx.drawRoundedRect(wx - 1.5, wy - 1.5, 19, 15, 6);
                gfx.endFill();
                const lit = n(f * 31 + c * 7) < 0.8;
                gfx.beginFill(lit ? 0xffd9a0 : 0x14100c, lit ? 0.9 : 1);
                gfx.drawRoundedRect(wx, wy, 16, 12, 5);
                gfx.endFill();
                gfx.beginFill(AC, 0.5);
                gfx.drawRect(wx - 1, wy + 12.5, 18, 1.5);
                gfx.endFill();
                if (n(f * 17 + c * 5) < 0.4) {
                    gfx.beginFill(0x2d6a4f);
                    gfx.drawRect(wx + 1, wy + 10, 14, 2);
                    gfx.endFill();
                    gfx.beginFill(0xef4444, 0.9);
                    gfx.drawCircle(wx + 4, wy + 10, 1);
                    gfx.drawCircle(wx + 11, wy + 10, 1);
                    gfx.endFill();
                }
            }
        }
        ...
        gfx.beginFill(0x000000, 0.18);
        gfx.drawRect(4, awnY + 12, b.w - 8, 2);
        gfx.endFill();
        // Shopfront glass: warm glow, pendant lamps, pastry-case silhouettes
        gfx.beginFill(0x1a1208);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        ...
        for (let gx = 16; gx < b.w - 16; gx += 24) {
            gfx.drawRect(gx, h - 19, 0.8, 4);
            gfx.drawCircle(gx + 0.4, h - 14, 1.8);
        }
        gfx.endFill();
        // Entrance
        gfx.beginFill(0x140d06);
        gfx.drawRect(b.w / 2 - 9, h - 20, 18, 18);
        gfx.endFill();
        gfx.beginFill(AC, 0.6);
        gfx.drawRect(b.w / 2 - 9, h - 20, 18, 2);
        gfx.endFill();
        gfx.beginFill(0xffe9a8, 0.8);
        gfx.drawCircle(b.w / 2 + 6, h - 11, 1);
        gfx.endFill();
        // Projecting shingle sign: steaming coffee cup on a round board
        const sx = -9,
            sy = h - 46;
        gfx.beginFill(0x5a3a22);
        gfx.drawRect(-14, sy - 11, 15, 2);
        gfx.endFill();
        gfx.beginFill(0x0e0a06, 0.95);
        gfx.drawCircle(sx, sy, 10);
        gfx.endFill();
        gfx.lineStyle(1.2, AC, 0.9);
        gfx.drawCircle(sx, sy, 10);
        gfx.lineStyle(0);
        gfx.beginFill(CREAM);
        gfx.drawRoundedRect(sx - 4, sy - 1, 8, 6, 1.5);
        gfx.endFill();
        gfx.lineStyle(1.2, CREAM, 0.9);
        gfx.drawCircle(sx + 5, sy + 2, 2);
        gfx.lineStyle(0);
        gfx.lineStyle(1, AC, 0.8);
        gfx.moveTo(sx - 2, sy - 3);
        gfx.lineTo(sx - 1, sy - 6);
        gfx.moveTo(sx + 1, sy - 3);
        gfx.lineTo(sx + 2, sy - 6);
        gfx.lineStyle(0);
        // Sidewalk terrace: striped umbrellas + tables flanking the entrance
        [18, b.w - 34].forEach((tx) => {
            gfx.beginFill(0x8a6a4a);
            gfx.drawRect(tx + 7.5, h - 26, 1.5, 26);
            gfx.endFill();
            ...
            gfx.drawRect(tx + 4, h - 8, 8, 1.5);
            gfx.drawRect(tx + 7.5, h - 8, 1, 8);
            gfx.drawRect(tx - 1, h - 5, 4, 1.2);
            gfx.drawRect(tx + 0.5, h - 5, 1, 5);
            gfx.drawRect(tx + 13, h - 5, 4, 1.2);
            gfx.drawRect(tx + 14.5, h - 5, 1, 5);
            gfx.endFill();
        });
    },
    ...
    _drawGymExt(gfx, b, h, floors, n) {
        const AC = 0x22d3ee,
            GLASS = 0xd6f6ff;
        // Athletic dark body
        gfx.beginFill(0x101822);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(AC, 0.05);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.04);
        gfx.drawRect(0, 8, 3, h - 8);
        ...
        gfx.endFill();
        // Cyan crown
        gfx.beginFill(AC, 0.92);
        gfx.drawRect(0, 0, b.w, 8);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.18);
        gfx.drawRect(0, 0, b.w, 2);
        gfx.endFill();
        gfx.beginFill(0x000000, 0.2);
        gfx.drawRect(0, 8, b.w, 2);
        gfx.endFill();
        // Rooftop lap pool (left) with lane glints + ladder, HVAC on the right
        const poolW = b.w * 0.26;
        gfx.beginFill(0x0e7490);
        gfx.drawRect(6, -6, poolW, 6);
        gfx.endFill();
        gfx.beginFill(0x38bdf8, 0.8);
        gfx.drawRect(6, -6, poolW, 4.5);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.5);
        for (let px = 12; px < poolW - 4; px += 14) gfx.drawRect(px, -4.5, 6, 0.8);
        gfx.endFill();
        gfx.beginFill(0xe2e8f0);
        gfx.drawRect(6 + poolW - 3, -8, 1, 8);
        gfx.drawRect(6 + poolW, -8, 1, 8);
        gfx.drawRect(6 + poolW - 3, -6.5, 4, 0.8);
        gfx.drawRect(6 + poolW - 3, -3.5, 4, 0.8);
        gfx.endFill();
        gfx.beginFill(0x475569);
        gfx.drawRect(b.w - 34, -5, 12, 5);
        gfx.endFill();
        gfx.beginFill(0x64748b, 0.7);
        gfx.drawRect(b.w - 32.5, -3.8, 9, 1);
        gfx.drawRect(b.w - 32.5, -2, 9, 1);
        gfx.endFill();
        // Full-height glass atrium (left) — training silhouettes per floor
        const atX = 8,
            atW = Math.max(52, b.w * 0.3);
        gfx.beginFill(0x0a1220);
        gfx.drawRect(atX, 12, atW, h - 34);
        gfx.endFill();
        gfx.beginFill(GLASS, 0.14);
        gfx.drawRect(atX, 12, atW, h - 34);
        gfx.endFill();
        for (let f = 0; f < floors - 1; f++) {
            const by = 16 + f * 18;
            if (by + 12 > h - 24) break;
            gfx.beginFill(GLASS, 0.55);
            gfx.drawRect(atX + 2, by, atW - 4, 12);
            gfx.endFill();
            gfx.beginFill(AC, 0.2);
            gfx.drawRect(atX + 2, by, atW - 4, 2.5);
            gfx.endFill();
            const cx2 = atX + atW / 2;
            ...
            if (kind === 0) {
                ...
                gfx.drawCircle(cx2 - 8, by + 6.7, 2.6);
                gfx.drawCircle(cx2 + 8, by + 6.7, 2.6);
        ...
        gfx.drawRect(atX + atW / 2 - 0.7, 12, 1.4, h - 34);
        gfx.endFill();
        // Hanging banner with dumbbell mark (right edge)
        const bnX = b.w - 24;
        gfx.beginFill(0x0c4a6e, 0.95);
        gfx.drawRect(bnX, 12, 16, 46);
        gfx.endFill();
        gfx.beginFill(AC, 0.9);
        gfx.drawRect(bnX, 12, 16, 3);
        gfx.endFill();
        gfx.beginFill(0x101822);
        gfx.drawPolygon([bnX, 58, bnX + 16, 58, bnX + 8, 52]);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.9);
        gfx.drawRect(bnX + 3, 30, 10, 1.6);
        gfx.drawRect(bnX + 2, 27, 2.4, 8);
        gfx.drawRect(bnX + 11.6, 27, 2.4, 8);
        gfx.endFill();
        ...
        for (let f = 0; f < floors - 1; f++) {
            ...
            for (let wx = atX + atW + 10; wx + 15 < bnX - 4; wx += 22) {
                ...
                gfx.beginFill(0x000000, 0.25);
                gfx.drawRect(wx - 1, wy - 1, 16, 13);
                gfx.endFill();
                gfx.beginFill(lit ? GLASS : 0x0a1018, lit ? 0.8 : 1);
                gfx.drawRect(wx, wy, 14, 11);
                gfx.endFill();
                if (lit) {
                    gfx.beginFill(AC, 0.25);
                    gfx.drawRect(wx, wy, 14, 2.5);
                    gfx.endFill();
                }
            }
        }
        // Reception floor: glass, turnstiles, cyan canopy
        gfx.beginFill(0x0a141e);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        gfx.beginFill(AC, 0.14);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        gfx.beginFill(AC, 0.8);
        gfx.drawRect(4, h - 22, b.w - 8, 2);
        gfx.endFill();
        gfx.beginFill(GLASS, 0.35);
        for (let lx = 12; lx < b.w - 28; lx += 26) gfx.drawRect(lx, h - 18, 16, 13);
        gfx.endFill();
        gfx.beginFill(0x05090f);
        gfx.drawRect(b.w / 2 - 10, h - 19, 20, 17);
        gfx.endFill();
        gfx.beginFill(AC, 0.55);
        gfx.drawRect(b.w / 2 - 10, h - 19, 20, 2);
        gfx.endFill();
        gfx.beginFill(0x94a3b8, 0.8);
        gfx.drawRect(b.w / 2 - 6, h - 9, 4, 1.2);
        gfx.drawRect(b.w / 2 + 2, h - 9, 4, 1.2);
        gfx.endFill();
        gfx.beginFill(AC, 0.85);
        gfx.drawRect(b.w / 2 - 15, h - 21, 30, 2.5);
        gfx.endFill();
    },
    ...
    _drawArenaExt(gfx, b, h, floors, n) {
        ...
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.04);
        gfx.drawRect(0, 14, 3, h - 14);
        gfx.endFill();
        gfx.beginFill(0x000000, 0.16);
        gfx.drawRect(b.w - 4, 14, 4, h - 14);
        gfx.endFill();
        gfx.beginFill(AC, 0.85);
        gfx.drawRect(0, 14, b.w, 2);
        gfx.endFill();
        const jbW = Math.min(b.w - 56, 150),
            jbX = (b.w - jbW) / 2;
        gfx.beginFill(0x140a10);
        gfx.drawRect(jbX - 6, 2, jbW + 12, 14);
        gfx.endFill();
        gfx.beginFill(AC, 0.9);
        gfx.drawRect(jbX - 6, 0, jbW + 12, 3);
        gfx.endFill();
        // ── JUMBOTRON — the arena's signature match screen ──
        const scrY = 16,
            scrH = Math.min(34, h - 60);
        gfx.beginFill(0x05070d);
        gfx.drawRect(jbX, scrY, jbW, scrH);
        gfx.endFill();
        gfx.lineStyle(1.5, 0x334155, 1);
        gfx.drawRect(jbX, scrY, jbW, scrH);
        gfx.lineStyle(0);
        ...
        const p1x = jbX + jbW * 0.22,
            p2x = jbX + jbW * 0.78,
            pcy = scrY + scrH * 0.42;
        gfx.beginFill(0x3b82f6, 0.9);
        gfx.drawRoundedRect(p1x - 12, pcy - 8, 24, 16, 3);
        gfx.endFill();
        gfx.beginFill(AC, 0.9);
        gfx.drawRoundedRect(p2x - 12, pcy - 8, 24, 16, 3);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.9);
        gfx.drawRect(p1x - 5, pcy - 3, 3, 3);
        gfx.drawRect(p1x + 2, pcy - 3, 3, 3);
        gfx.drawRect(p1x - 4, pcy + 3, 8, 1.5);
        gfx.drawRect(p2x - 5, pcy - 3, 3, 3);
        gfx.drawRect(p2x + 2, pcy - 3, 3, 3);
        gfx.drawRect(p2x - 4, pcy + 3, 8, 1.5);
        gfx.endFill();
        const vx = jbX + jbW / 2;
        gfx.lineStyle(2.2, GOLD, 1);
        gfx.moveTo(vx - 8, pcy - 6);
        gfx.lineTo(vx - 4.5, pcy + 4);
        gfx.lineTo(vx - 1, pcy - 6);
        gfx.moveTo(vx + 8, pcy - 6);
        gfx.lineTo(vx + 3, pcy - 6);
        gfx.lineTo(vx + 3, pcy - 1);
        gfx.lineTo(vx + 8, pcy - 1);
        gfx.lineTo(vx + 8, pcy + 4);
        gfx.lineTo(vx + 3, pcy + 4);
        gfx.lineStyle(0);
        ...
        [
            ...
            gfx.beginFill(pc, 0.92);
            gfx.drawRect(px, 18, 12, 26);
            gfx.endFill();
            gfx.beginFill(0x1c1016);
            gfx.drawPolygon([px, 44, px + 12, 44, px + 6, 38]);
            gfx.endFill();
            gfx.beginFill(GOLD, 0.95);
            gfx.drawCircle(px + 6, 26, 3);
            gfx.endFill();
            gfx.beginFill(GOLD, 0.7);
            gfx.drawRect(px + 2, 33, 8, 1.5);
            gfx.endFill();
        });
        // Floodlight masts at both roof edges
        [12, b.w - 12].forEach((mx) => {
            gfx.beginFill(0x64748b);
            gfx.drawRect(mx - 1, -2, 2, 16);
            gfx.endFill();
            gfx.beginFill(0x334155);
            gfx.drawRect(mx - 7, -8, 14, 6);
            gfx.endFill();
            gfx.beginFill(0xfef9c3, 0.9);
            gfx.drawRect(mx - 6, -7, 3, 4);
            gfx.drawRect(mx - 1.5, -7, 3, 4);
            gfx.drawRect(mx + 3, -7, 3, 4);
            gfx.endFill();
        });
        ...
        for (let f = 2; f < floors - 1; f++) {
            ...
            for (let wx = 10; wx < b.w - 24; wx += 24) {
                ...
                gfx.beginFill(0x000000, 0.25);
                gfx.drawRect(wx - 1, wy - 1, 16, 12);
                gfx.endFill();
                gfx.beginFill(lit ? 0xffd9c0 : 0x0e0a10, lit ? 0.8 : 1);
                gfx.drawRect(wx, wy, 14, 10);
                gfx.endFill();
            }
        }
        // Concourse floor: gold trim, grand arch gate, ticket booths
        gfx.beginFill(0x0e0a10);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        gfx.beginFill(AC, 0.1);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        gfx.beginFill(GOLD, 0.7);
        gfx.drawRect(4, h - 22, b.w - 8, 1.5);
        gfx.endFill();
        gfx.beginFill(0x05070d);
        gfx.drawRect(b.w / 2 - 12, h - 16, 24, 16);
        gfx.drawCircle(b.w / 2, h - 16, 12);
        gfx.endFill();
        gfx.lineStyle(1.5, GOLD, 0.8);
        gfx.drawCircle(b.w / 2, h - 16, 12);
        gfx.lineStyle(0);
        gfx.beginFill(0xffe9a8, 0.5);
        gfx.drawCircle(b.w / 2, h - 16, 2);
        gfx.endFill();
        [b.w * 0.22, b.w * 0.78 - 16].forEach((bx2) => {
            gfx.beginFill(0x1a1420);
            gfx.drawRect(bx2, h - 18, 16, 16);
            gfx.endFill();
            gfx.beginFill(AC, 0.85);
            gfx.drawRect(bx2 - 1, h - 19, 18, 2.5);
            gfx.endFill();
            gfx.beginFill(0xffe9a8, 0.75);
            gfx.drawRect(bx2 + 3, h - 14, 10, 7);
            gfx.endFill();
            gfx.beginFill(0x05070d, 0.8);
            gfx.drawRect(bx2 + 3, h - 9, 10, 1.5);
            gfx.endFill();
        });
    },
    ...
    _drawOpenSquareExt(gfx, b, h, floors, n) {
        ...
        gfx.beginFill(0x161020);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(AC, 0.06);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(0xffffff, 0.04);
        gfx.drawRect(0, 8, 3, h - 8);
        ...
        gfx.lineStyle(1, 0x94a3b8, 0.7);
        gfx.moveTo(4, -8);
        gfx.lineTo(b.w - 4, -8);
        gfx.lineStyle(0);
        for (let fx = 8, i2 = 0; fx < b.w - 8; fx += 12, i2++) {
            gfx.beginFill(pcols[i2 % 5], 0.9);
            gfx.drawPolygon([fx, -8, fx + 8, -8, fx + 4, -2]);
            gfx.endFill();
        }
        gfx.beginFill(0x94a3b8);
        gfx.drawRect(14, -14, 1.5, 6);
        gfx.endFill();
        gfx.beginFill(0xcbd5e1, 0.9);
        gfx.drawEllipse(11, -15, 5, 3.5);
        gfx.endFill();
        // ── LIVE CODE WALL — giant terminal pane, the hub's signature ──
        const twX = 10,
            twW = Math.max(64, b.w * 0.34),
            twY = 15,
            twH = Math.min(42, h - 44);
        gfx.beginFill(0x04070d);
        gfx.drawRect(twX, twY, twW, twH);
        gfx.endFill();
        gfx.lineStyle(1.2, CODE, 0.5);
        gfx.drawRect(twX, twY, twW, twH);
        gfx.lineStyle(0);
        gfx.beginFill(0xf87171);
        gfx.drawCircle(twX + 4, twY + 3.5, 1.2);
        gfx.endFill();
        gfx.beginFill(0xfbbf24);
        gfx.drawCircle(twX + 8, twY + 3.5, 1.2);
        gfx.endFill();
        gfx.beginFill(0x4ade80);
        gfx.drawCircle(twX + 12, twY + 3.5, 1.2);
        gfx.endFill();
        ...
        gfx.beginFill(CODE, 0.95);
        gfx.drawRect(twX + 4, twY + 8 + nLines * 3.6, 4, 2);
        gfx.endFill();
        // Git-branch emblem medallion (right of the code wall)
        const gx2 = twX + twW + (b.w - twX - twW) / 2,
            gy2 = 34;
        gfx.beginFill(0x0a0714, 0.85);
        gfx.drawRoundedRect(gx2 - 16, gy2 - 16, 32, 32, 5);
        gfx.endFill();
        gfx.lineStyle(1, AC, 0.6);
        gfx.drawRoundedRect(gx2 - 16, gy2 - 16, 32, 32, 5);
        gfx.lineStyle(0);
        gfx.lineStyle(2, 0xe2e8f0, 0.9);
        gfx.moveTo(gx2 - 6, gy2 - 9);
        gfx.lineTo(gx2 - 6, gy2 + 9);
        gfx.moveTo(gx2 - 6, gy2 - 2);
        gfx.lineTo(gx2 + 7, gy2 - 9);
        gfx.lineStyle(0);
        gfx.beginFill(CODE);
        gfx.drawCircle(gx2 - 6, gy2 - 9, 3);
        gfx.endFill();
        gfx.beginFill(AC);
        gfx.drawCircle(gx2 - 6, gy2 + 9, 3);
        gfx.endFill();
        gfx.beginFill(0xfbbf24);
        gfx.drawCircle(gx2 + 7, gy2 - 9, 3);
        gfx.endFill();
        ...
        for (let f = 0; f < floors - 1; f++) {
            ...
            for (let wx = startX; wx < b.w - 24; wx += 24) {
                ...
                gfx.endFill();
                gfx.beginFill(
                    lit ? (rr < 0.3 ? 0xd6ecff : rr < 0.6 ? 0xffe9c0 : 0xe9d5ff) : 0x0c0916,
                    lit ? 0.85 : 1
                );
                gfx.drawRect(wx, wy, 14, 10);
                gfx.endFill();
        ...
        gfx.beginFill(0x0d0a16);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        gfx.beginFill(AC, 0.1);
        gfx.drawRect(4, h - 22, b.w - 8, 20);
        gfx.endFill();
        gfx.beginFill(AC, 0.7);
        gfx.drawRect(4, h - 22, b.w - 8, 1.5);
        gfx.endFill();
        [
            [0x22d3ee, b.w * 0.14, 8],
            ...
            gfx.beginFill(gc, 0.3);
            gfx.drawEllipse(gx3, h - 10, gr, gr * 0.55);
            gfx.endFill();
            gfx.beginFill(gc, 0.18);
            gfx.drawEllipse(gx3 + gr * 0.8, h - 13, gr * 0.5, gr * 0.3);
            gfx.endFill();
        });
        ...
        gfx.moveTo(tgx - 6, tgy - 3);
        gfx.lineTo(tgx - 9, tgy);
        gfx.lineTo(tgx - 6, tgy + 3);
        gfx.moveTo(tgx + 6, tgy - 3);
        gfx.lineTo(tgx + 9, tgy);
        gfx.lineTo(tgx + 6, tgy + 3);
        gfx.moveTo(tgx + 2, tgy - 4);
        gfx.lineTo(tgx - 2, tgy + 4);
        gfx.lineStyle(0);
        for (let pi = 0; pi < 4; pi++) {
            ...
            gfx.endFill();
            gfx.beginFill(pcols[pi % 5], 0.8);
            gfx.drawRect(px2 + 1.5, h - 17.5, 7, 3);
            gfx.endFill();
            gfx.beginFill(0x94a3b8, 0.6);
            gfx.drawRect(px2 + 1.5, h - 12, 7, 1);
            gfx.drawRect(px2 + 1.5, h - 10, 5, 1);
            gfx.endFill();
        }
        gfx.beginFill(0x05070d);
        gfx.drawRect(b.w / 2 - 11, h - 19, 22, 17);
        gfx.endFill();
        gfx.beginFill(0x1e293b, 0.9);
        for (let dy2 = h - 17; dy2 < h - 4; dy2 += 3) gfx.drawRect(b.w / 2 - 10, dy2, 20, 1);
        gfx.endFill();
        gfx.beginFill(CODE, 0.6);
        gfx.drawRect(b.w / 2 - 11, h - 19, 22, 1.5);
        gfx.endFill();
    },
    ...
    _drawTimesExt(gfx, container, b, h, floors, n) {
        ...
        gfx.beginFill(0x262229);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        gfx.beginFill(CREAM, 0.08);
        gfx.drawRect(0, 8, b.w, h - 8);
        gfx.endFill();
        ...
        gfx.beginFill(INK, 0.98);
        gfx.drawRect(0, 0, b.w, 14);
        gfx.endFill();
        gfx.beginFill(CREAM, 0.9);
        gfx.drawRect(0, 0, b.w, 1.2);
        gfx.drawRect(0, 12.8, b.w, 1.2);
        gfx.endFill();
        ...
        const ccx = b.w / 2,
            ccy = 25;
        gfx.beginFill(0x0d0d12);
        gfx.drawCircle(ccx, ccy, 7);
        gfx.endFill();
        gfx.lineStyle(1.2, CREAM, 0.85);
        gfx.drawCircle(ccx, ccy, 7);
        gfx.lineStyle(0);
        gfx.lineStyle(1, 0xf5f0e6, 0.9);
        gfx.moveTo(ccx, ccy);
        gfx.lineTo(ccx, ccy - 4.5);
        gfx.moveTo(ccx, ccy);
        gfx.lineTo(ccx + 3.2, ccy + 1.5);
        gfx.lineStyle(0);
        ...
        for (let f = 0; f < floors - 1; f++) {
            ...
            for (let wx = 10; wx < b.w - 20; wx += 24) {
                ...
                gfx.drawRect(wx - 1, wy - 1, 16, 13);
                gfx.endFill();
                gfx.beginFill(lit ? 0xf5eeda : 0x121016, lit ? 0.85 : 1);
                gfx.drawRect(wx, wy, 14, 11);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.3);
                gfx.drawRect(wx, wy + 5, 14, 1);
                gfx.endFill();
            }
        }
        // ── PRESS HALL — printing press behind the ground-floor glass ──
        gfx.beginFill(0x0f0d12);
        gfx.drawRect(4, h - 24, b.w - 8, 22);
        gfx.endFill();
        gfx.beginFill(0xf5eeda, 0.1);
        gfx.drawRect(4, h - 24, b.w - 8, 22);
        gfx.endFill();
        gfx.beginFill(CREAM, 0.6);
        gfx.drawRect(4, h - 24, b.w - 8, 1.5);
        gfx.endFill();
        const prX = 10,
            prW = Math.max(46, b.w * 0.34);
        ...
        [
            ...
            gfx.beginFill(0x3f3f46);
            gfx.drawCircle(rx2, ry2, 3.5);
            gfx.endFill();
            gfx.beginFill(0x71717a, 0.9);
            gfx.drawCircle(rx2, ry2, 1.5);
            gfx.endFill();
        });
        gfx.beginFill(0x27272e);
        gfx.drawRect(prX + 2, h - 8, prW - 4, 6);
        gfx.endFill();
        gfx.beginFill(RED, 0.8);
        gfx.drawRect(prX + 2, h - 8, prW - 4, 1.2);
        gfx.endFill();
        // Entrance + fresh paper bundle
        gfx.beginFill(0x0a0a10);
        gfx.drawRect(b.w / 2 - 9, h - 20, 18, 18);
        gfx.endFill();
        gfx.beginFill(RED, 0.7);
        gfx.drawRect(b.w / 2 - 9, h - 20, 18, 2);
        gfx.endFill();
        gfx.beginFill(0xffe9a8, 0.7);
        gfx.drawCircle(b.w / 2 + 6, h - 11, 1);
        gfx.endFill();
        gfx.beginFill(0xe7e2d4, 0.9);
        gfx.drawRect(b.w / 2 + 12, h - 5, 9, 5);
        gfx.endFill();
        gfx.beginFill(0x9ca3af, 0.9);
        gfx.drawRect(b.w / 2 + 12, h - 3.5, 9, 0.8);
        gfx.endFill();
        // News kiosk (right wing) with red awning + racked papers
        const kx = b.w - 34;
        gfx.beginFill(0x1e3a2f);
        gfx.drawRect(kx, h - 16, 22, 16);
        gfx.endFill();
        gfx.beginFill(RED, 0.9);
        gfx.drawRect(kx - 2, h - 18, 26, 3);
        gfx.endFill();
        gfx.beginFill(0xf5eeda, 0.85);
        gfx.drawRect(kx + 2, h - 12, 5, 7);
        gfx.drawRect(kx + 9, h - 12, 5, 7);
        gfx.endFill();
        gfx.beginFill(0x0a0a10, 0.8);
        gfx.drawRect(kx + 3, h - 11, 3, 1);
        gfx.drawRect(kx + 10, h - 11, 3, 1);
        gfx.endFill();
        // Rooftop wire-service mast with beacon
        gfx.beginFill(0x64748b);
        gfx.drawRect(b.w - 22, -14, 1.5, 14);
        gfx.drawRect(b.w - 26, -8, 9, 1);
        gfx.endFill();
        gfx.beginFill(RED, 0.9);
        gfx.drawCircle(b.w - 21.2, -14.5, 1.5);
        gfx.endFill();
    },
    ...
    buildBuildings() {
        ...
        const ghostLights = [];
        this.lightLayer.children.forEach((c) => {
            if (c !== this.staticLightsGfx) ghostLights.push(c);
        });
        ghostLights.forEach((c) => {
            this.lightLayer.removeChild(c);
            c.destroy();
        });
        ...
        if (G.ceoRefs) {
            Object.values(G.ceoRefs).forEach((ceo) => {
                if (ceo && ceo.refCont && !ceo.refCont.destroyed) this.reflectionLayer.addChild(ceo.refCont);
            });
        }
        // Re-attach truck reflections
        if (G.cars) {
            G.cars.forEach((c) => {
                if (c && c.ref && !c.ref.destroyed) this.reflectionLayer.addChild(c.ref);
            });
        }

        const emojiFontStack =
            '"Twemoji Mozilla", "Apple Color Emoji", "Noto Color Emoji", "Segoe UI Emoji", sans-serif';

        BLDS.forEach((b) => {
            const lab = b.lab ? LABS[b.lab] : null;
            const colHex = lab
                ? parseInt(lab.color.slice(1), 16)
                : parseInt((b.color || '#6b7280').slice(1), 16);
            const floors = b.dynamicFl || b.fl;
            const h = floors * 18 + 24;

            const bx = b.x;
            const by = G.groundY - 24;
            ...
            if (b.id.startsWith('forest_') || b.id === 'forest_space') {
                ...
                gfx.drawRect(0, h - 6, b.w, 6);
                gfx.endFill();
                gfx.beginFill(0x2d6a4f);
                gfx.drawRect(0, h - 8, b.w, 2);
                gfx.endFill();
                ...
                for (let tx = 10; tx < b.w - 10; tx += 15 + sr() * 20) {
                    ...
                    gfx.beginFill(0x000000, 0.2);
                    gfx.drawEllipse(tx, h - 2, tw * 0.6, 4);
                    gfx.endFill();
                    gfx.beginFill(0x3d2914);
                    gfx.drawRect(tx - 3, h - 10 - th * 0.3, 6, th * 0.3 + 10);
                    gfx.endFill();
                ...
                if (estateStyle === 'brutalist') {
                    ...
                    gfx.endFill();
                    gfx.beginFill(0x334155);
                    gfx.drawRect(10, h - 55, 50, 55);
                    gfx.endFill();
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(b.w / 2 - 20, h - 80, 40, 80);
                    gfx.endFill();
                    // Cantilevered overhang
                    gfx.beginFill(0x3f4f63);
                    gfx.drawRect(5, h - 58, b.w - 10, 4);
                    gfx.endFill();
                    // Radar dish
                    gfx.beginFill(0x0f172a);
                    gfx.drawEllipse(35, h - 86, 22, 5);
                    gfx.endFill();
                    gfx.lineStyle(2, colHex);
                    gfx.drawCircle(35, h - 86, 8);
                    gfx.lineStyle(0);
                    // Accent stripe
                    gfx.beginFill(colHex);
                    gfx.drawRect(30, h - 80, b.w - 60, 2);
                    gfx.endFill();
                    // Laser slit windows
                    for (var sy = h - 70; sy < h - 10; sy += 14) {
                        gfx.beginFill(0xf87171, 0.3);
                        gfx.drawRect(40, sy, b.w - 80, 5);
                        gfx.endFill();
                        gfx.beginFill(0xfca5a5, 0.7);
                        gfx.drawRect(42, sy + 1, b.w - 84, 2);
                        gfx.endFill();
                    }
                    // Security bollards
                    gfx.beginFill(0x334155);
                    for (var boll = 0; boll < b.w; boll += 20) {
                        gfx.drawRect(boll, h - 8, 4, 8);
                    }
                    gfx.endFill();
                    ...
                    var pw = b.w - 30;
                    gfx.beginFill(0x0f172a);
                    gfx.drawRect(15, h - 90, pw, 90);
                    gfx.endFill();
                    // Glass curtain wall
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(18, h - 86, pw - 6, 82);
                    gfx.endFill();
                    // Floor-to-ceiling windows (3 floors)
                    for (var pf = 0; pf < 3; pf++) {
                        var wy = h - 82 + pf * 28;
                        gfx.beginFill(0x38bdf8, 0.25);
                        gfx.drawRect(22, wy, pw - 14, 22);
                        gfx.endFill();
                        gfx.beginFill(0x7dd3fc, 0.15);
                        gfx.drawRect(22, wy, pw - 14, 8);
                        gfx.endFill();
                        // Floor dividers
                        gfx.beginFill(0x334155);
                        gfx.drawRect(18, wy + 24, pw - 6, 3);
                        gfx.endFill();
                    }
                    // Rooftop terrace railing
                    gfx.beginFill(0xcbd5e1);
                    gfx.drawRect(10, h - 92, pw + 10, 2);
                    gfx.endFill();
                    gfx.lineStyle(1, 0x94a3b8, 0.5);
                    for (var rx = 14; rx < pw + 20; rx += 8) {
                        gfx.moveTo(rx, h - 92);
                        gfx.lineTo(rx, h - 98);
                    }
                    gfx.lineStyle(0);
                    // Accent crown
                    gfx.beginFill(colHex);
                    gfx.drawRect(15, h - 90, pw, 2);
                    gfx.endFill();
                    // Rooftop glow
                    gfx.beginFill(colHex, 0.15);
                    gfx.drawRect(10, h - 98, pw + 10, 6);
                    gfx.endFill();
                    // Entrance portico
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(b.w / 2 - 20, h - 10, 40, 10);
                    gfx.endFill();
                    gfx.beginFill(0x38bdf8, 0.3);
                    gfx.drawRect(b.w / 2 - 15, h - 8, 30, 6);
                    gfx.endFill();
                } else if (estateStyle === 'villa') {
                    // ── Google/Meta: California Villa — warm, open, terracotta, pool accent ──
                    // Main body
                    gfx.beginFill(0xfef3c7);
                    gfx.drawRect(15, h - 55, b.w - 30, 55);
                    gfx.endFill();
                    gfx.beginFill(0xfde68a);
                    gfx.drawRect(15, h - 55, b.w - 30, 3);
                    gfx.endFill();
                    // Terracotta roof
                    gfx.beginFill(0x92400e);
                    gfx.drawRect(10, h - 60, b.w - 20, 8);
                    gfx.endFill();
                    gfx.beginFill(0x78350f);
                    gfx.drawRect(10, h - 60, b.w - 20, 2);
                    gfx.endFill();
                    // Side wing
                    gfx.beginFill(0xfef3c7);
                    gfx.drawRect(b.w - 45, h - 45, 40, 45);
                    gfx.endFill();
                    gfx.beginFill(0x92400e);
                    gfx.drawRect(b.w - 50, h - 50, 48, 7);
                    gfx.endFill();
                    // Arched windows
                    ...
                    gfx.beginFill(0x22d3ee, 0.4);
                    gfx.drawRoundedRect(5, h - 12, 30, 8, 2);
                    gfx.endFill();
                    gfx.beginFill(0x67e8f9, 0.3);
                    gfx.drawRect(7, h - 11, 26, 3);
                    gfx.endFill();
                    // Palm tree silhouette
                    gfx.beginFill(0x5c4033);
                    gfx.drawRect(b.w - 15, h - 30, 3, 24);
                    gfx.endFill();
                    gfx.beginFill(0x166534);
                    gfx.drawEllipse(b.w - 13, h - 32, 12, 6);
                    gfx.endFill();
                    // Accent
                    gfx.beginFill(colHex);
                    gfx.drawRect(10, h - 60, b.w - 20, 2);
                    ...
                    // Foundation
                    gfx.beginFill(0x94a3b8);
                    gfx.drawRect(10, h - 8, b.w - 20, 8);
                    gfx.endFill();
                    // Main body
                    gfx.beginFill(0xf1f5f9);
                    gfx.drawRect(15, h - 58, b.w - 30, 50);
                    gfx.endFill();
                    // Shadow depth
                    gfx.beginFill(0xcbd5e1);
                    gfx.drawRect(15, h - 58, 3, 50);
                    gfx.endFill();
                    // Triangular pediment
                    gfx.beginFill(0xe2e8f0);
                    ...
                    // Grand door
                    gfx.beginFill(0x78350f);
                    gfx.drawRect(b.w / 2 - 8, h - 22, 16, 14);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawCircle(b.w / 2 + 4, h - 15, 1.5);
                    gfx.endFill();
                    // Accent
                    gfx.beginFill(colHex);
                    gfx.drawRect(12, h - 58, b.w - 24, 2);
                    gfx.endFill();
                } else if (estateStyle === 'chateau') {
                    // ── EU Labs: French Château — mansard roof, dormer windows, stone pillars ──
                    // Stone base
                    gfx.beginFill(0xe2e8f0);
                    gfx.drawRect(10, h - 52, b.w - 20, 52);
                    gfx.endFill();
                    gfx.beginFill(0xcbd5e1);
                    gfx.drawRect(10, h - 52, b.w - 20, 3);
                    gfx.drawRect(10, h - 52, 3, 52);
                    gfx.endFill();
                    // Mansard roof
                    gfx.beginFill(0x1e293b);
                    ...
                    for (var ry = h - 52; ry > h - 80; ry -= 5) {
                        ...
                        gfx.moveTo(7 + xOff, ry);
                        gfx.lineTo(b.w - 7 - xOff, ry);
                    }
                    gfx.lineStyle(0);
                    ...
                    for (var dx = b.w * 0.25; dx < b.w * 0.8; dx += b.w * 0.25) {
                        ...
                        gfx.endFill();
                        gfx.beginFill(0xfde68a, 0.6);
                        gfx.drawRect(dx - 4, h - 60, 8, 8);
                        gfx.endFill();
                    }
                    // Stone pillars
                    gfx.beginFill(0xffffff, 0.3);
                    for (var px2 = 18; px2 < b.w - 15; px2 += 28) {
                        gfx.drawRect(px2, h - 52, 5, 52);
                    }
                    gfx.endFill();
                    ...
                    // Accent
                    gfx.beginFill(colHex);
                    gfx.drawRect(5, h - 52, b.w - 10, 2);
                    gfx.endFill();
                    // Topiary hedges
                    gfx.beginFill(0x166534);
                    gfx.drawCircle(8, h - 10, 8);
                    gfx.drawCircle(b.w - 8, h - 10, 8);
                    gfx.endFill();
                    gfx.beginFill(0x1b4332);
                    gfx.drawCircle(8, h - 10, 6);
                    gfx.drawCircle(b.w - 8, h - 10, 6);
                    gfx.endFill();
                } else if (estateStyle === 'pagoda') {
                    // ── Chinese Labs: Pagoda Mansion — tiered roofs, red/gold, lanterns ──
                    // Main body (dark wood)
                    gfx.beginFill(0x44403c);
                    gfx.drawRect(15, h - 55, b.w - 30, 55);
                    gfx.endFill();
                    // Inner wall (warm)
                    gfx.beginFill(0x7c2d12, 0.6);
                    gfx.drawRect(18, h - 52, b.w - 36, 48);
                    gfx.endFill();
                    ...
                    for (var tier = 0; tier < 3; tier++) {
                        ...
                        gfx.endFill();
                        // Gold trim
                        gfx.beginFill(0xfbbf24, 0.7);
                        gfx.drawRect(tx + 2, ty + 5, tw - 4, 1);
                        gfx.endFill();
                    }
                    // Lattice windows
                    gfx.beginFill(0xfde68a, 0.4);
                    for (var lx = 25; lx < b.w - 25; lx += 22) {
                        gfx.drawRect(lx, h - 40, 14, 20);
                    }
                    gfx.endFill();
                    // Window lattice cross-hatching
                    gfx.lineStyle(1, 0x44403c, 0.6);
                    for (var lx2 = 25; lx2 < b.w - 25; lx2 += 22) {
                        gfx.moveTo(lx2 + 7, h - 40);
                        gfx.lineTo(lx2 + 7, h - 20);
                        gfx.moveTo(lx2, h - 30);
                        gfx.lineTo(lx2 + 14, h - 30);
                    }
                    gfx.lineStyle(0);
                    // Red lanterns
                    gfx.beginFill(0xdc2626);
                    gfx.drawCircle(20, h - 58, 4);
                    gfx.drawCircle(b.w - 20, h - 58, 4);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24, 0.8);
                    gfx.drawCircle(20, h - 58, 2);
                    gfx.drawCircle(b.w - 20, h - 58, 2);
                    gfx.endFill();
                    // Grand red door
                    gfx.beginFill(0xb91c1c);
                    gfx.drawRect(b.w / 2 - 10, h - 18, 20, 12);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawCircle(b.w / 2, h - 12, 2);
                    gfx.endFill();
                    // Accent
                    gfx.beginFill(colHex);
                    gfx.drawRect(15, h - 55, b.w - 30, 2);
                    gfx.endFill();
                } else {
                    // ── Fallback: Minimalist Modern — clean lines, flat roof, subtle glass ──
                    gfx.beginFill(0xf8fafc);
                    gfx.drawRect(20, h - 55, b.w - 40, 55);
                    gfx.endFill();
                    gfx.beginFill(0xe2e8f0);
                    gfx.drawRect(20, h - 55, b.w - 40, 3);
                    gfx.drawRect(20, h - 55, 3, 55);
                    gfx.endFill();
                    // Flat roof with parapet
                    gfx.beginFill(0x0f172a);
                    gfx.drawRect(15, h - 58, b.w - 30, 5);
                    gfx.endFill();
                    // Feature wall (darker accent block)
                    gfx.beginFill(0x334155);
                    gfx.drawRect(b.w - 50, h - 48, 30, 42);
                    gfx.endFill();
                    // Panoramic window
                    gfx.beginFill(0x38bdf8, 0.35);
                    gfx.drawRect(28, h - 44, b.w - 80, 22);
                    gfx.endFill();
                    gfx.beginFill(0x7dd3fc, 0.15);
                    gfx.drawRect(28, h - 44, b.w - 80, 8);
                    gfx.endFill();
                    // Window mullion
                    gfx.beginFill(0xcbd5e1);
                    gfx.drawRect(b.w / 2 - 1, h - 44, 2, 22);
                    gfx.endFill();
                    // Entrance
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(b.w / 2 - 12, h - 14, 24, 8);
                    gfx.endFill();
                    gfx.beginFill(0x38bdf8, 0.2);
                    gfx.drawRect(b.w / 2 - 10, h - 13, 20, 6);
                    gfx.endFill();
                    // Hedges
                    gfx.beginFill(0x166534);
                    gfx.drawRoundedRect(2, h - 10, 24, 8, 3);
                    gfx.drawRoundedRect(b.w - 26, h - 12, 24, 10, 3);
                    gfx.endFill();
                    // Accent
                    gfx.beginFill(colHex);
                    gfx.drawRect(15, h - 58, b.w - 30, 2);
                    gfx.endFill();
                }

                gfx.lineStyle(0); // Safety reset before sign
                const signBg = new PIXI.Graphics();
                signBg.beginFill(0x0a0a1a, 0.8);
                signBg.lineStyle(1, colHex, 0.5);
                signBg.drawRoundedRect(b.w / 2 - 40, h - 10, 80, 8, 2);
                signBg.endFill();
                container.addChild(signBg);

                const signTxt = new PIXI.Text(b.name.toUpperCase(), {
                ...
                b._stationSign = signTxt;
                b._stationCol = colHex;
            }
            // ─── DATA CENTER BUILDINGS ───
                ...
                if (isConstruction) {
                    // ── CONSTRUCTION SITE ──
                    // Dirt/foundation
                    gfx.beginFill(0x78582e);
                    gfx.drawRect(0, h - 8, b.w, 8);
                    gfx.endFill();
                    gfx.beginFill(0x92703a);
                    gfx.drawRect(0, h - 6, b.w, 2);
                    gfx.endFill();
                    // Partial structure (steel frame)
                    gfx.beginFill(0x475569, 0.6);
                    gfx.drawRect(10, h - 50, b.w - 20, 42);
                    gfx.endFill();
                    // Steel beams
                    gfx.beginFill(0x64748b);
                    for (var cx2 = 15; cx2 < b.w - 15; cx2 += 25) {
                        gfx.drawRect(cx2, h - 50, 4, 42);
                    }
                    for (var cy = h - 48; cy < h - 10; cy += 14) {
                        gfx.drawRect(10, cy, b.w - 20, 2);
                    }
                    gfx.endFill();
                    // Crane
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(b.w * 0.7, h - 95, 4, 87);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(b.w * 0.5, h - 95, b.w * 0.3, 3);
                    gfx.endFill();
                    // Crane cable
                    gfx.lineStyle(1, 0x94a3b8);
                    gfx.moveTo(b.w * 0.55, h - 92);
                    gfx.lineTo(b.w * 0.55, h - 60);
                    gfx.lineStyle(0);
                    // Dangling steel beam
                    gfx.beginFill(0x64748b);
                    gfx.drawRect(b.w * 0.52, h - 62, 8, 3);
                    gfx.endFill();
                    // Safety barriers
                    gfx.beginFill(0xef4444);
                    for (var bx2 = 0; bx2 < b.w; bx2 += 20) {
                        gfx.drawRect(bx2, h - 10, 8, 2);
                    }
                    gfx.endFill();
                    // Completion label
                    if (dc.completion) {
                        gfx.beginFill(0x000000, 0.7);
                        gfx.drawRect(b.w / 2 - 30, h - 30, 60, 12);
                        gfx.endFill();
                    }
                    // Accent
                    gfx.beginFill(opCol);
                    gfx.drawRect(10, h - 50, b.w - 20, 2);
                    gfx.endFill();
                } else {
                    // ── OPERATIONAL DATA CENTER ──
                    // Main structure — industrial, heavy
                    gfx.beginFill(0x334155);
                    gfx.drawRect(5, h - 65, b.w - 10, 65);
                    gfx.endFill();
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(8, h - 62, b.w - 16, 56);
                    gfx.endFill();
                    // Roof equipment
                    gfx.beginFill(0x475569);
                    gfx.drawRect(5, h - 68, b.w - 10, 5);
                    gfx.endFill();
                    // HVAC units on roof
                    gfx.beginFill(0x64748b);
                    for (var hvx = 15; hvx < b.w - 30; hvx += 30) {
                        gfx.drawRect(hvx, h - 75, 16, 8);
                        gfx.drawRect(hvx + 4, h - 78, 8, 4);
                    }
                    gfx.endFill();
                    // Server room windows (blue glow strips)
                    for (var wy2 = h - 58; wy2 < h - 12; wy2 += 14) {
                        gfx.beginFill(0x06b6d4, 0.3);
                        gfx.drawRect(12, wy2, b.w - 24, 8);
                        gfx.endFill();
                        gfx.beginFill(0x22d3ee, 0.15);
                        gfx.drawRect(12, wy2, b.w - 24, 3);
                        gfx.endFill();
                    }
                    // Loading dock
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(b.w / 2 - 15, h - 12, 30, 6);
                    gfx.endFill();
                    gfx.beginFill(0x475569);
                    gfx.drawRect(b.w / 2 - 12, h - 11, 24, 4);
                    gfx.endFill();
                    // Security fence posts
                    gfx.beginFill(0x475569);
                    gfx.drawRect(0, h - 8, 3, 8);
                    gfx.drawRect(b.w - 3, h - 8, 3, 8);
                    gfx.endFill();
                    // Accent stripe
                    gfx.beginFill(opCol);
                    gfx.drawRect(5, h - 65, b.w - 10, 2);
                    gfx.endFill();
                    // Power indicator LEDs
                    gfx.beginFill(0x4ade80);
                    for (var ledx = 20; ledx < b.w - 20; ledx += 18) {
                        gfx.drawCircle(ledx, h - 64, 1.5);
                    }
                    gfx.endFill();
                }
                ...
                dcSign.beginFill(0x0a0a1a, 0.9);
                dcSign.lineStyle(1, opCol, 0.6);
                dcSign.drawRoundedRect(b.w / 2 - dcSignW / 2, -30, dcSignW, 18, 3);
                dcSign.endFill();
                dcSign.beginFill(0x333333);
                dcSign.lineStyle(0);
                dcSign.drawRect(b.w / 2 - 10, -12, 4, 12);
                dcSign.drawRect(b.w / 2 + 6, -12, 4, 12);
                dcSign.endFill();
                container.addChild(dcSign);
                var dcTxt = new PIXI.Text(b.name.toUpperCase(), {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 9,
                    fill: opCol,
                    fontWeight: 'bold',
                ...
                if (isConstruction && dc.completion) {
                    var compTxt = new PIXI.Text(`EST. ${dc.completion}`, {
                        fontFamily: 'JetBrains Mono',
                        fontSize: 5,
                        fill: 0xfbbf24,
                    });
                    compTxt.anchor.set(0.5, 0.5);
                    compTxt.x = b.w / 2;
                    compTxt.y = h - 24;
            ...
            // ─── CHIP FAB BUILDINGS ───
                ...
                if (isConstruction) {
                    ...
                    gfx.endFill();
                    gfx.beginFill(0x64748b);
                    for (var fx = 15; fx < b.w - 15; fx += 25) {
                        gfx.drawRect(fx, h - 50, 4, 42);
                    }
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(b.w * 0.6, h - 85, 4, 77);
                    gfx.drawRect(b.w * 0.4, h - 85, b.w * 0.3, 3);
                    gfx.endFill();
                    gfx.beginFill(opCol);
                    gfx.drawRect(10, h - 50, b.w - 20, 2);
                    gfx.endFill();
                    ...
                    gfx.beginFill(0xe2e8f0);
                    gfx.drawRect(5, h - 60, b.w - 10, 60);
                    gfx.endFill();
                    gfx.beginFill(0xf8fafc);
                    gfx.drawRect(8, h - 57, b.w - 16, 51);
                    gfx.endFill();
                    // Cleanroom yellow lighting strips
                    for (var fy2 = h - 52; fy2 < h - 10; fy2 += 12) {
                        gfx.beginFill(0xfbbf24, 0.2);
                        gfx.drawRect(12, fy2, b.w - 24, 6);
                        gfx.endFill();
                        gfx.beginFill(0xfbbf24, 0.1);
                        gfx.drawRect(12, fy2, b.w - 24, 2);
                        gfx.endFill();
                    }
                    // Filtered air intakes on roof
                    gfx.beginFill(0xcbd5e1);
                    gfx.drawRect(5, h - 63, b.w - 10, 5);
                    gfx.endFill();
                    gfx.beginFill(0x94a3b8);
                    for (var ax = 12; ax < b.w - 20; ax += 20) {
                        gfx.drawRect(ax, h - 68, 12, 6);
                    }
                    gfx.endFill();
                    // Hazmat markings
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(b.w / 2 - 15, h - 8, 30, 2);
                    gfx.endFill();
                    // Accent
                    gfx.beginFill(opCol);
                    gfx.drawRect(5, h - 60, b.w - 10, 2);
                    gfx.endFill();
                }
                ...
                fabSign.beginFill(0x0a0a1a, 0.9);
                fabSign.lineStyle(1, opCol, 0.6);
                fabSign.drawRoundedRect(b.w / 2 - fabSignW / 2, -30, fabSignW, 18, 3);
                fabSign.endFill();
                fabSign.beginFill(0x333333);
                fabSign.lineStyle(0);
                fabSign.drawRect(b.w / 2 - 10, -12, 4, 12);
                fabSign.drawRect(b.w / 2 + 6, -12, 4, 12);
                fabSign.endFill();
                container.addChild(fabSign);
                var fabTxt = new PIXI.Text(b.name.toUpperCase(), {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 9,
                    fill: opCol,
                    fontWeight: 'bold',
                ...
                b._dcCol = opCol;
                if (isConstruction && dc.completion) {
                    var fabCompTxt = new PIXI.Text(`EST. ${dc.completion}`, {
                        fontFamily: 'JetBrains Mono',
                        fontSize: 5,
                        fill: 0xfbbf24,
                    });
                    fabCompTxt.anchor.set(0.5, 0.5);
                    fabCompTxt.x = b.w / 2;
                    fabCompTxt.y = h - 24;
                ...
                gfx.beginFill(0x2d6a4f);
                gfx.drawRect(0, h - 12, b.w, 12);
                gfx.endFill();
                gfx.beginFill(0x3d7a5f);
                gfx.drawRect(0, h - 12, b.w, 4);
                gfx.endFill();
                ...
                gfx.lineTo(25, h - 22);
                gfx.lineStyle(0);
                gfx.beginFill(0x8b5cf6);
                gfx.drawRect(17, h - 22, 10, 3);
                gfx.endFill();
                gfx.beginFill(0x555555);
                gfx.drawRect(b.w - 55, h - 12, 4, 30);
                gfx.endFill();
                gfx.beginFill(0x555555);
                gfx.drawRect(b.w - 45, h - 12, 4, 30);
                gfx.endFill();
                gfx.beginFill(0x666666);
                gfx.drawRect(b.w - 54, h - 20, 12, 2);
                gfx.endFill();
                gfx.beginFill(0x666666);
                gfx.drawRect(b.w - 54, h - 28, 12, 2);
                gfx.endFill();
                gfx.beginFill(0x666666);
                gfx.drawRect(b.w - 54, h - 36, 12, 2);
                gfx.endFill();
                gfx.beginFill(0x555555);
                gfx.drawRect(b.w - 54, h - 42, 14, 4);
                gfx.endFill();
                gfx.beginFill(0x22d3ee, 0.8);
                gfx.moveTo(b.w - 42, h - 40);
                gfx.lineTo(b.w - 20, h - 14);
                gfx.lineTo(b.w - 16, h - 14);
                gfx.lineTo(b.w - 40, h - 40);
                gfx.closePath();
                gfx.endFill();
                gfx.beginFill(0x22d3ee, 0.5);
                gfx.drawRect(b.w - 20, h - 16, 6, 4);
                gfx.endFill();

                const monW = 60;
                const monH = 80;
                gfx.beginFill(0x1a1a30);
                gfx.drawRect(b.w / 2 - monW / 2, h - monH, monW, monH);
                gfx.endFill();
                gfx.beginFill(0x2a2a42);
                gfx.drawRect(b.w / 2 - monW / 2 + 4, h - monH + 4, monW - 8, monH - 8);
                gfx.endFill();
                gfx.beginFill(0x22d3ee, 0.8);
                gfx.drawRect(b.w / 2 - 2, h - monH + 10, 4, monH - 20);
                gfx.endFill();
                gfx.beginFill(0xfacc15, 0.8);
                gfx.drawRect(b.w / 2 - monW / 2 - 4, h - 14, monW + 8, 4);
                gfx.endFill();

                gfx.lineStyle(2, 0xfacc15, 0.5);
                gfx.drawEllipse(b.w / 2, h - monH - 10, 30, 8);
                gfx.lineStyle(2, 0x22d3ee, 0.5);
                gfx.drawEllipse(b.w / 2, h - monH - 20, 20, 5);
                gfx.lineStyle(0);
                const monIcon = new PIXI.Text('🏆', {
                    fontFamily: emojiFontStack,
                    fontSize: 28,
                    fill: 0xfacc15,
                    dropShadow: true,
                    dropShadowColor: 0xfacc15,
                    dropShadowBlur: 10,
                    dropShadowDistance: 0,
                });
                monIcon.anchor.set(0.5, 0.5);
                monIcon.x = b.w / 2;
                monIcon.y = h - monH - 25;
                container.addChild(monIcon);
                b._monIcon = monIcon;
                ...
                // Dark earth ground
                gfx.beginFill(0x111118);
                gfx.drawRect(0, h - 12, b.w, 12);
                gfx.endFill();
                gfx.beginFill(0x1a1a28);
                gfx.drawRect(0, h - 14, b.w, 4);
                gfx.endFill();
                // Stone path
                gfx.beginFill(0x2a2a3a);
                gfx.drawRect(10, h - 8, b.w - 20, 4);
                gfx.endFill();
                for (let px = 12; px < b.w - 20; px += 8) {
                    gfx.beginFill(0x333344);
                    gfx.drawRect(px, h - 7, 6, 2);
                    gfx.endFill();
                }
                // Wrought iron fence posts
                gfx.beginFill(0x333344);
                gfx.drawRect(0, h - 30, 3, 20);
                gfx.drawRect(b.w - 3, h - 30, 3, 20);
                for (let fx = 20; fx < b.w; fx += 20) {
                    gfx.drawRect(fx, h - 28, 2, 16);
                }
                gfx.endFill();
                // Fence rails
                gfx.beginFill(0x2a2a3a);
                gfx.drawRect(0, h - 26, b.w, 1);
                gfx.drawRect(0, h - 18, b.w, 1);
                gfx.endFill();
                // Fog overlay
                gfx.beginFill(0x444466, 0.06);
                gfx.drawEllipse(b.w / 2, h - 10, b.w / 2, 14);
                gfx.endFill();

                // Eternal flame (center)
                const flameX = b.w / 2;
                gfx.beginFill(0x333344);
                gfx.drawRect(flameX - 6, h - 22, 12, 10);
                gfx.endFill(); // pedestal
                gfx.beginFill(0x444455);
                gfx.drawRect(flameX - 8, h - 24, 16, 4);
                gfx.endFill(); // rim
                const flame = new PIXI.Graphics();
                flame.beginFill(0xff6600, 0.7);
                flame.drawPolygon([flameX - 3, h - 24, flameX, h - 34, flameX + 3, h - 24]);
                flame.endFill();
                flame.beginFill(0xffaa00, 0.5);
                flame.drawPolygon([flameX - 2, h - 24, flameX, h - 30, flameX + 2, h - 24]);
                flame.endFill();
                ...
                for (const wx of [14, b.w - 14]) {
                    gfx.beginFill(0x2a2218);
                    gfx.drawRect(wx - 2, h - 55, 4, 43);
                    gfx.endFill();
                    // Drooping branches
                    ...
                    gfx.drawEllipse(wx, h - 52, 18, 8);
                    gfx.endFill();
                }

                // Per-model headstones
                const retired = G.models
                    ? G.models.filter((m) => {
                ...
                for (let si = 0; si < maxStones; si++) {
                    ...
                    const sh = 16 + (si % 3) * 6;
                    const labCol =
                        LABS[rm.lab] && LABS[rm.lab].color
                            ? parseInt(LABS[rm.lab].color.replace('#', ''), 16)
                            : 0x666688;
                    // Stone
                    gfx.beginFill(0x3a3a4a);
                    gfx.drawRoundedRect(sx, h - 14 - sh, 16, sh, 3);
                    gfx.endFill();
                    gfx.beginFill(0x4a4a5a);
                    gfx.drawRoundedRect(sx + 1, h - 13 - sh, 14, sh - 2, 2);
                    gfx.endFill();
                    // Cross or lab accent
                    gfx.beginFill(labCol, 0.5);
                    gfx.drawRect(sx + 6, h - 12 - sh, 4, 2);
                    gfx.endFill();
                    gfx.beginFill(labCol, 0.3);
                    gfx.drawRect(sx + 7, h - 14 - sh + 3, 2, 6);
                    gfx.endFill();
                    // Name lines (tiny)
                ...
                const hsHit = new PIXI.Graphics();
                hsHit.eventMode = 'static';
                hsHit.cursor = 'pointer';
                hsHit.beginFill(0xffffff, 0.001);
                hsHit.drawRect(stoneStartX, h - 40, maxStones * 22, 30);
                hsHit.endFill();
                ...
                hsHit.on('pointerover', (e) => {
                    ...
                    if (idx >= 0 && idx < maxStones && b._headstones[idx]) {
                        ...
                        const retDate = rm.ret
                            ? new Date(rm.ret).toLocaleDateString('en-US', {
                                  month: 'short',
                                  year: 'numeric',
                              })
                            : '?';
                ...
                // Count label
                const countTxt = new PIXI.Text(`${retired.length} retired`, {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 6,
                    fill: 0x6666aa,
                });
                ...
                // Dark metallic base
                gfx.beginFill(0x0a1020);
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                gfx.beginFill(bkCol, 0.04);
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                // Subtle panel lines
                for (let px = 0; px < b.w; px += 30) {
                    gfx.beginFill(0x111e30);
                    gfx.drawRect(px, 0, 1, h);
                    gfx.endFill();
                }
                // Brand crown band
                gfx.beginFill(bkCol, 0.9);
                gfx.drawRect(0, 0, b.w, 5);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.14);
                gfx.drawRect(0, 0, b.w, 1);
                gfx.endFill();
                gfx.beginFill(bkCol, 0.1);
                gfx.drawRect(0, 5, b.w, 2);
                gfx.endFill();
                // Floor slabs
                for (let fi = 1; fi < floors; fi++) {
                    const fy = fi * 18;
                    gfx.beginFill(0x1a2540);
                    gfx.drawRect(0, fy, b.w, 2);
                    gfx.endFill();
                }
                ...
                for (let fi = 0; fi < floors; fi++) {
                    for (let wx = 8; wx < b.w - 12; wx += 22) {
                        const wy = fi * 18 + 6;
                        gfx.beginFill(0x0a1830);
                        gfx.drawRect(wx, wy, 16, 10);
                        gfx.endFill();
                        ...
                        gfx.beginFill(
                            glowCol,
                            0.08 + this._labNoise((b.x | 0) + fi * 131 + wx * 7 + 1) * 0.1
                        );
                        gfx.drawRect(wx + 1, wy + 1, 14, 8);
                        gfx.endFill();
                    }
                }
                ...
                if (b.id === 'backbone_landing') {
                    ...
                    for (let ci = 0; ci < 4; ci++) {
                        const cx = 20 + ci * 40;
                        gfx.beginFill(0x1a2540);
                        gfx.drawRect(cx, h - 16, 24, 16);
                        gfx.endFill();
                        gfx.beginFill([0x22d3ee, 0x4ade80, 0xfacc15, 0xf43f5e][ci], 0.3);
                        gfx.drawRect(cx + 4, h - 12, 16, 3);
                        gfx.endFill();
                    }
                    // Armored cable stub coming from ground
                    gfx.beginFill(0x333850);
                    gfx.drawRect(b.w / 2 - 20, h - 8, 40, 8);
                    gfx.endFill();
                } else if (b.id === 'backbone_ixp') {
                    // Central hub: data convergence symbol
                    gfx.beginFill(bkCol, 0.12);
                    gfx.drawRect(b.w / 2 - 30, h / 2 - 15, 60, 30);
                    gfx.endFill();
                    gfx.beginFill(bkCol, 0.06);
                    gfx.drawRect(b.w / 2 - 40, h / 2 - 5, 80, 10);
                    gfx.endFill();
                    // Cable junction strips
                    for (let si = 0; si < 6; si++) {
                        const sy = h - 20 + si * 3;
                        const col = [0x22d3ee, 0x4ade80, 0xf43f5e, 0xfacc15, 0x8b5cf6, 0x3b82f6][si];
                        gfx.beginFill(col, 0.2);
                        gfx.drawRect(0, sy, b.w, 2);
                        gfx.endFill();
                    }
                    ...
                    for (let di = 0; di < 3; di++) {
                        ...
                        // Dish mount
                        gfx.beginFill(0x64748b);
                        gfx.drawRect(dx + 10, 6, 4, 16);
                        gfx.endFill();
                        // Dish
                        gfx.beginFill(0xcbd5e1, 0.25);
                        gfx.drawEllipse(dx + 12, 6, 22, 8);
                        gfx.endFill();
                        gfx.beginFill(0xe2e8f0, 0.15);
                        gfx.drawEllipse(dx + 12, 6, 18, 6);
                        gfx.endFill();
                        // Feed horn
                        gfx.beginFill(0xa855f7, 0.5);
                        gfx.drawCircle(dx + 12, 6, 2);
                        gfx.endFill();
                    }
                    ...
                    for (let ri = 0; ri < 5; ri++) {
                        ...
                        gfx.beginFill(0xf97316, 0.08);
                        gfx.drawRect(4, ry, b.w - 8, 8);
                        gfx.endFill();
                        // Status bar
                        const fill = 0.4 + this._labNoise((b.x | 0) + ri * 47) * 0.6;
                        gfx.beginFill(0x4ade80, 0.2);
                        gfx.drawRect(6, ry + 2, (b.w - 12) * fill, 4);
                        gfx.endFill();
                    }
                    ...
                    for (let fi = 1; fi < floors - 1; fi++) {
                        ...
                        for (let sx = 10; sx < b.w - 20; sx += 35) {
                            gfx.beginFill(0x0a0818);
                            gfx.drawRect(sx, fy, 28, 12);
                            gfx.endFill();
                            gfx.beginFill(bkCol, 0.06);
                            gfx.drawRect(sx + 1, fy + 1, 26, 10);
                            gfx.endFill();
                        }
                    }
                    ...
                    for (let ai = 0; ai < 3; ai++) {
                        const ax = 40 + ai * 65;
                        gfx.beginFill(0x64748b);
                        gfx.drawRect(ax, -8, 3, 12);
                        gfx.endFill();
                        gfx.beginFill(0x94a3b8);
                        gfx.drawRect(ax - 5, -4, 13, 2);
                        gfx.endFill();
                        gfx.beginFill(0xef4444, 0.5);
                        gfx.drawCircle(ax + 1, -8, 1.5);
                        gfx.endFill();
                    }
                }
                // Base / foundation
                gfx.beginFill(0x111e30);
                gfx.drawRect(0, h - 4, b.w, 4);
                gfx.endFill();
                gfx.beginFill(bkCol, 0.1);
                gfx.drawRect(0, h - 4, b.w, 1);
                gfx.endFill();
                ...
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                gfx.beginFill(agCol, 0.05);
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                // Vertical panel lines (circuit board feel)
                for (let px = 0; px < b.w; px += 28) {
                    gfx.beginFill(0x14101e);
                    gfx.drawRect(px, 0, 1, h);
                    gfx.endFill();
                }
                // Brand crown band
                gfx.beginFill(agCol, 0.9);
                gfx.drawRect(0, 0, b.w, 5);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.14);
                gfx.drawRect(0, 0, b.w, 1);
                gfx.endFill();
                gfx.beginFill(agCol, 0.1);
                gfx.drawRect(0, 5, b.w, 3);
                gfx.endFill();
                // Left edge highlight (glass tube read)
                gfx.beginFill(0xffffff, 0.03);
                gfx.drawRect(0, 8, 3, h - 8);
                gfx.endFill();
                // Floor slabs
                for (let fi = 1; fi < floors; fi++) {
                    const fy = fi * 18;
                    gfx.beginFill(0x1a1530);
                    gfx.drawRect(0, fy, b.w, 2);
                    gfx.endFill();
                }
                ...
                if (b.id === 'agents_orchestrator') {
                    ...
                    const cy = h * 0.5;
                    gfx.beginFill(agCol, 0.12);
                    gfx.drawCircle(b.w * 0.25, cy, 6);
                    gfx.drawCircle(b.w * 0.5, cy - 6, 6);
                    gfx.drawCircle(b.w * 0.75, cy, 6);
                    gfx.endFill();
                    gfx.lineStyle(1, agCol, 0.08);
                    gfx.moveTo(b.w * 0.25 + 6, cy);
                    gfx.lineTo(b.w * 0.5 - 6, cy - 6);
                    gfx.moveTo(b.w * 0.5 + 6, cy - 6);
                    gfx.lineTo(b.w * 0.75 - 6, cy);
                    gfx.lineStyle(0);
                    ...
                    for (let bi = 0; bi < 5; bi++) {
                        ...
                        gfx.beginFill(bc, 0.12);
                        gfx.drawRect(b.w * 0.2 + bi * 14, h * 0.6 - bh, 8, bh);
                        gfx.endFill();
                    }
                    ...
                    for (let ti = 0; ti < 4; ti++) {
                        gfx.beginFill(0xfbbf24, 0.06);
                        gfx.drawRect(b.w * 0.15, h * 0.3 + ti * 12, b.w * 0.7, 2);
                        gfx.endFill();
                    }
                }
                ...
                if (b.id === 'agents_orchestrator' || b.id === 'agents_sandbox') {
                    for (let ai = 0; ai < 3; ai++) {
                        const ax = 30 + ai * 60;
                        gfx.beginFill(0x64748b);
                        gfx.drawRect(ax, -8, 2, 12);
                        gfx.endFill();
                        gfx.beginFill(agCol, 0.5);
                        gfx.drawCircle(ax + 1, -9, 1.5);
                        gfx.endFill();
                    }
                }
                ...
                if (b.id === 'power_solar') {
                    ...
                    gfx.beginFill(0x2a2a20);
                    gfx.drawRect(0, h - 10, b.w, 10);
                    gfx.endFill();
                    gfx.beginFill(0x3a3a2c);
                    gfx.drawRect(0, h - 10, b.w, 3);
                    gfx.endFill();
                    // Perimeter fence posts
                    gfx.beginFill(0x4b5563, 0.6);
                    for (let fx2 = 2; fx2 < b.w; fx2 += 16) gfx.drawRect(fx2, h - 16, 1.5, 7);
                    gfx.endFill();
                    gfx.beginFill(0x64748b, 0.25);
                    gfx.drawRect(0, h - 15, b.w, 1);
                    gfx.endFill();
                    // Megapack-style battery containers (the BESS half of Solar + Storage)
                    for (let bi = 0; bi < 2; bi++) {
                        const bx2 = b.w - 78 + bi * 30;
                        gfx.beginFill(0xe5e7eb);
                        gfx.drawRect(bx2, h - 32, 26, 22);
                        gfx.endFill();
                        gfx.beginFill(0xcbd5e1);
                        gfx.drawRect(bx2, h - 32, 26, 3);
                        gfx.endFill();
                        gfx.beginFill(0x9ca3af);
                        gfx.drawRect(bx2, h - 14, 26, 2);
                        gfx.endFill();
                        ...
                        gfx.endFill();
                        gfx.beginFill(0x4ade80);
                        gfx.drawCircle(bx2 + 22, h - 29, 1.5);
                        gfx.endFill();
                    }
                    // Inverter shed
                    gfx.beginFill(0x475569);
                    gfx.drawRect(b.w - 40, h - 52, 32, 18);
                    gfx.endFill();
                    gfx.beginFill(0x334155);
                    gfx.drawRect(b.w - 40, h - 54, 32, 3);
                    gfx.endFill();
                    gfx.beginFill(0x22d3ee, 0.4);
                    gfx.drawRect(b.w - 33, h - 48, 9, 7);
                    gfx.endFill();
                    gfx.beginFill(0x64748b);
                    gfx.drawRect(b.w - 24, h - 34, 3, 2);
                    gfx.endFill(); // conduit
                    ...
                    for (let px = 8; px < b.w - 90; px += 32) {
                        gfx.beginFill(0x64748b);
                        gfx.drawRect(px + 13, h - 38, 3, 28);
                        gfx.endFill();
                        gfx.beginFill(0x64748b);
                        gfx.drawRect(px + 4, h - 38, 22, 2);
                        gfx.endFill();
                        const py = h - 44 - tilt * 8;
                        gfx.beginFill(0x1e3a8a);
                        gfx.drawRect(px, py, 28, 10);
                        gfx.endFill();
                        gfx.beginFill(0x2563eb, 0.5);
                        gfx.drawRect(px + 1, py + 1, 12, 8);
                        gfx.drawRect(px + 15, py + 1, 12, 8);
                        gfx.endFill();
                        // Glass glint tracks the sun
                        gfx.beginFill(0x93c5fd, 0.35);
                        gfx.drawRect(px + 2 + tilt * 30, py + 1, 5, 8);
                        gfx.endFill();
                        gfx.beginFill(0x3b82f6, 0.3);
                        gfx.drawRect(px, py, 28, 2);
                        gfx.endFill();
                    }
                    ...
                    gfx.drawRect(0, h - 10, b.w, 10);
                    gfx.endFill();
                    gfx.beginFill(0x2d4a2d, 0.7);
                    gfx.drawRect(0, h - 11, b.w, 4);
                    gfx.endFill();
                    ...
                    // Control cabin
                    gfx.beginFill(0x475569);
                    gfx.drawRect(5, h - 26, 25, 16);
                    gfx.endFill();
                    gfx.beginFill(0x334155);
                    gfx.drawRect(5, h - 28, 25, 3);
                    gfx.endFill();
                    gfx.beginFill(0x22d3ee, 0.3);
                    gfx.drawRect(10, h - 22, 8, 6);
                    gfx.endFill();
                    ...
                    for (let ti = 0; ti < 3; ti++) {
                        ...
                        // Foundation
                        gfx.beginFill(0x94a3b8);
                        gfx.drawRect(tx - 7, h - 12, 14, 4);
                        gfx.endFill();
                        gfx.beginFill(0x6b7280);
                        gfx.drawRect(tx - 5, h - 13, 10, 2);
                        gfx.endFill();
                        // Tapered tower with shade side
                        gfx.beginFill(0xf1f5f9, 0.9);
                        gfx.moveTo(tx - 4.5, h - 10);
                        gfx.lineTo(tx - 2, h - 72);
                        gfx.lineTo(tx + 2, h - 72);
                        gfx.lineTo(tx + 4.5, h - 10);
                        gfx.closePath();
                        gfx.endFill();
                        gfx.beginFill(0xcbd5e1, 0.7);
                        gfx.moveTo(tx + 1, h - 10);
                        gfx.lineTo(tx + 1.2, h - 72);
                        gfx.lineTo(tx + 2, h - 72);
                        gfx.lineTo(tx + 4.5, h - 10);
                        gfx.closePath();
                        gfx.endFill();
                        // Access door
                        gfx.beginFill(0x475569);
                        gfx.drawRect(tx - 2, h - 18, 4, 8);
                        gfx.endFill();
                        // Nacelle + hub + red aviation beacon
                        gfx.beginFill(0xe2e8f0);
                        gfx.drawRoundedRect(tx - 6, h - 76, 12, 7, 2);
                        gfx.endFill();
                        gfx.beginFill(0xb6c2ce);
                        gfx.drawRect(tx - 6, h - 70.5, 12, 1.5);
                        gfx.endFill();
                        gfx.beginFill(0x94a3b8);
                        gfx.drawCircle(tx, h - 72, 4);
                        gfx.endFill();
                        gfx.beginFill(0xef4444, 0.8);
                        gfx.drawCircle(tx, h - 77.5, 1.3);
                        gfx.endFill();
                    }
                    ...
                    [26, 78].forEach((cx2) => {
                        ...
                        gfx.lineTo(cx2 - 12, h - 62);
                        gfx.lineTo(cx2 - 15, h - 108);
                        gfx.lineTo(cx2 + 15, h - 108);
                        gfx.lineTo(cx2 + 12, h - 62);
                        ...
                        gfx.lineTo(cx2 + 5, h - 62);
                        gfx.lineTo(cx2 + 7, h - 108);
                        gfx.lineTo(cx2 + 15, h - 108);
                        gfx.lineTo(cx2 + 12, h - 62);
                        gfx.lineTo(cx2 + 22, h - 8);
                        gfx.closePath();
                        ...
                        gfx.beginFill(0x8d949c);
                        gfx.drawRect(cx2 - 15, h - 110, 30, 4);
                        gfx.endFill();
                        gfx.beginFill(0x565e66);
                        gfx.drawRect(cx2 - 12, h - 108, 24, 2);
                        gfx.endFill();
                    ...
                    gfx.beginFill(0xc7ccd1);
                    gfx.drawCircle(b.w - 42, h - 44, 24);
                    gfx.endFill();
                    gfx.beginFill(0x9aa2ab, 0.7);
                    gfx.drawCircle(b.w - 36, h - 40, 20);
                    gfx.endFill();
                    gfx.beginFill(0xc7ccd1);
                    gfx.drawCircle(b.w - 44, h - 46, 19);
                    gfx.endFill();
                    gfx.beginFill(0x565e66);
                    gfx.drawRect(b.w - 66, h - 44, 48, 36);
                    gfx.endFill();
                    // Turbine hall connecting dome to towers
                    gfx.beginFill(0x334155);
                    gfx.drawRect(b.w - 78, h - 36, 70, 28);
                    gfx.endFill();
                    gfx.beginFill(0x475569);
                    gfx.drawRect(b.w - 78, h - 38, 70, 3);
                    gfx.endFill();
                    for (let wx = b.w - 72; wx < b.w - 14; wx += 15) {
                        gfx.beginFill(0x22d3ee, 0.35);
                        gfx.drawRect(wx, h - 30, 9, 8);
                        gfx.endFill();
                    }
                    // Microsoft PPA accent — the whole plant is sold to one buyer
                    gfx.beginFill(0x0ea5e9, 0.55);
                    gfx.drawRect(b.w - 78, h - 40, 70, 2);
                    gfx.endFill();
                    // Switchyard: transformer + bushings
                    gfx.beginFill(0x374151);
                    gfx.drawRect(104, h - 26, 18, 18);
                    gfx.endFill();
                    gfx.beginFill(0x6b7280);
                    gfx.drawRect(106, h - 32, 2, 6);
                    gfx.drawRect(112, h - 32, 2, 6);
                    gfx.drawRect(118, h - 32, 2, 6);
                    gfx.endFill();
                    gfx.beginFill(0xe5e7eb, 0.8);
                    gfx.drawCircle(107, h - 33, 1.2);
                    gfx.drawCircle(113, h - 33, 1.2);
                    gfx.drawCircle(119, h - 33, 1.2);
                    gfx.endFill();
                    // Radiological warning placard
                    gfx.beginFill(0xfbbf24, 0.85);
                    gfx.drawRect(126, h - 22, 10, 10);
                    gfx.endFill();
                    gfx.beginFill(0x1f2937);
                    gfx.drawCircle(131, h - 17, 2.4);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawCircle(131, h - 17, 1);
                    gfx.endFill();
                    ...
                    for (let gi = 0; gi < 4; gi++) {
                        ...
                        gfx.beginFill(0xe5e7eb);
                        gfx.drawRect(gx2 + 10, h - 26, 16, 18);
                        gfx.endFill();
                        gfx.beginFill(0xf3f4f6);
                        gfx.drawRect(gx2 + 10, h - 26, 16, 3);
                        gfx.endFill();
                        gfx.beginFill(0x9ca3af);
                        gfx.drawRect(gx2 + 10, h - 10, 16, 2);
                        gfx.endFill();
                        // Exhaust stack with heat-darkened tip
                        gfx.beginFill(0xb0b7bf);
                        gfx.drawRect(gx2 + 17, h - 58, 7, 32);
                        gfx.endFill();
                        gfx.beginFill(0x8a929b);
                        gfx.drawRect(gx2 + 21.5, h - 58, 2.5, 32);
                        gfx.endFill();
                        gfx.beginFill(0x4b5563);
                        gfx.drawRect(gx2 + 16, h - 62, 9, 5);
                        gfx.endFill();
                        gfx.beginFill(0xef4444, 0.7);
                        gfx.drawCircle(gx2 + 20.5, h - 63.5, 1.2);
                        gfx.endFill();
                    }
                    // Substation transformer feeding the campus next door
                    gfx.beginFill(0x374151);
                    gfx.drawRect(b.w - 34, h - 30, 22, 22);
                    gfx.endFill();
                    gfx.beginFill(0x6b7280);
                    gfx.drawRect(b.w - 31, h - 36, 2, 6);
                    gfx.drawRect(b.w - 24, h - 36, 2, 6);
                    gfx.drawRect(b.w - 17, h - 36, 2, 6);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24, 0.6);
                    gfx.drawRect(b.w - 34, h - 30, 22, 2);
                    gfx.endFill();
                    ...
                    gfx.beginFill(0x155e75, 0.5);
                    gfx.drawRect(0, h - 78, b.w, 6);
                    gfx.endFill();
                    ...
                    gfx.lineTo(20, h - 30);
                    gfx.lineTo(25, h - 8);
                    gfx.lineTo(b.w - 25, h - 8);
                    gfx.lineTo(b.w - 20, h - 30);
                    gfx.lineTo(b.w - 14, h - 52);
                    gfx.lineTo(b.w - 5, h - 74);
                    gfx.closePath();
                    gfx.endFill();
                    // Shade side + expansion joint lines
                    gfx.beginFill(0x7d8896, 0.6);
                    gfx.moveTo(b.w * 0.62, h - 74);
                    gfx.lineTo(b.w * 0.66, h - 8);
                    gfx.lineTo(b.w - 25, h - 8);
                    gfx.lineTo(b.w - 14, h - 52);
                    gfx.lineTo(b.w - 5, h - 74);
                    gfx.closePath();
                    ...
                    gfx.beginFill(0x64748b);
                    gfx.drawRect(2, h - 78, b.w - 4, 5);
                    gfx.endFill();
                    gfx.beginFill(0x94a3b8, 0.7);
                    for (let rx2 = 6; rx2 < b.w - 6; rx2 += 10) gfx.drawRect(rx2, h - 82, 1.2, 4);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(b.w * 0.3, h - 92, 3, 14);
                    gfx.drawRect(b.w * 0.3 - 6, h - 92, 16, 3);
                    gfx.endFill();
                    // Spillway gates with falling water + stilling-basin foam
                    [b.w * 0.32, b.w * 0.5, b.w * 0.68].forEach((sx3) => {
                        gfx.beginFill(0x334155);
                        gfx.drawRect(sx3 - 7, h - 68, 14, 8);
                        gfx.endFill();
                        gfx.beginFill(0x7dd3fc, 0.55);
                        gfx.drawRect(sx3 - 5, h - 60, 10, 50);
                        gfx.endFill();
                        gfx.beginFill(0xe0f2fe, 0.5);
                        gfx.drawRect(sx3 - 2, h - 60, 3, 50);
                        gfx.endFill();
                        gfx.beginFill(0xffffff, 0.45);
                        gfx.drawEllipse(sx3, h - 9, 9, 3.5);
                        gfx.endFill();
                    });
                    // Powerhouse at the toe with lit windows
                    gfx.beginFill(0x475569);
                    gfx.drawRect(b.w / 2 - 22, h - 26, 44, 18);
                    gfx.endFill();
                    gfx.beginFill(0x334155);
                    gfx.drawRect(b.w / 2 - 22, h - 28, 44, 3);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24, 0.5);
                    gfx.drawRect(b.w / 2 - 16, h - 21, 8, 6);
                    gfx.drawRect(b.w / 2 - 4, h - 21, 8, 6);
                    gfx.drawRect(b.w / 2 + 8, h - 21, 8, 6);
                    gfx.endFill();
                } else if (b.id === 'power_smr') {
                    // ── HERMES 2 SMR (Kairos × Google) — under construction ──
                    gfx.beginFill(0x8b7355, 0.5);
                    gfx.drawRect(0, h - 8, b.w, 8);
                    gfx.endFill(); // dirt site
                    gfx.beginFill(0x6b7280);
                    gfx.drawRect(4, h - 10, b.w - 8, 3);
                    gfx.endFill();
                    // Reactor hall (modern, teal-trimmed)
                    gfx.beginFill(0x334155);
                    gfx.drawRect(10, h - 48, 66, 40);
                    gfx.endFill();
                    gfx.beginFill(0x2dd4bf, 0.6);
                    gfx.drawRect(10, h - 50, 66, 3);
                    gfx.endFill();
                    for (let wx = 16; wx < 68; wx += 16) {
                        gfx.beginFill(0x2dd4bf, 0.3);
                        gfx.drawRect(wx, h - 42, 10, 8);
                        gfx.endFill();
                    }
                    ...
                    [
                        ...
                        gfx.beginFill(dc, 0.9);
                        gfx.drawCircle(dx2, h - 20, 2);
                        gfx.endFill();
                    });
                    // Twin reactor modules with dome caps (one still skeletal)
                    gfx.beginFill(0xd1d5db);
                    gfx.drawRect(86, h - 40, 18, 32);
                    gfx.endFill();
                    gfx.beginFill(0xd1d5db);
                    gfx.drawCircle(95, h - 40, 9);
                    gfx.endFill();
                    gfx.beginFill(0x9ca3af, 0.6);
                    gfx.drawRect(99, h - 40, 5, 32);
                    gfx.endFill();
                    ...
                    gfx.beginFill(0xf59e0b);
                    gfx.drawRect(120, h - 74, 3, 66);
                    gfx.endFill();
                    gfx.beginFill(0xf59e0b);
                    gfx.drawRect(96, h - 74, 52, 3);
                    gfx.endFill();
                    gfx.beginFill(0xf59e0b, 0.7);
                    gfx.drawRect(142, h - 71, 2, 8);
                    gfx.endFill(); // counter-jib tie
                    gfx.beginFill(0x94a3b8);
                    gfx.drawRect(103, h - 71, 1.5, 16);
                    gfx.endFill(); // hook cable
                    gfx.beginFill(0x6b7280);
                    gfx.drawRect(100, h - 55, 8, 3);
                    gfx.endFill(); // hook block
                    gfx.beginFill(0xef4444, 0.8);
                    gfx.drawCircle(121.5, h - 76, 1.5);
                    gfx.endFill();
                } else if (b.id === 'power_fusion') {
                    // ── POLARIS FUSION (Helion × Microsoft) — pulsed FRC machine hall ──
                    gfx.beginFill(0x1f2430);
                    gfx.drawRect(0, h - 8, b.w, 8);
                    gfx.endFill();
                    // Sleek dark hall
                    gfx.beginFill(0x171c28);
                    gfx.drawRect(8, h - 54, b.w - 16, 46);
                    gfx.endFill();
                    gfx.beginFill(0x232a3a);
                    gfx.drawRect(8, h - 54, b.w - 16, 4);
                    gfx.endFill();
                    // Roofline LED strip (Helion purple)
                    gfx.beginFill(0xc084fc, 0.7);
                    gfx.drawRect(8, h - 56, b.w - 16, 2);
                    gfx.endFill();
                    ...
                    [
                        ...
                        gfx.beginFill(0x374151);
                        gfx.drawRect(cx3, cy3, 28, 26);
                        gfx.endFill();
                        gfx.beginFill(0x4b5563);
                        gfx.drawRect(cx3, cy3, 28, 3);
                        gfx.endFill();
                        gfx.beginFill(0xc084fc, 0.5);
                        for (let ly3 = cy3 + 6; ly3 < cy3 + 22; ly3 += 5) gfx.drawRect(cx3 + 3, ly3, 22, 1.5);
                        gfx.endFill();
                        gfx.beginFill(0xfbbf24, 0.8);
                        gfx.drawCircle(cx3 + 24, cy3 + 4.5, 1.2);
                        gfx.endFill();
                    });
                    // Central porthole revealing the FRC plasma (glow ring animated by PowerEnv)
                    const pcx = b.w / 2,
                        pcy = h - 32;
                    gfx.beginFill(0x0b0e16);
                    gfx.drawCircle(pcx, pcy, 15);
                    gfx.endFill();
                    gfx.lineStyle(2, 0x475569, 0.9);
                    gfx.drawCircle(pcx, pcy, 15);
                    gfx.lineStyle(0);
                    gfx.beginFill(0xf0abfc, 0.25);
                    gfx.drawCircle(pcx, pcy, 11);
                    gfx.endFill();
                    gfx.beginFill(0x22d3ee, 0.35);
                    gfx.drawCircle(pcx, pcy, 7);
                    gfx.endFill();
                    gfx.beginFill(0xffffff, 0.5);
                    gfx.drawCircle(pcx, pcy, 2.5);
                    gfx.endFill();
                    // Pulsing plasma halo — registered with PowerEnv for animation
                    const plasmaGlow = new PIXI.Graphics();
                    plasmaGlow.beginFill(0xe879f9, 0.5);
                    plasmaGlow.drawCircle(0, 0, 13);
                    plasmaGlow.endFill();
                    plasmaGlow.beginFill(0x22d3ee, 0.4);
                    plasmaGlow.drawCircle(0, 0, 8);
                    plasmaGlow.endFill();
                ...
                if (b.id === 'port_authority') {
                    ...
                    gfx.beginFill(0x1a2838);
                    gfx.drawRect(0, 0, b.w, h);
                    gfx.endFill();
                    gfx.beginFill(0x223848);
                    gfx.drawRect(0, 0, 8, h);
                    gfx.drawRect(b.w - 8, 0, 8, h);
                    gfx.endFill();
                    gfx.beginFill(0x2a4858);
                    gfx.drawRect(-2, -4, b.w + 4, 8);
                    gfx.endFill();
                    // Pilot-boat orange band (international harbor authority stripe)
                    gfx.beginFill(0xf97316, 0.7);
                    gfx.drawRect(0, 8, b.w, 3);
                    gfx.endFill();
                    for (let fy = 16; fy < h - 12; fy += 18)
                        for (let fx = 16; fx < b.w - 16; fx += 20) {
                            gfx.beginFill(0xfbbf24, 0.5);
                            gfx.drawRect(fx, fy, 12, 10);
                            gfx.endFill();
                        }
                    // Top-floor harbor-control glazing (cyan, watching the quay)
                    gfx.beginFill(0x22d3ee, 0.35);
                    gfx.drawRect(12, 14, b.w - 24, 9);
                    gfx.endFill();
                    gfx.beginFill(0x0a1628);
                    gfx.drawRect(b.w / 2 - 10, h - 16, 20, 16);
                    gfx.endFill();
                    gfx.beginFill(0x22d3ee, 0.3);
                    gfx.drawRect(b.w / 2 - 8, h - 14, 16, 2);
                    gfx.endFill();
                    // Radar mast + dish and signal halyard
                    gfx.beginFill(0x94a3b8);
                    gfx.drawRect(b.w / 2 - 1.5, -20, 3, 16);
                    gfx.endFill();
                    gfx.beginFill(0xe2e8f0);
                    gfx.drawPolygon([b.w / 2 - 8, -18, b.w / 2, -24, b.w / 2 + 8, -18]);
                    gfx.endFill();
                    gfx.beginFill(0xef4444, 0.9);
                    gfx.drawCircle(b.w / 2, -25.5, 1.5);
                    gfx.endFill();
                    gfx.beginFill(0xef4444);
                    gfx.drawRect(b.w - 24, -14, 7, 5);
                    gfx.endFill(); // signal flags
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(b.w - 24, -8, 7, 5);
                    gfx.endFill();
                    gfx.beginFill(0x64748b);
                    gfx.drawRect(b.w - 18, -16, 1.5, 16);
                    gfx.endFill();
                    // Quay bollards
                    gfx.beginFill(0x334155);
                    gfx.drawRect(6, h - 6, 5, 6);
                    gfx.drawRect(b.w - 11, h - 6, 5, 6);
                    gfx.endFill();
                    const anch = new PIXI.Text('⚓', { fontFamily: emojiFontStack, fontSize: 14 });
                    anch.anchor.set(0.5, 0.5);
                    anch.x = b.w / 2;
                    anch.y = -32;
                    container.addChild(anch);
                } else if (b.id === 'port_customs') {
                    // Export Control Office — federal navy facade, barrier gate, gold seal
                    gfx.beginFill(0x14213d);
                    gfx.drawRect(0, 0, b.w - 26, h);
                    gfx.endFill();
                    gfx.beginFill(0x1d2d50);
                    gfx.drawRect(0, 0, 6, h);
                    gfx.drawRect(b.w - 32, 0, 6, h);
                    gfx.endFill();
                    gfx.beginFill(0xe5e7eb, 0.9);
                    gfx.drawRect(-2, -4, b.w - 22, 6);
                    gfx.endFill(); // white cornice
                    // Portico columns
                    gfx.beginFill(0xd1d5db, 0.85);
                    for (let cx2 = 12; cx2 < b.w - 38; cx2 += 18) gfx.drawRect(cx2, h - 26, 4, 26);
                    gfx.endFill();
                    gfx.beginFill(0xd1d5db, 0.85);
                    gfx.drawRect(8, h - 28, b.w - 46, 3);
                    gfx.endFill();
                    // Lit paperwork windows (the licenses never sleep)
                    for (let fy = 10; fy < h - 32; fy += 16)
                        for (let fx = 14; fx < b.w - 42; fx += 18) {
                            gfx.beginFill(0xfef3c7, 0.55);
                            gfx.drawRect(fx, fy, 11, 9);
                            gfx.endFill();
                        }
                    // Gold department seal
                    gfx.beginFill(0xca8a04);
                    gfx.drawCircle((b.w - 26) / 2, h * 0.42, 7);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24);
                    gfx.drawCircle((b.w - 26) / 2, h * 0.42, 5);
                    gfx.endFill();
                    gfx.beginFill(0x14213d);
                    gfx.drawPolygon([
                        (b.w - 26) / 2,
                    ...
                    gfx.beginFill(0x374151);
                    gfx.drawRect(b.w - 24, h - 6, 24, 6);
                    gfx.endFill();
                    gfx.beginFill(0x4b5563);
                    gfx.drawRect(b.w - 22, h - 18, 5, 12);
                    gfx.endFill();
                    gfx.beginFill(0xef4444);
                    gfx.drawRect(b.w - 18, h - 17, 18, 3);
                    gfx.endFill();
                    gfx.beginFill(0xffffff);
                    for (let bx2 = b.w - 16; bx2 < b.w - 2; bx2 += 6) gfx.drawRect(bx2, h - 17, 3, 3);
                    gfx.endFill();
                    gfx.beginFill(0xef4444, 0.9);
                    gfx.drawCircle(b.w - 19.5, h - 21, 1.5);
                    gfx.endFill();
                } else if (b.id === 'port_warehouse') {
                    // GPU Warehouse — bonded corrugated hall, guarded 24/7
                    gfx.beginFill(0x1e293b);
                    gfx.drawRect(0, 6, b.w, h - 6);
                    gfx.endFill();
                    gfx.beginFill(0x27364a, 0.8);
                    gfx.drawRect(0, 6, b.w, 10);
                    gfx.endFill();
                    // Corrugated roof + skylight strip + HVAC
                    gfx.beginFill(0x334155);
                    for (let rx = 0; rx < b.w; rx += 8) {
                        gfx.drawRect(rx, 0, 4, 8);
                    }
                    gfx.endFill();
                    gfx.beginFill(0x475569);
                    gfx.drawRect(0, 0, b.w, 4);
                    gfx.endFill();
                    gfx.beginFill(0x38bdf8, 0.25);
                    gfx.drawRect(20, 2, b.w - 90, 3);
                    gfx.endFill();
                    gfx.beginFill(0x64748b);
                    gfx.drawRect(b.w - 52, -6, 16, 8);
                    gfx.drawRect(b.w - 30, -6, 16, 8);
                    gfx.endFill();
                    gfx.beginFill(0x94a3b8, 0.6);
                    gfx.drawRect(b.w - 50, -4, 12, 1.5);
                    gfx.drawRect(b.w - 28, -4, 12, 1.5);
                    gfx.endFill();
                    // Wall stencil: big GPU crate label
                    gfx.beginFill(0x76b900, 0.18);
                    gfx.drawRect(b.w / 2 - 26, 16, 52, 14);
                    gfx.endFill();
                    gfx.beginFill(0x76b900, 0.5);
                    gfx.drawRect(b.w / 2 - 26, 16, 52, 2);
                    gfx.endFill();
                    // Loading bay doors with status lights
                    let doorIdx = 0;
                    for (let dx = 15; dx < b.w - 30; dx += 50) {
                        gfx.beginFill(0x0a1628);
                        gfx.drawRect(dx, h - 30, 35, 30);
                        gfx.endFill();
                        gfx.beginFill(0xf59e0b, 0.2);
                        gfx.drawRect(dx + 2, h - 28, 31, 2);
                        gfx.drawRect(dx + 2, h - 20, 31, 2);
                        gfx.drawRect(dx + 2, h - 12, 31, 2);
                        gfx.endFill();
                        gfx.beginFill(doorIdx % 2 === 0 ? 0x4ade80 : 0xf59e0b, 0.9);
                        gfx.drawCircle(dx + 32, h - 32, 1.8);
                        gfx.endFill();
                        doorIdx++;
                    }
                    // Guard booth + camera
                    gfx.beginFill(0x334155);
                    gfx.drawRect(b.w - 16, h - 18, 14, 18);
                    gfx.endFill();
                    gfx.beginFill(0xfef3c7, 0.6);
                    gfx.drawRect(b.w - 13, h - 15, 8, 6);
                    gfx.endFill();
                    gfx.beginFill(0x94a3b8);
                    gfx.drawRect(2, 8, 2, 4);
                    gfx.endFill();
                    gfx.beginFill(0x1f2937);
                    gfx.drawRect(1, 6, 5, 3);
                    gfx.endFill();
                    gfx.beginFill(0xef4444, 0.9);
                    gfx.drawCircle(5.5, 7.5, 0.8);
                    gfx.endFill();
                    // NVIDIA green accent
                    gfx.beginFill(0x76b900, 0.3);
                    gfx.drawRect(0, h - 2, b.w, 4);
                    gfx.endFill();
                } else if (b.id === 'port_container') {
                    // Container Terminal — brand-color steel stacks + reach stacker
                    gfx.beginFill(0x2b3646);
                    gfx.drawRect(0, h - 8, b.w, 8);
                    gfx.endFill();
                    ...
                    stackCols.forEach((stack, siIdx) => {
                        ...
                        stack.forEach((scol, r) => {
                            gfx.beginFill(scol);
                            gfx.drawRect(sx2, h - 22 - r * 14, 30, 12);
                            gfx.endFill();
                            gfx.beginFill(0x000000, 0.18);
                            gfx.drawRect(sx2, h - 22 - r * 14, 30, 2);
                            gfx.endFill();
                            gfx.beginFill(0xffffff, 0.1);
                            for (let cx3 = sx2 + 3; cx3 < sx2 + 28; cx3 += 5)
                                gfx.drawRect(cx3, h - 20 - r * 14, 1.5, 8);
                            gfx.endFill();
                        });
                    });
                    ...
                    const rsx = b.w - 20;
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(rsx - 14, h - 18, 22, 10);
                    gfx.endFill();
                    gfx.beginFill(0x1f2937);
                    gfx.drawCircle(rsx - 9, h - 7, 4);
                    gfx.drawCircle(rsx + 3, h - 7, 4);
                    gfx.endFill();
                    gfx.beginFill(0x475569);
                    gfx.drawRect(rsx - 12, h - 26, 6, 9);
                    gfx.endFill(); // cab
                    gfx.beginFill(0xfef3c7, 0.6);
                    gfx.drawRect(rsx - 11, h - 25, 4, 4);
                    gfx.endFill();
                    gfx.beginFill(0x94a3b8);
                    gfx.drawPolygon([rsx - 4, h - 16, rsx + 14, h - 34, rsx + 16, h - 31, rsx - 1, h - 14]);
                    gfx.endFill(); // boom
                    gfx.beginFill(0x76b900);
                    gfx.drawRect(rsx + 8, h - 32, 18, 9);
                    gfx.endFill(); // lifted box
                    gfx.beginFill(0x000000, 0.15);
                    gfx.drawRect(rsx + 8, h - 32, 18, 1.5);
                    gfx.endFill();
                    // Floodlight mast
                    gfx.beginFill(0x64748b);
                    gfx.drawRect(2, h - 52, 3, 44);
                    gfx.endFill();
                    gfx.beginFill(0xfef3c7, 0.9);
                    gfx.drawRect(-2, h - 56, 11, 4);
                    gfx.endFill();
                    gfx.beginFill(0xfef3c7, 0.08);
                    gfx.drawPolygon([-2, h - 52, 9, h - 52, 26, h - 8, -14, h - 8]);
                    gfx.endFill();
                    ...
                    const spx = 30,
                        spy = h - 34;
                    gfx.beginFill(0x334155);
                    gfx.drawRect(spx - 14, h - 14, 4, 8);
                    gfx.drawRect(spx + 10, h - 14, 4, 8);
                    gfx.endFill();
                    gfx.beginFill(0xf1f5f9);
                    gfx.drawCircle(spx, spy, 22);
                    gfx.endFill();
                    gfx.beginFill(0xdbeafe, 0.8);
                    gfx.drawCircle(spx - 6, spy - 7, 12);
                    gfx.endFill();
                    gfx.beginFill(0x93c5fd, 0.5);
                    gfx.drawEllipse(spx, spy - 16, 14, 5);
                    gfx.endFill(); // frost crown
                    gfx.beginFill(0x1d4ed8, 0.8);
                    gfx.drawRect(spx - 20, spy - 2, 40, 5);
                    gfx.endFill(); // LHe band
                    gfx.beginFill(0xffffff, 0.35);
                    gfx.drawCircle(spx - 24, spy - 18, 2.5);
                    gfx.drawCircle(spx - 27, spy - 13, 1.8);
                    gfx.endFill(); // boil-off wisps
                    // Horizontal diesel tank
                    gfx.beginFill(0x6b7280);
                    gfx.drawRoundedRect(62, h - 30, 50, 20, 9);
                    gfx.endFill();
                    gfx.beginFill(0x9ca3af, 0.6);
                    gfx.drawRoundedRect(62, h - 30, 50, 6, 3);
                    gfx.endFill();
                    gfx.beginFill(0x334155);
                    gfx.drawRect(68, h - 11, 5, 3);
                    gfx.drawRect(100, h - 11, 5, 3);
                    gfx.endFill();
                    // Hazard diamond
                    gfx.beginFill(0xef4444);
                    gfx.drawPolygon([87, h - 27, 92, h - 22, 87, h - 17, 82, h - 22]);
                    gfx.endFill();
                    gfx.beginFill(0xffffff, 0.9);
                    gfx.drawRect(85.5, h - 24, 3, 4);
                    gfx.endFill();
                    // Manifold pipes linking sphere → tank → quay
                    gfx.beginFill(0x475569);
                    gfx.drawRect(spx + 20, h - 16, 44, 3);
                    gfx.endFill();
                    gfx.beginFill(0x22d3ee, 0.4);
                    gfx.drawRect(spx + 20, h - 15.5, 44, 1);
                    gfx.endFill();
                    gfx.beginFill(0x374151);
                    gfx.drawRect(56, h - 19, 4, 8);
                    gfx.endFill();
                    gfx.beginFill(0xef4444);
                    gfx.drawCircle(58, h - 20, 1.8);
                    gfx.endFill();
                    // Hazard stripes
                    gfx.beginFill(0xfbbf24);
                    for (let sx = 0; sx < b.w; sx += 12) {
                        gfx.drawRect(sx, h - 10, 6, 3);
                    }
                    gfx.endFill();
                    ...
                    gfx.beginFill(0xf59e0b);
                    gfx.drawRect(10, h - 8, 8, -70);
                    gfx.drawRect(b.w - 18, h - 8, 8, -70);
                    gfx.endFill();
                    gfx.lineStyle(2, 0xd97706, 0.8);
                    gfx.moveTo(14, h - 14);
                    gfx.lineTo(b.w - 14, h - 44);
                    gfx.moveTo(b.w - 14, h - 14);
                    gfx.lineTo(14, h - 44);
                    gfx.moveTo(14, h - 44);
                    gfx.lineTo(b.w - 14, h - 72);
                    gfx.lineStyle(0);
                    // Main beam extends over the water (left)
                    gfx.beginFill(0xfbbf24);
                    gfx.drawRect(-26, h - 78, b.w + 26, 8);
                    gfx.endFill();
                    gfx.beginFill(0xd97706);
                    gfx.drawRect(-26, h - 71, b.w + 26, 2);
                    gfx.endFill();
                    ...
                    gfx.moveTo(b.w / 2, h - 98);
                    gfx.lineTo(-22, h - 76);
                    gfx.moveTo(b.w / 2, h - 98);
                    gfx.lineTo(b.w - 6, h - 76);
                    gfx.lineStyle(0);
                    gfx.beginFill(0xef4444, 0.9);
                    gfx.drawCircle(b.w / 2, h - 102, 2);
                    gfx.endFill();
                    // Machinery house + operator cab
                    gfx.beginFill(0x475569);
                    gfx.drawRect(b.w - 34, h - 92, 22, 12);
                    gfx.endFill();
                    gfx.beginFill(0x334155);
                    gfx.drawRect(4, h - 70, 12, 9);
                    gfx.endFill();
                    gfx.beginFill(0x38bdf8, 0.6);
                    gfx.drawRect(6, h - 68, 8, 5);
                    gfx.endFill();
                ...
                // Main structure
                gfx.beginFill(0x1a2030);
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                // Side walls
                gfx.beginFill(0x222a38);
                gfx.drawRect(0, 0, 6, h);
                gfx.drawRect(b.w - 6, 0, 6, h);
                gfx.endFill();
                // Roof parapet
                gfx.beginFill(0x334155);
                gfx.drawRect(-2, -4, b.w + 4, 8);
                gfx.endFill();
                gfx.beginFill(0x475569);
                gfx.drawRect(-2, -4, b.w + 4, 3);
                gfx.endFill();
                // Floor lines
                for (let fy = 16; fy < h; fy += 18) {
                    gfx.beginFill(0x222a38);
                    gfx.drawRect(6, fy, b.w - 12, 2);
                    gfx.endFill();
                }
                ...
                // Door
                gfx.beginFill(0x0a0a18);
                gfx.drawRect(b.w / 2 - 8, h - 16, 16, 16);
                gfx.endFill();
                gfx.beginFill(0x334155);
                gfx.drawRect(b.w / 2 - 8, h - 16, 16, 2);
                gfx.endFill();
                gfx.beginFill(0xfbbf24, 0.4);
                gfx.drawCircle(b.w / 2 + 4, h - 8, 1);
                gfx.endFill();
                // Awning over door
                gfx.beginFill(0x475569);
                gfx.drawRect(b.w / 2 - 14, h - 20, 28, 3);
                gfx.endFill();
                // Worker icon badge
                ...
                const bodyH = h - 14;
                const bodyTop = 14;
                gfx.beginFill(p.wall);
                gfx.drawRect(10, bodyTop, b.w - 20, bodyH - bodyTop);
                gfx.endFill();
                // Side shadow
                gfx.beginFill(0x000000, 0.12);
                gfx.drawRect(b.w - 14, bodyTop, 4, bodyH - bodyTop);
                gfx.endFill();
                // Trim corners
                ...
                for (let ry = bodyTop; ry > 2; ry -= 4) {
                    ...
                    gfx.moveTo(6 + inset, ry);
                    gfx.lineTo(b.w - 6 - inset, ry);
                }
                gfx.lineStyle(0);

                // Chimney
                gfx.beginFill(0x78716c);
                gfx.drawRect(b.w - 36, 4, 8, bodyTop - 4);
                gfx.endFill();
                gfx.beginFill(0x44403c);
                gfx.drawRect(b.w - 37, 3, 10, 3);
                gfx.endFill();

                // Attic gable window (circle)
                gfx.beginFill(p.window, 0.8);
                gfx.drawCircle(b.w / 2, bodyTop - 6, 4);
                gfx.endFill();
                gfx.lineStyle(1, p.trim, 0.8);
                gfx.drawCircle(b.w / 2, bodyTop - 6, 4);
                gfx.moveTo(b.w / 2 - 4, bodyTop - 6);
                gfx.lineTo(b.w / 2 + 4, bodyTop - 6);
                gfx.moveTo(b.w / 2, bodyTop - 10);
                gfx.lineTo(b.w / 2, bodyTop - 2);
                gfx.lineStyle(0);
                ...
                [0.28, 0.72].forEach((frac) => {
                    ...
                    gfx.beginFill(p.trim);
                    gfx.drawRect(wx - 1, upWinY - 1, 16, 14);
                    gfx.endFill();
                    const lit = this._labNoise((b.x | 0) + wx * 13) > 0.5;
                    gfx.beginFill(lit ? p.window : 0x0a0a18, lit ? 0.85 : 1);
                    gfx.drawRect(wx, upWinY, 14, 12);
                    gfx.endFill();
                    // Window mullions
                    gfx.lineStyle(1, p.trim, 0.8);
                    gfx.moveTo(wx + 7, upWinY);
                    gfx.lineTo(wx + 7, upWinY + 12);
                    gfx.moveTo(wx, upWinY + 6);
                    gfx.lineTo(wx + 14, upWinY + 6);
                    gfx.lineStyle(0);
                    // Shutters
                    gfx.beginFill(p.trim);
                    gfx.drawRect(wx - 4, upWinY, 3, 12);
                    gfx.drawRect(wx + 15, upWinY, 3, 12);
                    gfx.endFill();
                });
                ...
                const gWinX = 18,
                    gWinY = bodyTop + 30;
                gfx.beginFill(p.trim);
                gfx.drawRect(gWinX - 2, gWinY - 2, 30, 20);
                gfx.endFill();
                const gLit = this._labNoise((b.x | 0) + 71) > 0.4;
                gfx.beginFill(gLit ? p.window : 0x0a0a18, gLit ? 0.85 : 1);
                gfx.drawRect(gWinX, gWinY, 26, 16);
                gfx.endFill();
                gfx.lineStyle(1, p.trim, 0.8);
                gfx.moveTo(gWinX + 13, gWinY);
                gfx.lineTo(gWinX + 13, gWinY + 16);
                gfx.moveTo(gWinX, gWinY + 8);
                gfx.lineTo(gWinX + 26, gWinY + 8);
                gfx.lineStyle(0);
                ...
                const doorX = b.w - 38,
                    doorY = h - 30;
                gfx.beginFill(p.trim);
                gfx.drawRect(doorX - 2, doorY - 2, 16, 18);
                gfx.endFill();
                gfx.beginFill(p.door);
                gfx.drawRect(doorX, doorY, 12, 16);
                gfx.endFill();
                gfx.beginFill(0xfbbf24, 0.9);
                gfx.drawCircle(doorX + 9, doorY + 9, 1);
                gfx.endFill(); // knob
                // Porch light glow
                gfx.beginFill(0xfbbf24, 0.35);
                gfx.drawCircle(doorX + 6, doorY - 6, 5);
                gfx.endFill();
                gfx.beginFill(0xfbbf24);
                gfx.drawCircle(doorX + 6, doorY - 6, 1.2);
                gfx.endFill();
                // Porch step
                gfx.beginFill(0x78716c);
                gfx.drawRect(doorX - 4, doorY + 16, 20, 3);
                gfx.endFill();

                // Driveway + garage hint
                gfx.beginFill(0x52525b);
                gfx.drawRect(b.w - 18, h - 10, 14, 10);
                gfx.endFill();
                gfx.beginFill(0x3f3f46);
                gfx.drawRect(b.w - 18, h - 8, 14, 2);
                gfx.endFill();

                // Small shrub by door
                gfx.beginFill(0x166534);
                gfx.drawCircle(doorX - 6, h - 10, 3);
                gfx.endFill();
                gfx.beginFill(0x14532d);
                gfx.drawCircle(doorX - 6, h - 11, 2);
                gfx.endFill();

                // Tree on lawn
                const treeX = 20;
                gfx.beginFill(0x5c4033);
                gfx.drawRect(treeX, h - 20, 3, 14);
                gfx.endFill();
                gfx.beginFill(0x166534);
                gfx.drawCircle(treeX + 1, h - 24, 7);
                gfx.endFill();
                gfx.beginFill(0x15803d);
                gfx.drawCircle(treeX + 1, h - 25, 5);
                gfx.endFill();
                gfx.beginFill(0x22c55e);
                gfx.drawCircle(treeX + 1, h - 26, 3);
                gfx.endFill();

                // House number plaque
                gfx.beginFill(0x1c1917);
                gfx.drawRect(doorX - 6, doorY - 14, 10, 5);
                gfx.endFill();
                const plate = new PIXI.Text(String(100 + idNum * 4), {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 4,
                    fill: 0xf5f5f4,
                });
                ...
                for (let f = 0; f < rows; f++) {
                    for (let c = 0; c < cols; c++) {
                        if (f === rows - 1 && (c === Math.floor(cols / 2) || c === Math.floor(cols / 2) - 1))
                            continue;
                        const wx = 16 + c * 24,
                            wy = 20 + f * 18;
                        const lit = this._labNoise((b.x | 0) + f * 131 + c * 17) > 0.4;

                        gfx.beginFill(0x05050a, 0.8);
                        gfx.drawRect(wx - 1, wy - 1, 14, 12);
                        gfx.endFill();
                        ...
                        if (lit) {
                            gfx.beginFill(0xeab308, 0.15);
                            gfx.drawRect(wx - 2, wy - 2, 16, 14);
                            gfx.endFill();
                        }
                    }
                }
                ...
                gfx.beginFill(0x0f172a);
                gfx.drawRect(b.w / 2 - 24, h - 30, 48, 30);
                gfx.endFill();
                gfx.beginFill(0x38bdf8, 0.2);
                gfx.drawRect(b.w / 2 - 20, h - 25, 18, 25);
                gfx.drawRect(b.w / 2 + 2, h - 25, 18, 25);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.5);
                ...
                gfx.drawRect(10, -10, 40, 10);
                gfx.drawRect(b.w - 50, -10, 40, 10);
                gfx.endFill();

                if (b.id === 'res_cn') {
                    gfx.beginFill(0x3d2914);
                    gfx.drawRect(28, -20, 3, 10);
                    gfx.drawRect(b.w - 31, -20, 3, 10);
                    gfx.endFill();
                    gfx.beginFill(0xffb7c5);
                    gfx.drawCircle(29, -25, 14);
                    gfx.drawCircle(19, -18, 10);
                    gfx.drawCircle(39, -18, 10);
                    gfx.drawCircle(b.w - 29, -25, 14);
                    gfx.drawCircle(b.w - 19, -18, 10);
                    gfx.drawCircle(b.w - 39, -18, 10);
                    gfx.endFill();
                } else if (b.id === 'res_us') {
                    gfx.beginFill(0x3d2914);
                    gfx.drawRect(28, -20, 4, 10);
                    gfx.drawRect(b.w - 32, -20, 4, 10);
                    gfx.endFill();
                    gfx.beginFill(0x2d6a4f);
                    ...
                    gfx.drawCircle(30, -30, 14);
                    gfx.drawCircle(18, -20, 12);
                    gfx.drawCircle(42, -20, 12);
                    gfx.drawCircle(b.w - 30, -30, 14);
                    gfx.drawCircle(b.w - 18, -20, 12);
                    gfx.drawCircle(b.w - 42, -20, 12);
                    gfx.endFill();
                }
            } else if (b.id.startsWith('metro_')) {
                gfx.beginFill(0x1a1a24);
                gfx.drawRect(0, h - 40, b.w, 40);
                gfx.endFill();
                gfx.beginFill(0x2a2a3e);
                gfx.drawRect(0, h - 45, b.w, 5);
                gfx.endFill();
                ...
                gfx.moveTo(0, h - 45);
                gfx.lineTo(20, h - 70);
                gfx.lineTo(b.w - 20, h - 70);
                gfx.lineTo(b.w, h - 45);
                gfx.lineStyle(0);

                gfx.beginFill(0x050508);
                gfx.drawRect(b.w / 2 - 30, h - 30, 60, 30);
                gfx.endFill();
                gfx.beginFill(0x222233);
                for (let sy = 0; sy < 30; sy += 4) {
                    gfx.drawRect(b.w / 2 - 30, h - 30 + sy, 60, 2);
                }
                gfx.endFill();

                const mSign = new PIXI.Text('🚇 METRO', {
                    fontFamily: emojiFontStack,
                    fontSize: 12,
                    fill: 0x4ade80,
                    dropShadow: true,
                    dropShadowColor: 0x4ade80,
                    dropShadowBlur: 8,
                    dropShadowDistance: 0,
                });
                mSign.anchor.set(0.5, 0.5);
                mSign.x = b.w / 2;
                mSign.y = h - 85;
                ...
                gfx.drawRect(5, h - 8, b.w - 10, 8);
                gfx.endFill();
                gfx.beginFill(0x222240);
                gfx.drawRect(8, h - 10, b.w - 16, 4);
                gfx.endFill();
                ...
                gfx.beginFill(0x050510);
                gfx.drawRect(20, -10, b.w - 40, 40);
                gfx.endFill();
                gfx.beginFill(0x0a0a20);
                gfx.drawRect(22, -8, b.w - 44, 36);
                gfx.endFill();
                // Screen border glow
                ...
                gfx.beginFill(0x22d3ee, 0.15);
                gfx.drawRect(22, 35, b.w - 44, 2);
                gfx.endFill();
                gfx.beginFill(0x22d3ee, 0.1);
                gfx.drawRect(20, 55, b.w - 40, 2);
                gfx.endFill();
                gfx.beginFill(0x22d3ee, 0.08);
                gfx.drawRect(18, h - 25, b.w - 36, 2);
                gfx.endFill();
                // Shadow
                gfx.beginFill(0x000000, 0.18);
                gfx.drawRect(0, h - 2, b.w, 4);
                ...
                capGlow.beginFill(0x22d3ee, 0.08);
                capGlow.drawEllipse(b.w / 2, -35, 20, 12);
                capGlow.endFill();
                ...
                // Dark brick facade
                gfx.beginFill(0x0f0a18);
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                gfx.beginFill(0x1a1028);
                gfx.drawRect(4, 4, b.w - 8, h - 4);
                gfx.endFill();
                // Brick texture
                for (let by = 8; by < h - 4; by += 6) {
                    const off = by % 12 === 0 ? 0 : 8;
                    for (let bx = off + 4; bx < b.w - 8; bx += 16) {
                        gfx.beginFill(0x1e1430, 0.5);
                        gfx.drawRect(bx, by, 14, 5);
                        gfx.endFill();
                    }
                }
                // Neon accent strips on facade
                gfx.beginFill(0xff00ff, 0.15);
                gfx.drawRect(0, 0, b.w, 3);
                gfx.endFill();
                gfx.beginFill(0x00ffff, 0.12);
                gfx.drawRect(0, h * 0.4, b.w, 2);
                gfx.endFill();
                gfx.beginFill(0xff00ff, 0.1);
                gfx.drawRect(0, h * 0.7, b.w, 2);
                gfx.endFill();
                // Side neon tubes (vertical)
                gfx.beginFill(0xff69b4, 0.3);
                gfx.drawRect(2, 10, 2, h - 20);
                gfx.endFill();
                gfx.beginFill(0x00ffff, 0.3);
                gfx.drawRect(b.w - 4, 10, 2, h - 20);
                gfx.endFill();
                // Windows with colored glow (bar interior visible)
                for (let wx = 15; wx < b.w - 20; wx += 28) {
                    gfx.beginFill(0x000000);
                    gfx.drawRect(wx, h * 0.3, 20, 22);
                    gfx.endFill();
                    const wCol = [0xff00ff, 0x00ffff, 0xff6b9d, 0xa855f7][Math.floor(wx / 28) % 4];
                    gfx.beginFill(wCol, 0.25);
                    gfx.drawRect(wx + 1, h * 0.3 + 1, 18, 20);
                    gfx.endFill();
                }
                // Stage area (ground floor)
                gfx.beginFill(0x2a1040);
                gfx.drawRect(b.w / 2 - 25, h - 28, 50, 28);
                gfx.endFill();
                gfx.beginFill(0xff00ff, 0.2);
                gfx.drawRect(b.w / 2 - 22, h - 25, 44, 22);
                gfx.endFill();
                // Microphone stand
                gfx.beginFill(0x888888);
                gfx.drawRect(b.w / 2 - 1, h - 22, 2, 14);
                gfx.endFill();
                gfx.beginFill(0xcccccc);
                gfx.drawCircle(b.w / 2, h - 23, 3);
                gfx.endFill();
                // Door
                gfx.beginFill(0x1a0a28);
                gfx.drawRect(10, h - 18, 16, 18);
                gfx.endFill();
                gfx.beginFill(0xff00ff, 0.4);
                gfx.drawRect(10, h - 18, 16, 2);
                gfx.endFill();
                gfx.beginFill(0xfbbf24);
                gfx.drawCircle(22, h - 9, 1.5);
                gfx.endFill();
                // Cocktail emoji sign
                const cSign = new PIXI.Text('🍸', {
                    fontFamily: emojiFontStack,
                    fontSize: 20,
                    fill: 0xffffff,
                });
                cSign.anchor.set(0.5, 0.5);
                cSign.x = b.w - 20;
                cSign.y = h * 0.15;
                ...
                const sr = () => {
                    ...
                    return (treeSeed & 0xffffff) / 0x1000000;
                };
                ...
                gfx.beginFill(0x1a3321);
                gfx.drawRect(-55, h - 6, b.w + 110, 6);
                gfx.endFill();
                gfx.beginFill(0x264a30);
                gfx.drawRect(-55, h - 4, b.w + 110, 2);
                gfx.endFill();
                ...
                if (sr() > 0.5) {
                    const lgX = -40 + sr() * 30;
                    gfx.beginFill(0x3a2a18);
                    gfx.drawRect(lgX, h - 7, 26, 3);
                    gfx.endFill();
                    gfx.beginFill(0x5a4028);
                    gfx.drawRect(lgX, h - 7, 26, 1);
                    gfx.endFill();
                    gfx.beginFill(0x2a5a30);
                    gfx.drawCircle(lgX + 2, h - 7, 1.8);
                    gfx.endFill(); // moss on the log
                }
                ...
                const drawPine = (tx, scale) => {
                    ...
                    gfx.drawRect(tx - 1.5 * scale, baseY - trunkH, 3 * scale, trunkH);
                    gfx.endFill();
                    gfx.beginFill(0x26170a);
                    gfx.drawRect(tx + 0.5 * scale, baseY - trunkH, 1 * scale, trunkH);
                    gfx.endFill();
                    ...
                    gfx.drawPolygon([
                        tx - canopyW * 0.45,
                        capBase - canopyH * 0.25,
                        tx,
                        capBase - canopyH * 0.65,
                        tx + canopyW * 0.45,
                        capBase - canopyH * 0.25,
                    ]);
                    gfx.endFill();
                    ...
                    gfx.drawPolygon([
                        ...
                        capBase - canopyH * 0.5,
                        tx,
                        capBase - canopyH * 0.95,
                        tx + canopyW * 0.35,
                        capBase - canopyH * 0.5,
                    ]);
                    gfx.endFill();
                    // A dab of sunlit highlight
                    gfx.beginFill(0x6fbb6f, 0.55);
                    gfx.drawCircle(tx - canopyW * 0.15, capBase - canopyH * 0.6, 1 * scale);
                    gfx.endFill();
                };
                ...
                gfx.drawEllipse(b.w / 2, h - 4, cbW * 0.48, 3);
                gfx.endFill();
                ...
                for (let ly = bodyTop; ly < h - 8; ly += logH) {
                    ...
                    gfx.beginFill(darker ? 0x5a3d24 : 0x6b4a2c);
                    gfx.drawRect(cbX, ly, cbW, logH);
                    gfx.endFill();
                    // Chinking highlight between logs
                    gfx.beginFill(0x2a1a0e);
                    gfx.drawRect(cbX, ly + logH - 1, cbW, 1);
                    gfx.endFill();
                    // End-grain circles at log tips for that stacked-log "notched corner" read
                    gfx.beginFill(0x3a240e);
                    gfx.drawCircle(cbX + 2, ly + logH / 2, 1.6);
                    gfx.drawCircle(cbX + cbW - 2, ly + logH / 2, 1.6);
                    gfx.endFill();
                }

                // ── PORCH STEP ──
                gfx.beginFill(0x3a2818);
                gfx.drawRect(cbX - 4, h - 8, cbW + 8, 4);
                gfx.endFill();
                gfx.beginFill(0x4a3020);
                gfx.drawRect(cbX - 4, h - 8, cbW + 8, 1);
                gfx.endFill();
                ...
                const winL = cbX + 14,
                    winR = cbX + cbW - winW - 14;
                for (const wx of [winL, winR]) {
                    // Frame (dark wood)
                    gfx.beginFill(0x2a1a0e);
                    gfx.drawRect(wx - 2, winY - 2, winW + 4, winH + 4);
                    gfx.endFill();
                    // Warm glow — amber at night, soft yellow by day
                    gfx.beginFill(0xffd37a, 0.92);
                    gfx.drawRect(wx, winY, winW, winH);
                    gfx.endFill();
                    gfx.beginFill(0xffeaa0, 0.55);
                    gfx.drawRect(wx, winY, winW, winH * 0.4);
                    gfx.endFill();
                    ...
                    // Sill
                    gfx.beginFill(0x3a240e);
                    gfx.drawRect(wx - 3, winY + winH, winW + 6, 2);
                    gfx.endFill();
                }
                ...
                gfx.beginFill(0xffd37a, 0.18);
                gfx.drawEllipse(b.w / 2, h - 6, 20, 3);
                gfx.endFill();
                // Door frame
                gfx.beginFill(0x2a1a0e);
                gfx.drawRect(doorX - 2, doorY - 2, doorW + 4, doorH + 4);
                gfx.endFill();
                // Door leaves
                gfx.beginFill(0x4a2e18);
                gfx.drawRect(doorX, doorY, doorW, doorH);
                gfx.endFill();
                gfx.beginFill(0x5a3a20);
                gfx.drawRect(doorX + 1, doorY + 1, winW / 3, doorH - 2);
                gfx.endFill();
                ...
                // Brass doorknob
                gfx.beginFill(0xfbbf24);
                gfx.drawCircle(doorX + doorW - 3, doorY + doorH / 2, 1.3);
                gfx.endFill();
                // Accent lantern above the door — institute-colored glow
                gfx.beginFill(0x2a1a0e);
                gfx.drawRect(doorX + doorW / 2 - 3, doorY - 7, 6, 6);
                gfx.endFill();
                gfx.beginFill(accentCol, 0.9);
                gfx.drawRect(doorX + doorW / 2 - 2, doorY - 6, 4, 4);
                gfx.endFill();
                gfx.beginFill(accentCol, 0.25);
                gfx.drawCircle(doorX + doorW / 2, doorY - 4, 6);
                gfx.endFill();
                ...
                gfx.endFill();
                // Main roof surface
                gfx.beginFill(0x3a2a1a);
                gfx.drawPolygon([cbX - roofOH, roofEaveY, b.w / 2, roofPeakY, cbX + cbW + roofOH, roofEaveY]);
                gfx.endFill();
                ...
                // Ridge cap
                gfx.beginFill(0x1a110a);
                gfx.drawRect(b.w / 2 - 4, roofPeakY - 1, 8, 2);
                gfx.endFill();
                ...
                const chimW = 10;
                gfx.beginFill(0x4a4a4a);
                gfx.drawRect(chimX, chimTopY, chimW, chimBaseY - chimTopY);
                gfx.endFill();
                // Stone dappling
                ...
                gfx.drawRect(chimX, chimTopY, chimW, 1);
                gfx.endFill();
                gfx.beginFill(0x2a2a2a);
                gfx.drawRect(chimX - 1, chimTopY - 2, chimW + 2, 2);
                gfx.endFill(); // cap
                // Soft smoke puff
                gfx.beginFill(0xe5e7eb, 0.35);
                gfx.drawCircle(chimX + chimW / 2 + 1, chimTopY - 6, 3.5);
                gfx.endFill();
                gfx.beginFill(0xd1d5db, 0.22);
                gfx.drawCircle(chimX + chimW / 2 + 3, chimTopY - 10, 3);
                gfx.endFill();
                ...
                // Sign post
                gfx.beginFill(0x3a2818);
                gfx.drawRect(b.w / 2 - 1.5, plaqueY + plaqueH, 3, 6);
                gfx.endFill();
                // Wooden sign background
                gfx.beginFill(0x3a2818);
                gfx.drawRect(plaqueX - 1, plaqueY - 1, plaqueW + 2, plaqueH + 2);
                gfx.endFill();
                gfx.beginFill(0x5a3a20);
                gfx.drawRect(plaqueX, plaqueY, plaqueW, plaqueH);
                gfx.endFill();
                gfx.beginFill(0x4a2e18);
                gfx.drawRect(plaqueX, plaqueY + plaqueH - 2, plaqueW, 2);
                gfx.endFill();
                ...
                // Marble/ivory facade
                gfx.beginFill(0xe9e4d2);
                gfx.drawRect(0, 0, b.w, h);
                gfx.endFill();
                gfx.beginFill(0xf4efdc);
                gfx.drawRect(3, 3, b.w - 6, h - 6);
                gfx.endFill();
                ...
                for (let i = 0; i < nCols; i++) {
                    ...
                    // Shaft
                    gfx.beginFill(0xf8f3df);
                    gfx.drawRect(cx, 10, colW, h - 22);
                    gfx.endFill();
                    // Shadow on right side (adds depth)
                    gfx.beginFill(0xc6bfa2, 0.7);
                    gfx.drawRect(cx + colW - 2, 10, 2, h - 22);
                    gfx.endFill();
                    // Highlight on left
                    gfx.beginFill(0xfffbe8, 0.5);
                    gfx.drawRect(cx + 1, 10, 1, h - 22);
                    gfx.endFill();
                    // Capital (top)
                    gfx.beginFill(0xd6d0b6);
                    gfx.drawRect(cx - 2, 8, colW + 4, 3);
                    gfx.endFill();
                    // Base
                    gfx.beginFill(0xd6d0b6);
                    gfx.drawRect(cx - 2, h - 14, colW + 4, 3);
                    gfx.endFill();
                }

                // Base steps (two tiers)
                gfx.beginFill(0xccc5ac);
                gfx.drawRect(-4, h - 10, b.w + 8, 5);
                gfx.endFill();
                gfx.beginFill(0xbab396);
                gfx.drawRect(-2, h - 5, b.w + 4, 5);
                gfx.endFill();
                ...
                gfx.drawRect(doorX - 2, doorY, doorW + 4, doorH);
                gfx.endFill();
                gfx.beginFill(0x2e2014);
                gfx.drawRect(doorX, doorY, doorW, doorH);
                gfx.endFill();
                gfx.beginFill(0x4a3422);
                gfx.drawRect(doorX + 2, doorY + 2, doorW - 4, doorH - 4);
                gfx.endFill();
                // Door split (double doors)
                gfx.lineStyle(1, 0x1a1008, 0.8);
                gfx.moveTo(doorX + doorW / 2, doorY + 2);
                gfx.lineTo(doorX + doorW / 2, doorY + doorH);
                gfx.lineStyle(0);
                // Brass knobs
                gfx.beginFill(0xfbbf24);
                gfx.drawCircle(doorX + doorW / 2 - 3, doorY + doorH / 2, 0.9);
                gfx.endFill();
                gfx.beginFill(0xfbbf24);
                gfx.drawCircle(doorX + doorW / 2 + 3, doorY + doorH / 2, 0.9);
                gfx.endFill();
                ...
                for (const lx of [doorX - 8, doorX + doorW + 8]) {
                    ...
                    gfx.drawRect(lx - 1, doorY + 4, 2, 4);
                    gfx.endFill();
                    gfx.beginFill(0xfbbf24, 0.9);
                    gfx.drawCircle(lx, doorY + 10, 2);
                    gfx.endFill();
                    gfx.beginFill(0xfffbe8, 0.7);
                    gfx.drawCircle(lx, doorY + 10, 1);
                    gfx.endFill();
                }
                ...
                for (const wx of [b.w / 2 - 50, b.w / 2 + 30]) {
                    ...
                    gfx.beginFill(0x1a1a28);
                    gfx.drawRect(wx, winY, 20, winH);
                    gfx.endFill();
                    gfx.beginFill(0xffe9a8, 0.55);
                    gfx.drawRect(wx + 1, winY + 1, 18, winH - 2);
                    gfx.endFill();
                    gfx.lineStyle(0.6, 0xa8a288, 0.8);
                    gfx.moveTo(wx + 10, winY + 1);
                    gfx.lineTo(wx + 10, winY + winH - 1);
                    gfx.moveTo(wx + 1, winY + winH / 2);
                    gfx.lineTo(wx + 19, winY + winH / 2);
                    gfx.lineStyle(0);
                }
                ...
                pole.beginFill(0x7a7a7a);
                pole.drawRect(-1, -28, 2, 30);
                pole.endFill();
                pole.beginFill(0xfbbf24);
                pole.drawCircle(0, -28, 1.8);
                pole.endFill();
                ...
                gfx.drawRect(0, 8, b.w, h - 8);
                gfx.endFill();
                gfx.beginFill(bc, 0.06);
                gfx.drawRect(0, 8, b.w, h - 8);
                gfx.endFill();
                // Left pilaster highlight + right shade (glassy tube read)
                gfx.beginFill(0xffffff, 0.04);
                gfx.drawRect(0, 8, 4, h - 8);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.18);
                gfx.drawRect(b.w - 5, 8, 5, h - 8);
                gfx.endFill();
                // Brand crown band across the parapet
                gfx.beginFill(bc, 0.92);
                gfx.drawRect(0, 0, b.w, 9);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.18);
                gfx.drawRect(0, 0, b.w, 2);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.2);
                gfx.drawRect(0, 9, b.w, 2);
                gfx.endFill();
                ...
                for (let ci = 0; ci < nCols; ci++) {
                    ...
                    for (let wy = glassTop; wy < glassBot; wy += 15) {
                        ...
                        if (lit) {
                            gfx.beginFill(bc, 0.18);
                            gfx.drawRect(wx, wy, colW - 6, 3);
                            gfx.endFill();
                        }
                    }
                    // Vertical mullion
                    gfx.beginFill(0x0a1018, 0.9);
                    gfx.drawRect(wx - 2, glassTop, 2, glassBot - glassTop);
                    gfx.endFill();
                }
                // Horizontal spandrel belts (floor slabs)
                gfx.beginFill(bc, 0.1);
                for (let sy = glassTop - 2; sy < glassBot; sy += 15) gfx.drawRect(6, sy, b.w - 12, 2);
                gfx.endFill();
                // Two-story glass lobby with brand portal
                const lobbyH = 20;
                gfx.beginFill(0x0a1622);
                gfx.drawRect(6, h - lobbyH - 2, b.w - 12, lobbyH);
                gfx.endFill();
                gfx.beginFill(bc, 0.16);
                gfx.drawRect(6, h - lobbyH - 2, b.w - 12, lobbyH);
                gfx.endFill();
                gfx.beginFill(bc, 0.8);
                gfx.drawRect(6, h - lobbyH - 2, b.w - 12, 2);
                gfx.endFill();
                ...
                gfx.beginFill(0x060b12);
                gfx.drawRect(b.w / 2 - 11, h - 18, 22, 18);
                gfx.endFill();
                gfx.beginFill(bc, 0.5);
                gfx.drawRect(b.w / 2 - 11, h - 18, 22, 2);
                gfx.endFill();
                gfx.lineStyle(1, bc, 0.6);
                gfx.moveTo(b.w / 2, h - 16);
                gfx.lineTo(b.w / 2, h);
                gfx.moveTo(b.w / 2 - 9, h - 9);
                gfx.lineTo(b.w / 2 + 9, h - 9);
                gfx.lineStyle(0);
                // Canopy
                gfx.beginFill(bc, 0.85);
                gfx.drawRect(b.w / 2 - 16, h - 20, 32, 2.5);
                gfx.endFill();
                // Rooftop setback + mechanical penthouse
                gfx.beginFill(0x0a1018);
                gfx.drawRect(b.w * 0.5 - 24, -8, 48, 10);
                gfx.endFill();
                gfx.beginFill(bc, 0.3);
                gfx.drawRect(b.w * 0.5 - 24, -8, 48, 2);
                gfx.endFill();
                ...
                for (let wx = 8; wx < b.w - 14; wx += 20) {
                    for (let wy = wTop; wy < wBot; wy += 15) {
                        const lit = lr() > 0.35;
                        gfx.beginFill(lit ? 0xeafff4 : 0x0e1a20, lit ? 0.7 : 1);
                        gfx.drawRect(wx, wy, 13, 11);
                        gfx.endFill();
                        if (lit) {
                            gfx.beginFill(ac, 0.22);
                            gfx.drawRect(wx, wy, 13, 3);
                            gfx.endFill();
                        }
                    }
                    gfx.beginFill(0x000000, 0.85);
                    gfx.drawRect(wx - 2, wTop, 2, wBot - wTop);
                    gfx.endFill();
                }
                // Clean-room lobby
                gfx.beginFill(0x0a1620);
                gfx.drawRect(6, h - 20, b.w - 12, 18);
                gfx.endFill();
                gfx.beginFill(ac, 0.15);
                gfx.drawRect(6, h - 20, b.w - 12, 18);
                gfx.endFill();
                gfx.beginFill(ac, 0.8);
                gfx.drawRect(6, h - 20, b.w - 12, 2);
                gfx.endFill();
                gfx.beginFill(0xeafff4, 0.3);
                for (let lx = 14; lx < b.w - 14; lx += 22) gfx.drawRect(lx, h - 17, 14, 12);
                gfx.endFill();
                // Airlock entrance
                gfx.beginFill(0x060d14);
                gfx.drawRect(b.w / 2 - 10, h - 17, 20, 17);
                gfx.endFill();
                gfx.beginFill(ac, 0.5);
                gfx.drawRect(b.w / 2 - 10, h - 17, 20, 2);
                gfx.endFill();
                gfx.beginFill(ac, 0.7);
                gfx.drawCircle(b.w / 2 + 6, h - 9, 1);
                gfx.endFill();
                ...
                for (let f = 0; f < floors; f++) {
                    ...
                    gfx.drawRect(ins, by2, b.w - ins * 2, 18);
                    gfx.endFill();
                    gfx.beginFill(colHex, 0.07);
                    gfx.drawRect(ins, by2, b.w - ins * 2, 18);
                    gfx.endFill();
                    // Depth: left highlight + right shade, hugging this band's edges
                    gfx.beginFill(0xffffff, 0.05);
                    gfx.drawRect(ins, by2, 3, 18);
                    gfx.endFill();
                    gfx.beginFill(0x000000, 0.18);
                    gfx.drawRect(b.w - ins - 5, by2, 5, 18);
                    gfx.endFill();
                    // Right-edge drop shadow cast beside this band
                    gfx.beginFill(0x000000, 0.12);
                    gfx.drawRect(b.w - ins, by2, 6, 18);
                    gfx.endFill();
                }
                // Ground band under the last floor (h-24..h always exists: h = floors*18+24)
                const gndY = 14 + floors * 18;
                gfx.beginFill(bodyCol);
                gfx.drawRect(0, gndY, b.w, h - gndY);
                gfx.endFill();
                gfx.beginFill(colHex, 0.07);
                gfx.drawRect(0, gndY, b.w, h - gndY);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.05);
                gfx.drawRect(0, gndY, 3, h - gndY);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.18);
                gfx.drawRect(b.w - 5, gndY, 5, h - gndY);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.12);
                gfx.drawRect(b.w, gndY, 6, h - gndY);
                gfx.endFill();

                const topIns = insetFor(0);

                // ── Crown band (rooftop stock-ticker sits over 0..14 when present) ──
                gfx.beginFill(colHex, 0.95);
                gfx.drawRect(topIns, 0, b.w - topIns * 2, 10);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.16);
                gfx.drawRect(topIns, 0, b.w - topIns * 2, 2);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.25);
                gfx.drawRect(topIns, 10, b.w - topIns * 2, 4);
                gfx.endFill();
                if (famous) {
                    // double brand stripe for the famous few
                    gfx.beginFill(0xffffff, 0.25);
                    gfx.drawRect(topIns, 6, b.w - topIns * 2, 1.5);
                    gfx.endFill();
                }
                ...
                if (style === 'setback') {
                    for (let f = 1; f < floors; f++) {
                        if (insetFor(f) !== insetFor(f - 1)) {
                            const ty2 = 14 + f * 18;
                            gfx.beginFill(colHex, 0.5);
                            gfx.drawRect(insetFor(f), ty2 - 2, b.w - insetFor(f) * 2, 2.5);
                            gfx.endFill();
                        }
                    }
                }
                ...
                if (style === 'campus' && floors >= 4) {
                    ...
                    gfx.beginFill(colHex, 0.5);
                    gfx.drawRect(0, podY - 2, 12, 2);
                    gfx.drawRect(b.w - 12, podY - 2, 12, 2);
                    gfx.endFill();
                    gfx.beginFill(0x94a3b8, 0.6);
                ...
                for (let f = 0; f < lastWinRow; f++) {
                    ...
                    for (let c = 0; c < cols2; c++) {
                        ...
                        gfx.drawRect(wx - 1.5, wy - 1.5, pw + 3, ph + 3);
                        gfx.endFill();
                        const lit = n < 0.62;
                        if (lit) {
                        ...
                        gfx.endFill();
                        if (lit) {
                            gfx.beginFill(colHex, 0.14);
                            gfx.drawRect(wx, wy, pw, 2.5);
                            gfx.endFill();
                        }
                ...
                if (hasLobby) {
                    gfx.beginFill(0x0a121e);
                    gfx.drawRect(4, h - lobbyH, b.w - 8, lobbyH - 2);
                    gfx.endFill();
                    gfx.beginFill(colHex, 0.12);
                    gfx.drawRect(4, h - lobbyH, b.w - 8, lobbyH - 2);
                    gfx.endFill();
                    gfx.beginFill(colHex, 0.75);
                    gfx.drawRect(4, h - lobbyH, b.w - 8, 2);
                    gfx.endFill();
                    // Warm reception glow panes
                    gfx.beginFill(0xffe9a8, 0.35);
                    for (let lx = 12; lx < b.w - 20; lx += 20)
                        gfx.drawRect(lx, h - lobbyH + 4, 13, lobbyH - 9);
                    gfx.endFill();
                    // Revolving door
                    gfx.beginFill(0x060b12);
                    gfx.drawRect(b.w / 2 - 9, h - 18, 18, 16);
                    gfx.endFill();
                    gfx.lineStyle(1, colHex, 0.55);
                    gfx.moveTo(b.w / 2, h - 17);
                    gfx.lineTo(b.w / 2, h - 3);
                    gfx.moveTo(b.w / 2 - 8, h - 10);
                    gfx.lineTo(b.w / 2 + 8, h - 10);
                    gfx.lineStyle(0);
                    // Canopy
                    gfx.beginFill(colHex, 0.85);
                    gfx.drawRect(b.w / 2 - 15, h - 20, 30, 2.5);
                    gfx.endFill();
                    gfx.beginFill(0x000000, 0.25);
                    gfx.drawRect(b.w / 2 - 15, h - 17.5, 30, 1.5);
                    gfx.endFill();
                } else {
                    gfx.beginFill(0x0a0a18);
                    gfx.drawRect(b.w / 2 - 6, h - 18, 12, 18);
                    gfx.endFill();
                    gfx.beginFill(colHex, 0.3);
                    gfx.drawRect(b.w / 2 - 6, h - 18, 12, 2);
                    gfx.endFill();
                }
                ...
                const px2 = topIns + 5;
                gfx.beginFill(0x070b12, 0.92);
                gfx.drawRoundedRect(px2, 17, 32, 32, 4);
                gfx.endFill();
                gfx.lineStyle(1.2, colHex, 0.7);
                gfx.drawRoundedRect(px2, 17, 32, 32, 4);
                gfx.lineStyle(0);
                ...
                if (b.w >= 110) {
                    ...
                    if (kind === 0) {
                        // HVAC pair
                        gfx.beginFill(0x475569);
                        gfx.drawRect(fx3, -6, 12, 6);
                        gfx.drawRect(fx3 + 16, -5, 10, 5);
                        gfx.endFill();
                        gfx.beginFill(0x64748b, 0.7);
                        gfx.drawRect(fx3 + 1.5, -4.5, 9, 1.2);
                        gfx.drawRect(fx3 + 1.5, -2.5, 9, 1.2);
                        gfx.endFill();
                    } else if (kind === 1) {
                        // water tank
                        gfx.beginFill(0x334155);
                        gfx.drawRect(fx3 + 2, -1.5, 10, 1.5);
                        gfx.endFill();
                        gfx.beginFill(0x6b7280);
                        gfx.drawRoundedRect(fx3 + 1, -9, 12, 8, 2);
                        gfx.endFill();
                        gfx.beginFill(0x9ca3af, 0.5);
                        gfx.drawRect(fx3 + 2.5, -8, 2, 6);
                        gfx.endFill();
                    } else if (kind === 2) {
                        // antenna cluster
                        gfx.beginFill(0x64748b);
                        gfx.drawRect(fx3, -9, 1.5, 9);
                        gfx.drawRect(fx3 + 7, -12, 1.5, 12);
                        gfx.drawRect(fx3 + 14, -7, 1.5, 7);
                        gfx.endFill();
                        gfx.beginFill(0xef4444, 0.8);
                        gfx.drawCircle(fx3 + 7.7, -12.8, 1.2);
                        gfx.endFill();
                    } else {
                        // roof garden
                        gfx.beginFill(0x1b4332);
                        gfx.drawRect(fx3, -3, 26, 3);
                        gfx.endFill();
                        gfx.beginFill(0x2d6a4f);
                ...
                if (b.isCheapest) {
                    ...
                    neon.x = b.w + 2;
                    neon.y = 40;
                    neon.rotation = Math.PI / 2;
                    container.addChild(neon);
                    b._neonSign = neon;
                }
            } else {
                gfx.beginFill(0x000000, 0.12);
                ...
                gfx.drawRect(0, 0, b.w, 14);
                gfx.endFill();
                gfx.beginFill(colHex, 1);
                gfx.drawRect(0, 0, b.w, 3);
                gfx.endFill();
                gfx.beginFill(0x000000, 0.15);
                gfx.drawRect(0, 11, b.w, 3);
                gfx.endFill();
                gfx.beginFill(colHex, 0.12);
                gfx.drawRect(0, 14, b.w, h - 14);
                gfx.endFill();
                gfx.beginFill(colHex, 0.06);
                gfx.drawRect(0, 14, b.w / 3, h - 14);
                gfx.endFill();
                gfx.beginFill(0xffffff, 0.02);
                gfx.drawRect((b.w * 2) / 3, 14, b.w / 3, h - 14);
                gfx.endFill();
                ...
                for (let ty = 14; ty < h; ty += 18) {
                    gfx.moveTo(0, ty);
                    gfx.lineTo(b.w, ty);
                }
                for (let tx = 24; tx < b.w; tx += 24) {
                    gfx.moveTo(tx, 14);
                    gfx.lineTo(tx, h);
                }
                gfx.lineStyle(0);
                ...
                for (let f = 0; f < floors; f++)
                    for (let c = 0; c < cols; c++) {
                        const lit = this._labNoise((b.x | 0) + f * 131 + c * 17) > 0.35;
                        const wx = 10 + c * 24,
                            wy = 20 + f * 18;
                        ...
                        gfx.lineStyle(0);
                        gfx.beginFill(colHex, 0.12);
                        gfx.drawRect(wx, wy + 4, 12, 1);
                        gfx.endFill();
                        gfx.beginFill(colHex, 0.12);
                        gfx.drawRect(wx + 5, wy, 1, 10);
                        gfx.endFill();
                    }
            ...
            if (
                ...
                !b.id.startsWith('forest_') &&
                !b.id.startsWith('dc_') &&
                !b.id.startsWith('fab_') &&
                !b.id.startsWith('suburb_') &&
                ...
                if (hasFounder) {
                    ...
                    // Pad surface
                    hpGfx.beginFill(0x334155, 0.8);
                    hpGfx.drawEllipse(hpX, hpY, 16, 5);
                    hpGfx.endFill();
                    // H marking
            ...
            if (
                ...
                beam.beginFill(0x22d3ee, 0.15);
                beam.drawRect(b.w / 2 - 15, -2000, 30, 2000);
                beam.endFill();
                beam.beginFill(0xffffff, 0.2);
                beam.drawRect(b.w / 2 - 5, -2000, 10, 2000);
                beam.endFill();
                beam.blendMode = PIXI.BLEND_MODES.ADD;
                beaconCont.addChild(beam);
                ...
                emitter.drawEllipse(b.w / 2, 0, 24, 6);
                emitter.endFill();
                beaconCont.addChild(emitter);
                container.addChild(beaconCont);
                b._beacon = { beam, emitter, crown };
            }
            ...
            const tickerSym =
                lab && lab.ticker ? lab.ticker : b.lab && FAB_TICKERS[b.lab] ? FAB_TICKERS[b.lab] : null;
            ...
            if (tickerSym && !b.id.startsWith('house_') && !b.id.startsWith('forest_') && !isConstruction) {
                ...
                tickBg.beginFill(0x000000, 0.85);
                tickBg.drawRect(0, 0, b.w, 14);
                tickBg.endFill();
                tickCont.addChild(tickBg);
                const mask = new PIXI.Graphics();
                mask.beginFill(0xffffff);
                mask.drawRect(0, -5, b.w, 24);
                mask.endFill();
                tickCont.addChild(mask);
                tickCont.mask = mask;
                ...
                tickTxt.y = 1;
                tickTxt.x = b.w;
                tickTxt.blendMode = PIXI.BLEND_MODES.ADD;
                tickCont.addChild(tickTxt);
                b._stockTicker = tickTxt;
                b._tickerW = b.w;
                b._tickerSym = tickerSym;
            ...
            if (b.id === 'backbone_ixp' && typeof BackboneZone !== 'undefined') {
                ...
                bkTickBg.beginFill(0x000000, 0.9);
                bkTickBg.drawRect(0, 0, b.w, 14);
                bkTickBg.endFill();
                bkTickCont.addChild(bkTickBg);
                const bkMask = new PIXI.Graphics();
                bkMask.beginFill(0xffffff);
                bkMask.drawRect(0, -5, b.w, 24);
                bkMask.endFill();
                bkTickCont.addChild(bkMask);
                bkTickCont.mask = bkMask;
                ...
                bkTickTxt.y = 1;
                bkTickTxt.x = b.w;
                bkTickTxt.blendMode = PIXI.BLEND_MODES.ADD;
                bkTickCont.addChild(bkTickTxt);
                b._bkTicker = bkTickTxt;
                b._bkTickerW = b.w;
            ...
            if (b.type === 'vcrow' && typeof VCRow !== 'undefined') {
                ...
                vTickBg.beginFill(0x000000, 0.9);
                vTickBg.drawRect(0, 0, b.w, 14);
                vTickBg.endFill();
                vTickCont.addChild(vTickBg);
                const vMask = new PIXI.Graphics();
                vMask.beginFill(0xffffff);
                vMask.drawRect(0, -5, b.w, 24);
                vMask.endFill();
                vTickCont.addChild(vMask);
                vTickCont.mask = vMask;
                ...
                vTickTxt.y = 1;
                vTickTxt.x = b.w;
                vTickTxt.blendMode = PIXI.BLEND_MODES.ADD;
                vTickCont.addChild(vTickTxt);
                b._vcTicker = vTickTxt;
                b._vcTickerW = b.w;
            ...
            if (
                ...
                const sign = new PIXI.Text(b.name, {
                ...
                if (lab) {
                    ...
                    gfx.lineStyle(0);
                    gfx.beginFill(0x333333);
                    gfx.drawRect(boardX + 10, boardY + boardH, 4, 10);
                    gfx.drawRect(boardX + boardW - 14, boardY + boardH, 4, 10);
                    gfx.endFill();
                    const logoTxt = new PIXI.Text(lab.name.toUpperCase(), {
                        fontFamily: 'JetBrains Mono',
                        fontSize: 10,
                        fontWeight: 'bold',
                        fill: 0xffffff,
            ...
            const neonConfig = {
                ...
                open_square: { text: '💻 OPEN SOURCE HUB', col: 0xa855f7, speed: 0.1, flicker: 0.35 },
            ...
            if (
                ...
                !b.id.startsWith('suburb_') &&
                !b.id.startsWith('res_') &&
                !b.id.startsWith('embassy_') &&
                !b.id.startsWith('align_') &&
                b.id !== 'graveyard' &&
                ...
                nc = {
                    text: (b.emoji || '🏢') + ' ' + (b.name || '').toUpperCase(),
                    col: 0x6688aa,
                    speed: 0.06,
                    flicker: 0.2,
                };
            ...
            container.on('pointerover', (e) => {
                if (typeof UI !== 'undefined') UI.showTooltip(e, b.name, b.tip || b.desc);
            });
    ...
    _buildFacadeDither() {
        ...
        let maxTex = 4096;
        try {
            maxTex = rnd.gl.getParameter(rnd.gl.MAX_TEXTURE_SIZE) || 4096;
        } catch (e) {
            /* no gl */
        }
    ...
    _pickNextWeather() {
        ...
        const row = table[this.weather] || table.clear || table.partly_cloudy || [['clear', 1]];
        let total = 0;
    ...
    _drawPortWater() {
        ...
        if (!this._portWaterCont) {
            if (!this.groundGfx || !this.groundGfx.parent || typeof PIXI.DisplacementFilter !== 'function')
                return;
            ...
            this._portWaterCont = cont;
            this._portWaterGfx = gfx;
            this._waterDispSprite = disp;
        ...
        if (typeof PortEnv !== 'undefined' && PortEnv.shipConts && PortZone.ships) {
            for (let i = 0; i < PortEnv.shipConts.length; i++) {
                ...
                if (night) {
                    ...
                    g.drawCircle(sx - 6, surfTop + 6, 1.6);
                    g.endFill();
                    g.beginFill(0x4ade80, 0.4);
                    g.drawCircle(sx + sw + 6, surfTop + 6, 1.6);
                    g.endFill();
                }
            }
        }
    ...
    _drawSnowAccum() {
        ...
        const ds = desert ? desert.start : Infinity,
            de = desert ? desert.end : -Infinity;
    ...
    drawWeather() {
        ...
        const xMin = wx - margin,
            xMax = wx + vw + margin;
        const yMin = wy - margin,
            yMax = wy + vh + margin;
        ...
        if (w === 'drizzle' || w === 'rain' || w === 'thunderstorm') {
            ...
            const rainAlpha =
                (w === 'drizzle' ? 0.22 : w === 'thunderstorm' ? 0.5 : 0.38) * (0.3 + intensity * 0.7);
            ...
            const splashCap = 60;
            const splashChance =
                (w === 'thunderstorm' ? 0.05 : w === 'rain' ? 0.035 : 0.014) * (0.4 + intensity * 0.6);
            ...
            for (let i = 0; i < drops.length; i++) {
                ...
                if (d.y > groundTop || d.y > yMax) {
                    ...
                    if (
                        ...
                        d.x >= xMin &&
                        d.x <= xMax &&
                        splashes.length < splashCap &&
                        (!desert || d.x < ds || d.x > de) &&
                        Math.random() < splashChance
                    ) {
            ...
            while (this.petals.length < target) {
                this.petals.push({
                    ...
                    leafCol: [0xd97706, 0xc2410c, 0x9a3412, 0x991b1b, 0xca8a04][
        ...
        if (this._wetness > 0.04) {
            ...
            for (let i = 0; i < pud.length; i++) {
                ...
                const rw = p.w,
                    rh = Math.max(2.5, p.w * 0.11);
                // Dark water body
                gfxG.beginFill(0x0a1420, wet * 0.5);
                gfxG.drawEllipse(p.x, py, rw, rh);
                gfxG.endFill();
                // Sky sheen on the near edge
                gfxG.beginFill(0x9fb8d4, wet * 0.16);
                gfxG.drawEllipse(p.x, py - rh * 0.3, rw * 0.85, rh * 0.5);
                gfxG.endFill();
                // Reflected-neon shimmer — night only, drifts and pulses
                if (nightNow) {
                    const shimmer = 0.1 + Math.sin(tick * 0.05 + p.ph) * 0.06;
                    if (shimmer > 0) {
    ...
    update(dp, night, occ) {
        ...
        const vp = this._vpEl;
        let sky;
        if (dp < 0.22) sky = 'linear-gradient(180deg,#080a1e,#0f0f28 50%,#141430)';
        else if (dp < 0.3) {
        ...
        } else if (dp < 0.72) sky = 'linear-gradient(180deg,#2d4a7a,#5a8fbb 50%,#87b5d6)';
        else if (dp < 0.84) {
        ...
        if (!night && dp > 0.3 && dp < 0.72) {
            if (_w === 'rain' || _w === 'drizzle')
        ...
        const isGoldenHour = (dp >= 0.72 && dp < 0.84) || (dp >= 0.22 && dp < 0.3);
        // Throttle celestial redraws to every 3rd frame (sun moves slowly)
        if (G.tick % 3 !== 0 && !this._celDirty) {
            /* skip redraw */
        } else {
            ...
            if (night) {
                ...
                cel.drawCircle(mx, my, mr * 2.1);
                cel.endFill();
                // Dark side base (barely visible earthshine disc)
                cel.beginFill(0x3a4356, 0.55);
                cel.drawCircle(mx, my, mr);
                cel.endFill();
                // Lit disc
                ...
                cel.drawCircle(mx - 4, my - 3, 2.2);
                cel.drawCircle(mx + 3, my + 4, 1.6);
                cel.drawCircle(mx + 5, my - 5, 1.2);
                cel.endFill();
                ...
                if (isGoldenHour) {
                    let ghI =
                        dp >= 0.72 ? 1 - Math.abs((dp - 0.78) / 0.06) : 1 - Math.abs((dp - 0.26) / 0.04);
                    ghI = Math.max(0, Math.min(1, ghI));

                    // Layered aura
                    cel.beginFill(0xff6622, 0.02 * ghI);
                    cel.drawCircle(sunX, sunY, 140);
                    cel.endFill();
                    cel.beginFill(0xff8833, 0.04 * ghI);
                    cel.drawCircle(sunX, sunY, 80);
                    cel.endFill();
                    cel.beginFill(0xffaa44, 0.08 * ghI);
                    cel.drawCircle(sunX, sunY, 40);
                    cel.endFill();
                    ...
                    for (let r = 0; r < 7; r++) {
                        const angle =
                            Math.PI * 0.2 +
                            (r / 6) * Math.PI * 0.6 +
                            Math.sin(G.tick * 0.003 + r * 1.7) * 0.04;
                        ...
                        cel.lineTo(
                            ex + Math.cos(angle + Math.PI / 2) * rayW,
                            ey + Math.sin(angle + Math.PI / 2) * rayW
                        );
                        cel.lineTo(
                            ex - Math.cos(angle + Math.PI / 2) * rayW,
                            ey - Math.sin(angle + Math.PI / 2) * rayW
                ...
                cel.drawCircle(sunX, sunY, isGoldenHour ? 18 : 15);
                cel.endFill();
            }
        ...
        if (G.tick % 2 === 0) {
            ...
            const _w = this.weather,
                _wi = this.weatherIntensity || 0;
            let cloudAlphaBase = -1;
            if (_w === 'rain' || _w === 'snow') cloudAlphaBase = 0.3 * (0.5 + _wi * 0.5);
            ...
            for (let ci = 0; ci < cLen; ci++) {
                const c = clouds[ci];
                c.x = c._bx + Math.sin(_tk * (c._drift || 0.003) + c._i) * 40;
                const ca = cloudAlphaBase >= 0 ? cloudAlphaBase : 0.1 + Math.sin(_tk * 0.001 + c._i) * 0.03;
        ...
        const _rw = this.weather,
            _rwi = this.weatherIntensity || 0;
        const _isRainy = _rw === 'rain' || _rw === 'drizzle' || _rw === 'thunderstorm';
        if (night) {
            if (_isRainy) targetRefAlpha = 0.55 + 0.4 * _rwi;
        ...
        for (let bi = 0; bi < bldLen; bi++) {
            ...
            if (b._stockTicker && b._tickerW && !b._stockTicker.destroyed) {
                ...
                if (doOcc && typeof API !== 'undefined' && API.stockPrices && API.stockPrices[b._tickerSym]) {
                    const sd = API.stockPrices[b._tickerSym];
                    b._stockTicker.text = `${b._tickerSym} $${sd.price} [${sd.change}]`;
                    b._stockTicker.style.fill = sd.color;
                    b._stockTicker.style.stroke = sd.color;
                    b._stockTicker.style.dropShadow = true;
                    b._stockTicker.style.dropShadowColor = sd.color;
                    b._stockTicker.style.dropShadowBlur = 10;
                }
            }
            // VC ticker scroll
                ...
                if (b._vcTicker.x + b._vcTicker.width < 0) {
                    b._vcTicker.text =
                        typeof VCRow !== 'undefined'
                            ? b._vcTickerIsCrypto
                                ? VCRow.getNextCryptoTickerItem()
                                : VCRow.getNextTickerItem()
                            : '';
            ...
            if (b._bkTicker && b._bkTickerW && !b._bkTicker.destroyed) {
                ...
                if (b._bkTicker.x + b._bkTicker.width < 0) {
                    b._bkTicker.text =
                        typeof BackboneZone !== 'undefined' ? BackboneZone.getNextTickerItem() : '';
            ...
            if (doOcc) {
                const list = occ[b.id] || [];
                const ct = list.length;
                if (b._sign && b._sign.text !== undefined) {
                    if (ct > 0) b._sign.text = `${b.name} [${ct}]`;
                    else b._sign.text = b.name;
                    b._sign.scale.set(1);
        ...
        if (doOcc && G.bloomFilter) {
            const targetBloom = night ? 1.8 : 0.8;
            G.bloomFilter.bloomScale += (targetBloom - G.bloomFilter.bloomScale) * 0.05;
        }

        if (G.tick % 120 === 0) {
            const park = G.bldById['park'];
            if (park && park._monIcon && !park._monIcon.destroyed) {
                const sorted = [...G.models]
                    .filter((m) => !m.ret || new Date(m.ret) > new Date())
                    .map((m) => {
                        const elo = BM[m.id]?.ELO || 0;
                        const avg = typeof avgBM === 'function' ? avgBM(m.id) : 0;
                        return { m, score: elo ? (elo - 1000) / 4.5 : avg };
                    })
                    .sort((a, b) => b.score - a.score);
                const top = sorted[0];
                if (top) {
                    const lab = LABS[top.m.lab] || LABS.other;
                    const display = BM[top.m.id]?.ELO
                        ? BM[top.m.id].ELO + ' Elo'
                        : (typeof avgBM === 'function' ? avgBM(top.m.id) : '??') + '%';
                    park._monIcon.text = lab.icon || '★';
                    park._monIcon.style.fill = lab.color;
                    park._monIcon.style.fontSize = 24;
                    park.tip = `Current #1 Leader:\n${top.m.name} (${display})`;
                }
            }
        }
    },
