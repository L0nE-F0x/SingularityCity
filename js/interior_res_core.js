...
const InteriorRes = {
    ...
    build(bld, layer) {
        ...
        const activeModels = G.models.filter(
            (m) =>
                (!m.ret || new Date(m.ret) > new Date()) &&
                (LABS[m.lab] && LABS[m.lab].region ? LABS[m.lab].region === resRegion : resRegion === 'eu')
        );
        ...
        if (typeof Underground !== 'undefined') {
            ...
            ugGfx.beginFill(0x05050a);
            ugGfx.drawRect(0, ugTopY, G.vpW, earthH);
            ugGfx.endFill();
        }
        ugGfx.beginFill(0x05050a);
        ugGfx.drawRect(0, ugTopY + earthH, G.vpW, 800);
        ugGfx.endFill();
        ...
        for (let f = minFloor; f < numFloors; f++) {
            ...
            if (isSilo && isEstate) {
                ...
                scanGfx.beginFill(0x1e293b);
                scanGfx.drawRect(shaftX - 10, fy + floorH - 40, 20, 36);
                scanGfx.endFill();
                scanGfx.beginFill(0x0f172a);
                scanGfx.drawRect(shaftX - 6, fy + floorH - 36, 12, 18);
                scanGfx.endFill();
                scanGfx.beginFill(0xef4444, 0.5);
                scanGfx.drawCircle(shaftX, fy + floorH - 22, 3);
                scanGfx.endFill();
            ...
            // ─── BASEMENT FLOOR (f === -1) ───
                ...
                if (isEstate) {
                    ...
                    if (G.ceoRefs && G.ceoRefs[bld.lab]) {
                        ...
                        this.ceoCarGfx.beginFill(colHex);
                        this.ceoCarGfx.drawRoundedRect(-22, -18, 44, 18, 4);
                        this.ceoCarGfx.endFill();
                        this.ceoCarGfx.beginFill(colHex, 0.8);
                        this.ceoCarGfx.drawRoundedRect(-12, -28, 24, 12, 4);
                        this.ceoCarGfx.endFill();
                        this.ceoCarGfx.beginFill(0x333333);
                        this.ceoCarGfx.drawCircle(-12, -1, 4);
                        this.ceoCarGfx.drawCircle(12, -1, 4);
                        this.ceoCarGfx.endFill();
                        this.ceoCarGfx.beginFill(0xffffff, 1.0);
                        this.ceoCarGfx.drawRect(20, -8, 4, 6);
                        this.ceoCarGfx.endFill();
                        this.ceoCarGfx.beginFill(0xff3333, 1.0);
                        this.ceoCarGfx.drawRect(-26, -10, 4, 4);
                        this.ceoCarGfx.endFill();
    ...
    onMove: (e) => {
        ...
        if (
            InteriorRes.bld &&
            InteriorRes.bld.id.startsWith('house_') &&
            InteriorRes.floors[-2] &&
            newY <= InteriorRes.minY + 30
        ) {
                ...
                case 'ceo_sleeping': {
                    if (!night) {
                        ...
                        if (!av._sleepGfx) {
                            ...
                            sg.drawRoundedRect(-21, bt - 9, 16, 8, 3);
                            sg.endFill();
                            sg.beginFill(0xfdd8b5);
                            sg.drawCircle(-13, bt - 6, 5);
                            sg.endFill();
                            sg.beginFill(0x2c1810, 0.7);
                            sg.drawEllipse(-13, bt - 11, 4, 2);
                            sg.endFill();
                            sg.beginFill(0x2c1810, 0.5);
                            sg.drawRect(-16, bt - 6, 2, 1);
                            sg.drawRect(-11, bt - 6, 2, 1);
                            sg.endFill();
                            sg.beginFill(0x2c1810, 0.2);
                            sg.drawRect(-14, bt - 3, 3, 0.7);
                            sg.endFill();
                            ...
                            sc.on('pointertap', () => {
                                if (typeof UI !== 'undefined') UI.selectModel(av.m);
                            });
                            sc.on('pointerover', (e) => {
                                if (typeof UI !== 'undefined') UI.showTooltip(e, av.m.name, 'CEO (sleeping)');
                            });
                            sc.on('pointerout', () => {
                        ...
                        av._z2.y = -8 + Math.sin(zt + 1) * 3;
                        av._z2.alpha = 0.3 + Math.sin(zt + 1) * 0.25;
                ...
                case 'working': {
                    ...
                    if (actData.act === 'sleep' && av.bedX !== undefined) {
                        ...
                        if (!av._sleepGfx) {
                            ...
                            const sg = new PIXI.Graphics();
                            sg.beginFill(0xddd8c8, 0.7);
                            sg.drawRoundedRect(-15, bt - 8, 14, 7, 3);
                            sg.endFill();
                            sg.beginFill(0xfdd8b5);
                            sg.drawCircle(-8, bt - 5, 4);
                            sg.endFill();
                            sg.beginFill(0x2c1810, 0.7);
                            sg.drawEllipse(-8, bt - 9, 3.5, 1.5);
                            sg.endFill();
                            sg.beginFill(0x2c1810, 0.5);
                            sg.drawRect(-10.5, bt - 5, 1.8, 0.8);
                            sg.drawRect(-6, bt - 5, 1.8, 0.8);
                            sg.endFill();
                            sg.beginFill(0x2c1810, 0.2);
                            sg.drawRect(-9, bt - 2.5, 2.5, 0.6);
                            sg.endFill();
                            sg.beginFill(col, 0.5);
                            sg.moveTo(-2, bt + 2);
                            sg.lineTo(-2, bt - 1);
                            ...
                            sc.on('pointerover', (e) => {
                                if (typeof UI !== 'undefined')
                                    UI.showTooltip(
                                        e,
                                        av.m.name,
                                        (LABS[av.m.lab] || { name: 'Lab' }).name + ' (sleeping)'
                                    );
                            });
                            sc.on('pointerout', () => {
                                if (typeof UI !== 'undefined') UI.hideTooltip();
                        ...
                        if (Math.random() < 0.002 && this.bubbles.length < 15) {
                            this.spawnBubble(
                                av,
                                ['Making coffee.', 'Watching the gecko.', 'Reading papers.'][
                                    Math.floor(Math.random() * 3)
                                ]
                            );
                        }
