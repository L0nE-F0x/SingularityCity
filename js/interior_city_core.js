...
const InteriorCity = {
    ...
    build(bld, layer) {
        ...
        const lab = LABS[bld.lab] ||
            LABS['other'] || {
                name: bld.name || 'Public',
                color: '#64748b',
                icon: '🏢',
                ticker: null,
                desc: '',
        ...
        const activeModels = G.models.filter(
            (m) => m.lab === bld.lab && (!m.ret || new Date(m.ret) > new Date())
        );
                ...
                if (isHQ && f === 0) {
                    ...
                    if (G.ceoRefs && G.ceoRefs[bld.lab]) {
                        ...
                        if (ceoRef.bld === bld.id) {
                            const ceoModel = {
                                ...
                                phase: 'released',
                                isCeo: true,
                                founderData: ceoRef.f,
                            };

                            if (ceoRef.wantsToEnter) {
                                let av = this.drawAvatar(
                                    ceoModel,
                                    this.startX + 180,
                                    this.totalH - 4,
                                    floorCont,
                    ...
                    floorModelsQueue.forEach((m, idx) => {
                        ...
                        if (refs && refs.bld === bld.id) {
                            ...
                            const deskX = isSeated ? seats[idx % seats.length] : this.startX + 80 + idx * 40;
                    ...
                    if (floorTheme === 'campsite') {
                        ...
                        this.drawRefreshmentStand(
                            floorCont,
                            this.startX + this.bldW / 2 - 60,
                            fy + floorH - 4
                        );
                        ...
                        this.drawViewingPlatform(
                            floorCont,
                            this.startX + this.bldW / 2 + 100,
                            fy + floorH - 4,
                            80
                        );
                        ...
                        this.drawAvatar(
                            {
                                ...
                                phase: 'released',
                                lab: 'other',
                                desc: 'Ensuring safe viewing distances.',
                            },
                            this.startX + 140,
                            fy + floorH - 4,
                            floorCont,
                        ...
                        if (this.drawMirrorWall)
                            this.drawMirrorWall(
                                floorCont,
                                this.startX + this.usableW / 2,
                                fy + floorH - 4,
                                this.usableW - 100
                            );
                        while (currX < this.startX + this.usableW - 120) {
                        ...
                        if (this.drawWaterCooler)
                            this.drawWaterCooler(floorCont, this.startX + this.usableW - 60, fy + floorH - 4);
                        ...
                        while (currX < this.startX + this.usableW - 120) {
                            if (Math.random() > 0.5) {
                                if (this.drawServerWeights)
                                    this.drawServerWeights(floorCont, currX, fy + floorH - 4);
                            } else {
                                if (this.drawWeightBench)
                                    this.drawWeightBench(floorCont, currX, fy + floorH - 4);
                            }
                            currX += 90;
                        }
                        if (this.drawVendingMachine)
                            this.drawVendingMachine(
                                floorCont,
                                this.startX + this.usableW - 50,
                                fy + floorH - 4
                            );
                    } else if (floorTheme === 'gym_combat') {
                        if (this.drawRing)
                            this.drawRing(floorCont, this.startX + this.usableW / 2, fy + floorH - 4);
                        ...
                        while (currX < this.startX + this.usableW - 120) {
                            ...
                            if (Math.random() > 0.6 && this.drawExerciseBall)
                                this.drawExerciseBall(floorCont, currX + 30, fy + floorH - 4);
                            currX += 70;
                        }
                        if (this.drawPlant)
                            this.drawPlant(floorCont, this.startX + this.usableW - 50, fy + floorH - 4);
                        if (!_isNightShift)
                            this.drawAvatar(
                                {
                                    id: 'yoga_inst',
                        ...
                        // Pool & steam room
                        if (this.drawPoolLane)
                            this.drawPoolLane(floorCont, this.startX + this.usableW / 2, fy + floorH - 4);
                        if (this.drawSteamRoom)
                            this.drawSteamRoom(floorCont, this.startX + this.usableW - 60, fy + floorH - 4);
                        if (this.drawLockerRow)
                            this.drawLockerRow(floorCont, this.startX + 60, fy + floorH - 4);
                        if (this.drawWaterCooler)
                            this.drawWaterCooler(floorCont, this.startX + 140, fy + floorH - 4);
                        ...
                        if (this.drawLeaderboard)
                            this.drawLeaderboard(floorCont, this.startX + this.usableW - 80, fy + floorH - 4);
                        if (this.drawVendingMachine)
                            this.drawVendingMachine(
                                floorCont,
                                this.startX + this.usableW - 140,
                                fy + floorH - 4
                            );
                        if (this.drawPlant) {
                            this.drawPlant(floorCont, this.startX + 180, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + 340, fy + floorH - 4);
                        }
                        ...
                        if (this.drawWaterCooler)
                            this.drawWaterCooler(floorCont, this.startX + this.usableW - 50, fy + floorH - 4);
                    } else if (floorTheme === 'arena_main') {
                        if (this.drawSpotlight) {
                            this.drawSpotlight(
                                floorCont,
                                this.startX + this.usableW * 0.35,
                                fy + floorH - 4,
                                0xef4444
                            );
                            this.drawSpotlight(
                                floorCont,
                                this.startX + this.usableW * 0.65,
                                fy + floorH - 4,
                                0x38bdf8
                            );
                        }
                        if (this.drawRing)
                            this.drawRing(floorCont, this.startX + this.usableW / 2, fy + floorH - 4);
                        if (this.drawAudienceStands)
                            this.drawAudienceStands(floorCont, this.startX + 100, fy + floorH - 4, 80);
                        if (this.drawAudienceStands)
                            this.drawAudienceStands(
                                floorCont,
                                this.startX + this.usableW - 100,
                                fy + floorH - 4,
                                80
                        ...
                        if (this.drawCommentaryDesk)
                            this.drawCommentaryDesk(
                                floorCont,
                                this.startX + this.usableW / 2,
                                fy + floorH - 4
                            );
                        if (this.drawJumbotron)
                            this.drawJumbotron(floorCont, this.startX + 100, fy + floorH - 4);
                        if (this.drawPlant)
                            this.drawPlant(floorCont, this.startX + this.usableW - 50, fy + floorH - 4);
                        if (!_isNightShift)
                            this.drawAvatar(
                                {
                                    ...
                                    phase: 'released',
                                    lab: 'other',
                                    desc: 'And the ELO shifts again!',
                                },
                                this.startX + this.usableW / 2 - 15,
                                fy + floorH - 4,
                                floorCont,
                                f,
                                true
                        ...
                        if (this.drawMenuBoard)
                            this.drawMenuBoard(floorCont, this.startX + 60, fy + floorH - 4);
                        this.drawBaristaCounter(floorCont, this.startX + this.usableW - 100, fy + floorH - 4);
                        if (this.drawCoffeeMachine)
                            this.drawCoffeeMachine(
                                floorCont,
                                this.startX + this.usableW - 160,
                                fy + floorH - 4
                            );
                        if (this.drawPastryDisplay)
                            this.drawPastryDisplay(
                                floorCont,
                                this.startX + this.usableW - 220,
                                fy + floorH - 4
                            );
                        ...
                        if (this.drawBarStool) {
                            this.drawBarStool(floorCont, this.startX + this.usableW - 130, fy + floorH - 4);
                            this.drawBarStool(floorCont, this.startX + this.usableW - 115, fy + floorH - 4);
                            this.drawBarStool(floorCont, this.startX + this.usableW - 100, fy + floorH - 4);
                        }
                        if (this.drawPlant) {
                            this.drawPlant(floorCont, this.startX + 100, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + 280, fy + floorH - 4);
                        }
                        if (!_isNightShift)
                            this.drawAvatar(
                                {
                        ...
                        if (this.drawCafeBookshelf) {
                            this.drawCafeBookshelf(floorCont, this.startX + 80, fy + floorH - 4);
                            this.drawCafeBookshelf(
                                floorCont,
                                this.startX + this.usableW - 80,
                                fy + floorH - 4
                            );
                        }
                        if (this.drawCafeCouch) {
                            this.drawCafeCouch(floorCont, this.startX + 160, fy + floorH - 4, 0x8b4513);
                            this.drawCafeCouch(floorCont, this.startX + 320, fy + floorH - 4, 0x6b3410);
                        }
                        this.drawCafeTable(floorCont, this.startX + 240, fy + floorH - 4);
                        if (this.drawStringLights)
                            this.drawStringLights(floorCont, this.startX + 60, fy + 8, this.usableW - 120);
                        if (this.drawPlant) {
                            this.drawPlant(floorCont, this.startX + 200, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + 380, fy + floorH - 4);
                        }
                    } else if (floorTheme === 'cafe_kitchen') {
                        // Back kitchen — ovens, prep stations
                        if (this.drawKitchenOven) {
                            this.drawKitchenOven(floorCont, this.startX + 100, fy + floorH - 4);
                            this.drawKitchenOven(floorCont, this.startX + 200, fy + floorH - 4);
                        }
                        if (this.drawPrepStation) {
                            this.drawPrepStation(floorCont, this.startX + 300, fy + floorH - 4);
                            this.drawPrepStation(floorCont, this.startX + 400, fy + floorH - 4);
                        }
                        if (this.drawWaterCooler)
                            this.drawWaterCooler(floorCont, this.startX + this.usableW - 50, fy + floorH - 4);
                        if (!_isNightShift)
                            this.drawAvatar(
                                {
                                    ...
                                    phase: 'released',
                                    lab: 'other',
                                    desc: 'Batch processing croissants.',
                                },
                                this.startX + 260,
                                fy + floorH - 4,
                                floorCont,
                        ...
                        if (this.drawOutdoorTable) {
                            this.drawOutdoorTable(floorCont, this.startX + 120, fy + floorH - 4);
                            this.drawOutdoorTable(floorCont, this.startX + 260, fy + floorH - 4);
                            this.drawOutdoorTable(floorCont, this.startX + 400, fy + floorH - 4);
                        }
                        if (this.drawStringLights)
                            this.drawStringLights(floorCont, this.startX + 60, fy + 6, this.usableW - 120);
                        if (this.drawPlant) {
                            this.drawPlant(floorCont, this.startX + 60, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + 190, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + 330, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + this.usableW - 50, fy + floorH - 4);
                        }
                        if (this.drawBiophilicDivider)
                            this.drawBiophilicDivider(
                                floorCont,
                                this.startX + this.usableW - 100,
                                fy + floorH - 4
                            );
                        ...
                        if (this.drawContributorWall)
                            this.drawContributorWall(floorCont, this.startX + 280, fy + floorH - 4);
                            ...
                            this.drawAvatar(
                                {
                                    ...
                                    phase: 'released',
                                    lab: 'other',
                                    desc: 'Reviewing pull requests since 2020.',
                                },
                                this.startX + 140,
                                fy + floorH - 4,
                                floorCont,
                        ...
                        if (this.drawWaterCooler)
                            this.drawWaterCooler(floorCont, this.startX + 60, fy + floorH - 4);
                        if (this.drawVendingMachine)
                            this.drawVendingMachine(
                                floorCont,
                                this.startX + this.usableW - 50,
                        ...
                        if (this.drawWhiteboard) {
                            this.drawWhiteboard(floorCont, this.startX + 100, fy + floorH - 4);
                            this.drawWhiteboard(floorCont, this.startX + 350, fy + floorH - 4);
                        }
                        if (this.drawCollaborationPod) {
                            this.drawCollaborationPod(
                                floorCont,
                                this.startX + 220,
                                fy + floorH - 4,
                                0xa855f7
                        ...
                        if (this.drawPlant) {
                            this.drawPlant(floorCont, this.startX + 160, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + 300, fy + floorH - 4);
                        }
                        if (!_isNightShift)
                            this.drawAvatar(
                                {
                                    ...
                                    phase: 'released',
                                    lab: 'other',
                                    desc: 'Squashing bugs, one PR at a time.',
                                },
                                this.startX + 260,
                                fy + floorH - 4,
                                floorCont,
                        ...
                        if (this.drawBiophilicDivider)
                            this.drawBiophilicDivider(floorCont, this.startX + 140, fy + floorH - 4);
                        if (this.drawStringLights)
                            this.drawStringLights(floorCont, this.startX + 60, fy + 6, this.usableW - 120);
                        if (this.drawPlant) {
                            this.drawPlant(floorCont, this.startX + 340, fy + floorH - 4);
                            this.drawPlant(floorCont, this.startX + this.usableW - 60, fy + floorH - 4);
                        }
                        ...
                        this.drawAvatar(
                            {
                                ...
                                phase: 'released',
                                lab: 'other',
                                desc: 'Collector of deprecated models.',
                            },
                            this.startX + this.usableW - 60,
                            fy + floorH - 4,
                            floorCont,
                            f,
                            true
                    ...
                    const floorVisitors = visitingModels.slice(
                        f * visitorsPerFloor,
                        (f + 1) * visitorsPerFloor
                    );
                            ...
                            bld.id === 'cafe'
                                ...
                                : bld.id === 'gym'
                                  ...
                                  : bld.id === 'arena'
                                    ...
                                    : bld.id === 'graveyard'
                                      ? 'resting'
                                      : bld.id === 'open_square' || bld.id === 'os_hub'
                                        ? 'collaborating'
                                        : 'working';
        ...
        if (isForest) {
            ...
            this.scene.y = G.vpH * 0.7 - groundSceneY;
    ...
    _drawCafeRoastery(cont, bld, fy, floorH) {
        ...
        g.beginFill(0x3a2518, 0.4);
        g.drawRect(this.startX, fy, this.usableW, floorH);
        g.endFill();
        // Brick wall (back)
        ...
        roaster.beginFill(0x1f1f1f);
        roaster.drawRect(-22, -32, 44, 30);
        roaster.endFill(); // body
        roaster.beginFill(0x78350f);
        roaster.drawCircle(0, -17, 11);
        roaster.endFill(); // drum
        roaster.beginFill(0xfbbf24, 0.7);
        roaster.drawCircle(0, -17, 6);
        roaster.endFill(); // flame glow
        roaster.beginFill(0x0f0f0f);
        roaster.drawRect(-20, -2, 8, 4);
        roaster.drawRect(12, -2, 8, 4);
        roaster.endFill(); // legs
        roaster.beginFill(0x475569);
        roaster.drawRect(14, -32, 4, 12);
        roaster.endFill(); // exhaust pipe
        roaster.x = this.startX + 70;
        roaster.y = fY;
        ...
        for (let si = 0; si < 3; si++) {
            ...
            sack.beginFill(0x78532a);
            sack.drawRoundedRect(-9, -16, 18, 16, 2);
            sack.endFill();
            sack.beginFill(0x5a3d1c);
            sack.drawRect(-9, -16, 18, 3);
            sack.endFill(); // dark top
            sack.beginFill(0x3a2510);
            sack.drawRect(-3, -16, 6, 2);
            sack.endFill(); // tie
            // Stenciled text
            sack.beginFill(0x1a0f05, 0.6);
            sack.drawRect(-6, -10, 12, 1);
            sack.drawRect(-6, -7, 10, 1);
            sack.endFill();
            sack.x = this.startX + 130 + si * 22;
            sack.y = fY;
            cont.addChild(sack);
        }
        ...
        for (let bi = 0; bi < 4; bi++) {
            ...
            barrel.beginFill(0x3d2010);
            barrel.drawEllipse(0, -12, 13, 14);
            barrel.endFill();
            barrel.beginFill(0x5a3018);
            barrel.drawEllipse(0, -12, 10, 12);
            barrel.endFill();
            // Metal bands
            barrel.beginFill(0x64748b);
            barrel.drawRect(-13, -18, 26, 1.5);
            barrel.drawRect(-13, -12, 26, 1.5);
            barrel.drawRect(-13, -6, 26, 1.5);
            barrel.endFill();
            // Bunghole
            barrel.beginFill(0x1a0a04);
            barrel.drawCircle(0, -12, 1.2);
            barrel.endFill();
            barrel.x = this.startX + 230 + (bi % 2) * 32;
            barrel.y = fY - Math.floor(bi / 2) * 26;
            cont.addChild(barrel);
        }
        // Stacked row behind them
        const barrelRack = new PIXI.Graphics();
        barrelRack.beginFill(0x2a1508);
        barrelRack.drawRect(this.startX + 218, fY - 54, 80, 2);
        barrelRack.endFill();
        barrelRack.beginFill(0x2a1508);
        barrelRack.drawRect(this.startX + 218, fY - 28, 80, 2);
        barrelRack.endFill();
        cont.addChild(barrelRack);

        // Tasting bar counter (far right)
        const counter = new PIXI.Graphics();
        counter.beginFill(0x1a0d04);
        counter.drawRect(-30, -16, 60, 16);
        counter.endFill();
        counter.beginFill(0x3a2010);
        counter.drawRect(-30, -16, 60, 3);
        counter.endFill(); // wood top
        // Wine glasses
        for (let gi = 0; gi < 3; gi++) {
            counter.beginFill(0xfde68a, 0.4);
            counter.drawRect(-22 + gi * 14, -22, 3, 6);
            counter.endFill();
            counter.beginFill(0xb91c1c, 0.6);
            counter.drawRect(-22 + gi * 14, -19, 3, 3);
            counter.endFill();
        }
        counter.x = this.startX + this.usableW - 80;
        counter.y = fY;
        ...
        for (let li = 0; li < 5; li++) {
            ...
            bulb.beginFill(0xfbbf24, 0.8);
            bulb.drawCircle(0, 0, 2);
            bulb.endFill();
            bulb.beginFill(0x78532a);
            bulb.drawRect(-0.5, -8, 1, 8);
            bulb.endFill();
            bulb.x = this.startX + 40 + (li * (this.usableW - 80)) / 4;
            bulb.y = fy + 14;
            cont.addChild(bulb);
        ...
        if (typeof PIXI.Text !== 'undefined') {
            const sign = new PIXI.Text('ROASTERY & CELLAR', {
            ...
            sign.x = this.startX + this.usableW / 2;
            sign.y = fy + 6;
            cont.addChild(sign);
        }
    },
    ...
    _drawGymLockers(cont, bld, fy, floorH) {
        ...
        for (let tx = this.startX; tx < this.startX + this.usableW; tx += 24) {
            g.beginFill(0x0f1a2a);
            g.drawRect(tx, fy + floorH - 6, 24, 6);
            g.endFill();
            g.beginFill(0x2a3a50, 0.4);
            g.drawRect(tx + 1, fy + floorH - 5, 22, 1);
            g.endFill();
        }
        cont.addChild(g);
        ...
        for (let li = 0; li < 8; li++) {
            ...
            locker.beginFill(lCol);
            locker.drawRect(0, -48, 18, 48);
            locker.endFill();
            locker.beginFill(0x06b6d4, 0.3);
            locker.drawRect(1, -47, 16, 2);
            locker.endFill(); // top highlight
            // Handle
            locker.beginFill(0xe0f2fe);
            locker.drawCircle(14, -22, 1.2);
            locker.endFill();
            // Vents
            for (let vi = 0; vi < 3; vi++) {
                locker.beginFill(0x164e63);
                locker.drawRect(4 + vi * 4, -10, 2, 1);
                locker.endFill();
            }
            // Number
            locker.beginFill(0x83e4fd, 0.8);
            locker.drawRect(6, -42, 6, 4);
            locker.endFill();
            locker.x = this.startX + 30 + li * 20;
            locker.y = fY;
        ...
        bench.beginFill(0x78532a);
        bench.drawRect(this.startX + 30, fY - 10, 160, 5);
        bench.endFill();
        bench.beginFill(0x3a2510);
        bench.drawRect(this.startX + 34, fY - 5, 4, 5);
        bench.drawRect(this.startX + 178, fY - 5, 4, 5);
        bench.endFill();
        cont.addChild(bench);

        // Towel pile
        const towels = new PIXI.Graphics();
        towels.beginFill(0xf8fafc);
        towels.drawRect(this.startX + 210, fY - 20, 20, 5);
        towels.endFill();
        towels.beginFill(0xe0f2fe);
        towels.drawRect(this.startX + 210, fY - 15, 20, 5);
        towels.endFill();
        towels.beginFill(0xf8fafc);
        towels.drawRect(this.startX + 210, fY - 10, 20, 5);
        towels.endFill();
        towels.beginFill(0xbae6fd);
        towels.drawRect(this.startX + 210, fY - 5, 20, 5);
        towels.endFill();
        cont.addChild(towels);

        // Sauna (right side, cedar)
        const sauna = new PIXI.Graphics();
        sauna.beginFill(0x7c3e1a);
        sauna.drawRect(this.startX + this.usableW - 140, fY - 55, 130, 55);
        sauna.endFill();
        // Cedar plank lines
        for (let py = fY - 50; py < fY; py += 5) {
            sauna.beginFill(0x5a2a0f);
            sauna.drawRect(this.startX + this.usableW - 140, py, 130, 1);
            sauna.endFill();
        }
        // Glass door
        sauna.beginFill(0x7dd3fc, 0.3);
        sauna.drawRect(this.startX + this.usableW - 90, fY - 45, 30, 45);
        sauna.endFill();
        sauna.beginFill(0x0891b2);
        sauna.drawRect(this.startX + this.usableW - 64, fY - 28, 2, 4);
        sauna.endFill(); // handle
        // Stones on heater
        sauna.beginFill(0x1a1a1a);
        sauna.drawRect(this.startX + this.usableW - 132, fY - 22, 16, 16);
        sauna.endFill();
        sauna.beginFill(0x475569);
        sauna.drawCircle(this.startX + this.usableW - 127, fY - 16, 2);
        sauna.drawCircle(this.startX + this.usableW - 120, fY - 14, 2);
        sauna.drawCircle(this.startX + this.usableW - 122, fY - 19, 1.5);
        sauna.endFill();
        // Heat glow
        sauna.beginFill(0xef4444, 0.3);
        sauna.drawRect(this.startX + this.usableW - 133, fY - 26, 18, 4);
        sauna.endFill();
        // "SAUNA" sign
        cont.addChild(sauna);
        const saunaTxt = new PIXI.Text('SAUNA', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xfb923c,
            fontWeight: 'bold',
        });
        saunaTxt.anchor.set(0.5);
        saunaTxt.x = this.startX + this.usableW - 75;
        saunaTxt.y = fY - 60;
    ...
    _drawArenaDojo(cont, bld, fy, floorH) {
        ...
        g.drawRect(this.startX + 80, fy + floorH - 8, this.usableW - 160, 8);
        g.endFill();
        g.beginFill(0x450a0a);
        g.drawRect(this.startX + 80, fy + floorH - 8, this.usableW - 160, 1);
        g.endFill();
        cont.addChild(g);
        ...
        for (let bi = 0; bi < 3; bi++) {
            ...
            // Chain
            bag.beginFill(0x64748b);
            bag.drawRect(-1, -50, 2, 16);
            bag.endFill();
            // Bag body
            bag.beginFill(0x1e293b);
            bag.drawRoundedRect(-7, -34, 14, 32, 2);
            bag.endFill();
            bag.beginFill(0x0f172a);
            bag.drawRect(-7, -34, 14, 3);
            bag.endFill(); // top cap
            bag.beginFill(0xdc2626);
            bag.drawRect(-6, -18, 12, 2);
            bag.endFill(); // red accent stripe
            bag.x = this.startX + 40 + bi * 32;
            bag.y = fY;
            cont.addChild(bag);
        }
        ...
        rack.beginFill(0x334155);
        rack.drawRect(this.startX + 150, fY - 38, 60, 3);
        rack.endFill(); // top bar
        rack.beginFill(0x334155);
        rack.drawRect(this.startX + 150, fY - 12, 60, 3);
        rack.endFill(); // lower bar
        rack.beginFill(0x475569);
        rack.drawRect(this.startX + 150, fY - 38, 3, 38);
        rack.drawRect(this.startX + 207, fY - 38, 3, 38);
        rack.endFill(); // posts
        // Dumbbells
        for (let di = 0; di < 5; di++) {
            rack.beginFill(0x0f172a);
            rack.drawRect(this.startX + 156 + di * 10, fY - 35, 8, 3);
            rack.endFill();
            rack.beginFill(0x1e293b);
            rack.drawCircle(this.startX + 156 + di * 10, fY - 33, 2);
            rack.drawCircle(this.startX + 164 + di * 10, fY - 33, 2);
            rack.endFill();
        }
        // Plate weights on lower bar
        for (let pi = 0; pi < 4; pi++) {
            rack.beginFill(0x0f172a);
            rack.drawCircle(this.startX + 160 + pi * 14, fY - 15, 4);
            rack.endFill();
            rack.beginFill(0xdc2626);
            rack.drawCircle(this.startX + 160 + pi * 14, fY - 15, 2);
            rack.endFill();
        }
        cont.addChild(rack);
        ...
        for (let ii = 0; ii < 2; ii++) {
            ...
            bath.beginFill(0x475569);
            bath.drawRect(-18, -18, 36, 18);
            bath.endFill();
            bath.beginFill(0x7dd3fc, 0.6);
            bath.drawRect(-16, -16, 32, 14);
            bath.endFill();
            // Ice cubes floating
            bath.beginFill(0xf0f9ff, 0.8);
            bath.drawRect(-12, -14, 4, 3);
            bath.drawRect(-2, -12, 5, 3);
            bath.drawRect(8, -15, 4, 3);
            bath.endFill();
            // Steam puff (cold mist)
            bath.beginFill(0xffffff, 0.2);
            bath.drawCircle(0, -22, 6);
            bath.drawCircle(-6, -24, 4);
            bath.drawCircle(6, -24, 4);
            bath.endFill();
            bath.x = this.startX + 240 + ii * 44;
            bath.y = fY;
            cont.addChild(bath);
        }
        ...
        medic.beginFill(0xf8fafc);
        medic.drawRect(this.startX + this.usableW - 70, fY - 28, 50, 28);
        medic.endFill();
        medic.beginFill(0xdc2626);
        medic.drawRect(this.startX + this.usableW - 49, fY - 24, 8, 3);
        medic.drawRect(this.startX + this.usableW - 46, fY - 27, 2, 9);
        medic.endFill(); // red cross
        medic.beginFill(0x94a3b8);
        medic.drawRect(this.startX + this.usableW - 66, fY - 18, 42, 2);
        medic.endFill(); // shelf
        medic.beginFill(0xef4444);
        medic.drawRect(this.startX + this.usableW - 62, fY - 16, 4, 6);
        medic.drawRect(this.startX + this.usableW - 54, fY - 16, 4, 6);
        medic.endFill(); // first-aid kits
        medic.beginFill(0x1e293b);
        medic.drawRect(this.startX + this.usableW - 46, fY - 16, 4, 6);
        medic.endFill();
        cont.addChild(medic);

        // Hanging spotlight
        const spot = new PIXI.Graphics();
        spot.beginFill(0x1a1a1a);
        spot.drawRect(-6, 0, 12, 4);
        spot.endFill();
        spot.beginFill(0xfbbf24, 0.3);
        spot.drawPolygon([-6, 4, 6, 4, 18, 60, -18, 60]);
        spot.endFill();
        spot.x = this.startX + this.usableW / 2;
        spot.y = fy + 2;
        cont.addChild(spot);
    ...
    _drawOSServerVault(cont, bld, fy, floorH) {
        ...
        for (let tx = this.startX; tx < this.startX + this.usableW; tx += 20) {
            ...
            g.drawRect(tx, fy + floorH - 6, 20, 1);
            g.endFill();
            g.beginFill(0x0a0a18);
            g.drawRect(tx, fy + floorH - 4, 1, 4);
            g.endFill();
        }
        cont.addChild(g);
        ...
        for (let r = 0; r < 2; r++) {
            for (let ri = 0; ri < 5; ri++) {
                const rack = new PIXI.Graphics();
                rack.beginFill(0x1e293b);
                rack.drawRect(0, -44, 22, 44);
                rack.endFill();
                rack.beginFill(0x0f172a);
                rack.drawRect(1, -43, 20, 42);
                rack.endFill();
                // Blinking LEDs
                for (let li = 0; li < 8; li++) {
                    const col = [0x22c55e, 0xf59e0b, 0x22d3ee][li % 3];
                    rack.beginFill(col, 0.8);
                    rack.drawRect(3, -40 + li * 5, 2, 2);
                    rack.endFill();
                    rack.beginFill(0x475569);
                    rack.drawRect(7, -40 + li * 5, 14, 1);
                    rack.endFill();
                }
                // Top status
                rack.beginFill(0x22c55e, 0.9);
                rack.drawRect(2, -43, 18, 1);
                rack.endFill();
                rack.x = this.startX + 50 + ri * 30;
                rack.y = fY - r * 10;
                cont.addChild(rack);
        ...
        panel.beginFill(0x0f172a);
        panel.drawRect(this.startX + this.usableW - 110, fy + 10, 90, 36);
        panel.endFill();
        panel.beginFill(0x1e293b);
        panel.drawRect(this.startX + this.usableW - 108, fy + 12, 86, 32);
        panel.endFill();
        ...
        const sign = new PIXI.Text('OPEN SOURCE CORE', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x22c55e,
            fontWeight: 'bold',
    ...
    _drawGraveyardCrypt(cont, bld, fy, floorH) {
        ...
        for (let bx = this.startX; bx < this.startX + this.usableW; bx += 28) {
            for (let by = fy + 2; by < fy + floorH; by += 14) {
                g.beginFill(0x0f0f12);
                g.drawRect(bx, by, 28, 1);
                g.endFill();
                g.beginFill(0x0f0f12);
                g.drawRect(bx, by, 1, 14);
                g.endFill();
            }
        }
        cont.addChild(g);
        ...
        for (let ni = 0; ni < 6; ni++) {
            ...
            niche.beginFill(0x0a0a0e);
            niche.drawRect(0, -40, 24, 38);
            niche.endFill();
            niche.beginFill(0x1e1e28);
            niche.drawRect(2, -38, 20, 34);
            niche.endFill();
            // Urn
            niche.beginFill(0x475569);
            niche.drawEllipse(12, -18, 6, 10);
            niche.endFill();
            niche.beginFill(0x64748b);
            niche.drawRect(9, -28, 6, 2);
            niche.endFill(); // neck
            niche.beginFill(0x94a3b8);
            niche.drawRect(7, -30, 10, 1);
            niche.endFill(); // rim
            // Nameplate
            niche.beginFill(0x3a3a44);
            niche.drawRect(4, -6, 16, 3);
            niche.endFill();
            niche.x = this.startX + 30 + ni * 32;
            niche.y = fY;
            cont.addChild(niche);
        }
        ...
        for (let ci = 0; ci < 5; ci++) {
            ...
            candle.beginFill(0xf8fafc);
            candle.drawRect(-1.5, -8, 3, 8);
            candle.endFill();
            candle.beginFill(0xfbbf24, 0.9);
            candle.drawRect(-0.5, -12, 1, 4);
            candle.endFill();
            candle.beginFill(0xfef3c7, 0.4);
            candle.drawCircle(0, -11, 3);
            candle.endFill();
            candle.x = this.startX + 240 + ci * 28;
            candle.y = fY;
        ...
        plaque.beginFill(0x1a1a24);
        plaque.drawRect(-40, -22, 80, 22);
        plaque.endFill();
        plaque.beginFill(0x2a2a34);
        plaque.drawRect(-38, -20, 76, 18);
        plaque.endFill();
        plaque.x = this.startX + this.usableW - 70;
        plaque.y = fY;
        cont.addChild(plaque);
        const plaqueTxt = new PIXI.Text('IN MEMORIAM', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x94a3b8,
            letterSpacing: 1,
        });
        plaqueTxt.anchor.set(0.5);
        plaqueTxt.x = this.startX + this.usableW - 70;
        plaqueTxt.y = fY - 14;
    ...
    _drawHQBasement(cont, bld, fy, floorH, colHex) {
        ...
        floor.beginFill(0x0a0a12);
        floor.drawRect(this.startX, fy, this.usableW, floorH);
        floor.endFill();
        cont.addChild(floor);
    ...
    _hqBunker_openai(cont, bld, fy, floorH, fY, colHex) {
        ...
        door.drawCircle(this.startX + 50, fY - 22, 20);
        door.endFill();
        door.beginFill(0x1e293b);
        door.drawCircle(this.startX + 50, fY - 22, 17);
        door.endFill();
        door.beginFill(0x475569);
        door.drawCircle(this.startX + 50, fY - 22, 6);
        door.endFill();
        door.lineStyle(2, 0x64748b);
        door.drawCircle(this.startX + 50, fY - 22, 17);
        door.lineStyle(0);
        // Retinal scanner
        door.beginFill(0x0f172a);
        door.drawRect(this.startX + 72, fY - 30, 10, 14);
        door.endFill();
        door.beginFill(0xef4444, 0.7);
        door.drawRect(this.startX + 74, fY - 26, 6, 6);
        door.endFill();
        cont.addChild(door);
        ...
        for (let ri = 0; ri < 6; ri++) {
            const rack = new PIXI.Graphics();
            rack.beginFill(0x1e293b);
            rack.drawRect(0, -46, 24, 46);
            rack.endFill();
            // GPU slots
            for (let gi = 0; gi < 8; gi++) {
                rack.beginFill(0x0a0f1a);
                rack.drawRect(2, -44 + gi * 5, 20, 4);
                rack.endFill();
                rack.beginFill(0x76b900);
                rack.drawRect(3, -43 + gi * 5, 2, 2);
                rack.endFill(); // NV green
                rack.beginFill(0x22c55e, 0.7);
                rack.drawRect(18, -43 + gi * 5, 2, 1);
                rack.endFill();
            }
            rack.x = this.startX + 100 + ri * 32;
            rack.y = fY;
            cont.addChild(rack);
            this.indoorLights.push({ g: rack, maxA: 0.9, type: 'server' });
        }
        ...
        pipes.beginFill(0xcbd5e1);
        pipes.drawRect(this.startX + 90, fy + 8, 220, 3);
        pipes.endFill();
        pipes.beginFill(0x7dd3fc, 0.5);
        pipes.drawRect(this.startX + 90, fy + 11, 220, 1);
        pipes.endFill();
        // Frost droplets
        for (let di = 0; di < 6; di++) {
            pipes.beginFill(0xf0f9ff, 0.6);
            pipes.drawCircle(this.startX + 110 + di * 32, fy + 14, 1);
            pipes.endFill();
        }
        cont.addChild(pipes);

        const sign = new PIXI.Text('GPU COLD VAULT', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x10a37f,
            fontWeight: 'bold',
    ...
    _hqBunker_anthropic(cont, bld, fy, floorH, fY, colHex) {
        ...
        for (let si = 0; si < 4; si++) {
            ...
            shelf.beginFill(0x78532a);
            shelf.drawRect(0, -50, 60, 50);
            shelf.endFill();
            shelf.beginFill(0x5a3018);
            shelf.drawRect(0, -50, 60, 2);
            shelf.drawRect(0, -34, 60, 2);
            shelf.drawRect(0, -18, 60, 2);
            shelf.drawRect(0, -2, 60, 2);
            shelf.endFill();
            // Books (amber-toned, academic)
            const bkCols = [0xd97706, 0xb45309, 0x78350f, 0x92400e, 0xa16207];
            for (let bi = 0; bi < 10; bi++) {
                shelf.beginFill(bkCols[bi % bkCols.length]);
                shelf.drawRect(2 + bi * 5.5, -48, 5, 14);
                shelf.endFill();
                shelf.beginFill(bkCols[(bi + 2) % bkCols.length]);
                shelf.drawRect(2 + bi * 5.5, -32, 5, 14);
                shelf.endFill();
                shelf.beginFill(bkCols[(bi + 3) % bkCols.length]);
                shelf.drawRect(2 + bi * 5.5, -16, 5, 14);
                shelf.endFill();
            }
            shelf.x = this.startX + 20 + si * 50;
            shelf.y = fY;
            cont.addChild(shelf);
        }
        ...
        table.beginFill(0x3a2510);
        table.drawEllipse(0, 0, 40, 14);
        table.endFill();
        table.beginFill(0x5a3a20);
        table.drawEllipse(0, -2, 38, 12);
        table.endFill();
        table.x = this.startX + this.usableW - 100;
        table.y = fY - 8;
        cont.addChild(table);
        ...
        for (let ti = 0; ti < 4; ti++) {
            ...
            screen.drawRect(-10, -12, 20, 12);
            screen.endFill();
            const red = ti % 2 === 0;
            screen.beginFill(red ? 0xef4444 : 0xd97706, 0.7);
            screen.drawRect(-8, -10, 16, 2);
            screen.endFill();
            screen.beginFill(red ? 0xdc2626 : 0xb45309, 0.5);
            screen.drawRect(-8, -7, 12, 1);
            screen.drawRect(-8, -5, 10, 1);
            screen.drawRect(-8, -3, 14, 1);
            screen.endFill();
        ...
        const sign = new PIXI.Text('CONSTITUTIONAL ARCHIVE', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xd97706,
            fontWeight: 'bold',
    ...
    _hqBunker_google(cont, bld, fy, floorH, fY, colHex) {
        ...
        for (let ci = 0; ci < 7; ci++) {
            ...
            cab.beginFill(col);
            cab.drawRect(0, -44, 22, 44);
            cab.endFill();
            cab.beginFill(0xf8fafc, 0.15);
            cab.drawRect(2, -42, 18, 2);
            cab.endFill();
            // Drawer divisions
            for (let di = 0; di < 4; di++) {
                cab.beginFill(0x0f172a, 0.4);
                cab.drawRect(0, -44 + di * 11, 22, 1);
                cab.endFill();
                cab.beginFill(0xf8fafc);
                cab.drawRect(9, -40 + di * 11, 4, 1);
                cab.endFill(); // handle
            }
            cab.x = this.startX + 30 + ci * 28;
            cab.y = fY;
            cont.addChild(cab);
        }
        ...
        tpu.beginFill(0x0f172a);
        tpu.drawRect(0, -50, 120, 50);
        tpu.endFill();
        tpu.beginFill(0x1e293b);
        tpu.drawRect(2, -48, 116, 46);
        tpu.endFill();
        // TPU chips
        for (let r = 0; r < 4; r++) {
            for (let c = 0; c < 8; c++) {
                tpu.beginFill(0x4285f4, 0.8);
                tpu.drawRect(8 + c * 14, -44 + r * 11, 10, 6);
                tpu.endFill();
                tpu.beginFill(0xfbbc04, 0.9);
                tpu.drawCircle(13 + c * 14, -41 + r * 11, 0.8);
                tpu.endFill();
            }
        }
        tpu.x = this.startX + this.usableW - 170;
        tpu.y = fY;
        cont.addChild(tpu);
        ...
        tpuLbl.anchor.set(0.5);
        tpuLbl.x = this.startX + this.usableW - 110;
        tpuLbl.y = fY - 55;
        cont.addChild(tpuLbl);

        const sign = new PIXI.Text('KNOWLEDGE GRAPH ARCHIVE', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x4285f4,
            fontWeight: 'bold',
    ...
    _hqBunker_meta(cont, bld, fy, floorH, fY, colHex) {
        ...
        seed.beginFill(0x1e293b);
        seed.drawRect(0, -50, 100, 50);
        seed.endFill();
        seed.beginFill(0x0f172a);
        seed.drawRect(2, -48, 96, 46);
        seed.endFill();
        // Upload progress bars
        for (let r = 0; r < 6; r++) {
            const progress = 0.4 + r * 0.1;
            seed.beginFill(0x1877f2, 0.3);
            seed.drawRect(6, -44 + r * 7, 88, 4);
            seed.endFill();
            seed.beginFill(0x1877f2, 0.9);
            seed.drawRect(6, -44 + r * 7, 88 * progress, 4);
            seed.endFill();
            seed.beginFill(0x22c55e);
            seed.drawCircle(3, -42 + r * 7, 1);
            seed.endFill();
        }
        seed.x = this.startX + 30;
        seed.y = fY;
        cont.addChild(seed);
        ...
        for (let ci = 0; ci < 4; ci++) {
            ...
            crate.beginFill(0x78532a);
            crate.drawRect(0, -26, 34, 26);
            crate.endFill();
            crate.beginFill(0x5a3d1c);
            crate.drawRect(0, -26, 34, 2);
            crate.drawRect(0, -14, 34, 2);
            crate.endFill();
            // Wood plank lines
            for (let px = 6; px < 34; px += 8) {
                crate.beginFill(0x3a2510, 0.4);
                crate.drawRect(px, -26, 1, 26);
                crate.endFill();
            }
            // Stencil
            crate.beginFill(0xfbbf24, 0.8);
            crate.drawRect(4, -10, 26, 1);
            crate.drawRect(4, -8, 20, 1);
            crate.endFill();
            crate.x = this.startX + 150 + (ci % 2) * 40;
            crate.y = fY - Math.floor(ci / 2) * 28;
            cont.addChild(crate);
        }

        // VR headsets on shelf (right)
        const shelf = new PIXI.Graphics();
        shelf.beginFill(0x475569);
        shelf.drawRect(this.startX + this.usableW - 100, fY - 20, 80, 2);
        shelf.endFill();
        for (let vi = 0; vi < 3; vi++) {
            shelf.beginFill(0x1e293b);
            shelf.drawRect(this.startX + this.usableW - 92 + vi * 24, fY - 32, 18, 12);
            shelf.endFill();
            shelf.beginFill(0x1877f2, 0.4);
            shelf.drawRect(this.startX + this.usableW - 90 + vi * 24, fY - 30, 14, 4);
            shelf.endFill();
            shelf.beginFill(0x0f172a);
            shelf.drawRect(this.startX + this.usableW - 92 + vi * 24, fY - 34, 18, 2);
            shelf.endFill();
        }
        cont.addChild(shelf);

        const sign = new PIXI.Text('OPEN WEIGHTS LIBRARY', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x1877f2,
            fontWeight: 'bold',
    ...
    _hqBunker_xai(cont, bld, fy, floorH, fY, colHex) {
        ...
        for (let ti = 0; ti < 3; ti++) {
            ...
            tank.beginFill(0x1e293b);
            tank.drawEllipse(0, -14, 14, 6);
            tank.endFill(); // top
            tank.beginFill(0x334155);
            tank.drawRect(-14, -14, 28, 30);
            tank.endFill(); // body
            tank.beginFill(0x0f172a);
            tank.drawEllipse(0, 16, 14, 4);
            tank.endFill(); // bottom
            // Warning stripe
            tank.beginFill(0xfbbf24);
            tank.drawRect(-14, -4, 28, 3);
            tank.endFill();
            tank.beginFill(0x0f172a);
            tank.drawRect(-14, -4, 28, 1);
            tank.endFill();
            // Label
            tank.beginFill(0xef4444, 0.8);
            tank.drawRect(-8, 2, 16, 3);
            tank.endFill();
            tank.x = this.startX + 40 + ti * 40;
            tank.y = fY - 20;
            cont.addChild(tank);
        }
        ...
        const pipes = new PIXI.Graphics();
        pipes.beginFill(0x64748b);
        pipes.drawRect(this.startX + 30, fY - 8, 120, 3);
        pipes.endFill();
        pipes.beginFill(0x475569);
        pipes.drawRect(this.startX + 30, fY - 5, 120, 1);
        pipes.endFill();
        cont.addChild(pipes);

        // Starship model display (center)
        const rocket = new PIXI.Graphics();
        rocket.beginFill(0xcbd5e1);
        rocket.drawRect(-3, -46, 6, 40);
        rocket.endFill();
        rocket.beginFill(0xe0e7ff);
        rocket.drawPolygon([-3, -46, 3, -46, 0, -54]);
        rocket.endFill();
        rocket.beginFill(0x1e293b);
        rocket.drawRect(-4, -6, 8, 2);
        rocket.drawPolygon([-4, -6, -8, 0, -4, 0]);
        rocket.drawPolygon([4, -6, 8, 0, 4, 0]);
        rocket.endFill();
        rocket.beginFill(0xef4444, 0.7);
        rocket.drawRect(-2, -2, 4, 6);
        rocket.endFill(); // exhaust glow
        rocket.x = this.startX + this.usableW / 2;
        rocket.y = fY;
        cont.addChild(rocket);
        // Pedestal
        const pedestal = new PIXI.Graphics();
        pedestal.beginFill(0x1a1a1a);
        pedestal.drawRect(this.startX + this.usableW / 2 - 12, fY - 4, 24, 4);
        pedestal.endFill();
        cont.addChild(pedestal);

        // Grok token "tokens per second" counter
        const counter = new PIXI.Graphics();
        counter.beginFill(0x0f172a);
        counter.drawRect(this.startX + this.usableW - 80, fY - 30, 60, 28);
        counter.endFill();
        counter.beginFill(0x000000);
        counter.drawRect(this.startX + this.usableW - 78, fY - 28, 56, 24);
        counter.endFill();
        // Fake readout
        counter.beginFill(0xfbbf24, 0.9);
        counter.drawRect(this.startX + this.usableW - 74, fY - 24, 48, 3);
        counter.endFill();
        counter.beginFill(0xfbbf24, 0.6);
        counter.drawRect(this.startX + this.usableW - 74, fY - 18, 36, 2);
        counter.drawRect(this.startX + this.usableW - 74, fY - 14, 42, 2);
        counter.drawRect(this.startX + this.usableW - 74, fY - 10, 30, 2);
        counter.endFill();
        cont.addChild(counter);
        this.indoorLights.push({ g: counter, maxA: 0.9, type: 'screen' });

        const sign = new PIXI.Text('XAI FUEL DEPOT', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xfbbf24,
            fontWeight: 'bold',
    ...
    _hqBunker_mistral(cont, bld, fy, floorH, fY, colHex) {
        ...
        arch.beginFill(0x3a3228);
        arch.drawRect(this.startX + 4, fy + 2, this.usableW - 8, 14);
        arch.endFill();
        for (let ax = this.startX + 8; ax < this.startX + this.usableW - 8; ax += 18) {
            arch.beginFill(0x1a1a1a, 0.4);
            arch.drawRect(ax, fy + 2, 1, 14);
            arch.endFill();
        }
        cont.addChild(arch);
        ...
        for (let r = 0; r < 3; r++) {
            for (let bi = 0; bi < 6; bi++) {
                const barrel = new PIXI.Graphics();
                barrel.beginFill(0x3d2010);
                barrel.drawEllipse(0, 0, 14, 15);
                barrel.endFill();
                barrel.beginFill(0x5a3018);
                barrel.drawEllipse(0, 0, 11, 13);
                barrel.endFill();
                // Bands
                barrel.beginFill(0x64748b);
                barrel.drawRect(-14, -7, 28, 1.5);
                barrel.drawRect(-14, 0, 28, 1.5);
                barrel.drawRect(-14, 7, 28, 1.5);
                barrel.endFill();
                // Mistral logo (brand)
                barrel.beginFill(0xff7000);
                barrel.drawRect(-4, -3, 8, 1);
                barrel.drawRect(-4, -1, 6, 1);
                barrel.drawRect(-4, 1, 8, 1);
                barrel.endFill();
            ...
            // Rack shelf
            const rack = new PIXI.Graphics();
            rack.beginFill(0x2a1508);
            rack.drawRect(this.startX + 30, fY - 2 - r * 24, 200, 2);
            rack.endFill();
            cont.addChild(rack);
        }
        ...
        // Tasting table
        const table = new PIXI.Graphics();
        table.beginFill(0x1a0d04);
        table.drawRect(-30, -14, 60, 14);
        table.endFill();
        table.beginFill(0x3a2010);
        table.drawRect(-30, -14, 60, 2);
        table.endFill();
        // Glasses of red
        for (let gi = 0; gi < 4; gi++) {
            table.beginFill(0xfde68a, 0.3);
            table.drawRect(-22 + gi * 12, -20, 3, 6);
            table.endFill();
            table.beginFill(0x7f1d1d);
            table.drawRect(-22 + gi * 12, -17, 3, 3);
            table.endFill();
        }
        table.x = this.startX + this.usableW - 80;
        table.y = fY;
        cont.addChild(table);

        // Hanging cellar lamp
        const lamp = new PIXI.Graphics();
        lamp.beginFill(0x1a1a1a);
        lamp.drawRect(-0.5, 0, 1, 10);
        lamp.endFill();
        lamp.beginFill(0xfbbf24, 0.8);
        lamp.drawCircle(0, 14, 3);
        lamp.endFill();
        lamp.beginFill(0xfef3c7, 0.3);
        lamp.drawCircle(0, 14, 6);
        lamp.endFill();
        lamp.x = this.startX + this.usableW / 2;
        lamp.y = fy + 14;
        cont.addChild(lamp);
        this.indoorLights.push({ g: lamp, maxA: 0.8, type: 'fire' });

        const sign = new PIXI.Text('LA CAVE DE MISTRAL', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xff7000,
            fontWeight: 'bold',
    ...
    _hqBunker_deepseek(cont, bld, fy, floorH, fY, colHex) {
        ...
        sheen.beginFill(0x111118);
        sheen.drawRect(this.startX, fy, this.usableW, floorH);
        sheen.endFill();
        sheen.beginFill(0x1e1e28, 0.6);
        sheen.drawRect(this.startX, fy + floorH - 10, this.usableW, 4);
        sheen.endFill();
        cont.addChild(sheen);
        ...
        const rack = new PIXI.Graphics();
        rack.beginFill(0x1e293b);
        rack.drawRect(-14, -50, 28, 50);
        rack.endFill();
        rack.beginFill(0x0f172a);
        rack.drawRect(-13, -49, 26, 48);
        rack.endFill();
        // Minimal LEDs — just a few
        rack.beginFill(0xef4444, 0.9);
        rack.drawRect(-10, -45, 2, 2);
        rack.endFill();
        rack.beginFill(0x22c55e, 0.8);
        rack.drawRect(-10, -38, 2, 2);
        rack.endFill();
        rack.beginFill(0xef4444, 0.9);
        rack.drawRect(-10, -20, 2, 2);
        rack.endFill();
        rack.x = this.startX + this.usableW / 2;
        rack.y = fY;
        cont.addChild(rack);
        ...
        for (let li = 0; li < 4; li++) {
            ...
            lantern.beginFill(0x1a1a1a);
            lantern.drawRect(-0.5, 0, 1, 12);
            lantern.endFill();
            lantern.beginFill(0xdc2626);
            lantern.drawEllipse(0, 18, 6, 8);
            lantern.endFill();
            lantern.beginFill(0xef4444, 0.5);
            lantern.drawEllipse(0, 18, 4, 6);
            lantern.endFill();
            lantern.beginFill(0xfbbf24);
            lantern.drawRect(-0.5, 26, 1, 3);
            lantern.endFill(); // tassel
    ...
    _hqBunker_alibaba(cont, bld, fy, floorH, fY, colHex) {
        ...
        wood.beginFill(0x5a3018);
        wood.drawRect(this.startX, fy, this.usableW, floorH);
        wood.endFill();
        for (let wx = this.startX; wx < this.startX + this.usableW; wx += 16) {
            wood.beginFill(0x3a1f10, 0.5);
            wood.drawRect(wx, fy, 1, floorH);
            wood.endFill();
        }
        cont.addChild(wood);
        ...
        for (let r = 0; r < 3; r++) {
            const shelf = new PIXI.Graphics();
            shelf.beginFill(0x78350f);
            shelf.drawRect(this.startX + 30, fY - 18 - r * 20, 180, 2);
            shelf.endFill();
            cont.addChild(shelf);
            for (let ti = 0; ti < 6; ti++) {
                const tablet = new PIXI.Graphics();
                tablet.beginFill(0x065f46);
                tablet.drawRect(0, -14, 12, 14);
                tablet.endFill();
                tablet.beginFill(0x10b981, 0.6);
                tablet.drawRect(1, -13, 10, 12);
                tablet.endFill();
                // Character carving
                tablet.beginFill(0x022c22);
                tablet.drawRect(3, -10, 6, 1);
                tablet.drawRect(3, -8, 4, 1);
                tablet.drawRect(3, -6, 6, 1);
                tablet.endFill();
        ...
        abacus.beginFill(0x78350f);
        abacus.drawRect(this.startX + this.usableW - 100, fY - 44, 80, 44);
        abacus.endFill();
        abacus.beginFill(0x5a2a0a);
        abacus.drawRect(this.startX + this.usableW - 98, fY - 42, 76, 40);
        abacus.endFill();
        // Rods + beads
        for (let rod = 0; rod < 5; rod++) {
            abacus.beginFill(0x92400e);
            abacus.drawRect(this.startX + this.usableW - 92 + rod * 15, fY - 40, 1, 36);
            abacus.endFill();
            for (let bd = 0; bd < 7; bd++) {
                abacus.beginFill(0xfbbf24);
                abacus.drawCircle(this.startX + this.usableW - 91.5 + rod * 15, fY - 38 + bd * 5, 1.5);
                abacus.endFill();
            }
        }
        ...
        for (let li = 0; li < 3; li++) {
            const lantern = new PIXI.Graphics();
            lantern.beginFill(0x1a1a1a);
            lantern.drawRect(-0.5, 0, 1, 10);
            lantern.endFill();
            lantern.beginFill(0xf97316);
            lantern.drawEllipse(0, 15, 5, 7);
            lantern.endFill();
            lantern.beginFill(0xfb923c, 0.6);
            lantern.drawEllipse(0, 15, 3, 5);
            lantern.endFill();
            lantern.x = this.startX + 80 + li * 60;
            lantern.y = fy + 8;
            cont.addChild(lantern);
            this.indoorLights.push({ g: lantern, maxA: 0.8, type: 'fire' });
        }

        const sign = new PIXI.Text('丝绸之路档案', {
            fontFamily: 'sans-serif',
            fontSize: 7,
            fill: 0xf97316,
            fontWeight: 'bold',
    ...
    _hqBunker_nvidia(cont, bld, fy, floorH, fY, colHex) {
        ...
        for (let bi = 0; bi < 4; bi++) {
            ...
            bin.beginFill(0x1e293b);
            bin.drawRect(-18, -28, 36, 28);
            bin.endFill();
            bin.beginFill(0x0f172a);
            bin.drawRect(-16, -26, 32, 24);
            bin.endFill();
            // Wafers (edge-on stack)
            for (let wi = 0; wi < 5; wi++) {
                bin.beginFill(0x76b900, 0.7);
                bin.drawCircle(0, -22 + wi * 4, 12);
                bin.endFill();
                bin.beginFill(0x4d7c0f, 0.6);
                bin.drawCircle(0, -22 + wi * 4, 10);
                bin.endFill();
            }
            bin.x = this.startX + 50 + bi * 50;
            bin.y = fY;
            cont.addChild(bin);
        }
        ...
        probe.beginFill(0xcbd5e1);
        probe.drawRect(-30, -40, 60, 40);
        probe.endFill();
        probe.beginFill(0xf1f5f9);
        probe.drawRect(-28, -38, 56, 36);
        probe.endFill();
        probe.beginFill(0x0f172a);
        probe.drawRect(-20, -30, 40, 20);
        probe.endFill();
        // Microscope arm
        probe.beginFill(0x475569);
        probe.drawRect(-2, -38, 4, 18);
        probe.drawRect(-8, -22, 16, 4);
        probe.endFill();
        probe.beginFill(0x76b900);
        probe.drawCircle(0, -18, 2);
        probe.endFill();
        probe.x = this.startX + this.usableW - 90;
        probe.y = fY;
        cont.addChild(probe);
        this.indoorLights.push({ g: probe, maxA: 0.8, type: 'screen' });

        const sign = new PIXI.Text('WAFER VAULT', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x76b900,
            fontWeight: 'bold',
    ...
    _hqBunker_microsoft(cont, bld, fy, floorH, fY, colHex) {
        ...
        for (let r = 0; r < 3; r++) {
            for (let ti = 0; ti < 8; ti++) {
                ...
                tape.beginFill(0x1e40af);
                tape.drawRect(0, -7, 22, 7);
                tape.endFill();
                tape.beginFill(0x3b82f6);
                tape.drawRect(1, -6, 20, 2);
                tape.endFill();
                tape.beginFill(0x0f172a);
                tape.drawCircle(6, -3, 1.5);
                tape.drawCircle(16, -3, 1.5);
                tape.endFill();
                tape.beginFill(0xf8fafc, 0.6);
                tape.drawRect(4, -5, 14, 1);
                tape.endFill(); // label
        ...
        for (let r = 0; r < 3; r++) {
            const shelf = new PIXI.Graphics();
            shelf.beginFill(0x64748b);
            shelf.drawRect(this.startX + 25, fY - 1 - r * 10, 210, 2);
            shelf.endFill();
            cont.addChild(shelf);
        }
        ...
        holo.beginFill(0x1e293b);
        holo.drawRect(this.startX + this.usableW - 100, fY - 50, 80, 50);
        holo.endFill();
        holo.beginFill(0x0f172a);
        holo.drawRect(this.startX + this.usableW - 98, fY - 48, 76, 46);
        holo.endFill();
        holo.beginFill(0x7dd3fc, 0.3);
        holo.drawRect(this.startX + this.usableW - 96, fY - 46, 72, 42);
        holo.endFill();
        // HoloLens headset silhouette
        holo.beginFill(0x334155);
        holo.drawRoundedRect(this.startX + this.usableW - 85, fY - 32, 50, 10, 4);
        holo.endFill();
        holo.beginFill(0x0ea5e9, 0.6);
        holo.drawRect(this.startX + this.usableW - 82, fY - 30, 44, 2);
        holo.endFill();
        cont.addChild(holo);
        this.indoorLights.push({ g: holo, maxA: 0.8, type: 'screen' });

        const sign = new PIXI.Text('AZURE COLD TIER', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x0ea5e9,
            fontWeight: 'bold',
    ...
    _hqBunker_generic(cont, bld, fy, floorH, fY, colHex) {
        ...
        parking.moveTo(this.startX + 80, fY);
        parking.lineTo(this.startX + 50, fY - 12);
        parking.moveTo(this.startX + 130, fY);
        parking.lineTo(this.startX + 100, fY - 12);
        parking.moveTo(this.startX + 180, fY);
        parking.lineTo(this.startX + 150, fY - 12);
        parking.lineStyle(0);
        ...
        for (let ci = 0; ci < 4; ci++) {
            const crate = new PIXI.Graphics();
            crate.beginFill(0x1e293b);
            crate.drawRect(0, -22, 24, 22);
            crate.endFill();
            crate.beginFill(0x334155);
            crate.drawRect(1, -21, 22, 2);
            crate.endFill();
            crate.beginFill(colHex, 0.6);
            crate.drawRect(3, -15, 18, 2);
            crate.endFill();
            // Frost
            crate.beginFill(0xbfdbfe, 0.4);
            crate.drawRect(0, -22, 24, 1);
            crate.endFill();
    ...
    _drawZoneUnderground(scene, bld, startX, bldW, surfaceY, belowY, floorH) {
        ...
        if (zone !== 'port' && zone !== 'silo') {
            ...
            g.beginFill(earthCol);
            g.drawRect(0, surfaceY, startX - 2, floorH);
            g.endFill();
            g.beginFill(earthCol);
            g.drawRect(startX + bldW + 2, surfaceY, vpW - startX - bldW - 2, floorH);
            g.endFill();
            g.beginFill(topsoilCol);
            g.drawRect(0, surfaceY, startX - 2, 6);
            g.endFill();
            g.beginFill(topsoilCol);
            g.drawRect(startX + bldW + 2, surfaceY, vpW - startX - bldW - 2, 6);
            g.endFill();
            // Grass strip at surface level
            g.beginFill(0x2d5a3f);
            g.drawRect(0, surfaceY - 4, startX - 2, 4);
            g.endFill();
            g.beginFill(0x2d5a3f);
            g.drawRect(startX + bldW + 2, surfaceY - 4, vpW - startX - bldW - 2, 4);
            g.endFill();
            // Foundation strip just below the basement floor
            g.beginFill(0x1a1810);
            g.drawRect(0, belowY - 4, vpW, 10);
            g.endFill();
        }
        ...
        if (typeof Underground !== 'undefined') {
            ...
            g.drawRect(0, undergroundY + totalDepth, vpW, 500);
            g.endFill();
        }
        scene.addChild(g);
        ...
        if (this._liveTrains) {
            try {
                this._liveTrains.destroy();
            } catch (e) {}
            this._liveTrains = null;
        }
    ...
    update() {
        ...
        if (this.starsLayer) {
            ...
            if (night) {
                this.starsLayer.children.forEach((s) => {
                    s.alpha = 0.15 + Math.abs(Math.sin(G.tick * 0.03 + s._phase)) * 0.5;
                });
            }
        }
        ...
        if (this.indoorLights) {
            const isWorkingHours = dp >= 0.35 && dp <= 0.8;
