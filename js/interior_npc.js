...
const InteriorNPC = {
    ...
    build(bld, layer) {
        ...
        sg.drawRect(bX + 12, bY + bH, 4, 8);
        sg.drawRect(bX + bW - 16, bY + bH, 4, 8);
        sg.endFill();
        rc.addChild(sg);
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            rg.beginFill(0x1a2030);
            rg.drawRect(shaftX + shaftW - 4, fy, 6, floorH);
            rg.endFill();
            rg.beginFill(0x0f1520);
            rg.drawRect(shaftX + 6, fy, shaftW - 12, floorH);
            rg.endFill();

            if (!isB) {
                rg.beginFill(0x151c28);
                rg.drawRect(startX, fy, usableW, 20);
                rg.endFill();
                rg.beginFill(0x151c28);
                rg.drawRect(startX, fy + 52, usableW, floorH - 52);
                rg.endFill();
                let wx = windowX;
                rg.beginFill(0x151c28);
                rg.drawRect(startX, fy + 20, windowX - startX, 32);
                rg.endFill();
                while (wx + 40 <= windowX + windowW) {
                    wx += 40;
                    if (wx + 20 <= windowX + windowW) {
                        rg.beginFill(0x151c28);
                        rg.drawRect(wx, fy + 20, 20, 32);
                        rg.endFill();
                    }
                    wx += 20;
                }
                rg.beginFill(0x151c28);
                rg.drawRect(wx - 20, fy + 20, startX + usableW - wx + 20, 32);
                rg.endFill();
                // Window frames
                const wf = new PIXI.Graphics();
                let cwx = windowX;
                while (cwx + 40 <= windowX + windowW) {
                    wf.lineStyle(2, 0x2a3448);
                    wf.drawRect(cwx, fy + 20, 40, 32);
                    wf.moveTo(cwx + 20, fy + 20);
                    wf.lineTo(cwx + 20, fy + 52);
                    wf.moveTo(cwx, fy + 36);
                    wf.lineTo(cwx + 40, fy + 36);
                    wf.lineStyle(0);
                    cwx += 60;
                }
                this.scene.addChild(wf);
            } else {
                rg.beginFill(0x10161e);
                rg.drawRect(startX, fy, usableW, floorH);
                rg.endFill();
            }
            rg.beginFill(0x0f1520);
            rg.drawRect(startX, fy + floorH - 6, usableW, 6);
            rg.endFill();
            rg.beginFill(0x0c1018);
            rg.drawRect(startX, fy, usableW, 3);
            rg.endFill();
            rg.beginFill(0x222a38);
            rg.drawRect(startX - 6, fy + floorH - 3, usableW + shaftW + 28, 3);
            rg.endFill();
            ...
            for (let li = 1; li <= 4; li++) {
                ...
                lg.beginFill(0xfbbf24, 0.3);
                lg.drawRect(lx - 8, fy + 2, 16, 2);
                lg.endFill();
                const bm = new PIXI.Graphics();
                bm.beginFill(0xfbbf24, 0.03);
                bm.moveTo(lx - 8, fy + 4);
                bm.lineTo(lx + 8, fy + 4);
                bm.lineTo(lx + 20, fy + floorH - 6);
                bm.lineTo(lx - 20, fy + floorH - 6);
                bm.closePath();
                bm.endFill();
                this.scene.addChild(lg, bm);
                this.indoorLights.push({ g: bm, maxA: 0.05, type: 'screen' });
            }

            // Elevator door
            const dr = new PIXI.Graphics();
            dr.beginFill(0x2a3448);
            dr.lineStyle(1, 0x1a2030);
        ...
        earth.beginFill(earthBase);
        earth.drawRect(0, groundY, startX - 6, floorH);
        earth.endFill();
        earth.beginFill(earthHi);
        ...
        if (typeof G !== 'undefined' && G.tracking && G.tracking.type === 'npc') {
            ...
            const cm =
                typeof NPCHousing !== 'undefined' && NPCHousing.commuters.find((c) => c.npc.id === trackedId);
            if (cm && cm.bld === bld.id) {
                // Find the resident data to get their color
                const trackedRes = residents.find(
                    (r) => 'npc_' + r.name.toLowerCase().replace(/\s/g, '_') === trackedId
                );
                ...
                body.beginFill(col);
                body.drawRoundedRect(-bw / 2, 0, bw, 14, bw * 0.1);
                body.endFill();
                body.y = -h + headH;
                ...
                if (G._addTrackHighlight) {
                    ...
                    if (hl) {
                        avObj._trackGlow = hl.glow;
                        avObj._trackArrow = hl.arrow;
                    }
                }
        ...
        // Position & scroll
        const bp = 56;
        const initY = G.vpH - bp - this.totalH + floorH;
        this.scene.y = initY;
        ...
        this._onMove = (e) => {
            if (!InteriorNPC.isDragging || !InteriorNPC.scene || InteriorNPC.scene.destroyed) return;
            let ny = InteriorNPC._startSceneY + (e.clientY - InteriorNPC._startY);
            ny = Math.max(InteriorNPC.minY, Math.min(ny, InteriorNPC.maxY));
            InteriorNPC.scene.y = ny;
        };
        this._onUp = () => {
            InteriorNPC.isDragging = false;
            if (InteriorNPC.layer) InteriorNPC.layer.cursor = 'grab';
        };
    ...
    _drawFoyer(c, sx, uw, pY, fy, fh, residents) {
        ...
        dg.eventMode = 'none';
        dg.beginFill(0x1e293b);
        dg.drawRect(sx + 40, pY - 18, 80, 18);
        dg.endFill();
        dg.beginFill(0x475569);
        dg.drawRect(sx + 40, pY - 20, 80, 3);
        dg.endFill();
        dg.beginFill(0x0a0a18);
        dg.drawRect(sx + 60, pY - 32, 20, 12);
        dg.endFill();
        dg.beginFill(0x22d3ee, 0.3);
        dg.drawRect(sx + 62, pY - 30, 16, 8);
        dg.endFill();
        c.addChild(dg);
        // Mailboxes
        const mb = new PIXI.Graphics();
        mb.eventMode = 'none';
        mb.beginFill(0x1e293b);
        mb.drawRect(sx + 160, pY - 40, 60, 40);
        mb.endFill();
        for (let my = pY - 38; my < pY - 4; my += 10) {
            for (let mx = sx + 164; mx < sx + 216; mx += 14) {
                mb.beginFill(0x334155);
                mb.drawRect(mx, my, 10, 7);
                mb.endFill();
            }
        }
        c.addChild(mb);
        ...
        couch.beginFill(0x334155, 0.8);
        couch.drawRoundedRect(sx + 260, pY - 12, 50, 12, 2);
        couch.endFill();
        couch.beginFill(0x334155, 0.5);
        couch.drawRoundedRect(sx + 256, pY - 18, 6, 18, 2);
        couch.drawRoundedRect(sx + 306, pY - 18, 6, 18, 2);
        couch.endFill();
        c.addChild(couch);
        // Plant
        const pl = new PIXI.Graphics();
        pl.eventMode = 'none';
        pl.beginFill(0x334155);
        pl.drawRect(sx + uw - 60, pY - 12, 14, 12);
        pl.endFill();
        pl.beginFill(0x166534);
        pl.drawEllipse(sx + uw - 53, pY - 18, 10, 8);
        pl.endFill();
        pl.beginFill(0x1b4332);
        pl.drawEllipse(sx + uw - 53, pY - 16, 7, 6);
        pl.endFill();
        c.addChild(pl);
        // Resident board
        const bd = new PIXI.Graphics();
        bd.eventMode = 'none';
        bd.beginFill(0x0f172a);
        bd.drawRect(sx + 360, pY - 45, 80, 40);
        bd.endFill();
        bd.beginFill(0x1e293b);
        bd.drawRect(sx + 362, pY - 43, 76, 36);
        bd.endFill();
        c.addChild(bd);
        const bdt = new PIXI.Text(`${residents.length} RESIDENTS`, {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: '#4ade80',
        });
        bdt.anchor.set(0.5, 0.5);
        bdt.x = sx + 400;
        bdt.y = pY - 30;
        c.addChild(bdt);
    ...
    _drawApartments(c, sx, uw, pY, fy, fh, floor, residents) {
        ...
        const flbl = new PIXI.Text('FL' + floor, {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: '#475569',
            letterSpacing: 1,
        ...
        for (let u = 0; u < unitsPerFloor; u++) {
            ...
            if (u > 0) {
                ...
                dv.beginFill(0x2a3040);
                dv.drawRect(ux - 2, fy + 4, 2, fh - 10);
                dv.endFill();
                c.addChild(dv);
            }
            ...
            bed.eventMode = 'none';
            bed.beginFill(0x1e293b);
            bed.drawRect(ux + 8, pY - 16, 40, 16);
            bed.endFill();
            bed.beginFill(col, 0.4);
            bed.drawRect(ux + 8, pY - 18, 40, 4);
            bed.endFill();
            bed.beginFill(0xffffff, 0.3);
            bed.drawRect(ux + 10, pY - 17, 12, 3);
            bed.endFill();
            unit.addChild(bed);
            // Desk + monitor
            const desk = new PIXI.Graphics();
            desk.eventMode = 'none';
            desk.beginFill(0x334155);
            desk.drawRect(ux + 60, pY - 20, 30, 20);
            desk.endFill();
            desk.beginFill(0x475569);
            desk.drawRect(ux + 60, pY - 22, 30, 3);
            desk.endFill();
            desk.beginFill(0x0a0a18);
            desk.drawRect(ux + 65, pY - 36, 20, 14);
            desk.endFill();
            desk.beginFill(col, 0.3);
            desk.drawRect(ux + 67, pY - 34, 16, 10);
            desk.endFill();
            unit.addChild(desk);
            // Chair
            const ch = new PIXI.Graphics();
            ch.eventMode = 'none';
            ch.beginFill(0x334155);
            ch.drawRect(ux + 70, pY - 10, 10, 10);
            ch.endFill();
            ch.beginFill(0x475569);
            ch.drawRect(ux + 68, pY - 14, 14, 4);
            ch.endFill();
            unit.addChild(ch);
            // Wardrobe
            const wd = new PIXI.Graphics();
            wd.eventMode = 'none';
            wd.beginFill(0x1e293b);
            wd.drawRect(ux + 100, pY - 40, 20, 40);
            wd.endFill();
            wd.beginFill(0x334155);
            wd.drawRect(ux + 102, pY - 38, 16, 36);
            wd.endFill();
            wd.beginFill(col, 0.2);
            wd.drawRect(ux + 104, pY - 36, 12, 14);
            wd.endFill();
            unit.addChild(wd);
            // Nameplate
            const nm = new PIXI.Text(resident.name, {
                fontFamily: 'JetBrains Mono',
                fontSize: 7,
                fill: col,
                fontWeight: 'bold',
            ...
            nm.y = fy + 14;
            unit.addChild(nm);
            const rl = new PIXI.Text(resident.role, {
                fontFamily: 'JetBrains Mono',
                fontSize: 5,
                fill: 0x64748b,
            });
            ...
            if (!isAtWork) {
                ...
                if (isSleeping) {
                    ...
                    sleeper.beginFill(0xddd8c8, 0.7);
                    sleeper.drawRoundedRect(ux + 9, bedTop - 10, 14, 8, 3);
                    sleeper.endFill();
                    // Head resting on pillow
                    sleeper.beginFill(0xfdd8b5);
                    sleeper.drawCircle(ux + 16, bedTop - 7, 4);
                    sleeper.endFill();
                    // Hair
                    sleeper.beginFill(0x2c1810, 0.7);
                    sleeper.drawEllipse(ux + 16, bedTop - 11, 3.5, 1.5);
                    sleeper.endFill();
                    // Closed eyes (small lashes)
                    sleeper.beginFill(0x2c1810, 0.5);
                    sleeper.drawRect(ux + 13.5, bedTop - 7, 1.8, 0.8);
                    sleeper.drawRect(ux + 17, bedTop - 7, 1.8, 0.8);
                    sleeper.endFill();
                    // Peaceful mouth
                    sleeper.beginFill(0x2c1810, 0.2);
                    sleeper.drawRect(ux + 15, bedTop - 4.5, 2.5, 0.6);
                    sleeper.endFill();
                    // Blanket covering body (contour hump)
                    sleeper.beginFill(col, 0.5);
                    sleeper.moveTo(ux + 22, bedTop);
                    sleeper.lineTo(ux + 22, bedTop - 3);
                    sleeper.quadraticCurveTo(ux + 31, bedTop - 13, ux + 46, bedTop - 2);
                    sleeper.lineTo(ux + 46, bedTop);
                    sleeper.closePath();
                    sleeper.endFill();
                    // Blanket top shimmer
                    sleeper.beginFill(col, 0.25);
                    sleeper.moveTo(ux + 23, bedTop - 1);
                    sleeper.lineTo(ux + 23, bedTop - 2);
                    sleeper.quadraticCurveTo(ux + 31, bedTop - 11, ux + 45, bedTop - 1);
                    sleeper.lineTo(ux + 45, bedTop - 1);
                    sleeper.closePath();
                    sleeper.endFill();
                    // Interactive wrapper
                    ...
                    sleeperCont.on('pointertap', () => {
                        if (typeof UI !== 'undefined')
                            UI.selectModel({
                                ...
                                role: _rl,
                                lab: 'other',
                                desc: _rl + '. Currently sleeping.',
                            });
                    });
                    sleeperCont.on('pointerover', (e) => {
                        if (typeof UI !== 'undefined') UI.showTooltip(e, _nm, _rl + ' (sleeping)');
                    });
    ...
    _drawBasement(c, sx, uw, pY, fy, fh) {
        ...
        for (let wx = sx + 30; wx < sx + uw / 2 - 20; wx += 50) {
            const wm = new PIXI.Graphics();
            wm.eventMode = 'none';
            wm.beginFill(0xf1f5f9);
            wm.drawRoundedRect(wx, pY - 35, 30, 35, 2);
            wm.endFill();
            wm.beginFill(0x94a3b8);
            wm.drawRect(wx + 2, pY - 33, 26, 8);
            wm.endFill();
            wm.beginFill(0x0a0a18);
            wm.drawCircle(wx + 15, pY - 14, 9);
            wm.endFill();
            wm.beginFill(0x22d3ee, 0.3);
            wm.drawCircle(wx + 15, pY - 14, 7);
            wm.endFill();
            wm.beginFill(0x4ade80);
            wm.drawCircle(wx + 6, pY - 30, 1.5);
            wm.endFill();
            c.addChild(wm);
        }
        // Storage cages
        for (let cx = sx + uw / 2 + 20; cx < sx + uw - 40; cx += 60) {
            ...
            cage.eventMode = 'none';
            cage.beginFill(0x1e293b, 0.5);
            cage.drawRect(cx, pY - 40, 40, 40);
            cage.endFill();
            cage.lineStyle(1, 0x475569, 0.4);
            for (let gy = pY - 38; gy < pY; gy += 8) {
                cage.moveTo(cx, gy);
                cage.lineTo(cx + 40, gy);
            }
            for (let gx = cx; gx <= cx + 40; gx += 8) {
                cage.moveTo(gx, pY - 40);
                cage.lineTo(gx, pY);
            }
            cage.lineStyle(0);
            // Stuff inside
            cage.beginFill(0x78582e, 0.5);
            cage.drawRect(cx + 5, pY - 15, 14, 15);
            cage.endFill();
            cage.beginFill(0x334155, 0.5);
            cage.drawRect(cx + 22, pY - 20, 12, 20);
            cage.endFill();
            c.addChild(cage);
        }
        ...
        rd.eventMode = 'none';
        rd.beginFill(0xef4444, 0.1);
        rd.drawRect(sx, fy + 4, uw, 2);
        rd.endFill();
        for (let rx = sx + 30; rx < sx + uw; rx += 60) {
            rd.beginFill(0xef4444, 0.3);
            rd.drawCircle(rx, fy + 5, 1.5);
            rd.endFill();
        }
        c.addChild(rd);
    },
    ...
    _npc(c, x, y, name, col) {
        ...
        if (typeof HumanAvatar !== 'undefined' && !HumanAvatar.isBot(name, npcId)) {
            const av = HumanAvatar.draw(c, {
                x,
                y,
                name: null, // tag drawn separately below for layout
                shirt: col,
                accent: col,
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Building Staff');
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
        });
    ...
    update() {
        ...
        this.indoorLights.forEach((l) => {
            ...
            l.g.alpha = l.maxA * (0.7 + Math.sin(G.tick * 0.02) * 0.3);
        });
        ...
        this.avatars.forEach((av, ci) => {
            ...
            if (av._isWalkIn) {
                ...
                if (Math.abs(dx) < av._walkSpeed) {
                    av.cont.x = av._walkTarget;
                    av._isWalkIn = false;
                    av._walkDir = 0;
                    av._walkTimer = 60 + Math.random() * 120;
                    av._minX = av._walkTarget - 50;
                    av._maxX = av._walkTarget + 50;
                } else {
                    av.cont.x += Math.sign(dx) * av._walkSpeed;
                    av.cont.scale.x = Math.sign(dx);
                    if (av.head) av.head.y = -32 + Math.sin(G.tick * 0.15 + av._phase) * 1.5;
                    if (av.legL) av.legL.y = Math.sin(G.tick * 0.25 + ci) * 4;
                    if (av.legR) av.legR.y = -Math.sin(G.tick * 0.25 + ci) * 4;
                }
                return;
            }
            if (av._isZzz) {
                ...
                av._z2.y = -8 + Math.sin(t + 1) * 3;
                av._z2.alpha = 0.3 + Math.sin(t + 1) * 0.25;
                av._z3.y = -18 + Math.sin(t + 2) * 3;
