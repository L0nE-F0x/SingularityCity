...
const InteriorBar = {
    ...
    build(bld, layer) {
        ...
        sg.beginFill(0x0a0514);
        sg.lineStyle(2, 0xff00ff, 0.6);
        ...
        sg.endFill();
        rc.addChild(sg);
        const lt = new PIXI.Text('🍸 NEON BAR', {
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            if (!isB) {
                ...
                rg.endFill(); // above windows
                rg.beginFill(wallCol);
                rg.drawRect(startX, fy + 54, bldW, floorH - 54);
                rg.endFill(); // below windows
                // Wall between windows
                let wx = startX + 40;
                rg.beginFill(wallCol);
                rg.drawRect(startX, fy + 20, 40, 34);
                rg.endFill(); // left of first window
                while (wx + 40 <= startX + bldW - 40) {
                    wx += 40;
                    if (wx + 20 <= startX + bldW - 40) rg.beginFill(wallCol);
            ...
            if (!isB) {
                ...
                ns.beginFill(nCol, 0.12);
                ns.drawRect(startX, fy + 2, bldW, 2);
                ns.endFill();
                ns.beginFill(nCol, 0.08);
                ns.drawRect(startX, fy + floorH * 0.5, bldW, 1);
                ns.endFill();
                ns.beginFill(0xff69b4, 0.2);
                ns.drawRect(startX + 2, fy + 6, 2, floorH - 12);
                ns.endFill();
                ns.beginFill(0x00ffff, 0.2);
                ns.drawRect(startX + bldW - 4, fy + 6, 2, floorH - 12);
                ns.endFill();
        ...
        earth.drawRect(0, groundY - 2, startX - 6, 6);
        earth.endFill();
        earth.beginFill(0x4a4a5a);
        earth.drawRect(startX + bldW + 6, groundY - 2, G.vpW - startX - bldW - 6, 6);
        earth.endFill();
        earth.beginFill(0x2d5a3f);
        earth.drawRect(0, groundY - 4, startX - 6, 4);
        earth.endFill();
        earth.beginFill(0x2d5a3f);
        earth.drawRect(startX + bldW + 6, groundY - 4, G.vpW - startX - bldW - 6, 4);
        earth.endFill();
        ...
        voidMask.beginFill(0x1a1810);
        voidMask.drawRect(0, basementBottom - 4, G.vpW, 10);
        voidMask.endFill();
        voidMask.beginFill(0x050508);
        voidMask.drawRect(0, undergroundY + undergroundH, G.vpW, 3000);
        voidMask.endFill();
        ...
        this._onMove = (e) => {
            if (!InteriorBar.isDragging || !InteriorBar.scene || InteriorBar.scene.destroyed) return;
            let ny = InteriorBar._startSceneY + (e.clientY - InteriorBar._startY);
            ny = Math.max(InteriorBar.minY, Math.min(ny, InteriorBar.maxY));
            InteriorBar.scene.y = ny;
        };
        this._onUp = () => {
            InteriorBar.isDragging = false;
            if (InteriorBar.layer) InteriorBar.layer.cursor = 'grab';
        };
    ...
    _drawMainBar(c, sx, bw, pY, fy, fh) {
        ...
        bg.eventMode = 'none';
        bg.beginFill(0x1a0a28);
        bg.drawRect(barX, pY - 20, barW, 20);
        bg.endFill();
        bg.beginFill(0x2a1040);
        bg.drawRect(barX, pY - 22, barW, 3);
        bg.endFill();
        // Bar top surface (shiny)
        bg.beginFill(0x3a1a50, 0.6);
        bg.drawRect(barX + 2, pY - 21, barW - 4, 2);
        bg.endFill();
        c.addChild(bg);
        // Bottles on shelf behind bar
        const shelf = new PIXI.Graphics();
        shelf.eventMode = 'none';
        shelf.beginFill(0x1a0a20);
        shelf.drawRect(barX, pY - 45, barW, 22);
        shelf.endFill();
        shelf.beginFill(0x2a1040);
        shelf.drawRect(barX, pY - 47, barW, 3);
        shelf.endFill();
        ...
        for (let bx = barX + 8; bx < barX + barW - 10; bx += 14) {
            ...
            shelf.beginFill(bc, 0.6);
            shelf.drawRect(bx, pY - 44 - bh + 22, 6, bh);
            shelf.endFill();
            shelf.beginFill(0xffffff, 0.3);
            shelf.drawRect(bx + 1, pY - 44 - bh + 22, 2, bh - 2);
            shelf.endFill();
        }
        c.addChild(shelf);
        ...
        for (let gx = barX + 15; gx < barX + barW - 20; gx += 40) {
            ...
            gl.eventMode = 'none';
            gl.beginFill(0xffffff, 0.3);
            gl.drawPolygon([gx, pY - 22, gx + 4, pY - 30, gx + 8, pY - 22]);
            gl.endFill();
            gl.beginFill([0xff00ff, 0x00ffff, 0xa855f7][Math.floor(gx / 40) % 3], 0.5);
            gl.drawRect(gx + 2, pY - 28, 4, 5);
            gl.endFill();
            c.addChild(gl);
        }
        // Bar stools
        for (let sx2 = barX + 20; sx2 < barX + barW - 20; sx2 += 35) {
            ...
            st.eventMode = 'none';
            st.beginFill(0x333344);
            st.drawRect(sx2, pY - 8, 2, 8);
            st.endFill();
            st.beginFill(0x4a4a6a);
            st.drawCircle(sx2 + 1, pY - 10, 5);
            st.endFill();
            c.addChild(st);
        }
        // Jukebox
        const jx = sx + bw - 80;
        const jb = new PIXI.Graphics();
        jb.eventMode = 'none';
        jb.beginFill(0x1a1030);
        jb.drawRoundedRect(jx, pY - 40, 30, 40, 4);
        jb.endFill();
        jb.beginFill(0xff00ff, 0.2);
        jb.drawRect(jx + 4, pY - 35, 22, 15);
        jb.endFill();
        jb.beginFill(0x00ffff, 0.15);
        jb.drawRect(jx + 4, pY - 18, 22, 8);
        jb.endFill();
        c.addChild(jb);
    ...
    _drawKaraoke(c, sx, bw, pY, fy, fh) {
        ...
        // Stage platform
        const stgX = sx + bw / 2 - 80,
            stgW = 160;
        const sg = new PIXI.Graphics();
        sg.eventMode = 'none';
        sg.beginFill(0x2a1040);
        sg.drawRect(stgX, pY - 8, stgW, 8);
        sg.endFill();
        sg.beginFill(0x3a1a50);
        ...
        for (let lx = stgX + 10; lx < stgX + stgW; lx += 20) {
            ...
            sg.drawCircle(lx, pY - 6, 3);
            sg.endFill();
        }
        c.addChild(sg);
        // Microphone stand (center stage)
        const mic = new PIXI.Graphics();
        mic.eventMode = 'none';
        mic.beginFill(0x888888);
        mic.drawRect(sx + bw / 2 - 1, pY - 30, 2, 22);
        mic.endFill();
        mic.beginFill(0xcccccc);
        mic.drawCircle(sx + bw / 2, pY - 32, 4);
        mic.endFill();
        c.addChild(mic);
        ...
        for (const spx of [stgX - 25, stgX + stgW + 5]) {
            ...
            sp.eventMode = 'none';
            sp.beginFill(0x111118);
            sp.drawRect(spx, pY - 35, 20, 35);
            sp.endFill();
            sp.beginFill(0x222230);
            sp.drawCircle(spx + 10, pY - 25, 6);
            sp.endFill();
            sp.beginFill(0x333340);
            sp.drawCircle(spx + 10, pY - 25, 3);
            sp.endFill();
            sp.beginFill(0x222230);
            sp.drawCircle(spx + 10, pY - 12, 4);
            sp.endFill();
            c.addChild(sp);
        }
        // Disco ball
        ...
        db.eventMode = 'none';
        db.beginFill(0xffffff, 0.4);
        db.drawCircle(sx + bw / 2, fy + 18, 8);
        db.endFill();
        for (let a = 0; a < Math.PI * 2; a += 0.5) {
            db.beginFill(0xffffff, 0.7);
            db.drawRect(sx + bw / 2 + Math.cos(a) * 6 - 1, fy + 18 + Math.sin(a) * 6 - 1, 2, 2);
            db.endFill();
        }
        db.beginFill(0x888888);
        db.drawRect(sx + bw / 2 - 1, fy + 3, 2, 8);
        db.endFill();
        c.addChild(db);
        const dbGlow = new PIXI.Graphics();
        dbGlow.beginFill(0xffffff, 0.04);
        dbGlow.drawEllipse(sx + bw / 2, fy + 18, 30, 15);
        dbGlow.endFill();
        dbGlow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(dbGlow);
        this.indoorLights.push({ g: dbGlow, maxA: 0.08, type: 'disco' });
        // Lyric screen
        const scr = new PIXI.Graphics();
        scr.eventMode = 'none';
        scr.beginFill(0x050510);
        scr.drawRect(stgX + 20, fy + 12, stgW - 40, 20);
        scr.endFill();
        scr.beginFill(0xff00ff, 0.15);
        scr.drawRect(stgX + 22, fy + 14, stgW - 44, 16);
        scr.endFill();
        c.addChild(scr);
        const lyricTxt = new PIXI.Text('♪ SING YOUR TRAINING DATA ♪', {
            fontFamily: 'JetBrains Mono',
            fontSize: 5,
            fill: '#ff69b4',
        });
        lyricTxt.anchor.set(0.5, 0.5);
        lyricTxt.x = sx + bw / 2;
        lyricTxt.y = fy + 22;
        c.addChild(lyricTxt);
        // Seating area
        for (let tx = sx + 30; tx < stgX - 20; tx += 40) {
            this._drawTable(c, tx, pY);
        }
        for (let tx = stgX + stgW + 30; tx < sx + bw - 30; tx += 40) {
            this._drawTable(c, tx, pY);
        }
        // NPC
    ...
    _drawVIP(c, sx, bw, pY, fy, fh) {
        ...
        for (let bx = sx + 30; bx < sx + bw - 60; bx += 100) {
            ...
            bt.eventMode = 'none';
            // Booth back
            bt.beginFill(0x2a1040);
            bt.drawRoundedRect(bx, pY - 30, 70, 8, 2);
            bt.endFill();
            bt.beginFill(0x3a1a50);
            bt.drawRoundedRect(bx - 4, pY - 32, 8, 32, 2);
            bt.drawRoundedRect(bx + 66, pY - 32, 8, 32, 2);
            bt.endFill();
            // Seat cushion
            bt.beginFill(0x4a2060, 0.8);
            bt.drawRoundedRect(bx + 4, pY - 22, 62, 10, 2);
            bt.endFill();
            // Table
            bt.beginFill(0x1a0a28);
            bt.drawRect(bx + 15, pY - 14, 40, 14);
            bt.endFill();
            bt.beginFill(0x2a1040);
            bt.drawRect(bx + 15, pY - 16, 40, 3);
            bt.endFill();
            // Candle
            bt.beginFill(0xfbbf24, 0.6);
            bt.drawRect(bx + 33, pY - 20, 4, 4);
            bt.endFill();
            bt.beginFill(0xff6600, 0.8);
            bt.drawPolygon([bx + 35, pY - 20, bx + 33, pY - 24, bx + 37, pY - 24]);
            bt.endFill();
            c.addChild(bt);
        }
        // Champagne bucket
        ...
        ch.eventMode = 'none';
        ch.beginFill(0x94a3b8);
        ch.drawRect(chx, pY - 18, 16, 18);
        ch.endFill();
        ch.beginFill(0xfbbf24, 0.4);
        ch.drawRect(chx + 2, pY - 16, 12, 10);
        ch.endFill();
        ch.beginFill(0x4ade80);
        ch.drawRect(chx + 5, pY - 26, 3, 10);
        ch.drawRect(chx + 9, pY - 24, 3, 8);
        ch.endFill();
        c.addChild(ch);
        // Velvet rope at entrance
        const vr = new PIXI.Graphics();
        vr.eventMode = 'none';
        vr.beginFill(0xfbbf24);
        vr.drawRect(sx + 20, pY - 18, 4, 18);
        vr.drawRect(sx + 50, pY - 18, 4, 18);
        vr.endFill();
        vr.beginFill(0xef4444, 0.6);
        vr.drawRect(sx + 24, pY - 14, 26, 3);
        vr.endFill();
        c.addChild(vr);
        // NPC
    ...
    _drawCellar(c, sx, bw, pY, fy, fh) {
        ...
        for (let rx = sx + 30; rx < sx + bw - 80; rx += 70) {
            ...
            rk.eventMode = 'none';
            rk.beginFill(0x3d2914);
            rk.drawRect(rx, pY - 45, 50, 45);
            rk.endFill();
            rk.beginFill(0x4a3520);
            rk.drawRect(rx + 2, pY - 43, 46, 41);
            rk.endFill();
            // Bottle rows
        ...
        for (let kx = sx + bw - 70; kx < sx + bw - 20; kx += 25) {
            const kg = new PIXI.Graphics();
            kg.eventMode = 'none';
            kg.beginFill(0x78582e);
            kg.drawEllipse(kx, pY - 12, 10, 12);
            kg.endFill();
            kg.beginFill(0x92703a);
            kg.drawRect(kx - 8, pY - 18, 16, 2);
            kg.drawRect(kx - 8, pY - 6, 16, 2);
            kg.endFill();
            kg.beginFill(0xfbbf24);
            kg.drawRect(kx - 2, pY - 8, 4, 4);
            kg.endFill();
            c.addChild(kg);
        }
        // Crates
        for (let cx2 = sx + bw / 2 - 30; cx2 < sx + bw / 2 + 30; cx2 += 22) {
            ...
            cr.eventMode = 'none';
            cr.beginFill(0x3d2914);
            cr.drawRect(cx2, pY - 18, 18, 18);
            cr.endFill();
            cr.beginFill(0x4a3520);
            cr.drawRect(cx2 + 2, pY - 16, 14, 14);
            cr.endFill();
            cr.lineStyle(1, 0x2a1f0e);
            cr.moveTo(cx2 + 9, pY - 16);
            cr.lineTo(cx2 + 9, pY - 2);
            cr.moveTo(cx2 + 2, pY - 9);
            cr.lineTo(cx2 + 16, pY - 9);
            cr.lineStyle(0);
            c.addChild(cr);
        }
        // Rat NPC (actual rat, not human)
        const rat = new PIXI.Container();
        rat.x = sx + 100;
        rat.y = pY;
        rat.zIndex = 5;
        const rg2 = new PIXI.Graphics();
        rg2.eventMode = 'none';
        // Body (grey oval)
        rg2.beginFill(0x6b6b7b);
        rg2.drawEllipse(0, -6, 10, 6);
        rg2.endFill();
        // Head
        rg2.beginFill(0x7b7b8b);
        rg2.drawEllipse(10, -8, 6, 5);
        rg2.endFill();
        // Ears
        rg2.beginFill(0xff9999, 0.6);
        rg2.drawCircle(8, -14, 3);
        rg2.drawCircle(13, -14, 3);
        rg2.endFill();
        // Eyes (beady)
        rg2.beginFill(0x000000);
        rg2.drawCircle(12, -9, 1.2);
        rg2.endFill();
        rg2.beginFill(0xffffff, 0.5);
        rg2.drawCircle(12.5, -9.5, 0.5);
        rg2.endFill();
        // Nose
        rg2.beginFill(0xff6b6b);
        rg2.drawCircle(16, -7, 1.5);
        rg2.endFill();
        // Whiskers
        ...
        rg2.moveTo(-10, -6);
        rg2.bezierCurveTo(-16, -12, -20, -2, -18, -8);
        rg2.lineStyle(0);
        // Feet
        rg2.beginFill(0xff9999, 0.5);
        rg2.drawEllipse(-4, 0, 3, 1.5);
        rg2.drawEllipse(4, 0, 3, 1.5);
        rg2.endFill();
        rat.addChild(rg2);
        const ratTxt = new PIXI.Text('Cellar Rat', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xf59e0b,
            fontWeight: 'bold',
        });
        ratTxt.anchor.set(0.5, 1);
        ratTxt.y = -18;
        rat.addChild(ratTxt);
        ...
        rat.on('pointertap', () => {
            if (typeof UI !== 'undefined')
                UI.selectModel({
                    ...
                    role: 'Cellar Rat',
                    lab: 'other',
                    desc: 'A resourceful rodent who guards the wine cellar. Technically not an employee but has seniority over everyone.',
                });
        });
        rat.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, 'Cellar Rat', 'Wine Cellar Guardian');
        });
        rat.on('pointerout', () => {
    ...
    _drawTable(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1a0a28);
        g.drawCircle(x + 10, y - 10, 12);
        g.endFill();
        g.beginFill(0x2a1040);
        g.drawCircle(x + 10, y - 10, 10);
        g.endFill();
        // Drink
        g.beginFill(0x00ffff, 0.3);
        g.drawRect(x + 6, y - 14, 4, 4);
        g.endFill();
        c.addChild(g);
    },
    ...
    _npc(c, x, y, name, col) {
        ...
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined')
                UI.selectModel({
                    ...
                    role: name,
                    lab: 'other',
                    desc: 'Neon Bar staff. Keeping the party going every night.',
                });
        });
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Neon Bar Staff');
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
        });
        ...
        if (typeof G !== 'undefined' && G.tracking && G._addTrackHighlight) {
            ...
            if (hl) {
                barAv._trackGlow = hl.glow;
                barAv._trackArrow = hl.arrow;
            }
        }
    ...
    _drawAvatar(m, x, y, container, floorIdx) {
        ...
        const lab =
            typeof LABS !== 'undefined'
                ? LABS[m.lab] || LABS.other || { color: '#64748b' }
                : { color: '#64748b' };
    ...
    update() {
        ...
        this.indoorLights.forEach((l) => {
            ...
            if (l.type === 'neon')
                l.g.alpha =
                    l.maxA * (0.6 + Math.sin(G.tick * 0.08) * 0.3 + (Math.random() < 0.05 ? -0.3 : 0));
        ...
        this.avatars.forEach((av, ci) => {
            ...
            if (!av.m || av.m.isNPC || av.m.id === 'npc_cellar_rat') {
                ...
                if (av._walkTimer <= 0) {
                    ...
                    av._walkTimer = 120 + Math.random() * 200;
                }
