...
const PowerEnv = {
    ...
    buildAnimations(charLayer) {
        ...
        if (nucBld) {
            for (let i = 0; i < 20; i++) {
                ...
                p.drawCircle(0, 0, 3 + Math.random() * 5);
                p.endFill();
        ...
        if (gasBld) {
            for (let i = 0; i < 16; i++) {
                const p = new PIXI.Graphics();
                p.beginFill(0x9ca3af, 0.1 + Math.random() * 0.1);
                ...
                p._speed = 0.25 + Math.random() * 0.35;
                p._phase = Math.random() * Math.PI * 2;
                p._resetY = gy - 66;
                charLayer.addChild(p);
    ...
    showGridPanel() {
        ...
        html +=
            '<div style="text-align:center;margin-bottom:12px"><span style="font-size:20px">⚡</span><br><span style="color:var(--cy);font-size:11px;letter-spacing:2px">POWER GRID STATUS</span></div>';
        ...
        if (live && live.sources.length > 0) {
            ...
            html +=
                '<div style="background:linear-gradient(135deg,rgba(34,211,238,0.06),transparent);border:1px solid #22d3ee33;border-radius:8px;padding:10px;margin-bottom:10px">';
            // Header
            html +=
                '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">';
            html +=
                '<div style="display:flex;align-items:center;gap:6px"><span style="font-size:7px;background:#22d3ee;color:#000;padding:2px 6px;border-radius:10px;font-weight:bold">REAL-WORLD</span><span style="font-size:10px;color:#fff;font-weight:bold">Global Power Grid</span></div>';
            html +=
                ...
                live.regionsTotal +
                ' regions · ' +
                (ageH < 1 ? '<1h ago' : ageH + 'h ago') +
                '</div></div>';

            // Summary cards
            html += '<div style="display:flex;gap:6px;margin-bottom:8px">';
            html +=
                '<div style="flex:1;background:var(--cd);border:1px solid var(--bd);border-radius:4px;padding:6px;text-align:center">';
            ...
            html +=
                ...
                live.plantCount.toLocaleString() +
                '</div>';
            html +=
                '<div style="font-size:7px;color:var(--t3)">' +
                live.sources.length +
                ' source types</div></div>';
            html +=
                '<div style="flex:1;background:var(--cd);border:1px solid var(--bd);border-radius:4px;padding:6px;text-align:center">';
            ...
            html +=
                '<div style="font-size:7px;color:var(--t3)">' +
                Math.round(live.totalMW).toLocaleString() +
                ' MW</div></div>';
            html +=
                '<div style="flex:1;background:var(--cd);border:1px solid var(--bd);border-radius:4px;padding:6px;text-align:center">';
            ...
            html +=
                ...
                live.renewPct.toFixed(1) +
                '%</div>';
            html +=
                '<div style="font-size:7px;color:var(--t3)">fossil: ' +
                live.fossilPct.toFixed(1) +
                '%</div></div></div>';
            ...
            live.sources.forEach((s) => {
                ...
                html +=
                    ...
                    '%;background:' +
                    s.color +
                    ';display:flex;align-items:center;justify-content:center;min-width:12px" title="' +
                    s.type +
                    ' ' +
                    pct.toFixed(1) +
                    '%">';
                html +=
                    '<span style="font-size:6px;color:#000;font-weight:bold">' +
            ...
            // Source table
            html +=
                '<table style="width:100%;border-collapse:collapse;font-size:8px;font-family:\'JetBrains Mono\',monospace">';
            html +=
                '<tr style="color:var(--t3);border-bottom:1px solid var(--bd)"><th style="text-align:left;padding:3px">Source</th><th style="text-align:right;padding:3px">Plants</th><th style="text-align:right;padding:3px">Capacity</th><th style="text-align:left;padding:3px;width:30%">Share</th></tr>';
            live.sources.forEach((s) => {
                const pct = live.totalMW > 0 ? (s.totalMW / live.totalMW) * 100 : 0;
                const gwStr =
                    s.totalMW >= 1000 ? (s.totalMW / 1000).toFixed(1) + ' GW' : Math.round(s.totalMW) + ' MW';
                const isRenew = renewTypes.has(s.type);
                html += '<tr style="border-bottom:1px solid rgba(255,255,255,0.03)">';
                html += '<td style="padding:3px">' + s.emoji + ' ' + s.type + '</td>';
                html +=
                    '<td style="text-align:right;padding:3px;color:var(--t3)">' +
                    s.count.toLocaleString() +
                    '</td>';
                html +=
                    '<td style="text-align:right;padding:3px;color:' +
                    (isRenew ? '#4ade80' : '#fff') +
                    ';font-weight:bold">' +
                    gwStr +
                    '</td>';
                html +=
                    '<td style="padding:3px"><div style="background:rgba(255,255,255,0.06);border-radius:2px;height:6px;overflow:hidden"><div style="width:' +
                    pct +
                    '%;height:100%;background:' +
                    s.color +
                    ';border-radius:2px"></div></div></td></tr>';
            });
            html += '</table>';
            ...
            if (live.byRegion.length > 0) {
                ...
                live.byRegion.forEach((r) => {
                    const gwStr =
                        r.totalMW >= 1000
                            ? (r.totalMW / 1000).toFixed(1) + ' GW'
                            : Math.round(r.totalMW) + ' MW';
            ...
            // Attribution
            html +=
                '<div style="text-align:right;font-size:6px;color:var(--t3);margin-top:4px">Data: <a href="https://www.openstreetmap.org" target="_blank" style="color:#22d3ee;text-decoration:none">OpenStreetMap</a> via Overpass API · <a href="https://openinframap.org" target="_blank" style="color:#22d3ee;text-decoration:none">OpenInfraMap</a></div>';
            html += '</div>';

            // Divider
            html +=
                '<div style="text-align:center;font-size:7px;color:var(--t3);margin:6px 0;border-top:1px dashed var(--bd);padding-top:6px">🎮 SINGULARITY CITY GRID</div>';
        } else {
            // Loading or failed — show retry
            html +=
                '<div style="text-align:center;font-size:7px;color:var(--t3);margin-bottom:8px;padding:6px;background:var(--cd);border:1px solid var(--bd);border-radius:4px">🌍 Global grid data loading from OpenStreetMap… <span style="cursor:pointer;color:var(--cy);text-decoration:underline" onclick="if(typeof API!==\'undefined\')API.fetchGlobalGrid().then(()=>PowerEnv.showGridPanel())">retry</span></div>';
        }
        ...
        html +=
            '<div style="background:var(--cd);border:1px solid var(--bd);border-radius:6px;padding:8px;margin-bottom:10px;text-align:center">';
        ...
        html +=
            ...
            ' MW</span>';
        html +=
            '<span style="font-size:9px;color:var(--t3)">/ ' + demand.toLocaleString() + ' MW demand</span>';
        html += '</div>';
        html +=
            '<div style="margin-top:6px;background:rgba(255,255,255,0.05);border-radius:4px;height:8px;overflow:hidden">';
        ...
        html +=
            '<div style="margin-top:4px;font-size:8px;color:' +
            balCol +
            ';font-weight:bold">' +
            balLabel +
            ' ' +
        ...
        html +=
            '<table style="width:100%;border-collapse:collapse;font-size:8px;font-family:\'JetBrains Mono\',monospace">';
        html +=
            '<tr style="color:var(--t3);border-bottom:1px solid var(--bd)"><th style="text-align:left;padding:4px">Source</th><th style="text-align:right;padding:4px">Output</th><th style="text-align:right;padding:4px">Capacity</th><th style="text-align:right;padding:4px">$/MWh</th><th style="text-align:center;padding:4px">Type</th></tr>';
        pz.SOURCES.forEach((s) => {
            ...
            const outCell =
                s.status === 'construction'
                    ...
                    : output.toLocaleString() +
                      ' MW' +
                      (s.id === 'power_fusion' ? ' <span style="color:#c084fc">⚡pulse</span>' : '');
            html += '<tr style="border-bottom:1px solid rgba(255,255,255,0.03)">';
            ...
            html +=
                '<td style="text-align:center;padding:4px"><span style="color:' +
                typeCol +
        ...
        if (typeof DC_FACILITIES !== 'undefined')
            DC_FACILITIES.forEach((dc) => {
                if (dc.power_mw) dcDemand += dc.power_mw;
            });
        html +=
            '<div style="margin-top:8px;padding:6px;background:var(--cd);border:1px solid var(--bd);border-radius:4px;font-size:7px;color:var(--t3)">';
