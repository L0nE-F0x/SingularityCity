...
const Entities = {
    ...
    init(layers) {
        ...
        if (this.undergroundLayer && this.trainLayer) {
            const trains = EntitiesGfx.initMetro(
        ...
        if (!G.ceoRefs) {
            ...
            if (typeof REAL_FOUNDERS !== 'undefined') {
                ...
                const atWork = !isWeekendNow && !night && dp >= 0.33 && dp <= 0.8;
    ...
    updateTrain() {
        ...
        const mEastX = G.bldById['metro_east']
            ? G.bldById['metro_east'].x + G.bldById['metro_east'].w / 2
            : 7000;
        ...
        const mLongX = G.bldById['metro_longevity']
            ? G.bldById['metro_longevity'].x + G.bldById['metro_longevity'].w / 2
            : null;
        ...
        if (this.trainWest) {
            ...
            if (this.trainWest.state === 'waiting') {
                if (Math.abs(this.trainWest.x - this.trainWest.st1) < 100)
                    this.trainWest.x = this.trainWest.st1;
                else if (Math.abs(this.trainWest.x - this.trainWest.st2) < 100)
                    this.trainWest.x = this.trainWest.st2;
        ...
        if (this.trainEast) {
            ...
            if (this.trainEast.state === 'waiting') {
                if (Math.abs(this.trainEast.x - this.trainEast.st1) < 100)
                    this.trainEast.x = this.trainEast.st1;
                else if (Math.abs(this.trainEast.x - this.trainEast.st2) < 100)
                    this.trainEast.x = this.trainEast.st2;
        ...
        if (this.trainMid && mMidX) {
            ...
            if (this.trainMid.state === 'waiting') {
                ...
                else if (Math.abs(this.trainMid.x - this.trainMid.st2) < 100)
                    this.trainMid.x = this.trainMid.st2;
        ...
        if (this.trainLongevity && mLongX) {
            ...
            if (this.trainLongevity.state === 'waiting') {
                if (Math.abs(this.trainLongevity.x - this.trainLongevity.st1) < 100)
                    this.trainLongevity.x = this.trainLongevity.st1;
                else if (Math.abs(this.trainLongevity.x - this.trainLongevity.st2) < 100)
                    this.trainLongevity.x = this.trainLongevity.st2;
        ...
        [this.trainWest, this.trainEast, this.trainMid, this.trainDC, this.trainLongevity].forEach((t) => {
            ...
            if (t.state === 'waiting') {
                ...
                if (t.timer <= 0) {
                    t.state = 'moving';
                    G._trainsDeparted++;
                    if (G._trainsDeparted >= 10) G.unlockAchieve('train_spotter');
                    ...
                    t.lightR.clear();
                    if (t.dir > 0) {
                        t.lightL.beginFill(0xef4444);
                        t.lightL.drawCircle(-175, 0, 4);
                        t.lightL.endFill();
                        t.lightR.beginFill(0x4ade80);
                        t.lightR.drawCircle(175, 0, 4);
                        t.lightR.endFill();
                    } else {
                        t.lightL.beginFill(0x4ade80);
                        t.lightL.drawCircle(-175, 0, 4);
                        t.lightL.endFill();
                        t.lightR.beginFill(0xef4444);
                        t.lightR.drawCircle(175, 0, 4);
                        t.lightR.endFill();
                    }
                }
    ...
    spawnCar() {
        ...
        if (typeof BLDS !== 'undefined') {
            ...
            nvHQ = BLDS.find(
                (b) => b.lab === 'nvidia' && !b.id.startsWith('fab_') && !b.id.startsWith('dc_')
            );
        }
    ...
    updateChatBubbles(dp) {
        ...
        const maxBubbles = Math.min(15, Math.floor(G.models.length / 3));
        let count = Object.keys(G.chatBubbles).length;
        G.models.forEach((m, i) => {
            if (count >= maxBubbles) return;
            if (G.chatBubbles[m.id] || Math.random() > 0.12) return;
            ...
            const isCotd =
                typeof CitizenOfDay !== 'undefined' && CitizenOfDay.isCotd && CitizenOfDay.isCotd(m.id);
            const pool =
                isCotd && CHAT_MSGS && CHAT_MSGS.press && Math.random() < 0.5
                    ? CHAT_MSGS.press
                    : CHAT_MSGS && CHAT_MSGS[act]
                      ? CHAT_MSGS[act]
                      : CHAT_MSGS
                        ? CHAT_MSGS.work
                        : ['...'];
            ...
            if (Math.random() < 0.2) {
                if (typeof Seasonal !== 'undefined') {
                    const sc = Seasonal.getSeasonalChat();
                    if (sc) {
                        msg = sc;
                    }
                }
                if (!msg && typeof ConferenceData !== 'undefined' && ConferenceData.isActive()) {
                    msg = ConferenceData.getConferenceChat();
                }
                if (!msg && typeof CourtData !== 'undefined' && CourtData.isModelSummoned(m.id)) {
                    msg = CourtData.CHAT_MSGS[Math.floor(Math.random() * CourtData.CHAT_MSGS.length)];
                }
            }
            ...
            if (msg) {
                /* seasonal/conference/goal override already set */
            ...
            G.chatBubbles[m.id] = { msg: msg, expire: G.tick + lifespan };
            count++;
        });
    },
    ...
    update(dp, night) {
        ...
        if (G.ceoRefs) {
            ...
            Object.values(G.ceoRefs).forEach((ceo) => {
                ...
                if (_isWeekend) {
                    ...
                    else if (blockHash < 35)
                        destBld = null; // quick drive
                    else destBld = hq;
                } else if (dp >= 0.55 && dp < 0.7) {
                    // Core afternoon — mostly at HQ, occasional errand
                    if (blockHash < 15)
                        destBld = null; // quick drive
                    else destBld = hq;
                } else if (dp >= 0.7 && dp < 0.82) {
                        ...
                        // and it's daytime — no chopper sorties at 3am, even for an imminent launch.
                            ...
                            if (spaceOrg && typeof SpaceData !== 'undefined' && SpaceData.launches) {
                                ...
                                const imminent = SpaceData.launches.find((l) => {
                                    ...
                                    return (
                                        org === spaceOrg &&
                                        new Date(l.net).getTime() - now < 7200000 &&
                                        new Date(l.net).getTime() - now > 0
                                    );
                                });
                                ...
                                if (imminent && !heli._spaceVisitDone) {
                                    ...
                                    if (mcBld) {
                                        ...
                                        if (typeof UI !== 'undefined')
                                            UI.addToast(
                                                `🚁 ${ceo.f.name} is heading to Mission Control for an upcoming ${SPACE_ORGS[spaceOrg].name} launch!`
                                            );
                                    }
                                }
                            }
                            ...
                            if (!spaceTrip && Math.random() < 0.002 && dp > 0.3 && dp < 0.8) {
                                ...
                                const hqBld = (G.bldsByLab[lab] || []).find(
                                    (b2) => !b2.id.startsWith('house_')
                                );
        ...
        G.cars = G.cars.filter((c) => {
            ...
            if (c._delivering && c._stops && c._stops.length > 0) {
                if (c._waitTimer > 0) {
                    ...
                    if (c.beam)
                        c.beam.alpha = this._headlightAlpha > 0.01 ? (G.tick % 40 < 20 ? 0.2 : 0.05) : 0;
                    return true;
                }
            ...
            const laneY = c.dir > 0 ? 26 : 12;
            c.gfx.y = G.groundY + laneY;
            ...
            const goingToPort =
                c._delivering &&
                c._stops &&
                c._stops[c._stopIdx] > portMinX &&
                c._stops[c._stopIdx] < portMaxX;
        ...
        const occ = {};
        if (!G.familyDestinations) G.familyDestinations = {};
        if (!G.lastBlock) G.lastBlock = -1;
        ...
        for (let i = 0; i < modelsLen; i++) {
            ...
            const stg = m._cachedStg;
            const sd = m._cachedSd;
            const sc = sd.size;
            const act = m._cachedAct;
            const bid = m._cachedBid;

            const ai =
                typeof ACTS !== 'undefined' && ACTS[act]
                    ? ACTS[act]
                    : typeof ACTS !== 'undefined' && ACTS['work']
                      ? ACTS['work']
                      : { indoor: true, icon: '💻', label: 'Processing' };
            ...
            let defaultHq =
                labBlds.find(
                    (x) => !x.id.startsWith('house_') && !x.id.startsWith('dc_') && !x.id.startsWith('fab_')
                ) ||
            ...
            const isSocial =
                ...
                act === 'train' ||
                act === 'arena';
            const block = Math.floor(dp * 24);

            if (isSocial && !night) {
                if (m._sb !== block) {
                    m._sb = block;
                    const r = Math.abs(Math.sin(i * 12.3 + block * 4.5));
                    if (G.lastBlock !== block) {
                        G.familyDestinations = {};
                        G.lastBlock = block;
                    }
                    const fk = m.lab + '_' + act;
                    if (r <= 0.8 && G.familyDestinations[fk]) m._sid = G.familyDestinations[fk];
                    else {
                        ...
                        if (sp && sp.length > 0) {
                            ...
                            if (sp.length > 0) {
                                ...
                                if (ch) {
                                    m._sid = ch.id;
                                    if (r <= 0.8) G.familyDestinations[fk] = ch.id;
                                }
                            }
                        }
                    }
            ...
            if (
                act === 'sleep' &&
                tBld &&
                !tBld.id.startsWith('res_') &&
                tBld.id !== 'graveyard' &&
                !tBld.id.startsWith('uni_')
            ) {
                const _region =
            ...
            const isIn = ai.indoor;
            const isR = stg === 'retired';
            const isRm = stg === 'rumored';
            if (isIn && !isR) {
                if (!occ[tBld.id]) occ[tBld.id] = [];
            ...
            if (refs.bld !== tBld.id) {
                if (isInside) {
                    ...
                    const stationSpread = Math.max(
                        -platformMaxSpread,
                        Math.min(platformMaxSpread, pseudoRandomOffset * 1.2)
                    );
                    ...
                    if (refs._metroLegs && refs._metroLegs.length > 0) {
                        ...
                        if (refs._metroState === 'entering') {
                            ...
                            if (
                                activeTrain &&
                                activeTrain.state === 'waiting' &&
                                Math.abs(activeTrain.x - s1) < 5
                            ) {
                    ...
                    if (atBuilding && refs._metroState === 'none' && !isR) {
                        ...
                        const lingerOK =
                            !night &&
                            (act === 'socialize' || act === 'lunch' || act === 'play' || act === 'share');
                        if (act === 'commute') {
                    ...
                    if (refs.c.visible) {
                        ...
                        if (
                            ...
                            for (let si = Math.max(0, i - 25); si < Math.min(modelsLen, i + 25); si++) {
                                ...
                                if (
                                    !otherRefs ||
                                    !otherRefs.c.visible ||
                                    otherRefs._streetState !== 'walking' ||
                                    otherRefs._chatTimer > 0
                                )
                                    continue;
                                ...
                                if (Math.abs(otherRefs.c.x - myX) < 30) {
                                    partnerObj = otherM;
                                    break;
                                }
                            }
                            if (partnerObj && Math.random() < 0.05) {
                                ...
                                chatBubbles[m.id] = {
                                    msg: streetTopics[Math.floor(Math.random() * streetTopics.length)],
                                    expire: tick + 100,
                                };
                                setTimeout(() => {
                        ...
                        if (refs._metroState === 'none' && !refs._metroLegs) {
                            const ws =
                                ...
                                weatherSpeedMod *
                                pSpeedMod *
                                (envIsRainy && act === 'commute' ? 1.5 : 1);
                            const wAmt =
                                (act === 'commute' ? (envIsRainy ? 120 : 80) : isSocial ? 50 : 20) * pScale;
                        ...
                        let legA = isR ? 0 : Math.sin(tick * 0.12 * weatherSpeedMod * pSpeedMod) * 2 * pScale;
                        ...
                        if (refs._metroState === 'riding') {
                            ...
                            const walkSpeed =
                                sd.speed * weatherSpeedMod * pSpeedMod * (act === 'commute' ? 1.5 : 1);
                            ...
                            if (!freezeX) {
                                if (Math.abs(distX) > walkSpeed) {
                                    ...
                                    legA = isR
                                        ? 0
                                        : Math.sin(tick * 0.15 * weatherSpeedMod * pSpeedMod) * 2 * pScale;
                                } else {
                        ...
                        refs.c.alpha = isR
                            ? 0.35 + Math.abs(Math.sin(tick * 0.03 + i)) * 0.45
                            : isRm
                              ? 0.55
                              : 1;
                        if (isR) {
                            if (refs.c.blendMode !== PIXI.BLEND_MODES.ADD)
                                refs.c.blendMode = PIXI.BLEND_MODES.ADD;
                        } else {
                            if (refs.c.blendMode !== PIXI.BLEND_MODES.NORMAL)
                                refs.c.blendMode = PIXI.BLEND_MODES.NORMAL;
                        }
                        // Ghost glow pulse
                        ...
                        if (isOutside && envIsRainy) {
                            ...
                            refs.umbrella.rotation =
                                (currentDir === 1 ? 0.1 : -0.1) + Math.sin(tick * 0.1 + i) * 0.05;
                        } else {
