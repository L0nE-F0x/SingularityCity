...
const InteriorAgents = {
    ...
    build(bld, layer) {
        ...
        const layouts = {
            agents_orchestrator: {
                ...
                roofLabel: 'ORCHESTRATION HUB',
                col: 0xf43f5e,
                npcs: ['Architect', 'Ops Lead'],
            },
            agents_toolshop: {
                floors: ['API Vault', 'Integration Lab', 'Auth Proxy', 'Registry Floor'],
                roofLabel: 'TOOL REGISTRY',
                col: 0xfbbf24,
                npcs: ['Tool Eng', 'API Ops'],
            },
            agents_sandbox: {
                floors: ['Eval Pit', 'Benchmark Hall', 'Replay Room', 'Observation Deck', 'Arena Floor'],
                roofLabel: 'SANDBOX ARENA',
                col: 0x4ade80,
                npcs: ['Sandbox Eng', 'Eval Lead'],
            },
            agents_deploy: {
                floors: ['Staging', 'Guardrail Room', 'Pipeline Floor', 'Release Deck'],
                roofLabel: 'DEPLOYMENT GATEWAY',
                col: 0xef4444,
                npcs: ['Deploy SRE', 'Guard Eng'],
            },
            agents_memory: {
                floors: ['Vector Store', 'Embedding Lab', 'Knowledge Graph', 'Episodic Cache', 'Archive'],
                roofLabel: 'MEMORY VAULT',
                col: 0xa855f7,
                npcs: ['Memory Eng', 'Graph Ops'],
            },
        };
    ...
    _drawReceptionDesk(c, x, y, col) {
        ...
        g.endFill();
        g.beginFill(0x222a40);
        g.drawRect(x + 43, y - 14, 4, 2);
        g.endFill();
        // Clipboard
    ...
    _drawFloorProps(c, sx, bw, pY, fy, fh, floorName, col, bldId) {
            ...
            if (fws.length) {
                ...
                fws.forEach((fw, i) => {
                    const row = Math.floor(i / perRow),
                        coln = i % perRow;
                    const cx = sx + 20 + coln * cardW,
                        cyy = fy + 14 + row * ((fh - 24) / 2);
                    const card = new PIXI.Container();
                    const cg = new PIXI.Graphics();
                    const fc = parseInt(fw.color.slice(1), 16);
                    cg.beginFill(0x0a0a18, 0.9);
                    cg.drawRoundedRect(cx, cyy, cardW - 8, (fh - 30) / 2, 3);
                    cg.endFill();
                    cg.lineStyle(1, fc, 0.55);
                    cg.drawRoundedRect(cx, cyy, cardW - 8, (fh - 30) / 2, 3);
                    cg.lineStyle(0);
                    cg.beginFill(fc, 0.18);
                    cg.drawRect(cx, cyy, cardW - 8, 8);
                    cg.endFill();
                    // A little "agent node" glyph
                    cg.beginFill(fc);
                    cg.drawCircle(cx + 8, cyy + 20, 3);
                    cg.endFill();
                    cg.lineStyle(1, fc, 0.6);
                    cg.drawCircle(cx + 8, cyy + 20, 6);
                    cg.lineStyle(0);
                    card.addChild(cg);
                    const nm = new PIXI.Text(fw.name, {
                        fontFamily: 'JetBrains Mono',
                        fontSize: 6,
                        fill: fc,
                        fontWeight: 'bold',
                    ...
                    nm.x = cx + 4;
                    nm.y = cyy + 1.5;
                    if (nm.width > cardW - 14) nm.scale.set((cardW - 14) / nm.width);
                    card.addChild(nm);
                    const pr = new PIXI.Text(fw.product || '', {
                        fontFamily: 'JetBrains Mono',
                        fontSize: 5.5,
                        fill: 0xcbd5e1,
                    });
                    pr.x = cx + 16;
                    pr.y = cyy + 16;
                    if (pr.width > cardW - 24) pr.scale.set((cardW - 24) / pr.width);
                    card.addChild(pr);
                    if (typeof UI !== 'undefined')
                        UI.tip(card, `${fw.icon} ${fw.name} — ${fw.ceo}`, fw.milestone);
                    c.addChild(card);
                });
            }
            return;
        ...
        if (fn.includes('command bridge') || fn.includes('ops deck') || fn.includes('swarm control')) {
            ...
            for (let mx = sx + 30; mx < sx + bw - 50; mx += 55) {
                ...
                g.drawRect(mx, fy + 12, 42, 28);
                g.endFill();
                g.beginFill(col, 0.12);
                g.drawRect(mx + 2, fy + 14, 38, 24);
                g.endFill();
                ...
                g.beginFill(dotCol, 0.8);
                g.drawCircle(mx + 38, fy + 16, 2);
                g.endFill();
            }
            // Operator desks
            for (let dx = sx + 50; dx < sx + bw - 80; dx += 100) {
                g.beginFill(0x1a2540);
                g.drawRect(dx, pY - 14, 60, 14);
                g.endFill();
                g.beginFill(0x222a40);
                g.drawRect(dx, pY - 16, 60, 3);
                g.endFill();
                g.beginFill(0x334155);
                g.drawCircle(dx + 30, pY - 4, 6);
                g.endFill();
            }
            ...
            for (let ni = 0; ni < 7; ni++) {
                ...
                // Node circle
                g.beginFill(0x0a0a18);
                g.drawCircle(nx, nodeY, 10);
                g.endFill();
                g.beginFill(nc, 0.2);
                g.drawCircle(nx, nodeY, 10);
                g.endFill();
                g.lineStyle(1.5, nc, 0.4);
                g.drawCircle(nx, nodeY, 10);
                g.lineStyle(0);
                // Inner icon (small filled circle)
                g.beginFill(nc, 0.6);
                g.drawCircle(nx, nodeY, 3);
                g.endFill();
            }
            // Floor-level server racks
            for (let rx = sx + 30; rx < sx + bw - 40; rx += 50) {
                g.beginFill(0x111825);
                g.drawRect(rx, pY - 22, 30, 22);
                g.endFill();
                g.beginFill(0x1a2540);
                g.drawRect(rx, pY - 22, 30, 3);
                g.endFill();
                for (let ru = 0; ru < 3; ru++) {
                    const uy = pY - 18 + ru * 6;
                    const ledCol = Math.random() > 0.3 ? 0x4ade80 : 0xfbbf24;
                    g.beginFill(ledCol, 0.5);
                    g.drawRect(rx + 3, uy + 1, 2, 2);
                    g.endFill();
                }
            }
            ...
            for (let rx = sx + 25; rx < sx + bw - 40; rx += 40) {
                g.beginFill(0x111825);
                g.drawRect(rx, fy + 12, 28, fh - 22);
                g.endFill();
                g.beginFill(0x1a2540);
                g.drawRect(rx, fy + 12, 28, 3);
                g.endFill();
                // Ports/connectors
                for (let pi = 0; pi < 6; pi++) {
                    const py = fy + 18 + pi * 8;
                    g.beginFill(0x0d1220);
                    g.drawRect(rx + 2, py, 24, 6);
                    g.endFill();
                    // Color-coded connector
                    const portCol = [0x22d3ee, 0x4ade80, 0xfbbf24, 0xf43f5e, 0xa855f7, 0x3b82f6][pi];
                    g.beginFill(portCol, 0.5);
                    g.drawRect(rx + 4, py + 2, 4, 2);
                    g.endFill();
                    // Activity indicator
                    if (Math.random() > 0.3) {
                        g.beginFill(portCol, 0.2);
                        g.drawRect(rx + 10, py + 2, 12, 2);
                        g.endFill();
                    }
                }
            }
            ...
            for (let mx = sx + 35; mx < sx + bw - 45; mx += 65) {
                g.beginFill(0x0a0a18);
                g.drawRect(mx, fy + 14, 50, 26);
                g.endFill();
                g.beginFill(col, 0.08);
                g.drawRect(mx + 2, fy + 16, 46, 22);
                g.endFill();
                // Shield icon
            ...
            for (let mx = sx + 30; mx < sx + bw - 50; mx += 70) {
                g.beginFill(0x0a0a18);
                g.drawRect(mx, fy + 12, 55, 32);
                g.endFill();
                g.beginFill(0x4ade80, 0.08);
                g.drawRect(mx + 2, fy + 14, 51, 28);
                g.endFill();
                // Bar chart bars
                for (let bi = 0; bi < 5; bi++) {
                    ...
                    g.drawRect(mx + 6 + bi * 9, fy + 42 - barH, 6, barH);
                    g.endFill();
                }
            }
            // Testing pods
            for (let px = sx + 50; px < sx + bw - 80; px += 100) {
                g.beginFill(0x111825);
                g.drawRect(px, pY - 18, 60, 18);
                g.endFill();
                g.beginFill(0x1a2540);
                g.drawRect(px, pY - 20, 60, 3);
                g.endFill();
                // Status light
                g.beginFill(Math.random() > 0.2 ? 0x4ade80 : 0xfbbf24, 0.6);
                g.drawCircle(px + 50, pY - 10, 3);
                g.endFill();
            }
            ...
            for (let mx = sx + 25; mx < sx + bw - 40; mx += 60) {
                g.beginFill(0x0a0a18);
                g.drawRect(mx, fy + 14, 48, 30);
                g.endFill();
                g.beginFill(0x0d1525);
                g.drawRect(mx + 2, fy + 16, 44, 26);
                g.endFill();
                // Scrolling log lines
                for (let li = 0; li < 6; li++) {
                    ...
                    g.drawRect(mx + 4, fy + 18 + li * 4, lineW, 2);
                    g.endFill();
                }
            }
            ...
            for (let rx = sx + 20; rx < sx + bw - 30; rx += 50) {
                g.beginFill(0x0d1525);
                g.drawRect(rx, fy + 14, 35, fh - 24);
                g.endFill();
                g.beginFill(0x141e30);
                g.drawRect(rx, fy + 14, 35, 3);
                g.endFill();
                for (let ru = 0; ru < 8; ru++) {
                    const uy = fy + 20 + ru * 6;
                    g.beginFill(0x0a0f1a);
                    g.drawRect(rx + 2, uy, 31, 4);
                    g.endFill();
                    for (let li = 0; li < 4; li++) {
                        const on = Math.random() > 0.25;
                        g.beginFill(on ? 0xa855f7 : 0x1a2530, on ? 0.5 : 0.2);
                        g.drawRect(rx + 4 + li * 7, uy + 1, 2, 2);
                        g.endFill();
                    }
                }
            }
            ...
            for (let si = 0; si < stages.length; si++) {
                ...
                // Pipeline box
                g.beginFill(0x111825);
                g.drawRect(stX, fy + 16, stageW - 10, 28);
                g.endFill();
                const stCol = si < 3 ? 0x4ade80 : si === 3 ? 0xfbbf24 : 0x22d3ee;
                g.beginFill(stCol, 0.1);
                g.drawRect(stX + 2, fy + 18, stageW - 14, 24);
                g.endFill();
                // Arrow connector
                ...
                g.drawRect(stX + 4, fy + 36, (stageW - 18) * fill, 4);
                g.endFill();
            }
        } else {
            // ── Generic ops floor ──
            for (let dx = sx + 50; dx < sx + bw - 60; dx += 80) {
                g.beginFill(0x1a2540);
                g.drawRect(dx, pY - 14, 50, 14);
                g.endFill();
                g.beginFill(0x222a40);
                g.drawRect(dx, pY - 16, 50, 3);
                g.endFill();
            }
        }
    ...
    _getNPCsForFloor(floorName, layout, floorIdx, numFloors) {
        ...
        if (fn.includes('command bridge') || fn.includes('ops deck')) {
            ...
            fn.includes('vector') ||
            fn.includes('embedding') ||
            fn.includes('knowledge') ||
            fn.includes('archive')
        ) {
            return [
    ...
    drawNPC(c, x, y, role, col) {
        ...
        head.drawRoundedRect(-bw * 0.4, 0, bw * 0.8, headH, headH * 0.2);
        head.endFill();
        // Visor/goggles instead of plain eyes — distinguishes from regular NPCs
        head.beginFill(colHex, 0.6);
        head.drawRect(-bw * 0.35, headH * 0.28, bw * 0.7, 3);
        head.endFill();
        // Visor lens dots
        head.beginFill(0xffffff, 0.7);
        head.drawRect(-bw * 0.15, headH * 0.32, 2, 2);
        head.drawRect(bw * 0.05, headH * 0.32, 2, 2);
        head.endFill();
        head.y = -h;
        ...
        antenna.drawRect(-0.5, -4, 1, 4);
        antenna.endFill();
        antenna.beginFill(colHex, 0.8);
        antenna.drawCircle(0, -5, 1.5);
        antenna.endFill();
        antenna.y = -h;
                    ...
                    if (av.timer <= 0) {
                        ...
                        if (r < 0.3) {
                            ...
                            if (Math.random() < 0.3) {
                                this.spawnBubble(
                                    av,
                                    AGENT_MSGS[Math.floor(Math.random() * AGENT_MSGS.length)]
                                );
                            }
                        }
                    }
