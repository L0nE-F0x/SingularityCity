...
const API = {
    ...
    _loadCoreCache() {
        try {
            ...
            if (typeof UI !== 'undefined' && UI.addLog)
                UI.addLog(`📦 Offline: city restored from your last visit (${ageDays}d old).`);
            return true;
        } catch (e) {
            return false;
        }
    ...
    async fetchCoreData() {
        ...
        try {
            const [labsRes, foundersRes, computeRes, bldsRes, actsRes, famRes, evRes, dcRes] =
                await Promise.all([
            ...
            LABS = {
                other: {
                    ...
                    icon: '🌐',
                    ticker: null,
                    desc: 'Independent entities and public spaces.',
                    region: 'eu',
                },
            };
    ...
    async fetchCloudEvents() {
        ...
        try {
            ...
            console.warn(
                '[CloudEvents] fetch failed (table may not exist yet):',
                (err && err.message) || err
            );
            return [];
        }
    ...
    async fetchCloudModels() {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                G.models.forEach((m) => {
                    const k = fuzzyNorm(m.name);
                    if (k) existingByName.set(k, m);
                });
                let added = 0,
                    dupeSkipped = 0;
                ...
                data.forEach((m) => {
                    ...
                    if (m.cost_input != null && m.cost_out != null) {
                        ...
                        window.COSTS[m.id] = {
                            input: parseFloat(m.cost_input),
                            output: parseFloat(m.cost_out),
                        };
                    }
                    if (m.ctx != null) {
                ...
                if (added > 0) {
                    if (typeof UI !== 'undefined')
                        UI.addLog(`☁️ Synced ${added} models from global database.`);
                }
                if (dupeSkipped > 0) {
                    console.warn(
                        `🔀 [Cloud] Skipped ${dupeSkipped} duplicate-name rows on load — dedupeModels() will reconcile Supabase.`
                    );
                }
                ...
                if (rejectIds.length > 0) {
                    console.warn(
                        `🚫 [Cloud] Filtered ${rejectIds.length} hallucinated models from display: ${rejectNames.join(', ')}`
                    );
        ...
        try {
            ...
            for (const hf of models) {
                ...
                const region = [
                    ...
                    ? 'us'
                    : ['Qwen', 'deepseek-ai', 'THUDM', '01-ai', 'baichuan-inc', 'internlm'].includes(org)
                      ? 'cn'
                      : 'eu';
                ...
                const isDupe = G.models.some(
                    (m) => m.name.toLowerCase().replace(/[^a-z0-9]/g, '') === safeName
                );
                if (isDupe) continue;
                ...
                if (this.supabase) {
                    try {
                        await this._cloudSubmit('models', this._dbSafeModel(nm));
                    } catch (e) {
                        /* silent */
                    }
                }
    ...
    async fetchZeroEval() {
        try {
            ...
            for (const ze of models) {
                ...
                if (existingFuzzy.has(fuzzyName)) {
                    benchUpdated++;
                    continue;
                } // fuzzy match caught a variant
    ...
    async fetchOpenRouter() {
        try {
            const isDeployed = !['localhost', '127.0.0.1'].includes(window.location.hostname);
            const url = isDeployed ? '/api/openrouter/models' : 'https://openrouter.ai/api/v1/models';
            const res = await fetch(url, { signal: AbortSignal.timeout(20000) });
            if (!res.ok) {
            ...
            for (const or of models) {
                ...
                if (or.created && typeof or.created === 'number') {
                    try {
                        releaseDate = new Date(or.created * 1000).toISOString().split('T')[0];
                    } catch (e) {}
                }
            ...
            if (added > 0 || updated > 0) {
                if (typeof UI !== 'undefined') {
                    if (added > 0) {
                        UI.addToast(
                            `🌐 OpenRouter: ${added} new model${added > 1 ? 's' : ''} (incl. preview/beta)`
                        );
                        if (typeof NOTIFY !== 'undefined')
                            NOTIFY.send(
                                'New AI Models',
        ...
        for (const feed of selectedFeeds) {
            try {
                ...
                if (d.status === 'ok' && d.items?.length > 0) {
                    this.liveNews = [
                        ...this.liveNews.filter((n) => n.source !== feed.source),
                        ...d.items
                            .slice(0, 12)
                            .map((i) => ({ headline: i.title, url: i.link, source: feed.source })),
                    ].slice(0, 40);
                    got = true;
                }
            } catch (e) {
            ...
            this.liveNews =
                fromCloud.length > 0
                    ...
                    : [
                          ...
                          {
                              headline: 'Live headlines temporarily unavailable — reconnecting to news feeds',
                              url: '#',
                              source: 'Fallback',
                          },
                      ];
    ...
    async fetchVCFunding() {
        ...
        try {
            ...
            console.warn('[VC Funding] Fetch failed:', e.message);
        }
    },
    ...
    async fetchSupplyChain() {
        ...
        try {
            ...
            console.warn('[Supply Chain] Fetch failed:', e.message);
        }
    },
    ...
    async fetchRegulationNews() {
        ...
        const regKeywords =
            /regulat|senate|congress|EU AI Act|compliance|safety|ban|lawsuit|copyright|FTC|antitrust|oversight|hearing|legislation|policy|GDPR|govern/i;
        ...
        try {
            const r = await fetch(
                'https://api.rss2json.com/v1/api.json?rss_url=' +
                    encodeURIComponent('https://www.theverge.com/rss/ai-artificial-intelligence/index.xml'),
                { signal: AbortSignal.timeout(8000) }
            );
            if (r.ok) {
                const d = await r.json();
                if (d.status === 'ok' && d.items?.length > 0) {
                    ...
                    regFiltered.forEach((i) => {
                        if (!this.regulationNews.find((n) => n.headline === i.title)) {
                            this.regulationNews.push({
                                headline: i.title,
                                url: i.link,
                                source: 'Regulation Feed',
                            });
                        }
                    });
                }
    ...
    async fetchVCDealsRSS() {
        ...
        const fundingVerbs =
            /\b(raises?|raised|secures?|secured|closes?|closed|lands?|landed|gets?|got|nabs?|nabbed|bags?|bagged|grabs?|grabbed|nets?|netted|funding)\b/i;
        ...
        for (const feed of feeds) {
            try {
                ...
                console.warn(`[VC RSS] ${feed.source}:`, e.message);
            }
        }
        ...
        for (const feed of feeds) {
            try {
                ...
                for (const item of d.items) {
                    ...
                    const cat = /ASML|EUV|DUV|lithograph/i.test(item.title)
                        ? 'lithography'
                        : /foundry|TSMC|Samsung.{0,6}fab|Intel.{0,6}fab|wafer|node|nanometer|\dnm/i.test(
                                item.title
                            )
                          ? 'foundry'
                          : /HBM|CoWoS|packaging|memory|DRAM|NAND|shortage/i.test(item.title)
                            ? 'bottleneck'
                            : 'accelerator';
                ...
                console.warn(`[Supply Chain RSS] ${feed.source}:`, e.message);
            }
        }
    ...
    async fetchAIEvents() {
        ...
        const eventKeywords =
            /\b(conference|summit|workshop|hackathon|symposium|keynote|demo day|launch event|developer day|devday|I\/O|Build|WWDC|re:Invent|Ignite|Connect|NeurIPS|ICML|ICLR|CVPR|AAAI|SIGMOD|KDD|NAACL|ACL|EMNLP|CoRL|RSS\b|IJCAI|ECCV|ICCV|WSDM|GTC|Microsoft Build|Google I\/O|Apple WWDC|AWS re:Invent|Dreamforce|CES\s+\d{4}|MWC\s+\d{4})\b/i;
        ...
        for (const feed of feeds) {
            try {
                ...
                /* silent — RSS feeds sometimes fail */
            }
        }
    ...
    async _fetchEventsFromLLM() {
        ...
        try {
            ...
            if (G.apiProvider === 'anthropic') {
                ...
                pl = {
                    model: G.modelId || PROVIDER_DEFAULT_MODELS.anthropic,
                    max_tokens: 2048,
                ...
                hd['Authorization'] = `Bearer ${G.authKey}`;
                pl = {
                    model: G.modelId || PROVIDER_DEFAULT_MODELS.openai,
                    max_tokens: 2048,
                    messages: [{ role: 'user', content: prompt }],
                };
            }
            ...
            console.warn('[Calendar LLM]', e.message);
        }
    },
    ...
    async fetchNewDataCenters() {
        if (
            this._dcScanning ||
            !G.authKey ||
            typeof DC_FACILITIES === 'undefined' ||
            typeof DCManager === 'undefined'
        )
            return;
...
{"facilities":[{"id":"dc_example","name":"Example DC","operator":"google","location":"City, State/Country","type":"datacenter","status":"operational","gpus":"50,000 H200","power_mw":400,"cooling":"Liquid cooling","desc":"One-sentence summary.","completion":null,"color":"#4285f4"}]}`;
            ...
            if (G.apiProvider === 'anthropic') {
                ...
                pl = {
                    model: G.modelId || PROVIDER_DEFAULT_MODELS.anthropic,
                    max_tokens: 2048,
                    system: `You are a real-time AI infrastructure data API. Respond strictly in minified JSON. Today is ${today}. Only return facilities you are certain exist in the real world.`,
                    messages: [{ role: 'user', content: prompt }],
                };
            } else if (G.apiProvider === 'google') {
                url = `https://generativelanguage.googleapis.com/v1beta/models/${G.modelId || PROVIDER_DEFAULT_MODELS.google}:generateContent?key=${G.authKey}`;
                ...
                pl = {
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: `You are a real-time AI infrastructure data API. Today is ${today}. Respond strictly in minified JSON. Only return facilities that are real.`,
                        },
            ...
            for (const f of parsed.facilities) {
                ...
                const facility = {
                    ...
                    status: ['operational', 'construction', 'planned'].includes(f.status)
                        ? f.status
                        : 'operational',
                    gpus: f.gpus || null,
                    ...
                    color:
                        typeof f.color === 'string' && /^#[0-9a-f]{6}$/i.test(f.color) ? f.color : '#64748b',
                };
            ...
            if (added > 0 && typeof UI !== 'undefined') {
                ...
                if (typeof NOTIFY !== 'undefined')
                    NOTIFY.send(
                        'New Compute!',
                        `${added} new data center${added === 1 ? '' : 's'} on the map`
                    );
            }
        } catch (e) {
    ...
    async fetchNetworkStatus() {
        ...
        for (const feed of feeds) {
            try {
                ...
                /* silent — status feeds sometimes fail */
            }
        }
    ...
    async fetchGlobalGrid() {
        ...
        try {
            ...
            if (sbUrl && sbKey) {
                const r = await fetch(`${sbUrl}/rest/v1/grid_data?id=eq.global&select=data,updated_at`, {
                ...
                if (r.ok) {
                    ...
                    if (rows.length > 0 && rows[0].data) {
                        ...
                        if (typeof UI !== 'undefined')
                            UI.addLog(
                                `⚡ Grid: ${this._gridData.plantCount.toLocaleString()} plants across ${this._gridData.regionsScanned} regions (live data)`
                            );
                        return;
                    }
                }
        ...
        try {
            ...
            if (typeof UI !== 'undefined')
                UI.addLog(
                    `⚡ Grid: ${this._gridData.plantCount.toLocaleString()} power plants across ${this._gridData.regionsScanned} regions`
                );
        } catch (e) {
    ...
    _mdToHtml(md) {
        return md
            ...
            .replace(
                /```(\w*)\n([\s\S]*?)```/g,
                '<pre style="background:rgba(0,0,0,0.4);padding:8px;border-radius:4px;overflow-x:auto;font-size:8px;margin:6px 0;border:1px solid var(--bd)"><code>$2</code></pre>'
            )
            .replace(
                /`([^`]+)`/g,
                '<code style="background:rgba(0,0,0,0.3);padding:1px 4px;border-radius:3px;font-size:8px">$1</code>'
            )
            .replace(/\*\*(.+?)\*\*/g, '<b>$1</b>')
            .replace(/\*(.+?)\*/g, '<i>$1</i>')
            .replace(
                /^### (.+)$/gm,
                '<div style="font-size:10px;font-weight:bold;color:var(--ac);margin:8px 0 4px">$1</div>'
            )
            .replace(
                /^## (.+)$/gm,
                '<div style="font-size:11px;font-weight:bold;color:var(--cy);margin:8px 0 4px">$1</div>'
            )
            .replace(
                /^# (.+)$/gm,
                '<div style="font-size:12px;font-weight:bold;color:#fff;margin:8px 0 4px">$1</div>'
            )
    ...
    async askAnalyst() {
        ...
        if (!q) return;
        if (!G.authKey) {
            if (typeof UI !== 'undefined') UI.addToast('❌ Set API key in Settings first.');
            return;
        }
        ...
        try {
            ...
            if (G.apiProvider === 'anthropic') {
                ...
                url = `https://generativelanguage.googleapis.com/v1beta/models/${G.modelId || PROVIDER_DEFAULT_MODELS.google}:generateContent?key=${G.authKey}`;
                const contents = this._chatHistory.map((m) => ({
                ...
                pl = {
                    model: G.modelId || PROVIDER_DEFAULT_MODELS.xai,
                    max_tokens: 4096,
                    messages: [{ role: 'system', content: sysPrompt }, ...this._chatHistory],
                };
            } else {
                url = 'https://api.openai.com/v1/chat/completions';
                hd['Authorization'] = `Bearer ${G.authKey}`;
            ...
            let txt = '';
            if (G.apiProvider === 'anthropic')
                txt = d.content?.[0]?.text || d.error?.message || 'No response';
            else if (G.apiProvider === 'google')
                txt = d.candidates?.[0]?.content?.parts?.[0]?.text || d.error?.message || 'No response';
            ...
            if (l)
                l.innerHTML = `<span style="color:#ef4444">❌ ${e.message.includes('Failed to fetch') ? 'Network error — check your connection and try again.' : e.message}</span>`;
        }
    },
    ...
    _familyToLab: {
        ...
        qwq: ['alibaba'],
        ernie: ['baidu'],
        glm: ['zhipu_ai', 'zhipu', 'thudm'],
        chatglm: ['zhipu_ai', 'zhipu', 'thudm'],
        command: ['cohere'],
        aya: ['cohere', 'cohereforai'],
        nova: ['amazon'],
        titan: ['amazon'],
        nemotron: ['nvidia'],
        nvlm: ['nvidia'],
        ...
        baichuan: ['baichuan'],
        internlm: ['shanghai_ai_lab'],
        internvl: ['shanghai_ai_lab'],
        falcon: ['tii'],
        jais: ['inception'],
        olmo: ['allen_ai', 'allenai'],
        olmoe: ['allen_ai', 'allenai'],
        tulu: ['allen_ai', 'allenai'],
        granite: ['ibm'],
        smollm: ['huggingface', 'huggingfaceh4'],
        bloom: ['bigscience'],
        bloomz: ['bigscience'],
        dbrx: ['databricks'],
    ...
    _verifyModel(m) {
        ...
        if (this._verifiedModelNames && this._verifiedModelNames.size > 0) {
            ...
            for (const v of this._verifiedModelNames) {
                if (
                    v.length >= 5 &&
                    (normName.includes(v) || (normName.length >= 5 && v.includes(normName)))
                ) {
                    return { ok: true };
                }
            }
        ...
        for (const [family, maxVer] of Object.entries(this._maxKnownVersions)) {
            ...
            const tol =
                this._frontierForwardTolerance[family] != null
                    ? this._frontierForwardTolerance[family]
                    : this._openWeightForwardTolerance;
            if (result.max != null && result.max > maxVer + tol) {
                return {
                    ok: false,
                    reason: `Version ${result.max} exceeds max known ${family} version ${maxVer}${tol ? ` (+${tol} forward tolerance)` : ''}`,
                };
            }
        }
        ...
        if (this._verifiedModelNames && this._verifiedModelNames.size > 0) {
            ...
            let isVerified =
                this._verifiedModelNames.has(normName) || this._verifiedModelNames.has(fuzzyName);
            if (!isVerified) {
                // Bidirectional containment with length floor 7 (handles lab-prefixed/date-suffixed names)
                for (const v of this._verifiedModelNames) {
                    if (
                        v.length >= 7 &&
                        (normName.includes(v) || (normName.length >= 7 && v.includes(normName)))
                    ) {
                        isVerified = true;
                        break;
                    }
            ...
            if (!isVerified) {
                return {
                    ok: false,
                    reason: `Unknown family + not in registry (likely hallucinated): ${m.name}`,
                };
            }
        }
    ...
    async doScan() {
        ...
        if (!G.authKey) {
            if (typeof UI !== 'undefined') UI.addLog('❌ No API key.');
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    ...
                    const weightedCategories = [
                        {
                            cat: 'CUTTING-EDGE 2025-2026 Flagships (the NEWEST model from each major lab)',
                            w: 50,
                        },
                        {
                            cat: 'Asian Tech Latest (DeepSeek-V3.2, DeepSeek-R2, Qwen3.5, Ernie 5, GLM-5, Yi-Lightning)',
                            w: 15,
                        },
                        {
                            cat: 'Open-weights Latest (Llama 4, Mistral Large 2, Gemma 3, Phi-4, Command-A)',
                            w: 15,
                        },
                        {
                            cat: 'Specialized/Niche (Coding, Audio, Vision, Medical, Robotics — 2025-2026 releases only)',
                            w: 10,
                        },
                    ...
                    for (const [family, maxVer] of Object.entries(this._maxKnownVersions)) {
                        ...
                        if (bestSeen < maxVer) {
                            missingFlagships.push(
                                `${family} v${maxVer} (${labs[0]}${bestSeen >= 0 ? `, we only have up to v${bestSeen}` : ', none tracked'})`
                            );
                        }
                    }
                    ...
                    const forwardNudge = Object.entries(frontierFams)
                        .map(
                            ([fam, label]) =>
                                `${label}: we currently know up to v${this._maxKnownVersions[fam]} — if a NEWER one has been officially released, RETURN IT`
                        )
                        .join('\n');
                    ...
                    const labCounts = {};
                    majorLabs.forEach((l) => {
                        labCounts[l] = G.models.filter((m) => m.lab === l).length;
                    });
                    const underrepresented = majorLabs
                        .filter((l) => labCounts[l] < 4)
                        .map((l) => `${l}(${labCounts[l]})`)
                        .join(', ');

                    // ─── FOUNDER GAP: Which labs have no CEO/founder tracked? ───
                    const allLabsWithModels = [...new Set(G.models.map((m) => m.lab))];
                    const founderLabs = new Set(
                        (typeof REAL_FOUNDERS !== 'undefined' ? REAL_FOUNDERS : []).map((f) => f.lab)
                    );
                    const labsMissingFounder = allLabsWithModels.filter(
                        (l) => l !== 'other' && !founderLabs.has(l)
                    );
...
{"models":[{"id":"model_id","name":"Full Model Name","lab":"lab_id","region":"us","founder_name":"CEO Name or null","released":"2025-01-01","retired":null,"phase":"released","os":false,"desc":"Summary.","personality":"Helpful","talent":"Coding","favSpot":"Server Room","benchmarks":{"MMLU":90,"HumanEval":85,"MATH":75,"GPQA":55},"arch":{"params":"200B","type":"Dense","tokens":"15T","compute":"1e25 FLOPs"},"ctx":200000,"cost_input":3.0,"cost_out":15.0}],"retirements":[],"elo_updates":[],"events":[],"lineage_updates":[]}`;
                    ...
                    if (G.apiProvider === 'anthropic') {
                        ...
                        pl = {
                            ...
                            max_tokens: 8192,
                            system:
                                'You are a real-time AI industry data API. Respond strictly in minified JSON format. No markdown. Only use real, verified data. Today is ' +
                                new Date().toISOString().split('T')[0] +
                                '. Any model publicly available via API as of today is "released", NOT "rumored". CRITICAL: Do NOT invent future model versions that do not exist yet. Only return models you are certain have been publicly released or officially announced. If unsure, omit the model.',
                            messages: [{ role: 'user', content: prompt }],
                        };
                    } else if (G.apiProvider === 'google') {
                        ...
                        pl = {
                            systemInstruction: {
                                parts: [
                                    {
                                        text:
                                            'You are a real-time AI industry data API. Respond strictly in valid JSON format. Only output real-world data. Do not truncate. Today is ' +
                                            new Date().toISOString().split('T')[0] +
                                            '. Any model publicly available via API today is "released", NOT "rumored". CRITICAL: Do NOT invent future model versions that do not exist yet. Only return models you are certain have been publicly released or officially announced. If unsure, omit the model.',
                                    },
                                ],
                            },
                            contents: [{ parts: [{ text: prompt }] }],
                            ...
                            messages: [
                                {
                                    role: 'system',
                                    content:
                                        'You are a real-time AI industry data API. Respond strictly in valid JSON format. Only use real, verified data. Today is ' +
                                        new Date().toISOString().split('T')[0] +
                                        '. CRITICAL: Any model that is publicly available via API as of today MUST have phase "released", NOT "rumored". Do NOT invent future model versions that do not exist yet. Only return models you are certain have been publicly released or officially announced. If unsure, omit the model.',
                                },
                                { role: 'user', content: prompt },
                            ],
                        };
                            ...
                            messages: [
                                {
                                    role: 'system',
                                    content:
                                        'You are a real-time AI industry data API. Respond strictly in valid JSON format. Only use real data. Today is ' +
                                        new Date().toISOString().split('T')[0] +
                                        '. Any model publicly available via API today is "released", NOT "rumored". CRITICAL: Do NOT invent future model versions that do not exist yet. Only return models you are certain have been publicly released or officially announced. If unsure, omit the model.',
                                },
                                { role: 'user', content: prompt },
                    ...
                    if (G.apiProvider === 'google') {
                        ...
                        if (data.candidates && data.candidates[0]) {
                            ...
                            if (fr === 'MAX_TOKENS') {
                                console.warn(
                                    `⚠️ [SCAN] Google hit MAX_TOKENS (thinking model used too many reasoning tokens). Attempting to salvage partial response...`
                                );
                            }
                        }
                    }
                    ...
                    if (!txt) {
                        throw new Error(
                            'API returned an empty text response. Check the Dev Console for the raw object dump.'
                        );
                    }
                    ...
                    try {
                        ...
                        console.warn(
                            '⚠️ [SCAN] JSON parse failed, attempting salvage of truncated response...'
                        );
                        try {
                            ...
                            if (modelsStart !== -1) {
                                ...
                                for (let ci = modelsStart; ci < cleanTxt.length; ci++) {
                                    ...
                                    if (ch === '}') {
                                        depth--;
                                        if (depth === 1) lastCompleteObj = ci;
                                    }
                                }
                                if (lastCompleteObj > 0) {
                                    const salvaged =
                                        cleanTxt.substring(0, lastCompleteObj + 1) +
                                        '],"retirements":[],"elo_updates":[],"events":[],"lineage_updates":[]}';
                    ...
                    if (
                        lastErr.includes('HTTP 401') ||
                        lastErr.includes('HTTP 403') ||
                        lastErr.includes('API returned HTTP 4')
                    ) {
                        throw loopErr;
                    }
            ...
            if (parsedData.models) {
                for (const m of parsedData.models) {
                    ...
                    if (isDuplicate) {
                        ...
                        const target = G.models.find(
                            (mod) =>
                                mod.id.toLowerCase().replace(/[^a-z0-9]/g, '') === safeId ||
                                mod.name.toLowerCase().replace(/[^a-z0-9]/g, '') === safeName
                        );

                        if (target) {
                            if (
                                (!window.COSTS || !window.COSTS[target.id]) &&
                                m.cost_input != null &&
                                m.cost_out != null
                            ) {
                                if (!window.COSTS) window.COSTS = {};
                                window.COSTS[target.id] = {
                                    input: parseFloat(m.cost_input),
                                    output: parseFloat(m.cost_out),
                                };
                            ...
                            if (needsUpdate && this.supabase) {
                                this._cloudSubmit(
                                    ...
                                    if (ok && typeof UI !== 'undefined')
                                        UI.addToast(`📈 Backfilled economic data for ${target.name}!`);
                                });
                            }
                        }
                        continue;
                    ...
                    const nm = {
                        ...
                        phase: m.phase || 'released',
                        os: m.os || false,
                        desc: m.desc || 'A new citizen!',
                        per: m.personality || 'Fresh face',
                        tal: m.talent || 'Being new',
                        fav: m.favSpot || 'Town Square',
                        _src: 'llm_scan',
                        ...
                        if (!existingFounder) {
                            ...
                            if (typeof Entities !== 'undefined' && G.ceoRefs && !G.ceoRefs[nm.lab]) {
                                if (Entities.carLayer && Entities.reflectionLayer) {
                                    const ceoObj = EntitiesGfx.initCEO(
                                        newFounder,
                                        Entities.carLayer,
                                        Entities.reflectionLayer
                                    );
                            ...
                            if (!BLDS.find((b) => b.id === 'house_' + nm.lab)) {
                                const hash = Array.from(nm.lab).reduce(
                                ...
                                if (typeof UI !== 'undefined')
                                    UI.addToast(`🏡 ${m.founder_name}'s Estate built in Billionaire's Row!`);
                            }

                            if (typeof UI !== 'undefined')
                                UI.addToast(
                                    `🧑‍💼 ${m.founder_name} (${labData.name}) has arrived in the city!`
                                );
                        }
                    }
                    ...
                    if (this.supabase) {
                        try {
                            ...
                            if (!ok)
                                console.error('Supabase Save Error: submit-data rejected or unreachable');
                        } catch (dbErr) {
            ...
            if (parsedData.lineage_updates) {
                ...
                if (famAdded > 0 && typeof UI !== 'undefined') {
                    UI.addToast(`🧬 Mapped ${famAdded} new family tree connections!`);
                    if (typeof NOTIFY !== 'undefined')
                        NOTIFY.send('Lineage Updated!', `🧬 ${famAdded} new model family connections mapped`);
                }
            }
            ...
            if (parsedData.events) {
                ...
                for (const ev of parsedData.events) {
                    if (
                        window.AI_EVENTS &&
                        !window.AI_EVENTS.find((e) => e.date === ev.date && e.name === ev.name)
                    ) {
                        window.AI_EVENTS.push(ev);
                        addedEvents++;
                        if (this.supabase) {
                ...
                if (addedEvents > 0 && typeof UI !== 'undefined') {
                    UI.addToast(`📅 Added ${addedEvents} new tech events to Calendar!`);
                    if (typeof NOTIFY !== 'undefined')
                        NOTIFY.send('Events Added!', `📅 ${addedEvents} new tech events on the calendar`);
                }
            }
            ...
            if (parsedData.retirements) {
                parsedData.retirements.forEach((rt) => {
                    ...
                    const m = G.models.find(
                        (mod) => mod.id.toLowerCase().replace(/[^a-z0-9]/g, '') === safeName
                    );
                    if (m && !m.ret) {
            ...
            if (typeof this.dedupeModels === 'function') {
                try {
                    await this.dedupeModels();
                } catch (e) {
                    console.warn('[Scan] Dedupe failed:', e);
                }
            }
        } catch (e) {
            ...
            if (
                ...
                if (typeof UI !== 'undefined')
                    UI.addLog(`⚠️ Network/CORS error. Try switching API providers.`);
            } else {
    ...
    async purgeHallucinations() {
        ...
        if (!this._zeroevalLoaded && typeof this.fetchZeroEval === 'function') {
            try {
                await this.fetchZeroEval();
            } catch (e) {
                /* registry will fall back to knownReal */
            }
        }
        ...
        try {
            ...
            if (localPurged > 0) {
                if (typeof UI !== 'undefined')
                    UI.addLog(`🧹 Purged ${localPurged} hallucinated models from view`);
    ...
    async dedupeModels() {
            ...
            for (const loser of losers) {
                ...
                if (window.BM && window.BM[loser.id]) {
                    ...
                    for (const [k, v] of Object.entries(window.BM[loser.id])) {
                        if (!window.BM[winner.id][k] || v > window.BM[winner.id][k])
                            window.BM[winner.id][k] = v;
                    }
        ...
        if (typeof UI !== 'undefined') {
            ...
            if (mergedCount > 0)
                UI.addToast(`🔀 Merged ${mergedCount} duplicate citizen${mergedCount > 1 ? 's' : ''}!`);
        }
