...
const InteriorVCRow = {
    ...
    NPCS: {
        reception: [
            { name: 'Receptionist', color: 0x94a3b8 },
            { name: 'Security', color: 0xef4444 },
        ],
        deal_room: [
            { name: 'VC Partner', color: 0x4ade80 },
            { name: 'Analyst', color: 0x22d3ee },
        ],
        partner_offices: [
            { name: 'GP', color: 0xfbbf24 },
            { name: 'Associate', color: 0x4ade80 },
        ],
        fund_ops: [
            { name: 'Controller', color: 0x94a3b8 },
            { name: 'Compliance', color: 0xa855f7 },
        ],
        rooftop_lounge: [{ name: 'LP Guest', color: 0xfbbf24 }],
        analytics: [
            { name: 'Quant', color: 0x22d3ee },
            { name: 'Data Eng', color: 0x06b6d4 },
        ],
        client_offices: [{ name: 'Client Mgr', color: 0x4ade80 }],
        trading_room: [
            { name: 'Trader', color: 0xef4444 },
            { name: 'Analyst', color: 0x22d3ee },
        ],
        coworking: [
            { name: 'Founder', color: 0xfbbf24 },
            { name: 'Hacker', color: 0x4ade80 },
        ],
        pitch_stage: [
            { name: 'Mentor', color: 0xfbbf24 },
            { name: 'Judge', color: 0xa855f7 },
        ],
        vault: [{ name: 'Guard', color: 0xef4444 }],
        trading_floor: [
            { name: 'Trader', color: 0xef4444 },
            { name: 'Broker', color: 0x22d3ee },
            { name: 'Algo Bot', color: 0x4ade80 },
        ],
        legal: [
            { name: 'Lawyer', color: 0x94a3b8 },
            { name: 'Paralegal', color: 0xa855f7 },
        ],
        boardroom: [
            { name: 'Director', color: 0xfbbf24 },
            { name: 'Secretary', color: 0x94a3b8 },
        ],
        executive: [
            { name: 'CEO', color: 0xfbbf24 },
            { name: 'CFO', color: 0x4ade80 },
        ],
        broker_floor: [
            { name: 'Broker', color: 0x22d3ee },
            { name: 'Clerk', color: 0x94a3b8 },
        ],
    },
    ...
    build(bld, layer) {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            s.drawCircle(0, 0, 0.5 + Math.random());
            s.endFill();
        ...
        const startX = 60,
            bldW = G.vpW - 120;
        const shaftW = 50,
            shaftX = startX + bldW - shaftW - 15;
        const usableW = bldW - shaftW - 20;

        // Accent color per building — matches the exterior brand palette
        const accents = {
            vcrow_apex: 0xe07a5f,
            vcrow_horizon: 0xb23b34,
            vcrow_thrive: 0x6366f1,
            vcrow_foundersfund: 0x38bdf8,
            vcrow_launchpad: 0xff6a00,
            vcrow_mgx: 0xc9a227,
            vcrow_titan: 0x9aa0a6,
            vcrow_exchange: 0xef4444,
        };
        ...
        const bW = 220,
            bH = 30,
            bX = startX + bldW / 2 - bW / 2,
            bY = roofH - bH - 8;
        const sg = new PIXI.Graphics();
        sg.beginFill(0x0a0a14);
        sg.lineStyle(2, accent, 0.5);
        sg.drawRect(bX, bY, bW, bH);
        sg.endFill();
        sg.lineStyle(0);
        sg.beginFill(0x333333);
        sg.drawRect(bX + 15, bY + bH, 4, 8);
        sg.drawRect(bX + bW - 19, bY + bH, 4, 8);
        sg.endFill();
        rc.addChild(sg);
        ...
        rl.beginFill(accent, 0.2);
        rl.drawRect(startX, roofH - 4, bldW, 4);
        rl.endFill();
        rc.addChild(rl);
        this.scene.addChild(rc);
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            if (!isBasement) {
                ...
                rg.drawRect(startX, fy, bldW, 20);
                rg.endFill();
                rg.beginFill(wallCol);
                rg.drawRect(startX, fy + 54, bldW, floorH - 54);
                rg.endFill();
                // Wall segments between windows
                let wx = startX + 40;
                rg.beginFill(wallCol);
                rg.drawRect(startX, fy + 20, 40, 34);
                rg.endFill();
                while (wx + 40 <= startX + bldW - 40) {
                    wx += 40;
                    if (wx + 20 <= startX + bldW - 40) {
                        rg.beginFill(wallCol);
                        rg.drawRect(wx, fy + 20, 20, 34);
                        rg.endFill();
                    }
                    wx += 20;
                }
                rg.beginFill(wallCol);
                rg.drawRect(wx - 20, fy + 20, startX + bldW - wx + 20, 34);
                rg.endFill();
                // Window frames
                const wf = new PIXI.Graphics();
                let wx2 = startX + 40;
                while (wx2 + 40 <= startX + bldW - 40) {
                    wf.lineStyle(2, 0x1a2030);
                    wf.drawRect(wx2, fy + 20, 40, 34);
                    wf.moveTo(wx2 + 20, fy + 20);
                    wf.lineTo(wx2 + 20, fy + 54);
                    wf.moveTo(wx2, fy + 37);
                    wf.lineTo(wx2 + 40, fy + 37);
                    wf.lineStyle(0);
                    wx2 += 60;
                }
            ...
            // Floor + ceiling lines
            rg.beginFill(0x0f0f18);
            rg.drawRect(startX, fy + floorH - 6, bldW, 6);
            rg.endFill();
            rg.beginFill(0x080810);
            rg.drawRect(startX, fy, bldW, 3);
            rg.endFill();
            rg.beginFill(0x1a1a2a);
            ...
            if (!isBasement) {
                ...
                ns.beginFill(accent, 0.08);
                ns.drawRect(startX, fy + 2, bldW, 2);
                ns.endFill();
                ns.beginFill(accent, 0.05);
                ns.drawRect(startX, fy + floorH * 0.5, bldW, 1);
                ns.endFill();
            ...
            else if (theme === 'partner_offices')
                this._drawPartnerOffices(fc, startX, bldW, pY, fy, floorH, accent);
            else if (theme === 'fund_ops') this._drawFundOps(fc, startX, bldW, pY, fy, floorH, accent);
            else if (theme === 'rooftop_lounge')
                this._drawRooftopLounge(fc, startX, bldW, pY, fy, floorH, accent);
            else if (theme === 'analytics') this._drawAnalytics(fc, startX, bldW, pY, fy, floorH, accent);
            else if (theme === 'client_offices')
                this._drawPartnerOffices(fc, startX, bldW, pY, fy, floorH, accent);
            else if (theme === 'trading_room' || theme === 'trading_floor')
                this._drawTradingFloor(fc, startX, bldW, pY, fy, floorH, accent);
        ...
        const shaftGfx = new PIXI.Graphics();
        shaftGfx.eventMode = 'none';
        // Shaft background
        shaftGfx.beginFill(0x0a0a12);
        shaftGfx.drawRect(shaftX, roofH, shaftW, (numFloors + 1) * floorH);
        shaftGfx.endFill();
        // Shaft walls
        shaftGfx.beginFill(0x1a1a2a);
        shaftGfx.drawRect(shaftX - 2, roofH, 2, (numFloors + 1) * floorH);
        shaftGfx.endFill();
        shaftGfx.beginFill(0x1a1a2a);
        shaftGfx.drawRect(shaftX + shaftW, roofH, 2, (numFloors + 1) * floorH);
        shaftGfx.endFill();
        // Rails
        shaftGfx.beginFill(0x333344, 0.4);
        shaftGfx.drawRect(shaftX + 4, roofH, 2, (numFloors + 1) * floorH);
        shaftGfx.endFill();
        shaftGfx.beginFill(0x333344, 0.4);
        shaftGfx.drawRect(shaftX + shaftW - 6, roofH, 2, (numFloors + 1) * floorH);
        shaftGfx.endFill();
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            // Door frame
            shaftGfx.beginFill(0x222230);
            shaftGfx.drawRect(shaftX + 8, doorY, shaftW - 16, 40);
            shaftGfx.endFill();
            // Double doors
            shaftGfx.beginFill(0x333348);
            shaftGfx.drawRect(shaftX + 10, doorY + 2, (shaftW - 20) / 2 - 1, 36);
            shaftGfx.endFill();
            shaftGfx.beginFill(0x333348);
            shaftGfx.drawRect(shaftX + 10 + (shaftW - 20) / 2 + 1, doorY + 2, (shaftW - 20) / 2 - 1, 36);
            shaftGfx.endFill();
            // Floor indicator
            shaftGfx.beginFill(accent, 0.5);
            shaftGfx.drawCircle(shaftX + shaftW / 2, doorY - 4, 3);
            shaftGfx.endFill();
        }
        // Elevator car (parked at ground floor)
        const carFy = roofH + (numFloors - 1) * floorH; // ground floor position
        shaftGfx.beginFill(0x444460);
        shaftGfx.drawRect(shaftX + 6, carFy + 10, shaftW - 12, floorH - 14);
        shaftGfx.endFill();
        shaftGfx.beginFill(0x555578);
        shaftGfx.drawRect(shaftX + 8, carFy + 12, shaftW - 16, 3);
        shaftGfx.endFill();
        // Elevator car interior light
        shaftGfx.beginFill(0xffffee, 0.08);
        shaftGfx.drawRect(shaftX + 8, carFy + 16, shaftW - 16, floorH - 30);
        shaftGfx.endFill();
        ...
        earth.beginFill(0x2a1a10);
        earth.drawRect(0, groundY, startX - 6, floorH);
        earth.endFill();
        earth.beginFill(0x3a2218);
        ...
        const undergroundH =
            typeof Underground !== 'undefined' ? Math.max(Underground.depthOf('east_rock') + 60, 300) : 300;
        const vm = new PIXI.Graphics();
        vm.beginFill(0x1a1810);
        vm.drawRect(0, basementBottom - 4, G.vpW, 10);
        vm.endFill();
        vm.beginFill(0x050508);
        vm.drawRect(0, undergroundY + undergroundH, G.vpW, 3000);
        vm.endFill();
        ...
        this._onMove = (e) => {
            if (!InteriorVCRow.isDragging || !InteriorVCRow.scene || InteriorVCRow.scene.destroyed) return;
            let ny = InteriorVCRow._startSceneY + (e.clientY - InteriorVCRow._startY);
            ny = Math.max(InteriorVCRow.minY, Math.min(ny, InteriorVCRow.maxY));
            InteriorVCRow.scene.y = ny;
        };
        this._onUp = () => {
            InteriorVCRow.isDragging = false;
            if (InteriorVCRow.layer) InteriorVCRow.layer.cursor = 'grab';
        };
    ...
    _drawBasement(c, sx, bw, pY, fy, fh, accent) {
        ...
        for (let ci = 0; ci < Math.min(5, Math.floor((bw - 120) / 90)); ci++) {
            ...
            // Body
            cg.beginFill(col);
            cg.drawRoundedRect(cx - 22, pY - 18, 44, 18, 4);
            cg.endFill();
            // Cabin/roof
            cg.beginFill(col, 0.8);
            cg.drawRoundedRect(cx - 12, pY - 28, 24, 12, 4);
            cg.endFill();
            // Windows
            cg.beginFill(0xffffff, 0.15);
            cg.drawRect(cx - 10, pY - 26, 9, 8);
            cg.drawRect(cx + 1, pY - 26, 9, 8);
            cg.endFill();
            // Wheels
            cg.beginFill(0x333333);
            cg.drawCircle(cx - 12, pY - 1, 4);
            cg.drawCircle(cx + 12, pY - 1, 4);
            cg.endFill();
            cg.beginFill(0x555555);
            cg.drawCircle(cx - 12, pY - 1, 2);
            cg.drawCircle(cx + 12, pY - 1, 2);
            cg.endFill();
            // Headlight
            cg.beginFill(0xffffff, 1.0);
            cg.drawRect(cx + 20, pY - 8, 4, 6);
            cg.endFill();
            // Brake light
            cg.beginFill(0xff3333, 1.0);
            cg.drawRect(cx - 26, pY - 10, 4, 4);
            cg.endFill();
            // Headlight beam glow
            cg.beginFill(0xffffee, 0.06);
            cg.drawPolygon([cx + 24, pY - 8, cx + 60, pY - 4, cx + 60, pY + 4, cx + 24, pY]);
            cg.endFill();
            c.addChild(cg);
        }
        ...
        up.eventMode = 'none';
        up.beginFill(0x222230);
        up.drawRect(sx + bw - 80, pY - 40, 40, 40);
        up.endFill();
        up.beginFill(0xef4444, 0.4);
        up.drawCircle(sx + bw - 60, pY - 30, 3);
        up.endFill();
        up.beginFill(0x4ade80, 0.4);
        up.drawCircle(sx + bw - 60, pY - 18, 3);
        up.endFill();
        c.addChild(up);
    },
    ...
    _drawReception(c, sx, bw, pY, fy, fh, accent) {
        ...
        g.beginFill(0x1a1a28, 0.5);
        g.drawRect(sx, pY - 4, bw, 4);
        g.endFill();
        g.beginFill(accent, 0.04);
        g.drawRect(sx, pY - 2, bw, 2);
        g.endFill();
        // Floor inlay pattern
        for (let ix = sx + 40; ix < sx + bw - 40; ix += 30) {
            g.beginFill(accent, 0.03);
            g.drawRect(ix, pY - 4, 20, 4);
            g.endFill();
        }
        ...
        g.drawRoundedRect(dx, pY - 24, dw, 24, 4);
        g.endFill();
        g.beginFill(0x222238);
        g.drawRect(dx + 3, pY - 22, dw - 6, 2);
        g.endFill();
        g.beginFill(accent, 0.15);
        g.drawRect(dx, pY - 24, dw, 3);
        g.endFill();
        // Logo backlit panel on desk front
        g.beginFill(0x0a0a14);
        g.drawRoundedRect(dx + dw / 2 - 25, pY - 18, 50, 14, 2);
        g.endFill();
        g.beginFill(accent, 0.25);
        g.drawRoundedRect(dx + dw / 2 - 22, pY - 15, 44, 8, 2);
        g.endFill();
        // Accent glow behind logo
        g.beginFill(accent, 0.04);
        g.drawRoundedRect(dx + dw / 2 - 30, pY - 20, 60, 18, 3);
        g.endFill();
        c.addChild(g);
        ...
        ph.eventMode = 'none';
        ph.beginFill(0x333344);
        ph.drawRect(dx + dw / 2 + 12, pY - 22, 8, 5);
        ph.endFill();
        ph.beginFill(0x444458);
        ph.drawRect(dx + dw / 2 + 13, pY - 20, 6, 2);
        ph.endFill();
        c.addChild(ph);
        ...
        for (const off of [sx + 25, sx + bw - 110]) {
            ...
            sf.eventMode = 'none';
            sf.beginFill(0x222240);
            sf.drawRoundedRect(off, pY - 16, 60, 12, 3);
            sf.endFill();
            sf.beginFill(0x2a2a50);
            sf.drawRoundedRect(off - 4, pY - 20, 8, 20, 2);
            sf.drawRoundedRect(off + 56, pY - 20, 8, 20, 2);
            sf.endFill();
            // Cushion details
            sf.beginFill(0x2e2e55, 0.5);
            sf.drawRect(off + 6, pY - 14, 20, 8);
            sf.drawRect(off + 30, pY - 14, 20, 8);
            sf.endFill();
            c.addChild(sf);
            // Coffee table in front
            const ct = new PIXI.Graphics();
            ct.eventMode = 'none';
            ct.beginFill(0x1a1a2a);
            ct.drawRect(off + 12, pY - 8, 36, 6);
            ct.endFill();
            ct.beginFill(accent, 0.06);
            ct.drawRect(off + 14, pY - 6, 32, 2);
            ct.endFill();
            // Magazine on table
            ct.beginFill(0xffffff, 0.08);
            ct.drawRect(off + 18, pY - 7, 8, 4);
            ct.endFill();
            c.addChild(ct);
        }
        // Company logo on back wall
        const wl = new PIXI.Graphics();
        wl.eventMode = 'none';
        wl.beginFill(accent, 0.06);
        wl.drawCircle(sx + bw / 2, fy + 30, 12);
        wl.endFill();
        wl.beginFill(accent, 0.12);
        wl.drawCircle(sx + bw / 2, fy + 30, 8);
        wl.endFill();
        wl.beginFill(accent, 0.2);
        wl.drawCircle(sx + bw / 2, fy + 30, 4);
        wl.endFill();
        c.addChild(wl);
    ...
    _drawDealRoom(c, sx, bw, pY, fy, fh, accent) {
        ...
        const tw = bw * 0.5,
            tx = sx + bw / 2 - tw / 2;
        g.beginFill(0x2a1a10);
        g.drawRoundedRect(tx, pY - 18, tw, 14, 4);
        g.endFill();
        g.beginFill(0x3a2a18);
        g.drawRoundedRect(tx + 3, pY - 16, tw - 6, 10, 2);
        g.endFill();
        // Table reflection highlight
        g.beginFill(0xffffff, 0.03);
        g.drawRect(tx + 10, pY - 15, tw - 20, 3);
        g.endFill();
        // Chairs around table — leather executive chairs
        for (let cx = tx + 12; cx < tx + tw - 10; cx += 22) {
            g.beginFill(0x222240);
            g.drawRoundedRect(cx, pY - 26, 12, 10, 3);
            g.endFill();
            g.beginFill(0x333358);
            g.drawRoundedRect(cx + 1, pY - 24, 10, 4, 2);
            g.endFill();
            g.beginFill(0x222240);
            g.drawRect(cx + 4, pY - 3, 4, 3);
            g.endFill();
        }
        // Term sheet documents + pens
        for (let dx = tx + 15; dx < tx + tw - 20; dx += 28) {
            g.beginFill(0xffffff, 0.12);
            g.drawRect(dx, pY - 15, 8, 10);
            g.endFill();
            g.beginFill(accent, 0.2);
            g.drawRect(dx + 1, pY - 13, 6, 1);
            g.drawRect(dx + 1, pY - 10, 4, 1);
            g.drawRect(dx + 1, pY - 8, 5, 1);
            g.endFill();
            // Pen
            g.beginFill(0x1a1a28);
            g.drawRect(dx + 10, pY - 12, 1, 8);
            g.endFill();
        }
        // Water carafe + glasses
        g.beginFill(0x93c5fd, 0.15);
        g.drawRect(tx + tw / 2 - 3, pY - 16, 6, 8);
        g.endFill();
        g.beginFill(0xffffff, 0.1);
        g.drawRect(tx + tw / 2 + 6, pY - 13, 3, 5);
        g.drawRect(tx + tw / 2 - 10, pY - 13, 3, 5);
        g.endFill();
        c.addChild(g);
        ...
        db.eventMode = 'none';
        db.beginFill(0x0a0a14);
        db.drawRect(dbx, dby, dbW, dbH);
        db.endFill();
        ...
        for (let ci = 0; ci < cols.length; ci++) {
            ...
            db.beginFill(accent, 0.08);
            db.drawRect(cx, dby + 2, colW - 2, 6);
            db.endFill();
            // Deal cards
            ...
            // Column separator
            if (ci > 0) {
                db.beginFill(0x333344, 0.3);
                db.drawRect(cx - 1, dby + 2, 1, dbH - 4);
                db.endFill();
            }
        }
        c.addChild(db);
        ...
        cam.eventMode = 'none';
        cam.beginFill(0x333344);
        cam.drawRect(sx + bw - 80, fy + 10, 12, 4);
        cam.endFill();
        cam.beginFill(0x4ade80, 0.5);
        cam.drawCircle(sx + bw - 74, fy + 10, 1.5);
        cam.endFill();
        c.addChild(cam);
    },
    ...
    _drawPartnerOffices(c, sx, bw, pY, fy, fh, accent) {
        ...
        for (let i = 0; i < numOffices; i++) {
            ...
            og.eventMode = 'none';
            // Glass walls
            og.beginFill(0x1a2030, 0.4);
            og.drawRect(ox, fy + 14, 2, fh - 20);
            og.endFill();
            og.beginFill(accent, 0.06);
            og.drawRect(ox + 2, fy + 14, offW - 4, fh - 20);
            og.endFill();
            // Desk
            og.beginFill(0x1a1a2a);
            og.drawRect(ox + 10, pY - 16, offW * 0.5, 12);
            og.endFill();
            og.beginFill(0x222240);
            og.drawRect(ox + 12, pY - 14, offW * 0.5 - 4, 8);
            og.endFill();
            c.addChild(og);
            // Monitor
            ...
            ch.eventMode = 'none';
            ch.beginFill(0x333350);
            ch.drawRoundedRect(ox + 10 + offW * 0.25 - 5, pY - 22, 10, 8, 2);
            ch.endFill();
            c.addChild(ch);
            // Bookshelf
    ...
    _drawFundOps(c, sx, bw, pY, fy, fh, accent) {
        ...
        for (let dx = sx + 40; dx < sx + bw - 80; dx += 70) {
            ...
            dg.eventMode = 'none';
            dg.beginFill(0x1a1a2a);
            dg.drawRect(dx, pY - 16, 50, 12);
            dg.endFill();
            dg.beginFill(0x222240);
            dg.drawRect(dx + 2, pY - 14, 46, 8);
            dg.endFill();
            c.addChild(dg);
        ...
        for (let fx = sx + bw - 70; fx < sx + bw - 20; fx += 20) {
            ...
            fg.eventMode = 'none';
            fg.beginFill(0x222230);
            fg.drawRect(fx, pY - 35, 16, 35);
            fg.endFill();
            for (let dy = pY - 32; dy < pY - 5; dy += 8) {
                fg.beginFill(0x333344);
                fg.drawRect(fx + 2, dy, 12, 6);
                fg.endFill();
                fg.beginFill(0x94a3b8, 0.3);
                fg.drawCircle(fx + 8, dy + 3, 1);
                fg.endFill();
            }
            c.addChild(fg);
        }
        ...
        pr.eventMode = 'none';
        pr.beginFill(0x333344);
        pr.drawRect(sx + 20, pY - 15, 25, 15);
        pr.endFill();
        pr.beginFill(0x4ade80, 0.4);
        pr.drawCircle(sx + 40, pY - 10, 2);
        pr.endFill();
        c.addChild(pr);
    },
    ...
    _drawRooftopLounge(c, sx, bw, pY, fy, fh, accent) {
        ...
        bg.drawRect(barX, pY - 20, barW, 20);
        bg.endFill();
        bg.beginFill(accent, 0.1);
        bg.drawRect(barX, pY - 22, barW, 3);
        bg.endFill();
        c.addChild(bg);
        // Bottles
        ...
        for (let gx = sx + 30; gx < barX - 40; gx += 90) {
            ...
            tg.eventMode = 'none';
            tg.beginFill(0x1a1a2a);
            tg.drawRect(gx + 15, pY - 10, 40, 10);
            tg.endFill();
            tg.beginFill(accent, 0.06);
            tg.drawRect(gx + 17, pY - 8, 36, 6);
            tg.endFill();
            c.addChild(tg);
            ...
            sf.eventMode = 'none';
            sf.beginFill(0x222240);
            sf.drawRoundedRect(gx, pY - 14, 60, 10, 3);
            sf.endFill();
            sf.beginFill(0x2a2a50);
            sf.drawRoundedRect(gx - 4, pY - 18, 8, 18, 2);
            sf.drawRoundedRect(gx + 56, pY - 18, 8, 18, 2);
            sf.endFill();
            c.addChild(sf);
        }
    ...
    _drawTradingFloor(c, sx, bw, pY, fy, fh, accent) {
        ...
        for (let row = 0; row < 2; row++) {
            ...
            for (let dx = sx + 25; dx < sx + bw - 80; dx += 60) {
                // Desk
                g.beginFill(0x1a1a2a);
                g.drawRoundedRect(dx, ry - 10, 45, 10, 2);
                g.endFill();
                g.beginFill(0x222238);
                g.drawRect(dx + 2, ry - 8, 41, 2);
                g.endFill();
                // Phone/intercom
                g.beginFill(0x333344);
                g.drawRect(dx + 38, ry - 8, 5, 4);
                g.endFill();
            }
        }
        c.addChild(g);
        ...
        const tkG = new PIXI.Graphics();
        tkG.eventMode = 'none';
        tkG.beginFill(0x0a0a14);
        tkG.drawRect(tkX, fy + 10, tkW, 16);
        tkG.endFill();
        tkG.beginFill(0x111120);
        tkG.drawRect(tkX + 1, fy + 11, tkW - 2, 14);
        tkG.endFill();
        ...
        for (let i = 0; i < syms.length; i++) {
            ...
            tkG.beginFill(col, 0.5);
            tkG.drawRect(sx2, fy + 12, symW - 4, 4);
            tkG.endFill();
            // Mini sparkline
        ...
        // Clock on wall
        const clk = new PIXI.Graphics();
        clk.eventMode = 'none';
        clk.beginFill(0x222230);
        clk.drawCircle(sx + bw - 40, fy + 20, 8);
        clk.endFill();
        clk.beginFill(0x0a0a14);
        clk.drawCircle(sx + bw - 40, fy + 20, 6);
        clk.endFill();
        clk.beginFill(0xef4444, 0.6);
        clk.drawRect(sx + bw - 40, fy + 15, 1, 5);
        clk.endFill();
        clk.beginFill(0xffffff, 0.4);
        clk.drawRect(sx + bw - 43, fy + 20, 4, 1);
        clk.endFill();
        c.addChild(clk);
    },
    ...
    _drawCoworking(c, sx, bw, pY, fy, fh, accent) {
        ...
        for (let tx = sx + 30; tx < sx + bw - 80; tx += 100) {
            ...
            tg.eventMode = 'none';
            tg.beginFill(0x1a1a2a);
            tg.drawRect(tx, pY - 16, 80, 12);
            tg.endFill();
            tg.beginFill(0x222240);
            tg.drawRect(tx + 2, pY - 14, 76, 8);
            tg.endFill();
            c.addChild(tg);
            // Laptops on table
            for (let lx = tx + 10; lx < tx + 70; lx += 20) {
                ...
                lg.eventMode = 'none';
                lg.beginFill(0x333344);
                lg.drawRect(lx, pY - 20, 10, 6);
                lg.endFill();
                lg.beginFill(accent, 0.2);
                lg.drawRect(lx + 1, pY - 19, 8, 4);
                lg.endFill();
                c.addChild(lg);
            }
        }
        ...
        wb.eventMode = 'none';
        wb.beginFill(0xffffff, 0.08);
        wb.drawRect(wbx, fy + 16, 40, 28);
        wb.endFill();
        wb.lineStyle(1, accent, 0.3);
        wb.moveTo(wbx + 5, fy + 24);
        wb.lineTo(wbx + 35, fy + 28);
        wb.moveTo(wbx + 5, fy + 30);
        wb.lineTo(wbx + 25, fy + 34);
        wb.lineStyle(0);
        c.addChild(wb);
        // Coffee station
    ...
    _drawPitchStage(c, sx, bw, pY, fy, fh, accent) {
        ...
        const sg = new PIXI.Graphics();
        sg.eventMode = 'none';
        sg.beginFill(0x222230);
        sg.drawRect(stgX, pY - 8, stgW, 8);
        sg.endFill();
        sg.beginFill(0x2a2a40);
        sg.drawRect(stgX, pY - 10, stgW, 3);
        sg.endFill();
        // Stage lights
        for (let lx = stgX + 10; lx < stgX + stgW; lx += 20) {
            sg.beginFill([0xfbbf24, 0x4ade80, 0x22d3ee][Math.floor(lx / 20) % 3], 0.4);
            sg.drawCircle(lx, pY - 6, 2);
            sg.endFill();
        }
        c.addChild(sg);
        // Podium
        ...
        pod.eventMode = 'none';
        pod.beginFill(0x1a1a2a);
        pod.drawRect(sx + bw / 2 - 10, pY - 30, 20, 22);
        pod.endFill();
        pod.beginFill(accent, 0.15);
        pod.drawRect(sx + bw / 2 - 6, pY - 26, 12, 14);
        pod.endFill();
        c.addChild(pod);
        ...
        for (let rx = sx + 20; rx < stgX - 20; rx += 30) {
            const ch = new PIXI.Graphics();
            ch.eventMode = 'none';
            ch.beginFill(0x222240);
            ch.drawRoundedRect(rx, pY - 10, 20, 8, 2);
            ch.endFill();
            c.addChild(ch);
        }
    ...
    _drawVault(c, sx, bw, pY, fy, fh, accent) {
        ...
        vd.eventMode = 'none';
        vd.beginFill(0x555570);
        vd.drawRoundedRect(vdx, pY - 45, 60, 45, 6);
        vd.endFill();
        vd.beginFill(0x666688);
        vd.drawCircle(vdx + 30, pY - 22, 15);
        vd.endFill();
        vd.beginFill(0x777799);
        vd.drawCircle(vdx + 30, pY - 22, 10);
        vd.endFill();
        // Spokes
        for (let a = 0; a < Math.PI * 2; a += Math.PI / 3) {
            vd.lineStyle(2, 0x555570);
            vd.moveTo(vdx + 30, pY - 22);
            vd.lineTo(vdx + 30 + Math.cos(a) * 10, pY - 22 + Math.sin(a) * 10);
        }
        vd.lineStyle(0);
        vd.beginFill(0x333344);
        vd.drawCircle(vdx + 30, pY - 22, 3);
        vd.endFill();
        c.addChild(vd);
        ...
        for (const offX of [sx + 30, sx + bw - 70]) {
            for (let row = 0; row < 3; row++) {
                for (let col = 0; col < 3 - row; col++) {
                    ...
                    gb.eventMode = 'none';
                    gb.beginFill(0xfbbf24, 0.8);
                    gb.drawRect(gx, gy, 10, 6);
                    gb.endFill();
                    gb.beginFill(0xfcd34d, 0.6);
                    gb.drawRect(gx + 1, gy + 1, 8, 2);
                    gb.endFill();
                    c.addChild(gb);
                }
            }
        ...
        for (let bx = sx + 100; bx < sx + bw - 100; bx += 14) {
            for (let by = fy + 14; by < fy + 46; by += 10) {
                ...
                sb.eventMode = 'none';
                sb.beginFill(0x333344);
                sb.drawRect(bx, by, 12, 8);
                sb.endFill();
                sb.beginFill(0xfbbf24, 0.2);
                sb.drawCircle(bx + 6, by + 4, 1);
                sb.endFill();
                c.addChild(sb);
            }
        }
    ...
    _drawLegal(c, sx, bw, pY, fy, fh, accent) {
        ...
        for (let dx = sx + 40; dx < sx + bw - 80; dx += 80) {
            ...
            for (let s = 0; s < 3; s++) {
                ...
                doc.eventMode = 'none';
                doc.beginFill(0xffffff, 0.12 + s * 0.03);
                doc.drawRect(dx + 5, pY - 18 - s * 3, 12, 3);
                doc.endFill();
                c.addChild(doc);
            }
        }
    ...
    _drawBoardroom(c, sx, bw, pY, fy, fh, accent) {
        ...
        const tg = new PIXI.Graphics();
        tg.eventMode = 'none';
        tg.beginFill(0x2a1a10);
        tg.drawEllipse(tx + tw / 2, pY - 12, tw / 2, 8);
        tg.endFill();
        tg.beginFill(0x3a2a18);
        tg.drawEllipse(tx + tw / 2, pY - 12, tw / 2 - 3, 5);
        tg.endFill();
        c.addChild(tg);
        // High-back chairs
        for (let cx = tx + 15; cx < tx + tw - 10; cx += 22) {
            ...
            ch.eventMode = 'none';
            ch.beginFill(0x222240);
            ch.drawRoundedRect(cx, pY - 26, 12, 14, 2);
            ch.endFill();
            ch.beginFill(0x333350);
            ch.drawRect(cx + 2, pY - 24, 8, 4);
            ch.endFill();
            c.addChild(ch);
        }
        // Wall screen
        ...
        for (let nx = tx + 20; nx < tx + tw - 20; nx += 40) {
            const np = new PIXI.Graphics();
            np.eventMode = 'none';
            np.beginFill(0xffffff, 0.12);
            np.drawRect(nx, pY - 14, 6, 4);
            np.endFill();
            np.beginFill(0x22d3ee, 0.2);
            np.drawRect(nx + 10, pY - 15, 4, 6);
            np.endFill();
            c.addChild(np);
        }
    },
    ...
    _drawExecutive(c, sx, bw, pY, fy, fh, accent) {
        ...
        const dg = new PIXI.Graphics();
        dg.eventMode = 'none';
        dg.beginFill(0x2a1a10);
        dg.drawRoundedRect(dx, pY - 20, 80, 16, 4);
        dg.endFill();
        dg.beginFill(0x3a2a18);
        dg.drawRect(dx + 4, pY - 18, 72, 10);
        dg.endFill();
        c.addChild(dg);
        // Executive chair
        const ech = new PIXI.Graphics();
        ech.eventMode = 'none';
        ech.beginFill(0x333350);
        ech.drawRoundedRect(dx + 30, pY - 30, 20, 14, 3);
        ech.endFill();
        ech.beginFill(0x444468);
        ech.drawRoundedRect(dx + 32, pY - 28, 16, 4, 2);
        ech.endFill();
        c.addChild(ech);
        ...
        // Trophy case
        const tcx = sx + 30;
        const tc = new PIXI.Graphics();
        tc.eventMode = 'none';
        tc.beginFill(0x222230);
        tc.drawRect(tcx, pY - 40, 40, 40);
        tc.endFill();
        tc.beginFill(0x1a1a28, 0.5);
        tc.drawRect(tcx + 3, pY - 37, 34, 34);
        tc.endFill();
        // Trophies inside
        tc.beginFill(0xfbbf24);
        tc.drawRect(tcx + 10, pY - 28, 6, 10);
        tc.drawRect(tcx + 7, pY - 28, 12, 3);
        tc.endFill();
        tc.beginFill(0xc0c0c0);
        tc.drawRect(tcx + 22, pY - 24, 5, 8);
        tc.drawRect(tcx + 20, pY - 24, 9, 3);
        tc.endFill();
        c.addChild(tc);
        // Bookshelf
        ...
        const pv = new PIXI.Graphics();
        pv.eventMode = 'none';
        pv.beginFill(0x1a1a28);
        pv.drawRect(pvx, fy + 14, 60, 25);
        pv.endFill();
        pv.beginFill(0x0a1020);
        pv.drawRect(pvx + 2, fy + 16, 56, 21);
        pv.endFill();
        // Mini skyline
        for (let bx = pvx + 5; bx < pvx + 55; bx += 8) {
            const bh = 6 + Math.random() * 12;
            pv.beginFill(0x222240);
            pv.drawRect(bx, fy + 35 - bh, 5, bh);
            pv.endFill();
            pv.beginFill(0xfbbf24, 0.3);
            pv.drawRect(bx + 1, fy + 36 - bh, 1, 1);
            pv.endFill();
        }
        c.addChild(pv);
    ...
    _npc(c, x, y, name, col, wallLeft, wallRight) {
        ...
        if (typeof HumanAvatar !== 'undefined' && !HumanAvatar.isBot(name, vcNpcId)) {
            ...
            sh.drawEllipse(0, 2, bw * 0.6, 3);
            sh.endFill();
            legL = new PIXI.Graphics();
            legL.beginFill(0x1a1a28);
            legL.drawRect(-2, 0, 3, 4);
            legL.endFill();
            legL.x = -bw * 0.15;
            legR = new PIXI.Graphics();
            legR.beginFill(0x1a1a28);
            legR.drawRect(-1, 0, 3, 4);
            legR.endFill();
            legR.x = bw * 0.15;
            body = new PIXI.Graphics();
            body.beginFill(col);
            body.drawRoundedRect(-bw / 2, -h + 11, bw, 16, 2);
            body.endFill();
            body.beginFill(0x0f0f18);
            body.drawRect(-1, -h + 12, 2, 8);
            body.endFill();
            head = new PIXI.Graphics();
            head.beginFill(0xfdd8b5);
            head.drawRoundedRect(-bw * 0.4, 0, bw * 0.8, 11, 3);
            head.endFill();
            head.beginFill(0x2c1810);
            head.drawCircle(-bw * 0.1, 4, 1);
            head.drawCircle(bw * 0.1, 4, 1);
            head.endFill();
            head.beginFill(0x1a1008);
            head.drawRoundedRect(-bw * 0.42, -1, bw * 0.84, 4, 2);
            head.endFill();
            head.y = -h;
        ...
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined')
                UI.selectModel({
                    ...
                    role: name,
                    lab: 'other',
                    desc: name + ' at ' + bldName + '. Part of the financial district workforce.',
                });
        });
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined' && UI.showTooltip) UI.showTooltip(e, name, bldName);
        });
        cont.on('pointerout', () => {
        ...
        if (typeof G !== 'undefined' && G.tracking && G._addTrackHighlight) {
            ...
            if (hl) {
                vcAv._trackGlow = hl.glow;
                vcAv._trackArrow = hl.arrow;
            }
        }
    ...
    _monitor(c, x, y, accent) {
        ...
        mg.eventMode = 'none';
        mg.beginFill(0x111118);
        mg.drawRect(x - 5, y - 10, 10, 8);
        mg.endFill();
        mg.beginFill(accent, 0.15);
        mg.drawRect(x - 4, y - 9, 8, 6);
        mg.endFill();
        mg.beginFill(0x333344);
        mg.drawRect(x - 1, y - 2, 2, 2);
        mg.endFill();
        c.addChild(mg);
    ...
    _plant(c, x, y) {
        const pg = new PIXI.Graphics();
        pg.eventMode = 'none';
        pg.beginFill(0x3d2914);
        pg.drawRect(x - 4, y - 12, 8, 12);
        pg.endFill();
        pg.beginFill(0x4a3520);
        pg.drawRect(x - 5, y - 14, 10, 3);
        pg.endFill();
        pg.beginFill(0x2d6a4f);
        pg.drawCircle(x, y - 18, 8);
        pg.endFill();
        pg.beginFill(0x40916c);
        pg.drawCircle(x - 4, y - 20, 5);
        pg.drawCircle(x + 4, y - 20, 5);
        pg.endFill();
        c.addChild(pg);
    },
    ...
    _bookshelf(c, x, y, w, h) {
        ...
        for (let sy = y - h + 4; sy < y - 4; sy += 10) {
            for (let bx = x + 2; bx < x + w - 4; bx += 5) {
                ...
                bg.beginFill(bc, 0.5);
                bg.drawRect(bx, sy + 8 - bh, 3, bh);
                bg.endFill();
            }
            bg.beginFill(0x3a2a18);
            bg.drawRect(x, sy + 8, w, 2);
            bg.endFill();
        }
        c.addChild(bg);
    },
    ...
    _wallScreen(c, x, y, w, h, accent) {
        ...
        sg.drawRect(x, y, w, h);
        sg.endFill();
        sg.beginFill(accent, 0.06);
        sg.drawRect(x + 2, y + 2, w - 4, h - 4);
        sg.endFill();
        // Scan line effect
        for (let ly = y + 4; ly < y + h - 2; ly += 3) {
            sg.beginFill(accent, 0.03);
            sg.drawRect(x + 2, ly, w - 4, 1);
            sg.endFill();
        }
        c.addChild(sg);
    ...
    _coffeeMachine(c, x, y) {
        const cg = new PIXI.Graphics();
        cg.eventMode = 'none';
        cg.beginFill(0x333344);
        cg.drawRect(x, y - 22, 18, 22);
        cg.endFill();
        cg.beginFill(0x444458);
        cg.drawRect(x + 2, y - 18, 14, 10);
        cg.endFill();
        cg.beginFill(0xef4444, 0.5);
        cg.drawCircle(x + 14, y - 16, 2);
        cg.endFill();
        // Cup
        cg.beginFill(0xffffff, 0.3);
        cg.drawRect(x + 5, y - 8, 8, 6);
        cg.endFill();
        cg.beginFill(0x6b4226, 0.5);
        cg.drawRect(x + 6, y - 7, 6, 3);
        cg.endFill();
        c.addChild(cg);
    },
