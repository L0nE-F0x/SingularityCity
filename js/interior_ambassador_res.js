...
const InteriorAmbassadorRes = {
    ...
    build(bld, layer) {
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            rg.beginFill(0xc6bfa2);
            rg.drawRect(startX - 6, fy, 6, floorH);
            rg.drawRect(startX + bldW, fy, 6, floorH);
            rg.endFill();
            ...
            if (!isBasement) {
                ...
                rg.beginFill(country.trimCol);
                rg.drawRect(startX, fy, bldW, 4);
                rg.endFill();
                rg.beginFill(accent, 0.2);
                rg.drawRect(startX, fy + 4, bldW, 1);
                rg.endFill();
                // Baseboard at bottom
                rg.beginFill(country.trimCol, 0.8);
                rg.drawRect(startX, fy + floorH - 10, bldW, 4);
                rg.endFill();
            } else {
                // Vaulted cellar — stone arches
                for (let bx = startX + 20; bx < startX + bldW - 20; bx += 60) {
                    rg.beginFill(0x1d180e, 0.5);
                    rg.drawEllipse(bx + 25, fy + 12, 24, 10);
                    rg.endFill();
                    rg.beginFill(country.wallCol === 0x2a2418 ? 0x3a2f22 : 0x3a2f22, 0.35);
                    rg.drawEllipse(bx + 25, fy + 12, 20, 8);
                    rg.endFill();
                }
                // Stone brick pattern
                for (let bx = startX; bx < startX + bldW; bx += 20) {
                    rg.beginFill(0x1d180e, 0.45);
                    rg.drawRect(bx + 1, fy + 30, 18, 2);
                    rg.endFill();
                    rg.beginFill(0x1d180e, 0.45);
                    rg.drawRect(bx + 11, fy + 44, 18, 2);
                    rg.endFill();
                    rg.beginFill(0x1d180e, 0.45);
                    rg.drawRect(bx + 1, fy + 58, 18, 2);
                    rg.endFill();
                }
            }

            // Floor slab divider
            rg.beginFill(0xa8a288);
            rg.drawRect(startX, fy + floorH - 6, bldW, 6);
            rg.endFill();
            rg.beginFill(0x8a8268);
            rg.drawRect(startX - 6, fy + floorH - 3, bldW + 12, 3);
            rg.endFill();
            ...
            const fl = new PIXI.Text(floorDef.label, {
                ...
                fontSize: 7,
                fill: isBasement ? 0xc6bfa2 : accent,
                letterSpacing: 2,
                fontWeight: 'bold',
            });
            fl.anchor.set(0.5, 0);
        ...
        earth.beginFill(0x3a2b18);
        earth.drawRect(0, groundY, startX - 6, floorH);
        earth.drawRect(startX + bldW + 6, groundY, G.vpW - startX - bldW - 6, floorH);
        earth.endFill();
        earth.beginFill(0x523d22);
        earth.drawRect(0, groundY, startX - 6, 6);
        earth.drawRect(startX + bldW + 6, groundY, G.vpW - startX - bldW - 6, 6);
        earth.endFill();
        // Manicured lawn strip
        earth.beginFill(0x3d6e3b);
        earth.drawRect(0, groundY - 2, startX - 6, 4);
        earth.drawRect(startX + bldW + 6, groundY - 2, G.vpW - startX - bldW - 6, 4);
        earth.endFill();
        // Garden hedges
        for (let hx = 10; hx < startX - 10; hx += 14) {
            earth.beginFill(0x2d5e2b);
            earth.drawCircle(hx, groundY - 2, 5);
            earth.endFill();
        }
        ...
        this._onMove = (e) => {
            if (
                !InteriorAmbassadorRes.isDragging ||
                !InteriorAmbassadorRes.scene ||
                InteriorAmbassadorRes.scene.destroyed
            )
                return;
    ...
    _drawRoof(c, sx, bw, roofH, flagCols, accent, warm, country, bldName) {
        ...
        if (st === 'pagoda') {
            ...
            g.endFill();
            // Gold trim
            g.beginFill(warm, 0.85);
            g.drawRect(sx + 12, roofH - 22, bw - 24, 2);
            g.endFill();
            g.beginFill(warm, 0.85);
            g.drawRect(sx + 50, roofH - 44, bw - 100, 2);
            g.endFill();
            // Ridge tiles
            for (let tx = sx + 20; tx < sx + bw - 20; tx += 10) {
                g.beginFill(0x6a1a0a, 0.65);
                g.drawRect(tx, roofH - 14, 8, 2);
                g.endFill();
            }
            ...
            g.endFill();
            // Flat top cap
            g.beginFill(0x1a1a28);
            g.drawRect(sx + 14, roofH - 46, bw - 28, 4);
            g.endFill();
            // Dormers
            const numDormers = 3;
            ...
            for (let di = 0; di < numDormers; di++) {
                ...
                g.drawRect(dx - 7, roofH - 30, 14, 14);
                g.endFill();
                g.beginFill(0xffe9a8, 0.55);
                g.drawRect(dx - 5, roofH - 28, 10, 10);
                g.endFill();
                g.beginFill(country.trimCol);
            ...
            // Gold gilded cornice
            g.beginFill(warm, 0.75);
            g.drawRect(sx, roofH - 4, bw, 4);
            g.endFill();
        } else if (st === 'haveli') {
            // Sandstone dome + carved parapet with cupolas
            g.beginFill(0xd9a04f);
            g.drawRect(sx, roofH - 24, bw, 24);
            g.endFill();
            // Jali lattice band
            for (let jx = sx + 6; jx < sx + bw - 6; jx += 8) {
                g.beginFill(0xb8843a, 0.6);
                g.drawRect(jx, roofH - 20, 4, 12);
                g.endFill();
                g.beginFill(0xf5d7a8, 0.5);
                g.drawRect(jx + 1, roofH - 19, 2, 10);
                g.endFill();
            }
            // Cupola domes
            for (const cx of [sx + 30, sx + bw - 30]) {
                g.beginFill(0xd9a04f);
                g.drawEllipse(cx, roofH - 24, 12, 10);
                g.endFill();
                g.beginFill(warm, 0.85);
                g.drawRect(cx - 1, roofH - 40, 2, 6);
                g.endFill();
                g.beginFill(warm, 0.85);
                g.drawCircle(cx, roofH - 41, 2);
                g.endFill();
            }
            // Central larger dome
            g.beginFill(0xd9a04f);
            g.drawEllipse(sx + bw / 2, roofH - 24, 22, 16);
            g.endFill();
            g.beginFill(warm, 0.4);
            g.drawEllipse(sx + bw / 2, roofH - 28, 18, 10);
            g.endFill();
        } else if (st === 'arabian') {
            // Flat parapet with crescent moon finial + crenelated trim
            g.beginFill(country.wallCol);
            g.drawRect(sx, roofH - 20, bw, 20);
            g.endFill();
            // Crenelations
            for (let tx = sx; tx < sx + bw; tx += 16) {
                g.beginFill(country.wallCol);
                g.drawRect(tx, roofH - 28, 10, 8);
                g.endFill();
            }
            // Gold trim line
            g.beginFill(warm, 0.85);
            g.drawRect(sx, roofH - 22, bw, 2);
            g.endFill();
            // Crescent moon finial centre
            const cx = sx + bw / 2;
            g.beginFill(warm, 0.9);
            g.drawCircle(cx, roofH - 40, 4);
            g.endFill();
            g.beginFill(country.wallCol);
            g.drawCircle(cx + 1.5, roofH - 41, 3.5);
            g.endFill();
            ...
            for (const cx of [sx + bw * 0.25, sx + bw * 0.75]) {
                g.beginFill(0x6b3026);
                g.drawRect(cx - 5, roofH - 40, 10, 24);
                g.endFill();
                g.beginFill(0x4a1f18);
                g.drawRect(cx - 5, roofH - 40, 10, 2);
                g.endFill();
                // Smoke puffs
                g.beginFill(0xc6bfa2, 0.35);
                g.drawCircle(cx, roofH - 44, 2);
                g.drawCircle(cx + 2, roofH - 47, 2.5);
                g.endFill();
            }
        } else {
            ...
            g.moveTo(sx, roofH - 12);
            g.lineTo(sx + bw / 2, 10);
            g.lineTo(sx + bw, roofH - 12);
            g.lineStyle(0);
            // Red brick chimney
            g.beginFill(0x6b3026);
            g.drawRect(sx + bw * 0.75, roofH - 36, 10, 22);
            g.endFill();
            g.beginFill(0x4a1f18);
            g.drawRect(sx + bw * 0.75, roofH - 36, 10, 2);
            g.endFill();
            // Architrave band
            g.beginFill(country.trimCol);
            g.drawRect(sx, roofH - 12, bw, 12);
            g.endFill();
            g.beginFill(accent, 0.35);
            g.drawRect(sx, roofH - 12, bw, 3);
            g.endFill();
        }
        c.addChild(g);
        ...
        pole.drawRect(-1.5, -62, 3, 62);
        pole.endFill();
        pole.beginFill(0xfbbf24);
        pole.drawCircle(0, -62, 3);
        pole.endFill();
        ...
        const countryCode =
            (this.bld && this.bld.country) ||
            (this.bld && this.bld.id ? this.bld.id.replace('diplomat_villa_', '') : 'us');
    ...
    _drawResidenceWindow(g, x, y, w, h, accent, trimCol, style) {
        ...
        if (style === 'pagoda') {
            ...
            g.beginFill(0x8a2a1a);
            g.moveTo(x - 6, y - 4);
            g.quadraticCurveTo(x + w / 2, y - 12, x + w + 6, y - 4);
            g.lineTo(x + w + 4, y - 2);
            g.lineTo(x - 4, y - 2);
            g.closePath();
            g.endFill();
            ...
            g.endFill();
            // Carved brackets
            g.beginFill(0xb8843a);
            g.drawRect(x - 3, y + h - 6, 3, 6);
            g.drawRect(x + w, y + h - 6, 3, 6);
            g.endFill();
        ...
        g.beginFill(accent, 0.55);
        g.drawRect(x - 2, y, 4, h);
        g.endFill();
        g.beginFill(accent, 0.55);
        g.drawRect(x + w - 2, y, 4, h);
        g.endFill();
        g.beginFill(trimCol, 0.45);
        g.drawRect(x - 2, y, 4, 2);
        g.drawRect(x + w - 2, y, 4, 2);
        g.endFill();
    },
    ...
    _drawFloorProps(c, sx, bw, pY, fy, fh, key, accent, warm, country, flagCols) {
        ...
        if (key === 'reception') {
            ...
            g.beginFill(country.floorCol);
            g.drawRect(sx, pY - 4, bw, 4);
            g.endFill();
            for (let ix = sx + 10; ix < sx + bw - 10; ix += 28) {
                g.beginFill(country.trimCol, 0.3);
                g.drawRect(ix, pY - 4, 14, 4);
                g.endFill();
            }
            // Accent runner rug down the centre
            const rugW = bw * 0.58,
                rugX = sx + bw / 2 - rugW / 2;
            g.beginFill(accent, 0.65);
            g.drawRect(rugX, pY - 4, rugW, 4);
            g.endFill();
            g.beginFill(warm, 0.35);
            g.drawRect(rugX + 10, pY - 3, rugW - 20, 2);
            g.endFill();
            g.beginFill(country.trimCol, 0.5);
            g.drawRect(rugX + 16, pY - 2, rugW - 32, 1);
            g.endFill();
            ...
            const tW = bw * 0.46,
                tX = sx + bw / 2 - tW / 2;
            g.beginFill(0x3a1f10);
            g.drawRoundedRect(tX, pY - 20, tW, 18, 3);
            g.endFill();
            g.beginFill(0x4a2e18);
            g.drawRect(tX + 2, pY - 20, tW - 4, 3);
            g.endFill();
            // White linen tablecloth
            g.beginFill(0xfffbe8, 0.92);
            g.drawRect(tX + 4, pY - 19, tW - 8, 14);
            g.endFill();
            g.beginFill(accent, 0.3);
            g.drawRect(tX + 4, pY - 19, tW - 8, 1);
            g.endFill();
            c.addChild(g);
            ...
            for (let ps = 0; ps < placeSettings; ps++) {
                ...
                const ps_g = new PIXI.Graphics();
                ps_g.eventMode = 'none';
                // Plate
                ps_g.beginFill(0xfffbe8);
                ps_g.drawCircle(psX, pY - 12, 2.5);
                ps_g.endFill();
                ps_g.beginFill(country.trimCol, 0.8);
                ps_g.drawCircle(psX, pY - 12, 1.2);
                ps_g.endFill();
                ps_g.beginFill(accent, 0.6);
                ps_g.drawCircle(psX, pY - 12, 0.6);
                ps_g.endFill();
                // Wine glass
                ps_g.beginFill(0xcce6ff, 0.55);
                ps_g.drawRect(psX - 0.8, pY - 18, 1.6, 3);
                ps_g.endFill();
                c.addChild(ps_g);
            }
            ...
            for (let ci = 0; ci < 3; ci++) {
                ...
                cg.eventMode = 'none';
                cg.beginFill(0xc0c6d0);
                cg.drawRect(cX - 1, pY - 14, 2, 6);
                cg.endFill();
                cg.beginFill(0xc0c6d0);
                cg.drawRect(cX - 4, pY - 20, 8, 2);
                cg.endFill();
                // Candles
                for (let cand = -1; cand <= 1; cand++) {
                    cg.beginFill(0xfffbe8);
                    cg.drawRect(cX + cand * 3 - 0.5, pY - 24, 1, 4);
                    cg.endFill();
                    // Flame
                    cg.beginFill(0xfbbf24, 0.9);
                    cg.drawEllipse(cX + cand * 3, pY - 26, 0.9, 1.4);
                    cg.endFill();
                    cg.beginFill(0xffe9a8, 0.7);
                    cg.drawEllipse(cX + cand * 3, pY - 26, 0.5, 1);
                    cg.endFill();
                }
                c.addChild(cg);
            ...
            for (let ci = 0; ci < 6; ci++) {
                ...
                ch.eventMode = 'none';
                // Seat
                ch.beginFill(0x3a1f10);
                ch.drawRect(cOff, pY - 6, 10, 6);
                ch.endFill();
                // High back
                ch.beginFill(0x4a2e18);
                ch.drawRect(cOff + 1, pY - 20, 8, 14);
                ch.endFill();
                ch.beginFill(accent, 0.75);
                ch.drawRect(cOff + 2, pY - 12, 6, 4);
                ch.endFill();
                c.addChild(ch);
            }

            // ─── Grand chandelier above table ───
            const ch2 = new PIXI.Graphics();
            ch2.eventMode = 'none';
            const chX = sx + bw / 2;
            ch2.beginFill(0x1a1a28);
            ch2.drawRect(chX - 0.5, fy + 8, 1, 16);
            ch2.endFill();
            ch2.beginFill(country.trimCol);
            ch2.drawCircle(chX, fy + 28, 8);
            ch2.endFill();
            ch2.beginFill(0xffe9a8, 0.85);
            ch2.drawCircle(chX, fy + 28, 5.5);
            ch2.endFill();
            // 8 crystal arms
            for (let arm = 0; arm < 8; arm++) {
                ...
                ch2.beginFill(country.trimCol);
                ch2.drawCircle(ax, ay, 2);
                ch2.endFill();
                ch2.beginFill(0xffe9a8, 0.8);
                ch2.drawCircle(ax, ay, 1.2);
                ch2.endFill();
                // Crystal droplet
                ch2.beginFill(0xcce6ff, 0.7);
                ch2.drawPolygon([ax, ay + 2, ax - 1, ay + 5, ax + 1, ay + 5]);
                ch2.endFill();
            }
            c.addChild(ch2);
            ...
            fp.eventMode = 'none';
            // Marble mantle
            fp.beginFill(country.trimCol);
            fp.drawRect(fpX - 4, fpY - 4, fpW + 8, 4);
            fp.endFill();
            fp.beginFill(0xe9e4d2);
            fp.drawRect(fpX, fpY, fpW, 34);
            fp.endFill();
            // Hearth opening
            fp.beginFill(0x1a0a04);
            fp.drawRect(fpX + 12, fpY + 8, fpW - 24, 22);
            fp.endFill();
            // Fire (only at night)
            fp.beginFill(0xfbbf24, 0.85);
            fp.drawEllipse(fpX + fpW / 2, fpY + 24, 8, 5);
            fp.endFill();
            fp.beginFill(0xbf0a30, 0.75);
            fp.drawEllipse(fpX + fpW / 2, fpY + 26, 5, 3);
            fp.endFill();
            fp.beginFill(0xffe9a8, 0.7);
            fp.drawEllipse(fpX + fpW / 2, fpY + 25, 3, 2);
            fp.endFill();
            // Logs
            fp.beginFill(0x3a1f10);
            fp.drawRect(fpX + 16, fpY + 27, 6, 3);
            fp.drawRect(fpX + 26, fpY + 26, 6, 3);
            fp.drawRect(fpX + 36, fpY + 27, 6, 3);
            fp.endFill();
            c.addChild(fp);
            ...
            port.eventMode = 'none';
            const portY = fy + 14;
            port.beginFill(country.trimCol);
            port.drawRect(fpX + 6, portY, fpW - 12, 32);
            port.endFill();
            port.beginFill(0x4a2e18);
            port.drawRect(fpX + 8, portY + 2, fpW - 16, 28);
            port.endFill();
            port.beginFill(accent, 0.45);
            port.drawRect(fpX + 10, portY + 4, fpW - 20, 24);
            port.endFill();
            // Silhouetted figure
            port.beginFill(0x1a1a28);
            port.drawEllipse(fpX + fpW / 2, portY + 14, 5, 6);
            port.endFill();
            port.beginFill(0x1a1a28);
            port.drawRect(fpX + fpW / 2 - 7, portY + 18, 14, 10);
            port.endFill();
            c.addChild(port);
            ...
            const cnX = sx + bw - 40,
                cnY = pY;
            const cn = new PIXI.Graphics();
            cn.eventMode = 'none';
            cn.beginFill(0x3a1f10);
            cn.drawRect(cnX, cnY - 16, 30, 16);
            cn.endFill();
            cn.beginFill(country.trimCol, 0.85);
            cn.drawRect(cnX, cnY - 16, 30, 2);
            cn.endFill();
            // Vase
            cn.beginFill(accent);
            cn.drawRect(cnX + 12, cnY - 22, 6, 6);
            cn.endFill();
            cn.beginFill(country.trimCol, 0.7);
            cn.drawRect(cnX + 12, cnY - 22, 6, 1);
            cn.endFill();
            // Flowers
            for (let fi = 0; fi < 5; fi++) {
                const fx = cnX + 10 + fi * 2.5;
                cn.beginFill(warm, 0.9);
                cn.drawCircle(fx, cnY - 26 - (fi % 2) * 2, 1.2);
                cn.endFill();
                cn.beginFill(0xfbbf24, 0.5);
                cn.drawCircle(fx, cnY - 26 - (fi % 2) * 2, 0.5);
                cn.endFill();
            }
            c.addChild(cn);
        } else if (key === 'study') {
            ...
            for (let ix = sx; ix < sx + bw; ix += 16) {
                ...
                g.drawRect(ix, pY - 4, 1, 4);
                g.endFill();
            }
            // Ornate rug
            const rugW = bw * 0.64,
                rugX = sx + bw / 2 - rugW / 2;
            g.beginFill(0x5a1a0a);
            g.drawRect(rugX, pY - 4, rugW, 4);
            g.endFill();
            g.beginFill(warm, 0.55);
            g.drawRect(rugX + 6, pY - 3, rugW - 12, 2);
            g.endFill();
            g.beginFill(country.trimCol, 0.5);
            g.drawRect(rugX + 12, pY - 2, rugW - 24, 1);
            g.endFill();
            c.addChild(g);
            ...
            bs.eventMode = 'none';
            bs.beginFill(0x3a1f10);
            bs.drawRect(bsX, bsY, bsW, bsH);
            bs.endFill();
            bs.beginFill(0x5a3d24);
            bs.drawRect(bsX, bsY, bsW, 3);
            bs.endFill();
            bs.beginFill(0x5a3d24);
            bs.drawRect(bsX, bsY + bsH - 3, bsW, 3);
            bs.endFill();
            ...
            for (let row = 0; row < shelves; row++) {
                ...
                // Shelf divider
                bs.beginFill(0x5a3d24);
                bs.drawRect(bsX, rowY + shelfH - 2, bsW, 1);
                bs.endFill();
                for (let col = 0; col < Math.floor(bsW / 4.5); col++) {
                    ...
                    bs.beginFill(bkCol);
                    bs.drawRect(bkX, rowY + (shelfH - 2 - bkH), 3.5, bkH);
                    bs.endFill();
                    bs.beginFill(country.trimCol, 0.3);
                    bs.drawRect(bkX, rowY + (shelfH - 2 - bkH), 3.5, 0.8);
                    bs.endFill();
                }
            }
            // Ladder on rail
            bs.beginFill(country.trimCol, 0.75);
            bs.drawRect(bsX + bsW * 0.6, bsY + 4, 2, bsH - 8);
            bs.endFill();
            bs.beginFill(0x5a3d24);
            bs.drawRect(bsX + bsW * 0.6 - 3, bsY + 6, 8, 2);
            bs.endFill();
            c.addChild(bs);
            ...
            dk.eventMode = 'none';
            dk.beginFill(0x3a1f10);
            dk.drawRect(dX, pY - 22, dW, 22);
            dk.endFill();
            dk.beginFill(0x4a2e18);
            dk.drawRect(dX, pY - 22, dW, 3);
            dk.endFill();
            // Green leather writing surface
            dk.beginFill(0x14532d);
            dk.drawRect(dX + 4, pY - 22, dW - 8, 4);
            dk.endFill();
            dk.beginFill(country.trimCol, 0.5);
            dk.drawRect(dX + 4, pY - 22, dW - 8, 1);
            dk.endFill();
            // Drawer handles
            dk.beginFill(country.trimCol);
            dk.drawCircle(dX + 10, pY - 10, 0.8);
            dk.drawCircle(dX + dW - 10, pY - 10, 0.8);
            dk.endFill();
            // Brass banker's lamp
            dk.beginFill(country.trimCol);
            dk.drawRect(dX + 8, pY - 30, 2, 8);
            dk.endFill();
            dk.beginFill(0x14532d);
            dk.drawRect(dX + 2, pY - 34, 14, 4);
            dk.endFill();
            dk.beginFill(country.trimCol, 0.75);
            dk.drawRect(dX + 4, pY - 32, 10, 1);
            dk.endFill();
            dk.beginFill(0xffe9a8, 0.8);
            dk.drawRect(dX + 4, pY - 30, 10, 2);
            dk.endFill();
            // Papers + quill
            dk.beginFill(0xfffbe8);
            dk.drawRect(dX + 22, pY - 22, 16, 10);
            dk.endFill();
            dk.beginFill(accent, 0.45);
            dk.drawRect(dX + 24, pY - 20, 12, 1);
            dk.endFill();
            dk.beginFill(accent, 0.45);
            dk.drawRect(dX + 24, pY - 18, 12, 1);
            dk.endFill();
            dk.beginFill(accent, 0.45);
            dk.drawRect(dX + 24, pY - 16, 12, 1);
            dk.endFill();
            dk.beginFill(0x1a1a28);
            dk.drawRect(dX + 42, pY - 24, 2, 12);
            dk.endFill();
            // Ink pot
            dk.beginFill(0x1a0a04);
            dk.drawRect(dX + 48, pY - 18, 4, 6);
            dk.endFill();
            dk.beginFill(country.trimCol);
            dk.drawRect(dX + 48, pY - 18, 4, 1);
            dk.endFill();
            c.addChild(dk);
            ...
            const chX = dX + dW / 2 - 8;
            ch.beginFill(0x3a1f10);
            ch.drawRoundedRect(chX, pY - 36, 16, 36, 3);
            ch.endFill();
            ch.beginFill(0x4a2e18);
            ch.drawRect(chX, pY - 36, 16, 3);
            ch.endFill();
            ch.beginFill(0x1a0a04);
            ch.drawRect(chX + 6, pY - 4, 4, 4);
            ch.endFill();
            c.addChild(ch);
            ...
            gb.eventMode = 'none';
            gb.beginFill(0x3a1f10);
            gb.drawRect(gbX - 6, gbY, 12, 6);
            gb.endFill();
            gb.beginFill(country.trimCol);
            gb.drawRect(gbX - 1, gbY - 4, 2, 4);
            gb.endFill();
            gb.beginFill(0x2a4a6e);
            gb.drawCircle(gbX, gbY - 14, 8);
            gb.endFill();
            gb.beginFill(0x3d6e3b, 0.8);
            gb.drawEllipse(gbX - 2, gbY - 14, 3, 2);
            gb.drawEllipse(gbX + 3, gbY - 16, 2.5, 1.5);
            gb.endFill();
            gb.lineStyle(0.5, country.trimCol, 0.75);
            gb.drawCircle(gbX, gbY - 14, 8);
            gb.lineStyle(0);
            c.addChild(gb);

            // ─── Leather Chesterfield reading chair in corner ───
            const rc = new PIXI.Graphics();
            rc.eventMode = 'none';
            ...
            sT.eventMode = 'none';
            sT.beginFill(0x3a1f10);
            sT.drawRect(cX - 6, cY - 10, 14, 10);
            sT.endFill();
            sT.beginFill(country.trimCol, 0.7);
            sT.drawRect(cX - 6, cY - 10, 14, 1);
            sT.endFill();
            // Decanter
            sT.beginFill(0xfbbf24, 0.85);
            sT.drawRect(cX - 2, cY - 18, 4, 8);
            sT.endFill();
            sT.beginFill(country.trimCol);
            sT.drawRect(cX - 1, cY - 20, 2, 2);
            sT.endFill();
            // Tumbler
            sT.beginFill(0xfbbf24, 0.7);
            sT.drawRect(cX + 3, cY - 15, 3, 5);
            sT.endFill();
            c.addChild(sT);
        } else if (key === 'master') {
            // ─── Plush carpet ───
            ...
            for (let ix = sx; ix < sx + bw; ix += 20) {
                g.beginFill(country.trimCol, 0.18);
                g.drawRect(ix, pY - 4, 1, 4);
                g.endFill();
            }
            ...
            g.drawRect(rugX, pY - 4, rugW, 4);
            g.endFill();
            g.beginFill(warm, 0.5);
            g.drawRect(rugX + 8, pY - 3, rugW - 16, 2);
            g.endFill();
            g.beginFill(country.trimCol, 0.5);
            g.drawRect(rugX + 14, pY - 2, rugW - 28, 1);
            g.endFill();
            ...
            // Base frame
            g.beginFill(0x3a1f10);
            g.drawRect(bdX, bdY - 4, bdW, 4);
            g.endFill();
            // Mattress
            g.beginFill(0xfffbe8);
            g.drawRect(bdX + 2, bdY - 16, bdW - 4, 12);
            g.endFill();
            g.beginFill(country.trimCol, 0.3);
            g.drawRect(bdX + 2, bdY - 16, bdW - 4, 1);
            g.endFill();
            // Duvet (folded over)
            g.beginFill(accent, 0.8);
            g.drawRect(bdX + 2, bdY - 16, bdW - 4, 6);
            g.endFill();
            g.beginFill(country.trimCol, 0.45);
            g.drawRect(bdX + 2, bdY - 11, bdW - 4, 1);
            g.endFill();
            // Four posters
            for (const postX of [bdX, bdX + bdW - 3]) {
                g.beginFill(0x3a1f10);
                g.drawRect(postX, bdY - 44, 3, 44);
                g.endFill();
                g.beginFill(country.trimCol);
                g.drawCircle(postX + 1.5, bdY - 46, 2);
                g.endFill();
            }
            // Canopy cross beam
            g.beginFill(0x3a1f10);
            g.drawRect(bdX, bdY - 44, bdW, 3);
            g.endFill();
            // Canopy fabric drape
            g.beginFill(accent, 0.5);
            g.drawRect(bdX, bdY - 41, 4, 14);
            g.drawRect(bdX + bdW - 4, bdY - 41, 4, 14);
            g.endFill();
            g.beginFill(country.trimCol, 0.3);
            g.drawRect(bdX + 2, bdY - 41, bdW - 4, 3);
            g.endFill();
            // Headboard
            g.beginFill(0x3a1f10);
            g.drawRect(bdX, bdY - 28, bdW, 12);
            g.endFill();
            g.beginFill(accent, 0.6);
            g.drawRect(bdX + 2, bdY - 26, bdW - 4, 8);
            g.endFill();
            g.beginFill(country.trimCol, 0.45);
            g.drawRect(bdX + 4, bdY - 24, bdW - 8, 1);
            g.endFill();
            c.addChild(g);

            // Pillows (2, always shown)
            const pwG = new PIXI.Graphics();
            pwG.eventMode = 'none';
            pwG.beginFill(0xfffbe8);
            pwG.drawRoundedRect(bdX + 4, bdY - 20, (bdW - 10) / 2, 6, 2);
            pwG.endFill();
            pwG.beginFill(0xfffbe8);
            pwG.drawRoundedRect(bdX + 6 + (bdW - 10) / 2, bdY - 20, (bdW - 10) / 2, 6, 2);
            pwG.endFill();
            pwG.beginFill(country.trimCol, 0.4);
            pwG.drawRect(bdX + 6, bdY - 20, (bdW - 14) / 2, 1);
            pwG.endFill();
            c.addChild(pwG);
            ...
            for (const nsX of [bdX - 20, bdX + bdW + 6]) {
                ...
                ns.eventMode = 'none';
                ns.beginFill(0x3a1f10);
                ns.drawRect(nsX, pY - 18, 16, 18);
                ns.endFill();
                ns.beginFill(0x4a2e18);
                ns.drawRect(nsX, pY - 18, 16, 3);
                ns.endFill();
                // Drawer handle
                ns.beginFill(country.trimCol);
                ns.drawCircle(nsX + 8, pY - 10, 0.8);
                ns.endFill();
                // Alarm clock
                ns.beginFill(0x1a1a28);
                ns.drawRect(nsX + 2, pY - 24, 6, 6);
                ns.endFill();
                ns.beginFill(warm, 0.85);
                ns.drawRect(nsX + 3, pY - 22, 4, 2);
                ns.endFill();
                // Table lamp
                ns.beginFill(country.trimCol);
                ns.drawRect(nsX + 12, pY - 24, 2, 6);
                ns.endFill();
                ns.beginFill(accent, 0.7);
                ns.drawPolygon([nsX + 10, pY - 28, nsX + 14, pY - 28, nsX + 16, pY - 24, nsX + 8, pY - 24]);
                ns.endFill();
                ns.beginFill(0xffe9a8, 0.5);
                ns.drawRect(nsX + 10, pY - 26, 4, 2);
                ns.endFill();
                c.addChild(ns);
            ...
            wb.eventMode = 'none';
            wb.beginFill(0x3a1f10);
            wb.drawRect(wbX, wbY, 48, 44);
            wb.endFill();
            wb.beginFill(0x4a2e18);
            wb.drawRect(wbX, wbY, 48, 4);
            wb.endFill();
            // Two doors with mirror panels
            wb.beginFill(country.trimCol, 0.4);
            wb.drawRect(wbX + 4, wbY + 6, 18, 32);
            wb.endFill();
            wb.beginFill(country.trimCol, 0.4);
            wb.drawRect(wbX + 26, wbY + 6, 18, 32);
            wb.endFill();
            wb.beginFill(country.trimCol);
            wb.drawCircle(wbX + 22, wbY + 22, 0.8);
            wb.drawCircle(wbX + 26, wbY + 22, 0.8);
            wb.endFill();
            c.addChild(wb);
            ...
            const vX = sx + bw - 66,
                vY = pY;
            const vn = new PIXI.Graphics();
            vn.eventMode = 'none';
            vn.beginFill(0x3a1f10);
            vn.drawRect(vX, vY - 18, 42, 18);
            vn.endFill();
            vn.beginFill(0x4a2e18);
            vn.drawRect(vX, vY - 18, 42, 3);
            vn.endFill();
            // Oval mirror
            vn.beginFill(country.trimCol);
            vn.drawEllipse(vX + 21, vY - 32, 14, 12);
            vn.endFill();
            vn.beginFill(0xcce6ff, 0.7);
            vn.drawEllipse(vX + 21, vY - 32, 12, 10);
            vn.endFill();
            // Perfume bottles
            vn.beginFill(warm, 0.85);
            vn.drawRect(vX + 4, vY - 22, 2, 4);
            vn.endFill();
            vn.beginFill(accent, 0.85);
            vn.drawRect(vX + 8, vY - 22, 2, 4);
            vn.endFill();
            vn.beginFill(country.trimCol, 0.85);
            vn.drawRect(vX + 12, vY - 22, 2, 4);
            vn.endFill();
            // Vanity stool
            vn.beginFill(0x3a1f10);
            vn.drawRect(vX + 16, vY - 6, 10, 6);
            vn.endFill();
            vn.beginFill(accent, 0.6);
            vn.drawRect(vX + 16, vY - 6, 10, 2);
            vn.endFill();
            c.addChild(vn);

            // ─── Framed art above bed ───
            const fArtX = bdX + bdW / 2 - 22,
                fArtY = fy + 10;
            const fa = new PIXI.Graphics();
            fa.eventMode = 'none';
            fa.beginFill(country.trimCol);
            fa.drawRect(fArtX, fArtY, 44, 22);
            fa.endFill();
            fa.beginFill(0x4a2e18);
            fa.drawRect(fArtX + 2, fArtY + 2, 40, 18);
            fa.endFill();
            fa.beginFill(accent, 0.4);
            fa.drawRect(fArtX + 4, fArtY + 4, 36, 14);
            fa.endFill();
            // Abstract flourish
            fa.beginFill(warm, 0.65);
            fa.drawRect(fArtX + 8, fArtY + 8, 14, 2);
            fa.endFill();
            fa.beginFill(country.trimCol, 0.6);
            fa.drawRect(fArtX + 22, fArtY + 12, 14, 2);
            fa.endFill();
            c.addChild(fa);
            ...
            cp.eventMode = 'none';
            const cpX = sx + bw / 2;
            cp.beginFill(country.trimCol, 0.35);
            cp.drawCircle(cpX, fy + 6, 10);
            cp.endFill();
            cp.beginFill(0x1a1a28);
            cp.drawRect(cpX - 0.5, fy + 6, 1, 20);
            cp.endFill();
            cp.beginFill(country.trimCol);
            cp.drawEllipse(cpX, fy + 30, 10, 6);
            cp.endFill();
            cp.beginFill(0xffe9a8, 0.8);
            cp.drawEllipse(cpX, fy + 30, 7, 4);
            cp.endFill();
            c.addChild(cp);
            this.indoorLights.push({ g: cp, maxA: 1, type: 'pendant' });
        } else if (key === 'cellar') {
            // ─── Stone floor ───
            g.beginFill(0x1a140a);
            g.drawRect(sx, pY - 4, bw, 4);
            g.endFill();
            for (let ix = sx; ix < sx + bw; ix += 30) {
                g.beginFill(0x2a1f12, 0.7);
                g.drawRect(ix, pY - 4, 28, 1);
                g.endFill();
            }
            ...
            for (let ri = 0; ri < Math.min(6, rackCount); ri++) {
                ...
                g.beginFill(0x3a1f10);
                g.drawRect(rkX, fy + 14, rkW, rkH);
                g.endFill();
                g.beginFill(0x4a2e18);
                g.drawRect(rkX, fy + 14, rkW, 3);
                g.endFill();
            ...
            tb.eventMode = 'none';
            tb.beginFill(0x3a1f10);
            tb.drawRect(tbX, tbY, 80, 12);
            tb.endFill();
            tb.beginFill(0x4a2e18);
            tb.drawRect(tbX, tbY, 80, 2);
            tb.endFill();
            // Wine decanter
            tb.beginFill(0x5a1a0a, 0.85);
            tb.drawRect(tbX + 12, tbY - 12, 6, 12);
            tb.endFill();
            tb.beginFill(0x3a0a04);
            tb.drawRect(tbX + 12, tbY - 14, 6, 2);
            tb.endFill();
            // Wine glasses
            for (let gi = 0; gi < 3; gi++) {
                const gX = tbX + 28 + gi * 10;
                tb.beginFill(0xcce6ff, 0.55);
                tb.drawRect(gX, tbY - 10, 3, 6);
                tb.endFill();
                tb.beginFill(0x5a1a0a, 0.7);
                tb.drawRect(gX, tbY - 8, 3, 3);
                tb.endFill();
                tb.beginFill(0xcce6ff, 0.4);
                tb.drawRect(gX, tbY - 4, 3, 1);
                tb.endFill();
                tb.beginFill(0xcce6ff, 0.55);
                tb.drawRect(gX + 1, tbY - 3, 1, 4);
                tb.endFill();
            }
            // Cheese board
            tb.beginFill(0x7c5a30);
            tb.drawRect(tbX + 60, tbY - 4, 16, 3);
            tb.endFill();
            tb.beginFill(0xffe9a8);
            tb.drawRect(tbX + 62, tbY - 7, 4, 3);
            tb.endFill();
            tb.beginFill(country.trimCol);
            tb.drawRect(tbX + 68, tbY - 6, 3, 2);
            tb.endFill();
            c.addChild(tb);
            ...
            sf.eventMode = 'none';
            sf.beginFill(0x1a1a28);
            sf.drawRect(sfX, sfY, 40, 40);
            sf.endFill();
            sf.beginFill(0x2a2a38);
            sf.drawRect(sfX + 2, sfY + 2, 36, 36);
            sf.endFill();
            // Dial
            sf.beginFill(country.trimCol);
            sf.drawCircle(sfX + 20, sfY + 20, 7);
            sf.endFill();
            sf.beginFill(0x1a1a28);
            sf.drawCircle(sfX + 20, sfY + 20, 5);
            sf.endFill();
            sf.beginFill(country.trimCol);
            sf.drawRect(sfX + 19.5, sfY + 14, 1, 6);
            sf.endFill();
            // Handle
            sf.beginFill(country.trimCol);
            sf.drawRect(sfX + 28, sfY + 18, 10, 4);
            sf.endFill();
            c.addChild(sf);
            ...
            sfLabel.anchor.set(0.5, 0);
            sfLabel.x = sfX + 20;
            sfLabel.y = sfY + 42;
            c.addChild(sfLabel);

            // ─── Bare bulb hanging ───
            const bu = new PIXI.Graphics();
            bu.eventMode = 'none';
            bu.beginFill(0x1a1a28);
            bu.drawRect(sx + bw / 2 - 0.5, fy + 6, 1, 12);
            bu.endFill();
            bu.beginFill(country.trimCol);
            bu.drawCircle(sx + bw / 2, fy + 22, 4);
            bu.endFill();
            bu.beginFill(0xffe9a8, 0.75);
            bu.drawCircle(sx + bw / 2, fy + 22, 2.5);
            bu.endFill();
            c.addChild(bu);
    ...
    _drawOrnamental(c, x, pY, country) {
        ...
        // Pot
        g.beginFill(country.trimCol);
        g.drawRect(x - 5, pY - 10, 10, 10);
        g.endFill();
        g.beginFill(country.accent, 0.4);
        g.drawRect(x - 5, pY - 10, 10, 2);
        g.endFill();

        if (st === 'pagoda') {
            // Bonsai tree
            g.beginFill(0x3a2a18);
            g.drawRect(x - 1, pY - 18, 2, 8);
            g.endFill();
            g.beginFill(0x166534, 0.85);
            g.drawEllipse(x - 4, pY - 20, 6, 3);
            g.drawEllipse(x + 4, pY - 20, 6, 3);
            g.drawEllipse(x, pY - 24, 5, 2.5);
            g.endFill();
            g.beginFill(country.warm, 0.5);
            g.drawCircle(x - 3, pY - 20, 0.6);
            g.drawCircle(x + 3, pY - 22, 0.6);
            g.endFill();
        } else if (st === 'haveli' || st === 'arabian') {
            // Tall palm
            g.beginFill(0x3a2a18);
            g.drawRect(x - 1, pY - 28, 2, 18);
            g.endFill();
            g.beginFill(0x166534, 0.9);
            g.drawEllipse(x - 8, pY - 30, 10, 3);
            g.drawEllipse(x + 8, pY - 30, 10, 3);
            g.drawEllipse(x - 6, pY - 34, 8, 2.5);
            g.drawEllipse(x + 6, pY - 34, 8, 2.5);
            g.drawEllipse(x, pY - 36, 3, 6);
            g.endFill();
            if (st === 'haveli') {
                g.beginFill(country.warm, 0.7);
                g.drawCircle(x - 2, pY - 32, 0.5);
                g.drawCircle(x + 3, pY - 34, 0.5);
                g.endFill();
            }
        } else {
            // Classical fern / ficus
            g.beginFill(0x3a2a18);
            g.drawRect(x - 1, pY - 22, 2, 12);
            g.endFill();
            g.beginFill(0x2d5e2b);
            g.drawEllipse(x, pY - 28, 8, 8);
            g.endFill();
            g.beginFill(0x3d7e3b, 0.7);
            g.drawEllipse(x - 2, pY - 30, 5, 4);
            g.drawEllipse(x + 3, pY - 29, 4, 3);
            g.endFill();
        }
        c.addChild(g);
    },
    ...
    _drawSleepingAmbassador(c, bedRect, accent, country) {
        ...
        sleeper.beginFill(0xfdd8b5);
        sleeper.drawCircle(bx + 10, by + 3, 4);
        sleeper.endFill();
        // Hair
        sleeper.beginFill(0x2c1810, 0.75);
        sleeper.drawEllipse(bx + 10, by + 1, 4, 1.8);
        sleeper.endFill();
        // Closed eyes
        sleeper.beginFill(0x2c1810, 0.55);
        sleeper.drawRect(bx + 7.5, by + 3, 1.8, 0.8);
        sleeper.drawRect(bx + 11, by + 3, 1.8, 0.8);
        sleeper.endFill();
        // Peaceful smile
        sleeper.beginFill(0x2c1810, 0.25);
        sleeper.drawRect(bx + 9, by + 5.5, 2.5, 0.6);
        sleeper.endFill();
        // Blanket hump over body (country-accent coloured duvet)
        sleeper.beginFill(accent, 0.82);
        sleeper.moveTo(bx + 14, by + 10);
        sleeper.lineTo(bx + 14, by + 5);
        sleeper.quadraticCurveTo(bx + bw / 2, by - 4, bx + bw - 4, by + 4);
        sleeper.lineTo(bx + bw - 4, by + 10);
        sleeper.closePath();
        sleeper.endFill();
        // Blanket highlight
        sleeper.beginFill(country.trimCol, 0.35);
        sleeper.moveTo(bx + 15, by + 8);
        sleeper.lineTo(bx + 15, by + 6);
        sleeper.quadraticCurveTo(bx + bw / 2, by - 2, bx + bw - 5, by + 6);
        sleeper.lineTo(bx + bw - 5, by + 8);
        sleeper.closePath();
        sleeper.endFill();

        const sleeperCont = new PIXI.Container();
        sleeperCont.addChild(sleeper);
        sleeperCont.eventMode = 'static';
        sleeperCont.cursor = 'pointer';
            ...
            if (typeof UI !== 'undefined' && UI.selectModel) {
                UI.selectModel({
                    ...
                    desc:
                        ...
                        ' of ' +
                        country.full +
                        '. Currently sleeping in their private residence.',
                });
            }
        });
        sleeperCont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, _nm, _rl);
        });
        sleeperCont.on('pointerout', () => {
    ...
    drawNPC(c, x, y, role, col, country) {
        ...
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined' && UI.selectModel) {
                const desc =
                    role +
                    '. ' +
                    (country ? country.full + " ambassador's residence." : 'Ambassador residence staff.');
                UI.selectModel({
                    id:
                        'amb_res_' +
                        role.replace(/\s/g, '_').toLowerCase() +
                        '_' +
                        (country ? country.full.split(' ')[0].toLowerCase() : 'x'),
                    name: role,
                    isNPC: true,
                    _trackType: 'npc',
                    role,
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined' && UI.showTooltip)
                UI.showTooltip(e, role, country ? country.full : 'Residence');
        });
        cont.on('pointerout', () => {
        ...
        const avObj = {
            ...
            tag,
            state: 'working',
            timer: 60 + Math.floor(Math.random() * 220),
            deskX: x,
            floorY: y,
            targetX: x,
            speed: 0.5,
    ...
    _updateAvatars() {
        const GENERIC_MSGS = [
            ...
            'Dinner at eight.',
            'Mrs. sends her regards.',
            'Fetching the decanter.',
                ...
                case 'working': {
                    ...
                    if (av.timer <= 0) {
                        ...
                        if (r < 0.25) {
                            ...
                            this._spawnBubble(
                                av,
                                GENERIC_MSGS[Math.floor(Math.random() * GENERIC_MSGS.length)]
                            );
                        } else {
