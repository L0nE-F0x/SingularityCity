...
const PortEnv = {
    ...
    buildOceanLife(charLayer) {
        ...
        for (let ai = 0; ai < 12; ai++) {
            ...
            strand._baseX = ax;
            strand._h = sh;
            strand._phase = Math.random() * Math.PI * 2;
    ...
    buildShips(charLayer) {
        ...
        PortZone.ships.forEach((ship, si) => {
            ...
            g.closePath();
            g.endFill();
            g.beginFill(ship.color, 0.5);
            g.drawRect(-8, sh * 0.5, sw + 8, 3);
            g.endFill();
            g.beginFill(0xef4444, 0.35);
            g.drawRect(-12, sh - 6, sw + 12, 2);
            g.endFill();
            // Deck
            g.beginFill(0x334155);
            g.drawRect(2, -2, sw - 20, 4);
            g.endFill();
            if (ship.special === 'euv') {
                // ── THE $380M LIFT — one giant white ASML crate, nothing else on deck ──
                g.beginFill(0xf8fafc);
                g.drawRect(18, -30, 78, 28);
                g.endFill();
                g.beginFill(0xe2e8f0);
                g.drawRect(18, -30, 78, 4);
                g.endFill();
                g.beginFill(0x1d4ed8, 0.9);
                g.drawRect(18, -18, 78, 5);
                g.endFill(); // ASML blue band
                ...
                g.drawRect(24, -27, 8, 6);
                g.endFill();
                g.beginFill(0x1f2937);
                g.drawPolygon([84, -22, 87, -27, 90, -22]);
                g.endFill();
                // Climate-control unit humming on top
                g.beginFill(0x475569);
                g.drawRect(72, -35, 16, 5);
                g.endFill();
                g.beginFill(0x4ade80);
                g.drawCircle(86, -32.5, 1.2);
                g.endFill();
                // Escort railing
                g.beginFill(0xfbbf24, 0.5);
                g.drawRect(12, -33, 92, 1.5);
                g.endFill();
            } else {
                ...
                for (let r = 0; r < 3; r++)
                    for (let c = 0; c < 6; c++) {
                        g.beginFill(cc[(c + r * 3 + si) % cc.length]);
                        g.drawRect(15 + c * 16, -6 - r * 9, 14, 7);
                        g.endFill();
                        g.beginFill(0x000000, 0.12);
                        g.drawRect(15 + c * 16, -6 - r * 9, 14, 1);
                        g.endFill();
                        g.beginFill(0xffffff, 0.08);
                        g.drawRect(15 + c * 16, -1 - r * 9, 14, 1);
                        g.endFill();
                    }
            }
            // Bridge (stern)
            g.beginFill(0xf1f5f9);
            g.drawRect(sw - 40, -36, 28, 32);
            g.endFill();
            g.beginFill(0x22d3ee, 0.4);
            for (let wi = 0; wi < 3; wi++) g.drawRect(sw - 38 + wi * 9, -30, 7, 5);
            g.endFill();
            g.beginFill(0x94a3b8);
            g.drawRect(sw - 28, -44, 2, 10);
            g.endFill();
            g.beginFill(0x475569);
            g.drawRect(sw - 32, -44, 10, 2);
            g.endFill();
            // Smokestack
            g.beginFill(0x1e293b);
            g.drawRect(sw - 44, -28, 7, 22);
            g.endFill();
            g.beginFill(ship.color);
            g.drawRect(sw - 44, -30, 7, 3);
            g.endFill();
            hull.addChild(g);
            sc.addChild(hull);
            ...
            const nm = new PIXI.Text((ship.flag ? ship.flag + ' ' : '') + ship.label, {
            ...
            if (comm) {
                const cl = new PIXI.Text(comm.emoji + ' ' + comm.name, {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 6,
                    fill: ship.color,
                });
            ...
            sc.on('pointertap', () => {
                if (typeof UI !== 'undefined' && ship.story)
                    UI.addToast(`${ship.flag || '🚢'} ${ship.label}: ${ship.story}`);
            ...
            sc.on('pointerover', (e) => {
                if (typeof UI !== 'undefined')
                    UI.showTooltip(
                        e,
                        `${ship.flag || ''} ${ship.label} — ex ${ship.origin || 'high seas'}`,
                        ship.story || 'Carrying: ' + (comm ? comm.name : ship.cargo)
                    );
            });
            sc.on('pointerout', () => {
    ...
    update() {
        ...
        if (this.shipConts.length) {
            ...
            PortZone.ships.forEach((ship, si) => {
                ...
                if (ship.state === 'sailing_in' || ship.state === 'sailing_out') {
                    ...
                    s.wake.moveTo(wx, s.sh - 2);
                    s.wake.lineTo(wx + wdir * 30, s.sh + 6);
                    s.wake.lineTo(wx + wdir * 25, s.sh - 4);
                    s.wake.closePath();
                    s.wake.moveTo(wx + wdir * 5, s.sh + 2);
                    s.wake.lineTo(wx + wdir * 45, s.sh + 10);
                    s.wake.lineTo(wx + wdir * 40, s.sh);
                    s.wake.closePath();
                    s.wake.endFill();
                }
            });
        ...
        this.fishSchools.forEach((fs) => {
            ...
            if (fs.cont.x > fs.maxX) {
                fs.dir = -1;
                fs.cont.scale.x = -1;
            }
            if (fs.cont.x < fs.minX) {
                fs.dir = 1;
                fs.cont.scale.x = 1;
            }
        });
        ...
        this.algaeStrands.forEach((a) => {
            ...
            a.moveTo(0, 0);
            a.lineTo(sway * 0.3, -a._h * 0.4);
            a.lineTo(sway, -a._h);
            a.lineTo(sway + 3, -a._h);
            a.lineTo(sway * 0.3 + 2, -a._h * 0.4);
            a.lineTo(2, 0);
            a.closePath();
            a.endFill();
            a.beginFill(a._col, 0.3);
            a.drawEllipse(sway + 1, -a._h - 2, 4, 3);
            a.endFill();
        });
    },
    ...
    _animateCrane() {
        ...
        cg.drawRect(tx - 6, h - 76, 12, 5);
        cg.endFill();
        // Cable
        cg.beginFill(0x888888);
        cg.drawRect(tx - 1, h - 71, 2, cableLen);
        cg.endFill();
        // Hook/spreader
        cg.beginFill(0xfbbf24);
        cg.drawRect(tx - 6, h - 71 + cableLen, 12, 3);
        cg.endFill();
        ...
        if (docked && Math.sin(G.tick * 0.02) > 0) {
            ...
            cg.drawRect(tx - 8, h - 71 + cableLen + 3, 16, 10);
            cg.endFill();
        }
    },
    ...
    showManifest() {
        ...
        html +=
            '<div style="text-align:center;margin-bottom:12px"><span style="font-size:20px">🚢</span><br><span style="color:var(--cy);font-size:11px;letter-spacing:2px">SINGULARITY CITY PORT</span><br><span style="color:var(--t3);font-size:8px">Global AI Supply Chain Manifest</span></div>';
        html +=
            '<table style="width:100%;border-collapse:collapse;font-size:8px;font-family:\'JetBrains Mono\',monospace">';
        html +=
            '<tr style="color:var(--t3);border-bottom:1px solid var(--bd)"><th style="text-align:left;padding:4px">Resource</th><th style="text-align:right;padding:4px">Price</th><th style="text-align:right;padding:4px">Change</th><th style="text-align:center;padding:4px">Supply</th></tr>';
        pz.COMMODITIES.forEach((c) => {
            const pr = pz.prices[c.id];
            const price = pr
                ? pr.price >= 1e6
                    ? '$' + (pr.price / 1e6).toFixed(pr.price >= 1e8 ? 0 : 1) + 'M'
                    : pr.price >= 10000
                      ? '$' + (pr.price / 1000).toFixed(1) + 'K'
                      : '$' + pr.price.toLocaleString()
                : '—';
            const chg = pr ? pr.change_pct : 0;
            const arrow = chg > 0 ? '▲' : chg < 0 ? '▼' : '—';
            const chgCol = chg > 10 ? '#ef4444' : chg > 0 ? '#f59e0b' : chg < -5 ? '#4ade80' : '#94a3b8';
            const supCol = pz.supplyColor(c.id);
            const supLbl = pz.supplyLabel(c.id);
            html += '<tr style="border-bottom:1px solid rgba(255,255,255,0.03)">';
            html +=
                '<td style="padding:4px" title="' +
                c.desc +
                '">' +
                c.emoji +
                ' ' +
                c.name +
                '<br><span style="color:var(--t3);font-size:6px">per ' +
                c.unit +
                '</span></td>';
            html += '<td style="text-align:right;padding:4px;color:#fff">' + price + '</td>';
            html +=
                '<td style="text-align:right;padding:4px;color:' +
                chgCol +
                '">' +
                arrow +
                ' ' +
        ...
        html += '</table>';
        html +=
            '<div style="text-align:center;margin-top:10px;color:var(--t3);font-size:7px">Prices are estimated market rates · Updated ' +
            (pz.prices.gpu_h100?.updated || 'N/A') +
            '</div>';
