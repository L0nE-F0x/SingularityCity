...
const InteriorLegacy = {
    ...
    build(bld, layer) {
        ...
        sg.endFill();
        rc.addChild(sg);
        const lt = new PIXI.Text('LEGACY SYSTEMS MUSEUM', {
        ...
        rc.addChild(lt);
        const bdg = new PIXI.Text('EST. 2023 \u2014 PRESERVING AI HISTORY', {
            fontFamily: 'JetBrains Mono',
            fontSize: 7,
            fill: 0x94a3b8,
            letterSpacing: 2,
        ...
        if (G.models && G.charRefs) {
            const visitors = G.models.filter((m) => {
        ...
        this._onMove = (e) => {
            if (!InteriorLegacy.isDragging || !InteriorLegacy.scene || InteriorLegacy.scene.destroyed) return;
            let ny = InteriorLegacy._startSceneY + (e.clientY - InteriorLegacy._startY);
            ny = Math.max(InteriorLegacy.minY, Math.min(ny, InteriorLegacy.maxY));
            InteriorLegacy.scene.y = ny;
        };
        this._onUp = () => {
            InteriorLegacy.isDragging = false;
            if (InteriorLegacy.layer) InteriorLegacy.layer.cursor = 'grab';
        };
    ...
    _drawArchitectures(c, sx, uw, pY, fy, fh, col) {
        ...
        this._exhibitCase(c, sx + 40, pY, 'RNN', 0xef4444, [
            'Recurrent loops',
            'Vanishing gradients',
            'seq2seq era',
        ]);
        this._exhibitCase(c, sx + 160, pY, 'LSTM', 0x4ade80, [
            'Memory cells',
            'Forget gates',
            'NLP backbone',
        ]);
        this._exhibitCase(c, sx + 280, pY, 'GAN', 0xa855f7, [
            'Generator vs',
            'Discriminator',
            'Mode collapse',
        ]);
        this._exhibitCase(c, sx + 400, pY, 'CNN', 0x22d3ee, [
            'Conv filters',
            'Pooling layers',
            'ImageNet king',
        ]);
    ...
    _drawTimeline(c, sx, uw, pY, fy, fh, col) {
        ...
        g.eventMode = 'none';
        g.beginFill(col, 0.3);
        g.drawRect(sx + 20, pY - 20, uw - 40, 3);
        g.endFill();
        c.addChild(g);
        // Timeline markers
        const milestones = [
    ...
    _drawHallOfFame(c, sx, uw, pY, fy, fh, col) {
        ...
        if (retiredModels.length === 0) {
            ...
            const placeholder = new PIXI.Text('Awaiting future inductees...', {
                fontFamily: 'JetBrains Mono',
                fontSize: 6,
                fill: 0x666688,
                fontStyle: 'italic',
    ...
    _welcomeDesk(c, x, y, col) {
        ...
        g.drawRect(x + 30, y - 35, 28, 14);
        g.endFill();
        g.beginFill(0x22d3ee, 0.2);
        g.drawRect(x + 32, y - 33, 24, 10);
        g.endFill();
        // "TICKETS" sign
        c.addChild(g);
        const ts = new PIXI.Text('TICKETS', {
            fontFamily: 'JetBrains Mono',
            fontSize: 5,
            fill: col,
            letterSpacing: 1,
    ...
    _velvetRope(c, x, y) {
        ...
        g.drawRect(x, y - 24, 4, 24);
        g.endFill();
        g.beginFill(0xc9a84c);
        g.drawCircle(x + 2, y - 26, 3);
        g.endFill();
        g.beginFill(0xc9a84c);
        g.drawRect(x + 50, y - 24, 4, 24);
        g.endFill();
        g.beginFill(0xc9a84c);
        g.drawCircle(x + 52, y - 26, 3);
        g.endFill();
        // Rope (catenary curve)
    ...
    _giftShop(c, x, y) {
        ...
        g.drawRect(x, y - 48, 55, 48);
        g.endFill();
        g.beginFill(0x4a4030);
        g.drawRect(x + 2, y - 46, 51, 2);
        g.endFill();
        g.beginFill(0x4a4030);
        g.drawRect(x + 2, y - 24, 51, 2);
        g.endFill();
        // Miniature items (colorful souvenirs)
        const cols = [0xef4444, 0x4ade80, 0x60a5fa, 0xfbbf24, 0xa855f7];
        for (let i = 0; i < 5; i++) {
            g.beginFill(cols[i]);
            g.drawRect(x + 4 + i * 10, y - 42, 8, 14);
            g.endFill();
        }
        for (let i = 0; i < 4; i++) {
            g.beginFill(cols[i + 1], 0.7);
            g.drawCircle(x + 8 + i * 12, y - 14, 4);
            g.endFill();
        }
        c.addChild(g);
        const lbl = new PIXI.Text('GIFT SHOP', {
            fontFamily: 'JetBrains Mono',
            fontSize: 5,
            fill: 0x94a3b8,
            letterSpacing: 1,
    ...
    _exhibitCase(c, x, y, title, col, lines) {
        ...
        g.drawRect(x, y - 50, 70, 50);
        g.endFill();
        g.lineStyle(1, col, 0.3);
        g.drawRect(x + 2, y - 48, 66, 44);
        g.lineStyle(0);
        // Glowing exhibit inside
        g.beginFill(col, 0.08);
        g.drawRect(x + 4, y - 46, 62, 40);
        g.endFill();
        // Exhibit icon — abstract neural net shape
        g.beginFill(col, 0.4);
        g.drawCircle(x + 35, y - 30, 8);
        g.endFill();
        g.beginFill(col, 0.2);
        g.drawCircle(x + 20, y - 35, 4);
        g.drawCircle(x + 50, y - 35, 4);
        g.endFill();
        g.beginFill(col, 0.2);
        g.drawCircle(x + 20, y - 20, 4);
        g.drawCircle(x + 50, y - 20, 4);
        g.endFill();
        ...
        gl.drawRect(x - 2, y - 52, 74, 54);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, gl);
        this.indoorLights.push({ g: gl, maxA: 0.06, type: 'blink' });
        // Title plaque
        const tt = new PIXI.Text(title, {
            fontFamily: 'JetBrains Mono',
            fontSize: 8,
            fill: col,
            fontWeight: 'bold',
    ...
    _infoPlaque(c, x, y, text) {
        ...
        g.drawRoundedRect(x, y - 14, 20, 10, 2);
        g.endFill();
        g.beginFill(0xc9a84c, 0.3);
        g.drawRoundedRect(x + 1, y - 13, 18, 8, 1);
        g.endFill();
        c.addChild(g);
    ...
    _timelineMarker(c, x, y, year, label, col) {
        ...
        const gl = new PIXI.Graphics();
        gl.eventMode = 'none';
        gl.beginFill(col, 0.06);
        gl.drawCircle(x, y - 38, 12);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, gl);
        this.indoorLights.push({ g: gl, maxA: 0.08, type: 'screen' });
        // Year label
        const yl = new PIXI.Text(year, {
            fontFamily: 'JetBrains Mono',
            fontSize: 7,
            fill: col,
            fontWeight: 'bold',
    ...
    _displayPanel(c, x, y, col) {
        ...
        g.drawRect(x, y - 48, 80, 42);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(x + 3, y - 45, 74, 36);
        g.endFill();
        const gl = new PIXI.Graphics();
        gl.eventMode = 'none';
        gl.beginFill(col, 0.1);
        gl.drawRect(x + 5, y - 43, 70, 32);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, gl);
        this.indoorLights.push({ g: gl, maxA: 0.12, type: 'screen' });
        // Scrolling text — "AI EVOLUTION"
        const dt = new PIXI.Text('THE EVOLUTION OF AI', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: col,
        });
    ...
    _portrait(c, x, y, model) {
        ...
        g.eventMode = 'none';
        // Ornate frame
        g.lineStyle(2, 0xc9a84c);
        g.drawRect(x, y - 50, 50, 40);
        g.lineStyle(0);
        g.beginFill(0x1a1a2e);
        g.drawRect(x + 3, y - 47, 44, 34);
        g.endFill();
        // Portrait silhouette
        g.beginFill(col, 0.3);
        g.drawCircle(x + 25, y - 35, 8);
        g.endFill();
        g.beginFill(col, 0.2);
        g.drawRect(x + 15, y - 26, 20, 12);
        g.endFill();
        c.addChild(g);
        // Clickable
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, model.name, 'Retired \u2014 Hall of Fame');
        });
        cont.on('pointerout', () => {
    ...
    _emptyFrame(c, x, y, col) {
        ...
        g.lineStyle(0);
        g.beginFill(0x111118);
        g.drawRect(x + 2, y - 48, 46, 36);
        g.endFill();
        g.beginFill(col, 0.05);
        g.drawRect(x + 4, y - 46, 42, 32);
        g.endFill();
        // Question mark
    ...
    _memorialTorch(c, x, y, col) {
        ...
        g.drawRect(x - 6, y - 35, 12, 35);
        g.endFill();
        g.beginFill(0x4a4a5a);
        g.drawRect(x - 8, y - 37, 16, 4);
        g.endFill();
        // Bowl
        g.beginFill(0xc9a84c);
        g.moveTo(x - 10, y - 37);
        ...
        const gl = new PIXI.Graphics();
        gl.eventMode = 'none';
        gl.beginFill(0xff8800, 0.08);
        gl.drawCircle(x, y - 48, 14);
        gl.endFill();
        gl.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(gl);
    ...
    _dustyCrate(c, x, y) {
        ...
        g.drawRect(x, y - 22, 30, 22);
        g.endFill();
        g.beginFill(0x5a4a3a);
        g.drawRect(x, y - 22, 30, 3);
        g.endFill();
        ...
        g.lineStyle(0);
        // Dust particles
        g.beginFill(0x8a7a6a, 0.3);
        g.drawCircle(x + 5, y - 25, 1);
        g.drawCircle(x + 22, y - 24, 0.8);
        g.drawCircle(x + 15, y - 26, 1.2);
        g.endFill();
        c.addChild(g);
    },
    ...
    _npc(c, x, y, name, col) {
        ...
        const npcModel = {
            ...
            phase: 'released',
            lab: 'other',
            desc: 'Museum staff \u2014 preserving AI history.',
        };
        cont.eventMode = 'static';
        cont.cursor = 'pointer';
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Museum Staff');
        });
        cont.on('pointerout', () => {
            if (typeof UI !== 'undefined') UI.hideTooltip();
        });
    ...
    update() {
        ...
        this.avatars.forEach((av) => {
            ...
            const ah = av._h || 32;
            if (av.head) {
                av.head.y = -ah + Math.sin(G.tick * 0.12 + av._phase) * 1.5;
            }
            if (av.body) {
                av.body.y = -ah + Math.round(ah * 0.4) + Math.abs(Math.sin(G.tick * 0.12 + av._phase)) * 1.5;
            }
            if (av.legL) {
