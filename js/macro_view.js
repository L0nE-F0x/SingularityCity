...
const MacroView = {
    ...
    _mmZones: [
        ...
        { id: 'frontier', emoji: '🌲', label: 'Frontier Pines', match: (b) => b.id === 'forest_space' },
        {
            ...
            match: (b) => b.id.startsWith('npc_apt_'),
        },
        {
            id: 'dc',
            emoji: '🖥️',
            label: 'Compute Dist.',
            match: (b) => b.id.startsWith('dc_') || b.id.startsWith('fab_') || b.id === 'metro_dc',
        },
        {
            id: 'res',
            emoji: '🏠',
            label: 'Residential',
            match: (b) => b.id.startsWith('res_') || b.id === 'metro_res',
        },
        ...
        { id: 'embassy', emoji: '🏛️', label: 'Embassy Row', match: (b) => b.id.startsWith('embassy_') },
        {
            id: 'diplomats',
            emoji: '🏘️',
            label: 'Diplomat Villas',
            match: (b) => b.id.startsWith('diplomat_villa_'),
        },
        ...
        { id: 'backbone', emoji: '🌐', label: 'The Backbone', match: (b) => b.id.startsWith('backbone_') },
        {
            ...
            match: (b) => b.id.startsWith('robotics_'),
        },
        {
            id: 'longevity',
            emoji: '🧬',
            label: 'Longevity Wing',
            match: (b) => b.id.startsWith('longevity_'),
        },
    ...
    buildMacroLayer() {
        ...
        for (let i = 0; i < this.vpW; i += 50) {
            grid.moveTo(i, 0);
            grid.lineTo(i, this.vpH);
        }
        for (let i = 0; i < this.vpH; i += 50) {
            grid.moveTo(0, i);
            grid.lineTo(this.vpW, i);
        }
        ...
        labsEntries.forEach(([labId, lab], i) => {
            ...
            const clusterInfo =
                typeof COMPUTE_DATA !== 'undefined' && COMPUTE_DATA.clusters
                    ? COMPUTE_DATA.clusters.find((c) => c.lab === labId) || {
                          name: 'Remote Cluster',
                          gpus: 0,
                          type: 'Unknown',
                          location: 'Unknown',
            ...
            nodeCont.on('pointerover', (e) => {
                ...
                if (typeof UI !== 'undefined')
                    UI.showTooltip(
                        e,
                        `${lab.name} Uplink`,
                        `${clusterInfo.name}<br><span style="color:#4ade80">${(clusterInfo.gpus / 1000).toFixed(0)}K GPUs</span>`,
                        false
                    );
            });
