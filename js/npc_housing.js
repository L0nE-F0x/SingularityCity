...
const NPCHousing = {
    REGISTRY: [
        ...
        },
        {
            id: 'npc_dip_eu',
            name: 'EU Amb.',
            role: 'EU Representative',
            workplace: 'embassy_eu',
            color: '#003399',
            shift: 'day',
        },
        {
        ...
        },
        {
            id: 'npc_dip_in',
            name: 'IN HC',
            role: 'Indian High Commissioner',
            workplace: 'embassy_in',
            color: '#ff9933',
            shift: 'day',
        },
        {
    ...
    buildings: [
        {
            ...
            fl: 5,
            emoji: '🏬',
            desc: 'Affordable housing for city facility workers.',
        },
        {
            id: 'npc_apt_2',
            name: 'Worker Block B',
            ...
            fl: 4,
            emoji: '🏬',
            desc: 'Compact apartments for night-shift staff.',
        },
        {
            id: 'npc_apt_3',
            name: 'Worker Block C',
            ...
            fl: 4,
            emoji: '🏬',
            desc: 'Staff quarters for space and tech workers.',
        },
        {
            id: 'npc_apt_4',
            name: 'Worker Block D',
            ...
            fl: 6,
            emoji: '🏬',
            desc: 'High-rise housing for Backbone and Robotics staff.',
        },
        {
            id: 'npc_apt_5',
            name: 'Worker Block E',
            ...
            fl: 5,
            emoji: '🏬',
            desc: 'Lab housing for Longevity Wing researchers.',
        },
        {
            id: 'npc_apt_6',
            name: 'Worker Block F',
            ...
            fl: 5,
            emoji: '🏬',
            desc: 'Mixed-use apartments for Power and Port workers.',
        },
    ],
    ...
    _drawAvatar(parent, npc) {
        ...
        if (typeof HumanAvatar !== 'undefined' && !HumanAvatar.isBot(npc.name, npc.id)) {
            ...
            const isExec =
                npc.role &&
                /Director|Commander|Commissioner|Ambassador|Representative|Foreman|Senior/.test(npc.role);
            ...
            c.on('pointertap', () => {
                if (typeof UI !== 'undefined')
                    UI.selectModel({
                        ...
                        role: npc.role,
                        lab: 'other',
                        desc: npc.role + '. Lives in the Worker Housing District.',
                    });
            });
            c.on('pointerover', (e) => {
                if (typeof UI !== 'undefined') UI.showTooltip(e, npc.name, npc.role);
            });
            c.on('pointerout', () => {
        ...
        dot.y = -h - 6;
        // Name tag
        const tag = new PIXI.Text(npc.name, {
    ...
    spawnCommuters(charLayer) {
        ...
        subset.forEach((npc, i) => {
            ...
            this.commuters.push({
                ...
                homeX,
                state: initState,
                targetX: shouldWork ? workX : homeX,
    ...
    _getWorkX(npc) {
        ...
        if (npc.workplace === 'university') {
            const b = G.bldById['uni_main'] || G.bldById['uni_dorm'];
            if (b) return b.x + b.w / 2;
        }
        if (npc.workplace === 'court') {
            const b = G.bldById['court_senate'] || G.bldById['court_hearing'];
            if (b) return b.x + b.w / 2;
        }
        if (npc.workplace === 'vcrow') {
            const b = G.bldById['vcrow_titan'] || G.bldById['vcrow_apex'];
            if (b) return b.x + b.w / 2;
        }
        if (map[npc.workplace]) {
            const b = G.bldById[map[npc.workplace]];
            if (b) return b.x + b.w / 2;
        }
        const hq = BLDS.find((b) => b.lab && !b.id.startsWith('house_'));
        return hq ? hq.x + hq.w / 2 : 3000;
    },
    ...
    _getWorkBld(npc) {
        ...
        if (npc.workplace === 'hq') {
            const b = BLDS.find((b) => b.lab && !b.id.startsWith('house_'));
            return b ? b.id : null;
        }
        if (map[npc.workplace]) {
            const b = G.bldById[map[npc.workplace]];
    ...
    _buildLegs(fromX, toX) {
        ...
        const legs = [];
        if (ia < ib) {
            for (let i = ia; i <= ib; i++) legs.push(list[i]);
        } else {
            for (let i = ia; i >= ib; i--) legs.push(list[i]);
        }
    ...
    _trainForLeg(s1, s2) {
        ...
        const dc = sx('metro_dc'),
            res = sx('metro_res'),
            hq = sx('metro_hq'),
            mid = sx('metro_mid'),
            east = sx('metro_east'),
            lon = sx('metro_longevity');
    ...
    stepMetroRide(mv) {
        ...
        if (rm.phase === 'descend') {
            ...
            if (!t) {
                // no train maps this leg — skip to exit
