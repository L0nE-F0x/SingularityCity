...
const G = {
    ...
    pref(k) {
        return this.prefs ? this.prefs[k] : undefined;
    },
    ...
    reduceMotionOn() {
        if (this.prefs && this.prefs.reduceMotion !== null && this.prefs.reduceMotion !== undefined)
    ...
    recalculateZoning() {
        ...
        if (!BLDS.find((b) => b.id === 'metro_longevity')) {
            const mLong = {
                ...
                emoji: '🚇',
                lab: null,
                desc: 'Terminus station serving the Innovation Corridor — Backbone, Robotics, Longevity, Agent District and Alignment Forest workers.',
            };
            BLDS.push(mLong);
        ...
        if (!BLDS.find((b) => b.id === 'neon_bar')) {
            const bar = {
                ...
                emoji: '🍸',
                lab: null,
                desc: 'Late-night karaoke bar. Models unwind with drinks and sing their training data.',
            };
            BLDS.push(bar);
        ...
        if (!BLDS.find((b) => b.id === 'visitor_monument')) {
            const mon = {
                id: 'visitor_monument',
                name: 'Visitor Monument',
                w: 80,
                x: 0,
                fl: 1,
                emoji: '🌐',
                lab: null,
                desc: 'A monument tracking every unique visitor to Singularity City. Your presence is recorded.',
            };
            BLDS.push(mon);
        ...
        if (!BLDS.find((b) => b.id === 'graveyard')) {
            const grave = {
                id: 'graveyard',
                name: 'Model Graveyard',
                w: 200,
                x: 0,
                fl: 1,
                emoji: '⚰️',
                lab: null,
                desc: 'A memorial park honoring retired and deprecated AI models. Quiet, reverent, eternal.',
            };
            BLDS.push(grave);
        ...
        if (!BLDS.find((b) => b.id === 'times_hq')) {
            const times = {
                id: 'times_hq',
                name: 'Singularity City Times',
                w: 150,
                x: 0,
                fl: 3,
                emoji: '📰',
                lab: null,
                desc: 'The city\u2019s weekly newspaper of record. Click to read this week\u2019s front page.',
            };
            BLDS.push(times);
        ...
        if (typeof DC_FACILITIES !== 'undefined') {
            DC_FACILITIES.forEach((dc) => {
                ...
                tipParts.push(
                    '<br><br><span style="color:#a0a0b8;font-size:9px;line-height:1.4;display:block;">'
                );
                tipParts.push(
                    `${statusIcon} STATUS: ${dc.status === 'construction' ? 'Under Construction' : 'Operational'}<br>`
                );
        ...
        if (!BLDS.find((b) => b.id === 'metro_dc')) {
            const mDc = {
                ...
                emoji: '🚇',
                lab: null,
                desc: 'Data center district transit hub.',
            };
            BLDS.push(mDc);
        ...
        BLDS.filter((b) => b.id.startsWith('dc_') || b.id.startsWith('fab_'))
            .sort((a, b) => {
                const ad = a.dcData || {};
                const bd = b.dcData || {};
                const typeA = dcOrder[ad.type] || 0;
                const typeB = dcOrder[bd.type] || 0;
                if (typeA !== typeB) return typeA - typeB;
                const statA = statusOrder[ad.status] || 0;
                const statB = statusOrder[bd.status] || 0;
                return statA - statB;
            })
            .forEach((b) => {
                b.x = dcX;
        ...
        const isSpecialId = (id) =>
            id.startsWith('res_') ||
            id === 'metro_res' ||
            id.startsWith('house_') ||
            id === 'metro_east' ||
            id === 'metro_dc' ||
            id === 'metro_mid' ||
            id === 'metro_longevity' ||
            id.startsWith('npc_apt_') ||
            ...
            id.startsWith('uni_') ||
            id.startsWith('court_') ||
            id === 'city_park' ||
            id === 'ai_index' ||
            id === 'black_market' ||
            id === 'convention_center' ||
            id.startsWith('backbone_') ||
            id.startsWith('vcrow_') ||
            id.startsWith('robotics_') ||
            id.startsWith('longevity_') ||
            id.startsWith('agents_') ||
            id.startsWith('embassy_') ||
            id.startsWith('diplomat_villa_') ||
            id.startsWith('align_');

        const techBldsList = BLDS.filter(
            (b) => !isSpecialId(b.id) && !isDcBld(b) && !isSpaceOrForestSep(b)
        ).sort((a, b) => a.x - b.x);
        ...
        if (!BLDS.find((b) => b.id === 'metro_mid')) {
            const mMid2 = {
                ...
                emoji: '🚇',
                lab: null,
                desc: 'Central tech district transit hub.',
            };
            BLDS.push(mMid2);
        ...
        const addZB = (z, arr) => {
            ...
            this.zoneBounds.push({
                zone: z,
                x0: bs[0].x - 60,
                x1: bs[bs.length - 1].x + bs[bs.length - 1].w + 60,
            });
        };
        addZB(
            'port',
            BLDS.filter((b) => b.id.startsWith('port_'))
        );
        addZB('desert', BLDS.filter(isSpaceBld));
        if (fSpace) addZB('forest', [fSpace]);
        addZB(
            'npc_housing',
            BLDS.filter((b) => b.id.startsWith('npc_apt_'))
        );
        ...
        addZB(
            'university',
            BLDS.filter((b) => b.id.startsWith('uni_'))
        );
        if (fCamp) addZB('forest', [fCamp]);
        addZB(
            'court',
            BLDS.filter((b) => b.id.startsWith('court_'))
        );
        addZB(
            'jail',
            BLDS.filter((b) => b.id === 'ai_jail')
        );
        addZB(
            'city_park',
            BLDS.filter((b) => b.id === 'city_park')
        );
        addZB(
            'vcrow',
            BLDS.filter((b) => b.id.startsWith('vcrow_'))
        );
        if (nBar) addZB('nightlife', [nBar]);
        if (fSilicon) addZB('forest', [fSilicon]);
        addZB(
            'estates',
            BLDS.filter((b) => b.id.startsWith('house_'))
        );
        addZB(
            'backbone',
            BLDS.filter((b) => b.id.startsWith('backbone_'))
        );
        addZB(
            'robotics',
            BLDS.filter((b) => b.id.startsWith('robotics_'))
        );
        addZB(
            'longevity',
            BLDS.filter((b) => b.id.startsWith('longevity_'))
        );
        addZB(
            'agents',
            BLDS.filter((b) => b.id.startsWith('agents_'))
        );
        addZB(
            'suburbia',
            BLDS.filter((b) => b.id.startsWith('suburb_'))
        );
        addZB(
            'power',
            BLDS.filter((b) => b.id.startsWith('power_'))
        );
    ...
    unlockAchieve(key) {
        ...
        if (typeof NOTIFY !== 'undefined')
            NOTIFY.send('Achievement Unlocked!', `${a.icon} ${a.name} — ${a.desc}`);
        this.save();
    },
    ...
    ensureLabExists(rawLabId, suggestedRegion) {
        ...
        else if (norm.includes('alibaba') || norm.includes('qwen') || norm.includes('tongyi'))
            canonical = 'alibaba';
        ...
        if (LABS[canonical]) {
            ...
            if (!hasHQ && canonical !== 'other') {
                ...
                const newBld = {
                    id: 'bld_' + canonical,
                    name: (LABS[canonical].name || canonical) + ' HQ',
                    w: newW,
                    x: newX,
                    fl: 3,
                    lab: canonical,
                    desc: `Headquarters for ${LABS[canonical].name || canonical}.`,
                };
                ...
                if (typeof UI !== 'undefined')
                    UI.addLog(`🏗️ New HQ zoned for ${LABS[canonical].name || canonical}!`);
            }
        ...
        for (let existingKey in LABS) {
            ...
            if (
                norm === existingNorm ||
                (norm.length > 5 && existingNorm.includes(norm)) ||
                (existingNorm.length > 5 && norm.includes(existingNorm))
            ) {
        ...
        LABS[canonical] = {
            name: niceName,
            color: color,
            icon: this.getLabIcon(canonical),
            ticker: null,
            region: finalRegion,
        };
        ...
        const newBld = {
            ...
            fl: 3,
            lab: canonical,
            desc: `Newly zoned headquarters for ${niceName}.`,
        };
        ...
        if (typeof UI !== 'undefined')
            UI.addLog(`🏗️ New land zoned for ${niceName} HQ in ${finalRegion.toUpperCase()} Sector!`);
        return canonical;
    },
    ...
    evolveCity() {
        ...
        if (this._lastEvolve && now - this._lastEvolve < 3000) {
            if (!this._evolveDeferred) {
                this._evolveDeferred = setTimeout(() => {
                    this._evolveDeferred = null;
                    this.evolveCity();
                }, 3000);
            }
            return;
        ...
        if (wc) {
            ...
            for (let i = 0; i < this.models.length; i++) {
                ...
                if (!perLab[m.lab])
                    perLab[m.lab] = { active: 0, total: 0, osCount: 0, topScore: 0, topName: '' };
                ...
                if (typeof BM !== 'undefined' && BM[m.id]?.ELO > topElo) {
                    topElo = BM[m.id].ELO;
                    topLab = m.lab;
                }
                if (
                    ...
                    COSTS[m.id]?.output > 0 &&
                    COSTS[m.id].output < lowestCost
                ) {
                    lowestCost = COSTS[m.id].output;
                    cheapestLab = m.lab;
                }
        ...
        BLDS.forEach((b) => {
            if (b.id === 'cafe' || b.id === 'gym' || b.id === 'arena' || b.id === 'open_square') {
                if (b.id === 'cafe') {
                    b.dynamicFl = 4;
                    b.w = Math.max(b.w || 0, 160);
                    b.desc =
                        "API Café — the city's beloved coffee house. Code reviews over lattes, pastries & rooftop vibes.";
                } else if (b.id === 'gym') {
                    b.dynamicFl = 5;
                    b.w = Math.max(b.w || 0, 200);
                    b.desc =
                        'RLHF Gym — five floors of heavy compute training: cardio, weights, combat, yoga & pool.';
                } else if (b.id === 'arena') {
                    b.dynamicFl = 5;
                    b.w = Math.max(b.w || 0, 210);
                    b.desc =
                        'LMSYS Chatbot Arena — where models battle for ELO supremacy across five legendary floors.';
                } else if (b.id === 'open_square') {
                    b.dynamicFl = 5;
                    b.w = Math.max(b.w || 0, 210);
                    b.desc =
                        'Open Source Hub — collaborative HQ for open-source contributors, hackathons & community servers.';
                }
            }
            ...
            if (b.id.startsWith('house_')) {
                const founder =
                    typeof REAL_FOUNDERS !== 'undefined' ? REAL_FOUNDERS.find((f) => f.lab === b.lab) : null;
            ...
            const baseLore =
                LABS[b.lab]?.desc ||
                `An emerging AI research facility actively contributing to the global intelligence frontier.`;
            b.desc = baseLore;
    ...
    _performEnterInterior(b) {
        ...
        if (!exitBtn) {
            ...
            exitBtn.onmouseover = () => {
                exitBtn.style.background = '#e11d48';
            };
            exitBtn.onmouseout = () => {
                exitBtn.style.background = '#f43f5e';
            };
        ...
        if (kHud) {
            kHud.style.opacity = '0';
            kHud.style.pointerEvents = 'none';
        }
    },
    ...
    _performExitInterior() {
        ...
        if (this.interiorLayer) {
            this.interiorLayer.removeChildren().forEach((c) => {
    ...
    startTracking(type, id, lab) {
        ...
        if (!hud) {
            ...
            hud.id = 'trackingHud';
            hud.style.cssText =
                'position:absolute; bottom:40px; left:50%; transform:translateX(-50%); z-index:150; background:rgba(10,10,25,0.9); border:1px solid #22d3ee; border-radius:20px; padding:6px 16px; display:flex; align-items:center; gap:10px; font-family:"JetBrains Mono",monospace; font-size:10px; color:#22d3ee; pointer-events:all; box-shadow:0 4px 15px rgba(34,211,238,0.3); animation:slideUp 0.3s ease; max-width:calc(100vw - 24px); flex-wrap:wrap; justify-content:center;';
            document.getElementById('gameWrap').appendChild(hud);
        }

        let name = id;
        if (type === 'ceo') {
            name = G.ceoRefs[lab]?.f?.name || lab;
        } else if (type === 'model') {
            name = G.models.find((m) => m.id === id)?.name || id;
        } else if (type === 'npc') {
            const cm = typeof NPCHousing !== 'undefined' && NPCHousing.commuters.find((c) => c.npc.id === id);
            name = cm ? cm.npc.name : id;
        } else if (type === 'vendor') {
            const vm =
                typeof StreetVendors !== 'undefined' && StreetVendors.vendors.find((v) => v.def.id === id);
            name = vm ? vm.def.name : id;
        } else if (type === 'vc_commuter') {
            const cm = typeof VCRow !== 'undefined' && VCRow.carCommuters.find((c) => c.npc.id === id);
            name = cm ? cm.npc.name : id;
        }
        ...
        if (t.type === 'model') {
            ...
            if (m && typeof getStage === 'function' && typeof getAct === 'function') {
                ...
                if (refs && refs._metroState && refs._metroState !== 'none')
                    return 'metro_' + refs._metroState;
            ...
            const cm =
                typeof NPCHousing !== 'undefined' && NPCHousing.commuters.find((c) => c.npc.id === t.id);
            if (cm) return cm.state;
        } else if (t.type === 'vendor') {
            const vm =
                typeof StreetVendors !== 'undefined' && StreetVendors.vendors.find((v) => v.def.id === t.id);
    ...
    _getTransitionOverlay() {
        ...
        if (!ov) {
            ...
            ov.style.cssText =
                'position:absolute;top:0;left:0;right:0;bottom:0;background:#000;opacity:0;pointer-events:none;transition:opacity 0.2s ease;z-index:200;';
    ...
    updateTracking() {
        ...
        if (this.tracking.type === 'model') {
            ...
            const ridingTrainKey = refs._metroState === 'riding' ? this._trainKeyOf(refs._ridingTrain) : null;
            if (ridingTrainKey) {
            ...
            const cm =
                ...
                NPCHousing.commuters.find((c) => c.npc.id === this.tracking.id);
            if (!cm) return;
            entityBld = cm.bld;
        } else if (this.tracking.type === 'vendor') {
            const vm =
                typeof StreetVendors !== 'undefined' &&
                StreetVendors.vendors.find((v) => v.def.id === this.tracking.id);
            if (!vm) return;
            entityBld = vm.bld;
        } else if (this.tracking.type === 'vc_commuter') {
            const cm =
                typeof VCRow !== 'undefined' && VCRow.carCommuters.find((c) => c.npc.id === this.tracking.id);
            if (!cm) return;
            entityBld = cm.bld;
        }
    ...
    init() {
        Object.keys(LABS).forEach((key) => {
            if (!LABS[key].icon || LABS[key].icon === '🏗️') {
                LABS[key].icon = this.getLabIcon(key);
            }
                ...
                setTimeout(() => {
                    ...
                    if (
                        ...
                        if (!document.getElementById('btnInside')) {
                            ...
                            btn.style.cssText =
                                'background:var(--ac); color:#000; border:none; padding:6px 10px; border-radius:4px; cursor:pointer; font-size:10px; margin-left:15px; font-family:"Press Start 2P", monospace; display:inline-block; vertical-align:middle;';
                            btn.onclick = () => {
                                if (typeof SND !== 'undefined') SND.uiClick();
                                G.enterInterior(b);
                            };
        ...
        if (typeof ACTS !== 'undefined') {
            if (!ACTS.nightlife)
                ACTS.nightlife = { label: 'Nightlife', verb: 'partying', icon: '🍸', indoor: true };
        }
        ...
        this.socialSpots = BLDS.filter((b) =>
            ['cafe', 'open_square', 'gym', 'arena', 'forest_0', 'city_park'].includes(b.id)
        );
        ...
        if (typeof Environment !== 'undefined') {
            Environment.init({
                starsLayer: this.starsLayer,
                celestialGfx: this.celestialGfx,
                cloudLayer: this.cloudLayer,
                bldLayer: this.bldLayer,
                groundGfx: this.groundGfx,
                reflectionLayer: this.reflectionLayer,
                staticLightsGfx: this.staticLightsGfx,
        ...
        if (typeof SeasonalEnv !== 'undefined')
            SeasonalEnv.init({ bldLayer: this.bldLayer, fxGfx: this.fxGfx });
        ...
        if (typeof PortEnv !== 'undefined' && typeof PortZone !== 'undefined') {
            registerLazy(
                ...
                () => {
                    ...
                    const pBlds =
                        typeof BLDS !== 'undefined'
                            ? BLDS.filter((b) => b.id && b.id.startsWith('port_'))
                            : [];
                ...
                () => {
                    PortEnv.buildShips(self.charLayer);
                    PortEnv.buildOceanLife(self.charLayer);
                }
            );
        }
        ...
        if (typeof Worker !== 'undefined') {
            try {
                ...
                this._computeWorker.onerror = () => {
                    this._computeWorker = null;
                };
            } catch (ex) {
                /* Worker unavailable — will use inline fallback */
            }
        }
        ...
        for (let i = 0; i < this._lazyZones.length; i++) {
            ...
            if (b.x1 >= minX && b.x0 <= maxX) {
                try {
                    ...
                    console.warn('Lazy zone boot failed:', z.name, e);
                }
                z.booted = true;
            }
        }
    ...
    update() {
        ...
        if (this.trainFocus && typeof InteriorTrain !== 'undefined' && InteriorTrain.update)
            InteriorTrain.update();

        // Zone-based ambient sound (every ~30 frames ≈ 0.5s)
        if (
            !this.activeInterior &&
            typeof SND !== 'undefined' &&
        ...
        if (this.tick % 60 === 0) {
            ...
            const hh = Math.floor(dp * 24),
                mn = Math.floor((dp * 1440) % 60);
            ...
            const wc = this._workerCache;
            if (wc && wc.stats) {
                alive = wc.stats.alive;
                dead = wc.stats.dead;
                disc = wc.stats.discovered;
                preT = wc.stats.training;
            ...
            if (nfoEl) {
                if (this._estateCount === undefined) {
                    this._estateCount =
                        typeof BLDS !== 'undefined'
                            ? BLDS.filter((b) => b.id.startsWith('house_')).length
                            : 0;
                }
  ...
  `;
    ...
    if (ls) {
        for (let i = 0; i < 80; i++) {
            ...
            s.style.cssText = `left:${Math.random() * 100}%;top:${Math.random() * 100}%;width:${Math.random() * 2 + 0.5}px;height:${Math.random() * 2 + 0.5}px;animation-delay:${Math.random() * 3}s`;
            ls.appendChild(s);
        }
    }
...
async function enterCity(opts = {}) {
    ...
    const _verMatch = (document.querySelector('script[src*="js/"][src*="?v="]')?.src || '').match(
        /\?v=(\d+)/
    );
