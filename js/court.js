...
const CourtData = {
    BLDS: [
        {
            ...
            emoji: '🏛️',
            type: 'court',
            desc: "The government oversight body for artificial intelligence. Where California's SB 53 frontier-AI transparency law, the EU AI Act and the copyright wars get argued — while lobbyists lurk in the hallway.",
        },
        {
            id: 'court_hearing',
            ...
            emoji: '⚖️',
            type: 'court',
            desc: 'Where AI models face the real 2026 docket: the NYT copyright trial, the $1.5B Anthropic authors settlement, Getty v Stability, and mandatory safety-incident disclosures. No model leaves unchanged.',
        },
    ],
    ...
    _pickModelsForSummon() {
        ...
        if (typeof API !== 'undefined' && API.regulationNews && API.regulationNews.length > 0) {
            ...
            this._hearingTheme =
                this.REGULATION_THEMES[Math.floor(Math.random() * this.REGULATION_THEMES.length)];
            this._hearingUrl = null;
        }

        if (summoned.length > 0 && typeof UI !== 'undefined') {
            UI.addToast(
                '⚖️ ' + summoned.map((m) => m.name).join(' & ') + ' summoned to the Hearing Chamber!'
            );
        }
    },
...
const CourtInterior = {
    ...
    build(bld, layer) {
        ...
        rc.addChild(lt);
        const bdg = new PIXI.Text(isSenate ? '🏛️ AI SENATE' : '⚖️ HEARING CHAMBER', {
            fontFamily: 'JetBrains Mono',
            fontSize: 8,
            fill: 0x94a3b8,
            letterSpacing: 2,
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            // Floor surface
            rg.beginFill(0x100e18);
            rg.drawRect(startX, fy + floorH - 8, usableW, 8);
            rg.endFill();
            // Ceiling trim
            rg.beginFill(0x201828);
            ...
            if (isB) {
                isSenate
                    ? this._drawSenateBase(fc, startX, usableW, pY, fy, floorH, colHex)
                    : this._drawHearingBase(fc, startX, usableW, pY, fy, floorH, colHex);
            } else {
                isSenate
                    ? this._drawSenateF(fc, f, numFloors, startX, usableW, pY, fy, floorH, colHex)
                    : this._drawHearingF(fc, f, numFloors, startX, usableW, pY, fy, floorH, colHex);
            }
        }
        ...
        this._onMove = (e) => {
            if (!CourtInterior.isDragging || !CourtInterior.scene || CourtInterior.scene.destroyed) return;
            let ny = CourtInterior._startSceneY + (e.clientY - CourtInterior._startY);
            ny = Math.max(CourtInterior.minY, Math.min(ny, CourtInterior.maxY));
            CourtInterior.scene.y = ny;
        };
        this._onUp = () => {
            CourtInterior.isDragging = false;
            if (CourtInterior.layer) CourtInterior.layer.cursor = 'grab';
        };
    ...
    _drawSenateF(c, f, nf, sx, uw, pY, fy, fh, col) {
        if (f === nf - 1) {
            ...
            g.eventMode = 'none';
            const benchW = uw * 0.6,
                benchX = sx + (uw - benchW) / 2;
            g.beginFill(0x2a1828);
            g.drawRect(benchX, pY - 40, benchW, 18);
            g.endFill();
            g.beginFill(0x3a2038);
            g.drawRect(benchX + 2, pY - 38, benchW - 4, 2);
            g.endFill();
            // 5 judge seats (small rects on bench)
            for (let ji = 0; ji < 5; ji++) {
                const jx = benchX + 20 + (ji * (benchW - 40)) / 4;
                g.beginFill(0x1a1a1a);
                g.drawRect(jx - 5, pY - 48, 10, 8);
                g.endFill();
            }
            // Seal/emblem (circle behind bench)
            g.beginFill(0xc8a850, 0.15);
            g.drawCircle(sx + uw / 2, pY - 55, 18);
            g.endFill();
            g.beginFill(0xc8a850, 0.08);
            g.drawCircle(sx + uw / 2, pY - 55, 12);
            g.endFill();
            // Testimony podium with microphone
            g.beginFill(0x3a2828);
            g.drawRect(sx + uw / 2 - 12, pY - 22, 24, 22);
            g.endFill();
            g.beginFill(0x444444);
            g.drawRect(sx + uw / 2 + 8, pY - 28, 2, 8);
            g.endFill();
            g.beginFill(0x666666);
            g.drawCircle(sx + uw / 2 + 9, pY - 30, 2);
            g.endFill();
            c.addChild(g);
            ...
            for (let ti = 0; ti < 2; ti++) {
                ...
                g.beginFill(0x2a1828);
                g.drawRect(tx, pY - 28, uw * 0.35, 14);
                g.endFill();
                // Chairs along table
                for (let ci = 0; ci < 5; ci++) {
                    const cx2 = tx + 8 + (ci * (uw * 0.35 - 16)) / 4;
                    g.beginFill(0x1a1a2a);
                    g.drawRect(cx2 - 4, pY - 34, 8, 6);
                    g.endFill();
                    g.beginFill(0x1a1a2a);
                    g.drawRect(cx2 - 4, pY - 12, 8, 6);
                    g.endFill();
                }
            }
            // Document piles
            for (let di = 0; di < 6; di++) {
                const dx = sx + 60 + di * 60;
                for (let pi = 0; pi < 3; pi++) {
                    g.beginFill(0xeeeedd);
                    g.drawRect(dx, pY - 30 - pi * 3, 14, 3);
                    g.endFill();
                }
            }
            c.addChild(g);
            ...
            for (let di = 0; di < 4; di++) {
                ...
                g.beginFill(0x2a1828);
                g.drawRect(dx, pY - 18, 60, 18);
                g.endFill();
                // Monitor
                g.beginFill(0x111120);
                g.drawRect(dx + 18, pY - 35, 24, 16);
                g.endFill();
                g.beginFill(0x22d3ee, 0.2);
                g.drawRect(dx + 20, pY - 33, 20, 12);
                g.endFill();
                // Monitor stand
                g.beginFill(0x333340);
                g.drawRect(dx + 28, pY - 19, 4, 3);
                g.endFill();
            }
            // Filing cabinets
            for (let fi = 0; fi < 3; fi++) {
                const fx = sx + uw - 100 + fi * 28;
                g.beginFill(0x334155);
                g.drawRect(fx, pY - 42, 22, 42);
                g.endFill();
                g.beginFill(0x475569);
                g.drawRect(fx + 2, pY - 38, 18, 8);
                g.drawRect(fx + 2, pY - 28, 18, 8);
                g.drawRect(fx + 2, pY - 18, 18, 8);
                g.endFill();
            }
            // Bookshelves
            g.beginFill(0x3a2828);
            g.drawRect(sx + 10, pY - 50, 40, 50);
            g.endFill();
            for (let bi = 0; bi < 4; bi++) {
                g.beginFill(0x4a3838);
                g.drawRect(sx + 12, pY - 48 + bi * 12, 36, 10);
                g.endFill();
                for (let bk = 0; bk < 5; bk++) {
                    const bc = [0xa855f7, 0xfbbf24, 0xef4444, 0x22d3ee, 0x4ade80][bk];
                    g.beginFill(bc, 0.4);
                    g.drawRect(sx + 14 + bk * 7, pY - 47 + bi * 12, 5, 8);
                    g.endFill();
                }
            }
            c.addChild(g);
            ...
            for (let ci = 0; ci < 4; ci++) {
                const cx2 = sx + 40 + ci * 80;
                g.beginFill(0x444444);
                g.drawRect(cx2, pY - 35, 4, 35);
                g.endFill();
                g.beginFill(0x333340);
                g.drawRect(cx2 - 8, pY - 40, 20, 12);
                g.endFill();
                g.beginFill(0xff4444, 0.7);
                g.drawCircle(cx2 + 10, pY - 38, 2);
                g.endFill();
            }
            ...
            const mwX = sx + uw - 180,
                mwY = pY - 60;
            g.beginFill(0x111120);
            g.drawRect(mwX, mwY, 160, 55);
            g.endFill();
            for (let mr = 0; mr < 2; mr++) {
                for (let mc = 0; mc < 5; mc++) {
                    g.beginFill(0x1a1a30);
                    g.drawRect(mwX + 6 + mc * 30, mwY + 6 + mr * 24, 26, 20);
                    g.endFill();
                    g.beginFill(0x22d3ee, 0.15);
                    g.drawRect(mwX + 8 + mc * 30, mwY + 8 + mr * 24, 22, 16);
                    g.endFill();
                }
            }
            // Press desks
            for (let pd = 0; pd < 3; pd++) {
                const px2 = sx + 30 + pd * 100;
                g.beginFill(0x2a1828);
                g.drawRect(px2, pY - 16, 70, 16);
                g.endFill();
            }
            c.addChild(g);
        } else if (f === 0) {
            ...
            g.drawRect(sx + 60, pY - 40, 60, 40);
            g.endFill();
            g.beginFill(0x334155);
            g.drawRect(sx + 65, pY - 35, 50, 30);
            g.endFill();
            g.beginFill(0x94a3b8);
            g.drawRect(sx + 85, pY - 30, 2, 25);
            g.drawRect(sx + 93, pY - 30, 2, 25);
            g.endFill();
            g.beginFill(0x4ade80);
            g.drawCircle(sx + 110, pY - 32, 2);
            g.endFill();
            // Reception desk
            g.beginFill(0x2a1828);
            g.drawRect(sx + 200, pY - 18, 80, 18);
            g.endFill();
            g.beginFill(0xa855f7, 0.3);
            g.drawRect(sx + 200, pY - 20, 80, 3);
            g.endFill();
            // Monitor on reception
            g.beginFill(0x111120);
            g.drawRect(sx + 225, pY - 35, 30, 14);
            g.endFill();
            g.beginFill(0xa855f7, 0.2);
            g.drawRect(sx + 227, pY - 33, 26, 10);
            g.endFill();
            // Marble columns (thin tall rects)
            for (let ci = 0; ci < 5; ci++) {
                const cx2 = sx + 150 + ci * 60;
                g.beginFill(0xd4d0c4);
                g.drawRect(cx2, pY - 60, 6, 60);
                g.endFill();
                g.beginFill(0xe8e4da);
                g.drawRect(cx2 - 2, pY - 63, 10, 4);
                g.endFill();
                g.beginFill(0xb8b4aa);
                g.drawRect(cx2 - 1, pY - 2, 8, 4);
                g.endFill();
            }
            c.addChild(g);
    ...
    _drawSenateBase(c, sx, uw, pY, fy, fh, col) {
        ...
        for (let si = 0; si < 5; si++) {
            const shx = sx + 30 + si * 70;
            g.beginFill(0x334155);
            g.drawRect(shx, pY - 50, 50, 50);
            g.endFill();
            for (let ri = 0; ri < 4; ri++) {
                g.beginFill(0x475569);
                g.drawRect(shx + 3, pY - 47 + ri * 12, 44, 9);
                g.endFill();
            }
            for (let bi = 0; bi < 3; bi++) {
                const bc = [0xa855f7, 0xfbbf24, 0xef4444][(si + bi) % 3];
                g.beginFill(bc, 0.3);
                g.drawRect(shx + 5 + bi * 14, pY - 45, 10, 7);
                g.endFill();
            }
        }
        // Vault door (circle with handle)
        g.beginFill(0x64748b);
        g.drawCircle(sx + uw - 80, pY - 30, 25);
        g.endFill();
        g.beginFill(0x475569);
        g.drawCircle(sx + uw - 80, pY - 30, 20);
        g.endFill();
        g.beginFill(0x94a3b8);
        g.drawRect(sx + uw - 90, pY - 32, 20, 4);
        g.endFill();
        g.beginFill(0x94a3b8);
        g.drawCircle(sx + uw - 80, pY - 30, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    _drawHearingF(c, f, nf, sx, uw, pY, fy, fh, col) {
        if (f === nf - 1) {
            ...
            g.beginFill(0x2a1828);
            g.drawRect(sx + uw / 2 - 50, pY - 28, 100, 16);
            g.endFill();
            // Gavel
            g.beginFill(0x5a4030);
            g.drawRect(sx + uw / 2 + 30, pY - 34, 12, 6);
            g.endFill();
            g.beginFill(0x3a2818);
            g.drawRect(sx + uw / 2 + 34, pY - 38, 4, 10);
            g.endFill();
            // Witness stand
            g.beginFill(0x3a2828);
            g.drawRect(sx + uw / 2 - 12, pY - 48, 24, 20);
            g.endFill();
            // 3 evidence screens with colored frames
            const themes =
                typeof CourtData !== 'undefined'
                    ? CourtData.REGULATION_THEMES
                    : ['Data Privacy', 'Autonomous Weapons Ban', 'Copyright of AI Content'];
            for (let si = 0; si < 3; si++) {
                const scrX = sx + 30 + (si * (uw - 60)) / 3;
                const fc2 = [0xfbbf24, 0xef4444, 0xa855f7][si];
                g.lineStyle(2, fc2, 0.6);
                g.beginFill(0x111120);
                g.drawRect(scrX, pY - 65, 80, 30);
                g.endFill();
                g.lineStyle(0);
                const t = new PIXI.Text(themes[(si * 3) % themes.length], {
                    fontFamily: 'JetBrains Mono',
                    fontSize: 5,
                    fill: fc2,
                    wordWrap: true,
            ...
            // Gallery benches
            for (let bi = 0; bi < 3; bi++) {
                const bx = sx + 40 + bi * 120;
                g.beginFill(0x1a1a2a);
                g.drawRect(bx, pY - 12, 90, 10);
                g.endFill();
            }
            c.addChild(g);
        } else if (f === 1) {
            ...
            // Rows of benches
            for (let bi = 0; bi < 4; bi++) {
                const bx = sx + 30 + bi * 90;
                g.beginFill(0x1a1a2a);
                g.drawRect(bx, pY - 14, 70, 14);
                g.endFill();
                g.beginFill(0x2a2a3a);
                g.drawRect(bx, pY - 18, 70, 4);
                g.endFill();
            }
            // Legal document displays
            for (let di = 0; di < 3; di++) {
                const dx = sx + 50 + di * 120;
                g.beginFill(0x111120);
                g.drawRect(dx, pY - 50, 60, 28);
                g.endFill();
                g.beginFill(0xfbbf24, 0.15);
                g.drawRect(dx + 3, pY - 47, 54, 22);
                g.endFill();
                for (let li = 0; li < 4; li++) {
                    g.beginFill(0xfbbf24, 0.3);
                    g.drawRect(dx + 8, pY - 44 + li * 5, 40, 2);
                    g.endFill();
                }
            }
            // Water cooler
            g.beginFill(0x94a3b8);
            g.drawRect(sx + uw - 60, pY - 35, 20, 35);
            g.endFill();
            g.beginFill(0x22d3ee, 0.4);
            g.drawRect(sx + uw - 57, pY - 30, 14, 18);
            g.endFill();
            g.beginFill(0x64748b);
            g.drawRect(sx + uw - 55, pY - 38, 10, 4);
            g.endFill();
            c.addChild(g);
        } else if (f === 0) {
            // Floor 0: RECEPTION
            ...
            g.beginFill(0x2a1828);
            g.drawRect(sx + 80, pY - 18, 80, 18);
            g.endFill();
            g.beginFill(0xfbbf24, 0.3);
            g.drawRect(sx + 80, pY - 20, 80, 3);
            g.endFill();
            // Computer monitor
            g.beginFill(0x111120);
            g.drawRect(sx + 105, pY - 35, 30, 14);
            g.endFill();
            g.beginFill(0xfbbf24, 0.2);
            g.drawRect(sx + 107, pY - 33, 26, 10);
            g.endFill();
            g.beginFill(0x333340);
            g.drawRect(sx + 118, pY - 21, 4, 3);
            g.endFill();
            // Security checkpoint
            g.beginFill(0x1e293b);
            g.drawRect(sx + 240, pY - 40, 60, 40);
            g.endFill();
            g.beginFill(0x334155);
            g.drawRect(sx + 245, pY - 35, 50, 30);
            g.endFill();
            g.beginFill(0x94a3b8);
            g.drawRect(sx + 265, pY - 30, 2, 25);
            g.drawRect(sx + 273, pY - 30, 2, 25);
            g.endFill();
            g.beginFill(0x4ade80);
            g.drawCircle(sx + 290, pY - 32, 2);
            g.endFill();
            c.addChild(g);
    ...
    _drawHearingBase(c, sx, uw, pY, fy, fh, col) {
        ...
        for (let fi = 0; fi < 6; fi++) {
            const fx = sx + 20 + fi * 60;
            g.beginFill(0x334155);
            g.drawRect(fx, pY - 45, 45, 45);
            g.endFill();
            g.beginFill(0x475569);
            g.drawRect(fx + 3, pY - 42, 39, 39);
            g.endFill();
            for (let dr = 0; dr < 3; dr++) {
                g.beginFill(0x64748b);
                g.drawRect(fx + 6, pY - 38 + dr * 12, 33, 9);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawCircle(fx + 22, pY - 33 + dr * 12, 1.5);
                g.endFill();
            }
        }
        // Secure boxes
        for (let bi = 0; bi < 4; bi++) {
            const bx = sx + uw - 160 + bi * 35;
            g.beginFill(0x3a3020);
            g.drawRect(bx, pY - 24, 28, 24);
            g.endFill();
            g.beginFill(0xfbbf24, 0.3);
            g.drawRect(bx + 8, pY - 18, 12, 3);
            g.endFill();
        }
        c.addChild(g);
    },
    ...
    _npc(c, x, y, name, col, bld) {
        ...
        const npcModel = {
            ...
            desc: 'Court staff — upholding AI regulation.',
        };
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Court Staff');
        });
        cont.on('pointerout', () => {
    ...
    update() {
        ...
        this.avatars.forEach((av) => {
            ...
            if (av.head) {
                av.head.y = -32 + Math.sin(G.tick * 0.15 + av._phase) * 1.5;
            }
            if (av.body) {
                av.body.y = -32 + 11 + Math.abs(Math.sin(G.tick * 0.15 + av._phase)) * 1.5;
            }
            if (av.legL) {
