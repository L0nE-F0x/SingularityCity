...
const Personality = {
    ...
    deriveTraits(m) {
        ...
        if (archType.includes('multimodal') || archType.includes('vision') || archType.includes('image'))
            creative += 0.3;
        ...
        if (
            nm.includes('code') ||
            nm.includes('coder') ||
            nm.includes('starcoder') ||
            nm.includes('deepseek-coder')
        )
            coding += 0.4;
        if (
            nm.includes('vision') ||
    ...
    getBuildingBias(m, currentAct, dp) {
        ...
        if (currentAct === 'lunch' || currentAct === 'socialize') {
            ...
            if (dom === 'creative' && Math.random() < t.creative)
                return Math.random() < 0.6 ? 'city_park' : 'park'; // creatives love the park
    ...
    CHAT: {
        coding: [
            'refactoring the attention layer...',
            'git push --force 😈',
            'who needs comments anyway',
            'O(n log n) feels RIGHT',
            'debugging at 3am hits different',
            'vim or emacs? trick question, use VS Code',
            "rubber duck said it's a type error",
        ...
        social: [
            'hey everyone! 👋',
            'love this city energy',
            "let's form a study group!",
            'anyone tried the new cafe?',
        ...
        creative: [
            ...
            'what if we tried it upside down?',
            'this sunset inspires me',
            'art is just compressed information',
            'generating new aesthetics',
            'pixel by pixel...',
            'beauty in the gradient',
            'the medium is the message',
            'visualizing the latent space',
            'composing a symphony of tokens',
            'creative block? never heard of her',
        ...
        analytical: [
            ...
            "p < 0.001 or it didn't happen",
            'optimizing the objective function',
            'Bayesian reasoning kicks in',
            'the math checks out',
