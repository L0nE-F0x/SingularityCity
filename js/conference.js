...
const ConferenceData = {
    CONFERENCES: [
        ...
        },
        {
            ...
            color: 0xfbbf24,
            hex: '#fbbf24',
            theme: 'Conference on Computer Vision and Pattern Recognition',
        },
        {
            id: 'aaai',
            name: 'AAAI',
            ...
            color: 0xa78bfa,
            hex: '#a78bfa',
            theme: 'Association for the Advancement of AI',
        },
    ],
    ...
    _injectBuilding() {
        ...
        const bld = {
            ...
            desc:
                ...
                ' is in session — posters, keynotes, and demo booths.',
        ...
        if (G.tick % 3600 === 0) {
            ...
            if (!was && this._active) {
                ...
                if (typeof UI !== 'undefined')
                    UI.addToast('🎓 ' + this._active.name + ' conference has begun!');
                ...
                this._bld = null;
                if (typeof Environment !== 'undefined' && Environment.buildBuildings)
...
const ConferenceInterior = {
    ...
    build(bld, layer) {
        ...
        sg.endFill();
        rc.addChild(sg);
        const lt = new PIXI.Text((conf.name + ' Convention Center').toUpperCase(), {
        ...
        rc.addChild(lt);
        const bdg = new PIXI.Text('🎓 CONVENTION CENTER', {
            fontFamily: 'JetBrains Mono',
            fontSize: 8,
            fill: 0x94a3b8,
            letterSpacing: 2,
        ...
        this._onMove = (e) => {
            if (
                !ConferenceInterior.isDragging ||
                !ConferenceInterior.scene ||
                ConferenceInterior.scene.destroyed
            )
                return;
            let ny = ConferenceInterior._startSceneY + (e.clientY - ConferenceInterior._startY);
            ny = Math.max(ConferenceInterior.minY, Math.min(ny, ConferenceInterior.maxY));
            ConferenceInterior.scene.y = ny;
        };
        this._onUp = () => {
            ConferenceInterior.isDragging = false;
            if (ConferenceInterior.layer) ConferenceInterior.layer.cursor = 'grab';
        };
    ...
    _drawKeynoteHall(c, sx, uw, pY, fy, fh, col, conf) {
        ...
        g.eventMode = 'none';
        const stageW = uw * 0.6,
            stageX = sx + (uw - stageW) / 2;
        g.beginFill(0x2a1f4e);
        g.drawRoundedRect(stageX, pY - 20, stageW, 20, 4);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawRect(stageX + 2, pY - 20, stageW - 4, 3);
        g.endFill();
        // Presentation screen with conference-colored border
        g.lineStyle(2, col, 0.8);
        g.beginFill(0x0a0a1e);
        g.drawRect(stageX + stageW / 2 - 60, pY - 60, 120, 35);
        g.endFill();
        g.lineStyle(0);
        // Screen content lines
        for (let i = 0; i < 3; i++) {
            g.beginFill(0x334466, 0.6);
            g.drawRect(stageX + stageW / 2 - 50, pY - 55 + i * 10, 100, 3);
            g.endFill();
        }
        // Podium
        g.beginFill(0x3a3060);
        g.drawRect(stageX + stageW / 2 - 8, pY - 18, 16, 16);
        g.endFill();
        g.beginFill(col, 0.5);
        g.drawRect(stageX + stageW / 2 - 5, pY - 16, 10, 6);
        g.endFill();
        // Spotlight beams (additive blend triangles)
        const sp = new PIXI.Graphics();
        sp.eventMode = 'none';
        sp.blendMode = PIXI.BLEND_MODES.ADD;
        sp.beginFill(col, 0.06);
        sp.moveTo(stageX + stageW * 0.3, fy + 4);
        sp.lineTo(stageX + stageW * 0.15, pY - 20);
        sp.lineTo(stageX + stageW * 0.45, pY - 20);
        sp.closePath();
        sp.endFill();
        sp.beginFill(col, 0.06);
        sp.moveTo(stageX + stageW * 0.7, fy + 4);
        sp.lineTo(stageX + stageW * 0.55, pY - 20);
        sp.lineTo(stageX + stageW * 0.85, pY - 20);
        sp.closePath();
        sp.endFill();
    ...
    _drawPosterSession(c, sx, uw, pY, fy, fh, col) {
        ...
        for (let i = 0; i < numPosters; i++) {
            ...
            g.drawRect(px, pY - 55, 24, 45);
            g.endFill();
            // Conference-colored header strip
            g.beginFill(col);
            g.drawRect(px, pY - 55, 24, 6);
            g.endFill();
            // Gray content lines
            for (let ln = 0; ln < 4; ln++) {
                g.beginFill(0xaaaaaa);
                g.drawRect(px + 3, pY - 46 + ln * 9, 18, 2);
                g.endFill();
            }
            // Small desk in front of each
            g.beginFill(0x1e293b);
            g.drawRect(px - 2, pY - 8, 28, 8);
            g.endFill();
            g.beginFill(col, 0.2);
            g.drawRect(px - 2, pY - 10, 28, 2);
            g.endFill();
        }
        c.addChild(g);
    },
    ...
    _drawDemoBooths(c, sx, uw, pY, fy, fh, col) {
        ...
        for (let i = 0; i < numBooths; i++) {
            ...
            g.eventMode = 'none';
            // Booth frame rect
            g.lineStyle(2, labCol, 0.6);
            g.beginFill(0x1a1830);
            g.drawRect(bx, pY - 50, 50, 50);
            g.endFill();
            g.lineStyle(0);
            // Lab-colored accent top
            g.beginFill(labCol);
            g.drawRect(bx, pY - 50, 50, 6);
            g.endFill();
            // Demo screen (small dark rect)
            g.beginFill(0x0a0a18);
            g.drawRect(bx + 8, pY - 40, 34, 18);
            g.endFill();
            g.beginFill(labCol, 0.2);
            g.drawRect(bx + 10, pY - 38, 30, 14);
            g.endFill();
            // Booth desk
            g.beginFill(0x1e293b);
            g.drawRect(bx + 5, pY - 8, 40, 8);
            g.endFill();
            c.addChild(g);
        }
    },
    ...
    _drawRegistration(c, sx, uw, pY, fy, fh, col, conf) {
        ...
        const deskW = uw * 0.5,
            deskX = sx + (uw - deskW) / 2;
        g.beginFill(0x1e293b);
        g.drawRect(deskX, pY - 16, deskW, 16);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawRect(deskX, pY - 18, deskW, 3);
        g.endFill();
        // Badge printer (small rect with green light)
        g.beginFill(0x222240);
        g.drawRect(deskX + deskW + 15, pY - 14, 20, 14);
        g.endFill();
        g.beginFill(0x4ade80);
        g.drawCircle(deskX + deskW + 25, pY - 16, 2);
        g.endFill();
        // Info board
        g.beginFill(0x1a1a30);
        g.drawRect(sx + 30, pY - 50, 30, 40);
        g.endFill();
        g.beginFill(col, 0.4);
        g.drawRect(sx + 30, pY - 50, 30, 5);
        g.endFill();
        for (let ln = 0; ln < 3; ln++) {
            g.beginFill(0x444466);
            g.drawRect(sx + 34, pY - 42 + ln * 10, 22, 3);
            g.endFill();
        }
        // Welcome banner with conference name
        g.beginFill(col, 0.7);
        g.drawRect(sx + uw / 2 - 60, fy + 8, 120, 14);
        g.endFill();
        c.addChild(g);
        const bt = new PIXI.Text('WELCOME TO ' + conf.name, {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0xffffff,
            fontWeight: 'bold',
    ...
    _drawBasement(c, sx, uw, pY, fy, fh, col) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            g.drawRect(cx, pY - 25, 30, 25);
            g.endFill();
            g.beginFill(0x4a4030);
            g.drawRect(cx + 2, pY - 23, 26, 21);
            g.endFill();
            g.beginFill(0x5a5040);
            g.drawRect(cx + 5, pY - 15, 20, 2);
            g.endFill();
        }
        // Cable drums (circles)
        for (let i = 0; i < 3; i++) {
            ...
            g.drawCircle(dx, pY - 12, 10);
            g.endFill();
            g.beginFill(0x222240);
            g.drawCircle(dx, pY - 12, 6);
            g.endFill();
            g.beginFill(col, 0.3);
            g.drawCircle(dx, pY - 12, 2);
            g.endFill();
        }
        // AV rack
        g.beginFill(0x1a1a2e);
        g.drawRect(sx + uw / 2 - 15, pY - 45, 30, 45);
        g.endFill();
        g.beginFill(0x111120);
        g.drawRect(sx + uw / 2 - 12, pY - 42, 24, 39);
        g.endFill();
        for (let s = pY - 40; s < pY - 5; s += 8) {
            g.beginFill(0x1a1a30);
            g.drawRect(sx + uw / 2 - 10, s, 20, 6);
            g.endFill();
            g.beginFill(0x4ade80);
            g.drawCircle(sx + uw / 2 - 6, s + 3, 1);
            g.endFill();
        }
        // Conduit lines
        g.beginFill(0xef4444, 0.15);
        g.drawRect(sx, fy + 4, uw, 2);
        g.endFill();
        for (let cx2 = sx + 30; cx2 < sx + uw; cx2 += 60) {
            g.beginFill(0xef4444, 0.3);
            g.drawCircle(cx2, fy + 5, 1.5);
            g.endFill();
        }
        c.addChild(g);
    },
    ...
    _npc(c, x, y, name, col, bld) {
        ...
        const npcModel = {
            ...
            desc: 'Conference staff — managing the event.',
        };
        ...
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Conference Staff');
        });
        cont.on('pointerout', () => {
