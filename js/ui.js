...
const NOTIFY = {
    ...
    send(t, b) {
        if (this.permission === 'granted') {
            try {
                new Notification(t, { body: b, icon: '/icon-192.png', badge: '/favicon-32.png' });
            } catch (e) {}
        }
    },
};
...
const UI = {
    ...
    updateTicker() {
        ...
        if (!item) {
            const src =
                typeof API !== 'undefined' && API.liveNews && API.liveNews.length > 0 ? API.liveNews : NEWS;
            const rawItem = src[(typeof API !== 'undefined' ? API.newsIdx : 0) % src.length] || {
                headline: 'Welcome to Singularity City!',
                url: '#',
            };
            item = rawItem;
        }
        ...
        if (item.url && item.url !== '#') {
            ...
            const sb = item.source
                ? `<span style="color:var(--cy);font-size:8px;margin-right:6px">[${safeSource}]</span>`
                : '';
    ...
    generateCityTickerEvent() {
        ...
        if (roll < 0.2) {
            const withElo = G.models
                .filter((m) => BM[m.id] && BM[m.id].ELO)
                .sort((a, b) => (BM[b.id].ELO || 0) - (BM[a.id].ELO || 0));
            if (withElo.length >= 2) {
                const top = withElo[0];
                const lab = LABS[top.lab] || { name: top.lab };
                return {
                    headline: `${top.name} holds the #1 ELO ranking at ${BM[top.id].ELO} — ${lab.name} leads the frontier`,
                    icon: '👑',
                    color: '#fbbf24',
                };
            }
        ...
        if (roll < 0.35) {
            ...
            return {
                headline: `Singularity City population: ${count} AI models across ${labs} labs`,
                icon: '🏙️',
                color: '#4ade80',
            };
        }
        ...
        if (roll < 0.5) {
            ...
            if (labKeys.length >= 2) {
                ...
                return {
                    headline: `${LABS[a].name} (${countA} models) vs ${LABS[b].name} (${countB} models) — the race continues`,
                    icon: '⚔️',
                    color: '#a78bfa',
                };
            }
        ...
        if (roll < 0.65) {
            const withCost = G.models.filter(
                (m) => COSTS[m.id] && COSTS[m.id].input && COSTS[m.id].input > 0
            );
            if (withCost.length > 0) {
                const cheapest = withCost.sort((a, b) => COSTS[a.id].input - COSTS[b.id].input)[0];
                return {
                    headline: `Budget pick: ${cheapest.name} at $${COSTS[cheapest.id].input}/1M input tokens`,
                    icon: '💰',
                    color: '#4ade80',
                };
            }
        ...
        if (roll < 0.8) {
            ...
            if (active.length > 0) {
                ...
                return {
                    headline: `Spotlight: ${m.name} by ${lab.name}${stat}`,
                    icon: '✦',
                    color: lab.color || '#22d3ee',
                };
            }
        }
        ...
        if (dp < 0.25)
            return {
                headline: 'Night shift: AI models are sleeping in the residential zone',
                icon: '🌙',
                color: '#94a3b8',
            };
        if (dp < 0.35)
            return {
                headline: 'Dawn commute: models heading to their HQs via metro',
                icon: '🌅',
                color: '#fb923c',
            };
        if (dp > 0.45 && dp < 0.55)
            return {
                headline: 'Lunch rush at the café — models socializing in the park',
                icon: '☕',
                color: '#fbbf24',
            };
        if (dp > 0.82)
            return {
                headline: 'Evening commute: models heading home to the residential zone',
                icon: '🌆',
                color: '#f97316',
            };

        return {
            headline: `${G.models.length} AI citizens going about their day in Singularity City`,
            icon: '🏙️',
            color: '#22d3ee',
        };
    },
    ...
    addLog(msg) {
        ...
        document.getElementById('logEntries').innerHTML = this.scanLog
            .map((e) => `<div class="slog-e"><span class="slog-tm">${e.t}</span>${e.msg}</div>`)
            .join('');
    },
    ...
    tip(obj, title, sub) {
        ...
        if (
            obj.listenerCount &&
            (obj.listenerCount('pointerover') > 0 || obj.listenerCount('pointertap') > 0)
        )
            return obj;
        obj._tipT = title;
        obj._tipS = sub || '';
    ...
    initInteriorTips() {
        ...
        const structural = [
            'drawRoof',
            'drawRoomInterior',
            'drawNegativeSpaceWall',
            'drawBasementInterior',
            'drawSiloInterior',
            'drawDoor',
            'drawAvatar',
            'drawNPC',
        ...
        if (typeof InteriorRes !== 'undefined')
            UI.autoTipModule(InteriorRes, { overrides, exclude: structural });
        if (typeof InteriorCity !== 'undefined')
            UI.autoTipModule(InteriorCity, { overrides, exclude: structural });
        if (typeof SpaceInterior !== 'undefined')
            UI.autoTipModule(SpaceInterior, { overrides, exclude: structural });
        ...
        if (typeof InteriorDC !== 'undefined')
            UI.autoTipModule(InteriorDC, {
                include: {
        ...
        if (typeof UniversityInterior !== 'undefined')
            UI.autoTipModule(UniversityInterior, {
                include: {
        ...
        if (typeof InteriorLegacy !== 'undefined')
            UI.autoTipModule(InteriorLegacy, {
                include: {
        ...
        if (typeof InteriorVCRow !== 'undefined')
            UI.autoTipModule(InteriorVCRow, {
                include: {
        ...
        if (typeof InteriorBar !== 'undefined')
            UI.autoTipModule(InteriorBar, {
                include: {
        ...
        if (typeof InteriorBlackMarket !== 'undefined')
            UI.autoTipModule(InteriorBlackMarket, {
                include: {
        ...
        if (typeof CourtInterior !== 'undefined')
            UI.autoTipModule(CourtInterior, {
                include: {
        ...
        if (typeof ConferenceInterior !== 'undefined')
            UI.autoTipModule(ConferenceInterior, {
                include: {
        ...
        if (typeof InteriorNPC !== 'undefined')
            UI.autoTipModule(InteriorNPC, {
                include: {
        ...
        if (typeof InteriorAmbassadorRes !== 'undefined')
            UI.autoTipModule(InteriorAmbassadorRes, {
                include: {
        ...
        if (typeof InteriorLongevity !== 'undefined')
            UI.autoTipModule(InteriorLongevity, {
                include: {
                    _buildFloorProps: { title: 'Lab Equipment' },
                    _drawReceptionDesk: { title: 'Reception Desk' },
                },
            });
        if (typeof InteriorEmbassy !== 'undefined')
            UI.autoTipModule(InteriorEmbassy, {
                include: {
    ...
    panelTab(tab, btn) {
        ...
        else document.querySelector('.ipanel-tab')?.classList.add('active');
        const m = this.selModel;
        if (!m) return;
        const ct = document.getElementById('panelContent');
        if (!ct) return;

        if (tab !== 'bench' && UI.eloChart) {
            UI.eloChart.destroy();
            UI.eloChart = null;
        }
        if (tab !== 'cost' && UI.roiChart) {
            UI.roiChart.destroy();
            UI.roiChart = null;
        }
        ...
        if (tab === 'info') {
          ...
          <div class="ipanel-grid" style="padding-top:5px">
            ...
            if (!c || (c.input === undefined && c.output === undefined)) {
                ct.innerHTML =
                    '<div style="padding:20px;text-align:center;color:var(--t3);font-size:10px">No pricing data.</div>';
                return;
            }
            const a = typeof avgBM === 'function' ? avgBM(m.id) : 0;
            const bang = a && c.output ? (a / c.output).toFixed(1) : null;
          ...
          ${bang ? `<div class="ipanel-stat"><span class="ipanel-lbl">Bang/Buck</span><span class="ipanel-val" style="color:#22d3ee">${bang}</span></div>` : ''}</div>`;
            ...
            setTimeout(() => {
                ...
                const scatterData = G.models
                    .filter((x) => COSTS[x.id] && typeof avgBM === 'function' && avgBM(x.id))
                    .map((x) => {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    label: function (ctx) {
                                        return `${ctx.raw.modelName}: $${ctx.raw.x} / ${ctx.raw.y}%`;
                                    },
                                },
                            },
                        },
    ...
    selectBld(b) {
        ...
        if (isDcFab) {
            col =
                (lab && lab.color) ||
                (typeof DC_OPERATORS !== 'undefined' && DC_OPERATORS[b.lab] && DC_OPERATORS[b.lab].color) ||
                (b.dcData && b.dcData.color) ||
                '#06b6d4';
        } else {
        ...
        if (isDcFab) {
            ...
            const opName =
                (lab && lab.name) ||
                (typeof DC_OPERATORS !== 'undefined' && DC_OPERATORS[b.lab] && DC_OPERATORS[b.lab].name) ||
                b.lab ||
                '';
            subtitle =
                (isDc ? 'Data Center' : 'Chip Fabrication') + (opName ? ` · ${escapeHTML(opName)}` : '');
        ...
        // ─── SPACE ZONE building info ───
            ...
            const org = b.org && typeof SPACE_ORGS !== 'undefined' ? SPACE_ORGS[b.org] : null;
            const veh =
                b.org && typeof SpaceRockets !== 'undefined'
                    ? SpaceRockets.vehicleName(b.org)
                    : org && org.vehicle;
            html += `<div style="margin:0 16px 16px;padding:10px;background:var(--cd);border:1px solid var(--bd);border-radius:6px;font-size:10px;line-height:1.6">`;
        ...
        // ─── POWER FACILITY panel — real 2026 AI-energy deal + live output ───
            ...
            if (src.online)
                html += `<div>${src.status === 'construction' ? '🚧' : '✅'} <b>Status:</b> ${escapeHTML(src.online)}</div>`;
        ...
        // ─── AI COURT panel — real 2026 legal docket ───
            ...
            if (typeof CourtData !== 'undefined' && Array.isArray(CourtData.REGULATION_THEMES)) {
                ...
                CourtData.REGULATION_THEMES.forEach((t) => {
                    html += `<span style="font-size:9px;padding:2px 6px;background:var(--sf);border:1px solid var(--bd);border-radius:3px">${escapeHTML(t)}</span>`;
                });
                html += `</div></div>`;
            }
        }
        // ─── AI ACADEMY panel — real 2026 curriculum + faculty ───
            ...
            if (Array.isArray(b.curriculum) && b.curriculum.length) {
                ...
                html += `<div style="display:flex;flex-wrap:wrap;gap:4px">`;
                b.curriculum.forEach((c) => {
                    html += `<span style="font-size:9px;padding:2px 6px;background:var(--sf);border:1px solid var(--bd);border-radius:3px">${escapeHTML(c)}</span>`;
                });
                html += `</div></div>`;
            }
            if (Array.isArray(b.faculty) && b.faculty.length) {
                ...
                html += `<div style="display:flex;flex-wrap:wrap;gap:4px">`;
                b.faculty.forEach((f) => {
                    html += `<span style="font-size:9px;padding:2px 6px;background:var(--sf);border:1px solid var(--bd);border-radius:3px">${escapeHTML(f)}</span>`;
                });
                html += `</div></div>`;
            }
        ...
        // ─── THE BACKBONE panel — live network + real 2026 infra brief ───
            ...
            if (b.milestone)
                html += `<div style="margin-top:4px;color:var(--t2)">📡 ${escapeHTML(b.milestone)}</div>`;
            html += `</div>`;
        ...
        // ─── AGENT DISTRICT panel — live agent ops + real 2026 framework roster ───
            ...
            const headline = {
                agents_orchestrator:
                    '🎛️ <b>Pattern:</b> role-based crews + stateful graphs coordinate the swarm',
            ...
            if (typeof AGENT_FRAMEWORKS !== 'undefined') {
                ...
                Object.values(AGENT_FRAMEWORKS).forEach((fw) => {
                    ...
                    if (fw.milestone)
                        html += `<div style="font-size:9px;color:var(--t2);margin-top:1px">${escapeHTML(fw.milestone)}</div>`;
                    html += `</div>`;
                });
                html += `</div>`;
            }
        ...
        // ─── LONGEVITY WING panel — live research + real 2026 AI-bio roster ───
            ...
            const headline = {
                ...
                longevity_discovery:
                    "💊 <b>Milestone:</b> Insilico's Rentosertib — first AI-designed drug validated in humans",
                longevity_trials:
                    '🏥 <b>In the clinic:</b> Isomorphic & Retro dosing the first AI-designed drugs (2026)',
            ...
            if (typeof LONGEVITY_COMPANIES !== 'undefined') {
                ...
                Object.values(LONGEVITY_COMPANIES).forEach((co) => {
                    ...
                    if (co.milestone)
                        html += `<div style="font-size:9px;color:var(--t2);margin-top:1px">${escapeHTML(co.milestone)}</div>`;
                    html += `</div>`;
                });
                html += `</div>`;
            }
        ...
        // ─── VC ROW panel — real 2026 fund profile ───
            ...
            if (firm) {
                ...
                if (firm.recentDeal)
                    html += `<div>🤝 <b>Recent deal:</b> ${escapeHTML(firm.recentDeal)}</div>`;
                html += `</div>`;
                if (firm.milestone) {
                    html += `<div style="margin:0 16px 16px;padding:10px;background:var(--cd);border:1px solid var(--bd);border-radius:6px;font-size:10px;line-height:1.6">`;
                    html += `<div style="font-weight:700;margin-bottom:4px;color:${col}">📡 LATEST MILESTONE</div>`;
        ...
        // ─── ALIGNMENT INSTITUTE panel — safety research brief ───
            ...
            const shieldCol =
                typeof b.shield === 'number' ? '#' + b.shield.toString(16).padStart(6, '0') : col;
            html += `<div style="margin:0 16px 16px;padding:10px;background:var(--cd);border:1px solid var(--bd);border-radius:6px;font-size:10px;line-height:1.6">`;
            ...
            if (b.milestone)
                html += `<div style="margin-top:6px;padding-top:6px;border-top:1px dashed var(--bd)">📡 <b>Latest:</b> <span style="color:var(--t2)">${escapeHTML(b.milestone)}</span></div>`;
        ...
        // ─── EMBASSY panel — AI policy brief for the country ───
            ...
            const accentCol =
                typeof b.accent === 'number' ? '#' + b.accent.toString(16).padStart(6, '0') : col;
            html += `<div style="margin:0 16px 16px;padding:10px;background:var(--cd);border:1px solid var(--bd);border-radius:6px;font-size:10px;line-height:1.6">`;
            ...
            if (b.stance)
                html += `<div style="margin-top:4px">⚖️ <b>Stance:</b> ${escapeHTML(b.stance)}</div>`;
            if (b.milestone)
                html += `<div style="margin-top:6px;padding-top:6px;border-top:1px dashed var(--bd)">📡 <b>2026:</b> <span style="color:var(--t2)">${escapeHTML(b.milestone)}</span></div>`;
        ...
        // ─── VISITOR MONUMENT panel ───
            ...
            const vt =
                typeof VisitorTracker !== 'undefined'
                    ? VisitorTracker
        ...
        // ─── HQ model list (exclude DC/fab and space) ───
            ...
            if (res.length > 0) {
                ...
                res.forEach((r) => {
                    const s = getStage(r.rel, r.ret, r.phase);
                    const a = typeof avgBM === 'function' ? avgBM(r.id) : 0;
        ...
        const isConstructionSite =
            (b.id.startsWith('dc_') || b.id.startsWith('fab_')) &&
            b.dcData &&
            b.dcData.status === 'construction';
        if (!isConstructionSite) {
            G.models.forEach((m, i) => {
                const stg = getStage(m.rel, m.ret, m.phase);
                const { act } = getAct(stg, dp, i, m);
                const ai =
                    typeof ACTS !== 'undefined' && ACTS[act]
                        ? ACTS[act]
                        : { indoor: true, icon: '💻', verb: 'processing' };
                ...
                if (refs && refs.bld === b.id) {
                    ...
                    if (b.id === 'ai_jail' && typeof JailData !== 'undefined' && !JailData.isJailed(m.id))
                        return;
        ...
        if (curOcc.length > 0) {
            ...
            const msg =
                nearby > 0
                    ? `🌳 ${nearby} citizen${nearby > 1 ? 's' : ''} enjoying the park`
                    : '🌳 An open-air social space for citizens to gather';
    ...
    renderCensusList() {
        ...
        const matches = (m) => {
            ...
            const hay = [m.name, m.tal, m.lab, lab?.name, m.phase, relYear, m.arch?.params]
                .filter(Boolean)
                .join(' ')
                .toLowerCase();
        ...
        if (q && total === 0) {
            ...
            h =
                `<div style="font-size:9px;color:var(--t2);margin-bottom:4px">${total} match${total === 1 ? '' : 'es'}</div>` +
                h;
        }
    ...
    showCompare() {
        ...
        const pan = document.getElementById('comparePan');
        const ms = this.compareList.map((id) => G.models.find((m) => m.id === id)).filter(Boolean);
        if (ms.length < 2) {
            pan.innerHTML =
                '<button class="ipanel-x" onclick="document.getElementById(\'compareOv\').classList.remove(\'open\')">✕</button><div class="ov-title">⚖️ COMPARE</div><div style="text-align:center;padding:40px;color:var(--t3)">Select 2+ models via ⚖️ buttons.</div>';
            return;
        }
    ...
    showLaunches() {
        ...
        if (!launches.length) {
            ...
            launches.forEach((l, i) => {
                ...
                const dateStr = net.toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
                });
                const timeStr = net.toLocaleTimeString('en-US', {
                    hour: '2-digit',
                    minute: '2-digit',
                    hour12: true,
                });
    ...
    showFamilyTree() {
        ...
        labKeys.forEach((lk) => {
            ...
            edges.forEach((e) => {
                nodes.add(e.id);
                if (e.children) e.children.forEach((c) => nodes.add(c));
            });
            const childSet = new Set();
            edges.forEach((e) => {
                if (e.children) e.children.forEach((c) => childSet.add(c));
            });
            ...
            let treeCount = 0;
            nodes.forEach((n) => {
                if (G.models.find((x) => x.id === n)) treeCount++;
            });
            ...
            <div style="display:flex;align-items:center;gap:5px;flex:1;padding:5px 10px;background:var(--sf);border:1px solid ${retired ? 'var(--bd)' : lab.color + '22'};border-radius:6px;cursor:pointer;transition:border-color 0.15s,background 0.15s;${retired ? 'opacity:0.5;' : ''}"
              ...
              ${bmBar}
                ...
                children.forEach((c, i) => {
                    o += render(c, depth + 1, i === children.length - 1);
                });
                return o;
            };
    ...
    showCalendar() {
        G.unlockAchieve('calendar_view');
        document.getElementById('calendarOv').classList.add('open');
        const pan = document.getElementById('calendarPan');
        const now = new Date();
        const up = AI_EVENTS.filter((e) => new Date(e.date) >= now).sort(
            (a, b) => new Date(a.date) - new Date(b.date)
        );
        const past = AI_EVENTS.filter((e) => new Date(e.date) < now).sort(
        ...
        if (up.length) {
            h +=
                '<div style="font-size:9px;color:var(--ac);margin-bottom:8px;font-weight:700">UPCOMING</div>';
            up.forEach((e) => {
                const d = new Date(e.date);
                const days = Math.ceil((d - now) / 864e5);
                const tc = e.type === 'conference' ? '#22d3ee' : e.type === 'release' ? '#4ade80' : '#facc15';
        ...
        if (past.length) {
            ...
            past.forEach((e) => {
                h += `<div style="padding:6px 8px;background:var(--cd);border:1px solid var(--bd);border-radius:4px;margin-bottom:3px;opacity:.6"><div style="font-size:8px;color:var(--t2)">${new Date(e.date).toLocaleDateString('en', { month: 'short', day: 'numeric' })} — ${e.name}</div></div>`;
            });
        }
        h += '</div>';
        pan.innerHTML = h;
    ...
    showCompute() {
        document.getElementById('computeOv').classList.add('open');
        const pan = document.getElementById('computePan');
    ...
    showCostDashboard() {
        ...
        const models = G.models
            .filter((m) => COSTS[m.id] && COSTS[m.id].input > 0)
            .sort((a, b) => (COSTS[a.id]?.input || 999) - (COSTS[b.id]?.input || 999));
        if (!models.length) {
            pan.innerHTML = `<button class="ipanel-x" onclick="document.getElementById('costOv').classList.remove('open')">✕</button><div class="ov-title">💰 PRICE WAR TRACKER</div><div style="text-align:center;padding:40px;color:var(--t3)">No pricing data yet.</div>`;
            return;
        }

        // Find cheapest frontier (top 15 by benchmark)
        const ranked = G.models
            .filter((m) => BM[m.id] && avgBM(m.id))
            .sort((a, b) => avgBM(b.id) - avgBM(a.id));
        const frontier = ranked.slice(0, 15);
        const cheapFrontier = frontier
            .filter((m) => COSTS[m.id] && COSTS[m.id].input > 0)
            .sort((a, b) => COSTS[a.id].input - COSTS[b.id].input);
        ...
        if (models[0]) {
            const ch = models[0];
            const chLab = LABS[ch.lab] || { name: ch.lab, color: '#64748b' };
        ...
        if (cheapFrontier[0]) {
            const cf = cheapFrontier[0];
            const cfLab = LABS[cf.lab] || { name: cf.lab, color: '#64748b' };
            const cfAvg = avgBM(cf.id);
        ...
        // Best bang-for-buck
        const withBang = models
            .filter((m) => avgBM(m.id) && COSTS[m.id].output > 0)
            .map((m) => ({ m, bang: avgBM(m.id) / COSTS[m.id].output }))
            .sort((a, b) => b.bang - a.bang);
        if (withBang[0]) {
            const bb = withBang[0].m;
            const bbLab = LABS[bb.lab] || { name: bb.lab, color: '#64748b' };
        ...
        sortedLabs.forEach((labKey) => {
            ...
            labModels.forEach((m) => {
                ...
                h += avg
                    ? `<span style="font-size:8px;color:${avg > 80 ? '#4ade80' : '#facc15'};width:30px;text-align:right">${avg}%</span>`
    ...
    showNewsPanel() {
        document.getElementById('newsOv').classList.add('open');
        const pan = document.getElementById('newsPan');
        ...
        if (!API.liveNews.length)
            h +=
                '<div style="text-align:center;padding:30px;color:var(--t3);font-size:9px">Fetching headlines...</div>';
        else {
            if (API.liveNews[0]?.source === 'Fallback')
                h +=
                    '<div style="text-align:center;padding:8px;margin-bottom:8px;font-size:8px;color:var(--pk);background:#f472b610;border:1px solid #f472b622;border-radius:4px">Showing fallback headlines</div>';
    ...
    showAnalyst() {
        document.getElementById('analystOv').classList.add('open');
        const pan = document.getElementById('analystPan');
        const providerName =
            { xai: 'Grok', openai: 'ChatGPT', anthropic: 'Claude', google: 'Gemini' }[G.apiProvider] || 'AI';
    ...
    _settingsToggleSection() {
        ...
        const reduceMotionOn =
            typeof G.reduceMotionOn === 'function' ? G.reduceMotionOn() : p.reduceMotion === true;
        const uiScale = p.uiScale || 1;
        const scaleOpt = (v, lbl) =>
            `<option value="${v}" ${Math.abs(uiScale - v) < 0.01 ? 'selected' : ''}>${lbl}</option>`;
        return `
    ...
    showSettings() {
        ...
        document.getElementById('settingsPan').innerHTML =
            `<button class="ipanel-x" onclick="document.getElementById('settingsOv').classList.remove('open')">✕</button><div class="ov-title">⚙️ SETTINGS</div>
    ...
    updateModelDatalist() {
        ...
        if (!dl) return;
        let o = [];
        if (pv === 'xai') o = ['grok-3-latest'];
        else if (pv === 'openai') o = ['gpt-4o', 'o3-mini'];
        else if (pv === 'anthropic') o = ['claude-sonnet-4-20250514'];
    ...
    setupMobileGestures() {
        ...
        const onTouchStart = (e) => {
            ...
            const scrollableTarget = e.target.closest(
