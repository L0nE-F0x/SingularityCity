...
const InteriorDC = {
    ...
    build(bld, layer) {
        ...
        // ─── VOID + DATA CABLES (rendered AFTER elevator to cover the shaft below basement) ───
        ...
        // Underground tunnel area (like the metro exterior)
        vm.beginFill(0x050508); vm.drawRect(0, basementBottom + 6, G.vpW, 3000); vm.endFill();
        // Data cable runs (colorful horizontal lines like exterior metro tunnel)
        ...
        this._onMove = (e) => { if(!InteriorDC.isDragging) return; let ny=InteriorDC._startSceneY+(e.clientY-InteriorDC._startY); ny=Math.max(InteriorDC.minY,Math.min(ny,InteriorDC.maxY)); InteriorDC.scene.y=ny; };
