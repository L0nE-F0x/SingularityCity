...
const InteriorPower = {
    ...
    build(bld, layer) {
        ...
        const layouts = {
            power_nuclear: {
                floors: ['Waste Storage', 'Turbine Hall', 'Reactor Core', 'Control Room'],
                roofLabel: 'CRANE CLEAN ENERGY',
                col: 0x4ade80,
                npcs: ['Reactor Tech', 'Grid Operator'],
            },
            power_coal: {
                floors: ['Fuel Skids', 'Turbine Gensets', 'Exhaust Deck', 'Control Deck'],
                roofLabel: 'GAS TURBINE ARRAY',
                col: 0xf59e0b,
                npcs: ['Turbine Foreman'],
            },
            power_hydro: {
                floors: ['Turbine Hall', 'Generator Room', 'Control Room'],
                roofLabel: 'COLUMBIA HYDRO',
                col: 0x06b6d4,
                npcs: ['Dam Keeper'],
            },
            power_solar: {
                floors: ['Battery Vault', 'Inverter Room', 'Monitoring Station'],
                roofLabel: 'SOLAR + STORAGE',
                col: 0xfbbf24,
                npcs: ['Solar Engineer'],
            },
            power_wind: {
                floors: ['Nacelle Access', 'Monitoring Hub'],
                roofLabel: 'SUNZIA WIND',
                col: 0x60a5fa,
                npcs: ['Turbine Tech'],
            },
            power_smr: {
                floors: ['TRISO Fuel Bay', 'Reactor Cell', 'Control Room'],
                roofLabel: 'HERMES 2 · KAIROS',
                col: 0x2dd4bf,
                npcs: ['SMR Engineer'],
            },
            power_fusion: {
                floors: ['Capacitor Vault', 'Fusion Chamber', 'Pulse Control'],
                roofLabel: 'POLARIS · HELION',
                col: 0xc084fc,
                npcs: ['Plasma Physicist'],
            },
        };
        const layout = layouts[bld.id] || {
            floors: ['Operations'],
        ...
        sg.lineStyle(2, layout.col, 0.8);
        sg.drawRect(bX, bY, bW, bH);
        sg.endFill();
        sg.lineStyle(0);
        rc.addChild(sg);
        const lt = new PIXI.Text(layout.roofLabel, {
            fontFamily: 'JetBrains Mono',
            fontSize: 11,
            fontWeight: 'bold',
            fill: '#' + layout.col.toString(16).padStart(6, '0'),
            letterSpacing: 2,
        });
        ...
        if (src && (src.operator || src.offtaker)) {
            ...
            const dt = new PIXI.Text(dealTxt, {
                fontFamily: 'JetBrains Mono',
                fontSize: 7,
                fill: 0x94a3b8,
                letterSpacing: 1,
            ...
            if (typeof UI !== 'undefined')
                UI.tip(dt, 'The Deal', src.milestone || 'Real 2026 AI-energy deal');
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            const fl = new PIXI.Text(floorName.toUpperCase(), {
                fontFamily: 'JetBrains Mono',
                fontSize: 7,
                fill: layout.col,
                letterSpacing: 2,
        ...
        vm.drawRect(0, vmY - 4, G.vpW, 10);
        vm.endFill();
        vm.beginFill(0x050508);
        vm.drawRect(0, vmY + 6, G.vpW, 3000);
        vm.endFill();
        const cc = [0xef4444, 0x22d3ee, 0x4ade80, 0xfbbf24, 0xa855f7];
        for (let cy = vmY + 20; cy < vmY + 120; cy += 6) {
            vm.beginFill(cc[Math.floor(Math.random() * cc.length)], 0.15 + Math.random() * 0.25);
            vm.drawRect(0, cy + Math.random() * 3, G.vpW, 1 + Math.random() * 2);
            vm.endFill();
        }
        for (let px = 80; px < G.vpW; px += 150) {
            vm.beginFill(0x111115);
            vm.drawRect(px, vmY + 6, 20, 100);
            vm.endFill();
        }
        ...
        this._onMove = (e) => {
            if (!InteriorPower.isDragging || !InteriorPower.scene || InteriorPower.scene.destroyed) return;
            let ny = InteriorPower._startSceneY + (e.clientY - InteriorPower._startY);
            ny = Math.max(InteriorPower.minY, Math.min(ny, InteriorPower.maxY));
            InteriorPower.scene.y = ny;
        };
        this._onUp = () => {
            InteriorPower.isDragging = false;
            if (InteriorPower.layer) InteriorPower.layer.cursor = 'grab';
        };
    ...
    _drawFloorProps(c, sx, bw, pY, fy, fh, floorName, col, bldId) {
        ...
        if (fn.includes('control')) {
            ...
            for (let mx = sx + 40; mx < sx + bw - 60; mx += 70) {
                ...
                g.drawRect(mx, fy + 14, 50, 30);
                g.endFill();
                g.beginFill(col, 0.2);
                g.drawRect(mx + 2, fy + 16, 46, 26);
                g.endFill();
                for (let ly = fy + 18; ly < fy + 40; ly += 6) {
                    g.beginFill(col, 0.15);
                    g.drawRect(mx + 4, ly, 42, 2);
                    g.endFill();
                }
            }
            for (let dx = sx + 60; dx < sx + bw - 80; dx += 90) {
                g.beginFill(0x334155);
                g.drawRect(dx, pY - 16, 50, 16);
                g.endFill();
                g.beginFill(0x475569);
                g.drawRect(dx, pY - 18, 50, 3);
                g.endFill();
                g.beginFill(0x334155);
                g.drawRect(dx + 15, pY - 8, 14, 8);
                g.endFill();
            }
            ...
            const rx = sx + bw / 2;
            g.beginFill(0x334155);
            g.drawEllipse(rx, pY - 25, 50, 25);
            g.endFill();
            g.beginFill(0x475569);
            g.drawEllipse(rx, pY - 28, 45, 8);
            g.endFill();
            g.beginFill(col, 0.15);
            g.drawEllipse(rx, pY - 25, 35, 18);
            g.endFill();
            g.beginFill(0xfbbf24);
            g.drawRect(rx - 20, pY - 12, 40, 3);
            g.endFill(); // hazard line
            // Pipes
            for (let px = sx + 40; px < sx + bw - 40; px += 80) {
                g.beginFill(0x94a3b8);
                g.drawRect(px, fy + 10, 6, fh - 16);
                g.endFill();
                g.beginFill(0x475569);
                g.drawRect(px - 1, fy + 20, 8, 4);
                g.drawRect(px - 1, fy + 50, 8, 4);
                g.endFill();
            }
            ...
            for (let tx = sx + 60; tx < sx + bw - 60; tx += 100) {
                g.beginFill(0x334155);
                g.drawEllipse(tx + 25, pY - 15, 30, 15);
                g.endFill();
                g.beginFill(0x475569);
                g.drawEllipse(tx + 25, pY - 18, 25, 5);
                g.endFill();
                g.beginFill(col, 0.2);
                g.drawCircle(tx + 25, pY - 15, 8);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawRect(tx + 50, pY - 20, 8, 20);
                g.endFill(); // shaft
            }
        } else if (fn.includes('boiler')) {
            // Boiler drums
            for (let bx = sx + 50; bx < sx + bw - 50; bx += 80) {
                g.beginFill(0x78582e);
                g.drawEllipse(bx + 20, pY - 20, 25, 20);
                g.endFill();
                g.beginFill(0xef4444, 0.2);
                g.drawEllipse(bx + 20, pY - 20, 18, 14);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawRect(bx + 10, fy + 10, 4, fh - 16);
                g.drawRect(bx + 30, fy + 10, 4, fh - 16);
                g.endFill();
            }
            this._npc(c, sx + 100, pY, 'Stoker', 0xf59e0b);
        } else if (fn.includes('conveyor')) {
            g.beginFill(0x334155);
            g.drawRect(sx + 30, pY - 8, bw - 60, 8);
            g.endFill();
            for (let cx = sx + 40; cx < sx + bw - 40; cx += 20) {
                g.beginFill(0x1e293b);
                g.drawRect(cx, pY - 10, 8, 10);
                g.endFill();
            }
            for (let cx = sx + 50; cx < sx + bw - 50; cx += 30) {
                g.beginFill(0x444444);
                g.drawRect(cx, pY - 12, 14, 6);
                g.endFill();
            } // coal lumps
            ...
            for (let dx = sx + 40; dx < sx + bw - 40; dx += 35) {
                ...
                g.beginFill(dc, 0.5);
                g.drawRect(dx, pY - 22, 18, 22);
                g.endFill();
                g.beginFill(0x000000, 0.2);
                g.drawRect(dx + 2, pY - 20, 14, 18);
                g.endFill();
                if (fn.includes('waste')) {
                    g.beginFill(0xfbbf24);
                    g.drawRect(dx + 5, pY - 18, 8, 2);
                    g.drawRect(dx + 7, pY - 16, 4, 6);
                    g.endFill();
                } // hazard symbol
            }
            ...
            const mx = sx + bw / 2;
            g.beginFill(0x1f2430);
            g.drawRect(mx - 90, pY - 32, 180, 24);
            g.endFill();
            g.beginFill(0x374151);
            g.drawRect(mx - 96, pY - 30, 8, 20);
            g.drawRect(mx + 88, pY - 30, 8, 20);
            g.endFill();
            ...
            for (let cx2 = mx - 80; cx2 <= mx + 80; cx2 += 20) {
                g.beginFill(0xc084fc, 0.55);
                g.drawRect(cx2 - 2, pY - 34, 4, 28);
                g.endFill();
            }
            // Plasma core glow (brightest at center — where the FRCs collide)
            g.beginFill(0xe879f9, 0.35);
            g.drawEllipse(mx, pY - 20, 60, 6);
            g.endFill();
            g.beginFill(0x22d3ee, 0.45);
            g.drawEllipse(mx, pY - 20, 30, 4);
            g.endFill();
            g.beginFill(0xffffff, 0.6);
            g.drawEllipse(mx, pY - 20, 10, 2.5);
            g.endFill();
        } else if (fn.includes('capacitor')) {
            // Capacitor bank cabinets + overhead busbars
            g.beginFill(0xf59e0b, 0.5);
            g.drawRect(sx + 30, fy + 10, bw - 60, 3);
            g.endFill();
            for (let cx3 = sx + 40; cx3 < sx + bw - 60; cx3 += 55) {
                g.beginFill(0x374151);
                g.drawRect(cx3, pY - 34, 40, 34);
                g.endFill();
                g.beginFill(0x4b5563);
                g.drawRect(cx3, pY - 34, 40, 4);
                g.endFill();
                g.beginFill(0xc084fc, 0.5);
                for (let ly = pY - 26; ly < pY - 6; ly += 6) g.drawRect(cx3 + 4, ly, 32, 2);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawRect(cx3 + 18, fy + 13, 3, fy + 40 - (fy + 13));
                g.endFill();
                g.beginFill(0xfbbf24);
                g.drawCircle(cx3 + 35, pY - 30, 1.6);
                g.endFill();
            }
            ...
            for (let hx = sx + 50; hx < sx + bw - 70; hx += 90) {
                g.beginFill(0x475569);
                g.drawRect(hx, fy + 12, 44, 24);
                g.endFill();
                g.beginFill(0x334155);
                g.drawPolygon([hx, fy + 36, hx + 44, fy + 36, hx + 28, pY - 12, hx + 16, pY - 12]);
                g.endFill();
                g.beginFill(0x2dd4bf, 0.7);
                for (let pi = 0; pi < 8; pi++)
                    g.drawCircle(hx + 8 + (pi % 4) * 10, fy + 18 + Math.floor(pi / 4) * 9, 3);
                g.endFill();
                g.beginFill(0x1f2937);
                g.drawRect(hx + 18, pY - 12, 8, 12);
                g.endFill();
            }
        } else if (fn.includes('fuel')) {
            // Gas fuel skids — pipe manifolds, valves, bottle rack
            g.beginFill(0xb45309);
            g.drawRect(sx + 30, pY - 22, bw - 60, 4);
            g.endFill();
            g.beginFill(0xd97706, 0.6);
            g.drawRect(sx + 30, pY - 21, bw - 60, 1.5);
            g.endFill();
            for (let vx = sx + 60; vx < sx + bw - 60; vx += 70) {
                g.beginFill(0x374151);
                g.drawRect(vx, pY - 28, 14, 14);
                g.endFill();
                g.beginFill(0xef4444);
                g.drawCircle(vx + 7, pY - 30, 3);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawRect(vx + 5, pY - 14, 4, 14);
                g.endFill();
            }
            for (let bx2 = sx + bw - 120; bx2 < sx + bw - 60; bx2 += 12) {
                g.beginFill(0x9ca3af);
                g.drawRoundedRect(bx2, pY - 26, 8, 26, 3);
                g.endFill();
                g.beginFill(0xef4444, 0.7);
                g.drawRect(bx2 + 2, pY - 28, 4, 3);
                g.endFill();
            }
        } else if (fn.includes('battery')) {
            // Grid battery racks with state-of-charge bars
            for (let bx3 = sx + 40; bx3 < sx + bw - 60; bx3 += 60) {
                g.beginFill(0xe5e7eb, 0.9);
                g.drawRect(bx3, pY - 32, 44, 32);
                g.endFill();
                g.beginFill(0xcbd5e1);
                g.drawRect(bx3, pY - 32, 44, 4);
                g.endFill();
                const soc = 0.4 + ((bx3 * 7) % 50) / 100;
                g.beginFill(0x1f2937);
                g.drawRect(bx3 + 5, pY - 24, 34, 6);
                g.endFill();
                g.beginFill(0x4ade80, 0.9);
                g.drawRect(bx3 + 6, pY - 23, 32 * soc, 4);
                g.endFill();
        ...
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined')
                UI.selectModel({
                    ...
                    name,
                    isNPC: true,
                    role: name + ' — Power Grid',
                    lab: 'other',
                    desc: 'Power Grid facility staff.',
                });
        });
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined') UI.showTooltip(e, name, 'Power Grid Staff');
        });
        cont.on('pointerout', () => {
