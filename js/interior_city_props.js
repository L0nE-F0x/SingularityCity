...
const InteriorCityProps = {
    ...
    drawRoof(roofH, startX, usableW, colHex, lab, bld) {
        ...
        const textToDisplay = bld.lab
            ? safeLabName.toUpperCase()
            : `${bld.emoji || ''} ${(bld.name || 'HQ').toUpperCase()}`.trim();
        ...
        if (starsLayer) {
            ...
            if (night) {
                starsLayer.children.forEach((s) => {
                    s.alpha = 0.15 + Math.abs(Math.sin(G.tick * 0.03 + (s._phase || 0))) * 0.5;
                });
            }
        }
    },
    ...
    drawRoomInterior(gfx, x, y, w, h, colHex, isCeo, windowX, windowW, theme = 'general') {
        ...
        if (theme === 'server_core') {
            ...
            wallCol = 0x111115;
            floorCol = 0x0a0a0f;
            if (theme === 'arena_commentary') {
                wallCol = 0x0f1020;
                floorCol = 0x0a0a15;
            }
            if (theme === 'arena_trophy') {
                wallCol = 0x1a1510;
                floorCol = 0x100e08;
                lightCol = 0xfbbf24;
                beamAlpha = 0.15;
            ...
            wallCol = 0x271e1a;
            floorCol = 0x17120f;
            if (theme === 'cafe_rooftop') {
                wallCol = 0x1a2030;
                floorCol = 0x0f1520;
                lightCol = 0xfbbf24;
                beamAlpha = 0.1;
            }
            if (theme === 'cafe_kitchen') {
                wallCol = 0x1e2025;
                floorCol = 0x14161a;
            }
        ...
        if (theme === 'server_core') {
            lightCol = 0x38bdf8;
            beamAlpha = 0.1;
        } else if (theme.startsWith('arena')) {
            lightCol = 0xef4444;
            beamAlpha = 0.25;
        } else if (theme === 'zen_garden') {
            lightCol = 0x86efac;
    ...
    drawLake(c, x, y) {
        ...
        g.drawEllipse(x, y - 5, 90, 18);
        g.endFill();
        g.beginFill(0x0284c7, 0.6);
        g.drawEllipse(x, y - 4, 85, 14);
        g.endFill();
    ...
    drawTent(c, x, y, col) {
        ...
        g.eventMode = 'none';
        g.beginFill(cHex);
        g.drawPolygon([x, y - 40, x - 30, y - 5, x + 30, y - 5]);
        g.endFill();
        g.beginFill(0x11111a);
        g.drawPolygon([x, y - 30, x - 10, y - 5, x + 10, y - 5]);
        g.endFill();
    ...
    drawCampfire(c, x, y) {
        ...
        g.drawPolygon([x - 8, y - 5, x, y - 18, x + 8, y - 5]);
        g.endFill();
        g.beginFill(0xfacc15);
        g.drawPolygon([x - 4, y - 5, x, y - 12, x + 4, y - 5]);
        g.endFill();
        c.addChild(g);

        const glow = new PIXI.Graphics();
        glow.beginFill(0xf97316, 0.3);
        glow.drawCircle(x, y - 10, 35);
        glow.endFill();
    ...
    drawPunchingBag(c, x, y) {
        ...
        g.drawRect(x - 15, y - 80, 30, 6);
        g.endFill();
        g.beginFill(0x888888);
        g.drawRect(x - 1, y - 74, 2, 20);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawRoundedRect(x - 12, y - 54, 24, 45, 6);
        g.endFill();
        g.beginFill(0x991b1b);
        g.drawRect(x - 12, y - 40, 24, 15);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawWeightBench(c, x, y) {
        ...
        g.drawRoundedRect(x - 20, y - 15, 40, 6, 2);
        g.endFill();
        g.beginFill(0x444444);
        g.drawRect(x - 15, y - 9, 4, 9);
        g.drawRect(x + 11, y - 9, 4, 9);
        g.endFill();
        g.beginFill(0x555555);
        g.drawRect(x - 25, y - 30, 4, 30);
        g.drawRect(x + 21, y - 30, 4, 30);
        g.endFill();
        g.beginFill(0xcccccc);
        g.drawRect(x - 30, y - 30, 60, 3);
        g.endFill();
        g.beginFill(0x222222);
        g.drawRect(x - 30, y - 38, 6, 19);
        g.drawRect(x + 24, y - 38, 6, 19);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawTreadmill(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1a1a2e);
        g.drawPolygon([x - 25, y, x + 20, y, x + 25, y - 8, x - 20, y - 8]);
        g.endFill();
        g.beginFill(0x111111);
        g.drawPolygon([x - 23, y - 1, x + 18, y - 1, x + 23, y - 7, x - 18, y - 7]);
        g.endFill();
        g.beginFill(0x33334a);
        g.drawRect(x + 15, y - 35, 6, 30);
        g.endFill();
        g.beginFill(0x111111);
        g.drawRect(x + 10, y - 45, 12, 12);
        g.endFill();
        const glow = new PIXI.Graphics();
        glow.beginFill(0x22d3ee, 0.8);
        glow.drawRect(x + 12, y - 43, 8, 8);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawServerWeights(c, x, y) {
        ...
        g.drawRect(x - 25, y - 10, 50, 2);
        g.endFill();
        g.beginFill(0x11111a);
        g.drawRect(x - 20, y - 20, 10, 20);
        g.drawRect(x + 10, y - 20, 10, 20);
        g.endFill();
        g.beginFill(0x4ade80);
        g.drawCircle(x - 18, y - 15, 1);
        g.drawCircle(x + 12, y - 15, 1);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawRing(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1a1a2e);
        g.drawRect(x - 60, y - 10, 120, 10);
        g.endFill();
        g.beginFill(0x0ea5e9, 0.4);
        g.drawRect(x - 60, y - 10, 120, 2);
        g.endFill();
        g.beginFill(0x33334a);
        g.drawRect(x - 60, y - 40, 4, 30);
        g.drawRect(x + 56, y - 40, 4, 30);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawRect(x - 60, y - 35, 120, 2);
        g.endFill();
        g.beginFill(0xffffff);
        g.drawRect(x - 60, y - 25, 120, 2);
        g.endFill();
        g.beginFill(0x3b82f6);
        g.drawRect(x - 60, y - 15, 120, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawPlant(c, x, y) {
        ...
        g.drawRect(x - 6, y - 10, 12, 10);
        g.endFill();
        g.beginFill(0x4ade80);
        g.drawCircle(x, y - 16, 6);
        g.drawCircle(x - 5, y - 12, 5);
        g.drawCircle(x + 5, y - 12, 5);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawAvatar(arg1, arg2, arg3, arg4) {
        ...
        if (arg1 && typeof arg1.addChild === 'function') {
            ...
            container =
                this.scene || (typeof InteriorCityCore !== 'undefined' ? InteriorCityCore.scene : null);
            x = arg1;
            y = arg2;
            col = arg3;
        }
        ...
        g.eventMode = 'none';
        g.beginFill(cHex);
        g.drawRoundedRect(x - 6, y - 12, 12, 12, 2);
        g.endFill();
        g.beginFill(0xfdd8b5);
        g.drawCircle(x, y - 16, 5);
        g.endFill();
    ...
    drawReceptionDesk(c, x, y, col) {
        ...
        g.drawRect(x - 15, y - 22, 50, 22);
        g.endFill();
        g.beginFill(cHex, 0.8);
        g.drawRect(x - 17, y - 22, 54, 4);
        g.endFill();
        g.beginFill(0x11111a);
    ...
    drawCouches(c, x, y, col) {
        ...
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(cHex, 0.5);
        g.drawRect(x - 20, y - 10, 40, 10);
        g.beginFill(cHex, 0.3);
    ...
    drawChair(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1a1a2e);
        g.drawRect(x - 5, y - 18, 4, 12);
        g.drawRect(x - 5, y - 8, 12, 3);
        g.beginFill(0x33334a);
        g.drawRect(x + 1, y - 5, 2, 3);
        g.drawRect(x - 3, y - 2, 10, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawBeanbagAndHandheld(c, x, y) {
        ...
        g.drawEllipse(x, y - 5, 12, 5);
        g.drawCircle(x, y - 8, 7);
        g.endFill();
        g.beginFill(0x11111a);
        g.drawRect(x - 12, y - 15, 6, 10);
        g.endFill();
        g.beginFill(0x22d3ee);
        g.drawRect(x - 11, y - 14, 4, 6);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawPingPongTable(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x33334a);
        g.drawRect(x - 15, y - 15, 4, 15);
        g.drawRect(x + 11, y - 15, 4, 15);
        g.endFill();
        g.beginFill(0x10b981);
        g.drawRect(x - 20, y - 17, 40, 4);
        g.endFill();
        g.beginFill(0xffffff);
        g.drawRect(x - 1, y - 22, 2, 5);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawIndoorPool(c, x, y) {
        ...
        g.drawRect(x - 30, y - 4, 60, 4);
        g.endFill();
        g.beginFill(0x0284c7, 0.6);
        g.drawRect(x - 28, y - 3, 56, 3);
        g.endFill();
        const glow = new PIXI.Graphics();
        glow.beginFill(0x38bdf8, 0.3);
        glow.drawRect(x - 28, y - 15, 56, 12);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawBiophilicDivider(c, x, y) {
        ...
        g.drawCircle(x - 4, y - 15, 8);
        g.drawCircle(x + 4, y - 12, 6);
        g.drawCircle(x, y - 20, 7);
        g.endFill();
        g.beginFill(0x22c55e);
        g.drawCircle(x - 2, y - 16, 4);
        g.drawCircle(x + 2, y - 14, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawCanteen(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x33334a);
        g.drawRect(x - 25, y - 15, 4, 15);
        g.drawRect(x + 21, y - 15, 4, 15);
        g.endFill();
        g.beginFill(0xf1f5f9);
        g.drawRect(x - 30, y - 18, 60, 4);
        g.endFill();
        g.beginFill(0xd97706);
        g.drawRect(x - 20, y - 8, 8, 3);
        g.drawRect(x, y - 8, 8, 3);
        g.drawRect(x + 12, y - 8, 8, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawDeskAndPC(c, x, y, col) {
        ...
        // ─── Desk slab + fascia ───
        g.beginFill(0x1e293b);
        g.drawRect(x - 18, y - 20, 36, 4);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawRect(x - 18, y - 16, 36, 1);
        g.endFill();
        // Front desk legs
        g.beginFill(0x33334a);
        ...
        g.beginFill(0x252538);
        g.drawRect(x + 5, y - 15, 11, 13);
        g.endFill();
        ...
        // ─── Keyboard + key dots ───
        g.beginFill(0x0f172a);
        g.drawRect(x - 9, y - 22, 12, 2);
        g.endFill();
        g.beginFill(0x1e293b);
        g.drawRect(x - 9, y - 22, 12, 0.5);
        g.endFill();
        ...
        // ─── Mousepad + mouse ───
        g.beginFill(0x33334a, 0.6);
        g.drawRect(x + 4, y - 22, 8, 2);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawRect(x + 7, y - 22, 3, 1.5);
        g.endFill();
        ...
        if (cSeed > 0.3) {
            ...
            g.beginFill(mugCol);
            g.drawRect(x - 17, y - 23, 3, 3);
            g.endFill();
            g.beginFill(0x000000, 0.35);
            g.drawRect(x - 14, y - 22, 1, 1.5);
            g.endFill();
            if (cSeed > 0.7) {
                g.beginFill(0xffffff, 0.3);
                g.drawCircle(x - 15.5, y - 26, 0.8);
                g.endFill();
            }
        }
        if (cSeed > 0.55) {
            ...
            g.drawRect(x - 2, y - 26, 2.5, 2);
            g.endFill();
        }
        if (cSeed > 0.4 && cSeed < 0.7) {
            // Paper stack
            g.beginFill(0xf1f5f9);
            g.drawRect(x - 5, y - 21, 5, 1);
            g.endFill();
            g.beginFill(0x94a3b8, 0.55);
            g.drawRect(x - 4, y - 20.5, 3, 0.4);
            g.endFill();
        }
        if (cSeed > 0.62) {
            // Tiny desk plant on right corner
            g.beginFill(0x854d0e);
            g.drawRect(x + 14, y - 22, 3, 2);
            g.endFill();
            g.beginFill(0x166534);
            g.drawCircle(x + 15, y - 23, 1.5);
            g.endFill();
            g.beginFill(0x22c55e);
            g.drawCircle(x + 16, y - 24, 1.2);
            g.endFill();
        }
        if (cSeed > 0.85) {
            ...
            g.lineStyle(0);
            g.beginFill(0x1e293b);
            g.drawCircle(x - 8, y - 31, 1);
            g.endFill();
        }

        // ─── Cable to floor ───
        g.beginFill(0x111118, 0.6);
        g.drawRect(x - 4, y - 22, 0.6, 6);
        g.endFill();
    ...
    drawCollaborationPod(c, x, y, col) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawEllipse(x, y - 15, 20, 15);
        g.endFill();
        g.beginFill(cHex, 0.3);
        g.drawEllipse(x, y - 15, 18, 13);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawRect(x - 10, y - 18, 20, 6);
        g.endFill();
        g.beginFill(cHex);
        g.drawRect(x - 8, y - 17, 16, 4);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawLoungeNook(c, x, y, col) {
        ...
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(cHex, 0.4);
        g.drawRect(x - 20, y - 10, 20, 10);
        g.drawRect(x - 20, y - 20, 10, 20);
        g.endFill();
        g.beginFill(0x33334a);
        g.drawRect(x - 5, y - 12, 15, 12);
        g.endFill();
        g.beginFill(0x22c55e);
        g.drawCircle(x + 2, y - 16, 6);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawTrophy(c, x, y, isGold) {
        ...
        g.endFill();
        g.beginFill(col);
        g.drawPolygon([x - 10, y - 30, x + 10, y - 30, x + 4, y - 10, x - 4, y - 10]);
        g.endFill();
        g.beginFill(col);
        g.drawCircle(x - 12, y - 25, 4);
        g.drawCircle(x + 12, y - 25, 4);
        g.endFill();
        g.beginFill(0x000000);
        g.drawCircle(x - 12, y - 25, 2);
        g.drawCircle(x + 12, y - 25, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawExecutiveLounge(c, x, y, col) {
        ...
        const g = new PIXI.Graphics();
        g.eventMode = 'none';
        g.beginFill(cHex, 0.2);
        g.drawEllipse(x, y - 2, 30, 4);
        g.endFill();
        g.beginFill(0x1a1a2e);
        g.drawRect(x - 25, y - 12, 15, 12);
        g.drawRect(x + 10, y - 12, 15, 12);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawRect(x - 8, y - 8, 16, 8);
        g.endFill();
        g.beginFill(0xffffff, 0.8);
        g.drawRect(x - 5, y - 10, 4, 2);
        g.drawRect(x + 1, y - 10, 4, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawCommandCenter(c, x, y, col) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1a1a2e);
        g.drawRect(x - 20, y - 15, 40, 15);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawRect(x - 18, y - 35, 10, 10);
        g.drawRect(x - 4, y - 38, 12, 12);
        g.drawRect(x + 10, y - 35, 10, 10);
        g.endFill();
        const glow = new PIXI.Graphics();
        glow.beginFill(cHex, 0.5);
        glow.drawRect(x - 17, y - 34, 8, 8);
        glow.drawRect(x - 3, y - 37, 10, 10);
        glow.drawRect(x + 11, y - 34, 8, 8);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawPrivateOasis(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x - 15, y - 8, 30, 8);
        g.endFill();
        g.beginFill(0x0284c7);
        g.drawRect(x - 12, y - 6, 24, 4);
        g.endFill();
        g.beginFill(0x166534);
        g.drawCircle(x - 15, y - 15, 10);
        g.drawCircle(x + 15, y - 12, 8);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawBossDesk(c, x, y, col) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1a1a2e);
        g.drawRect(x - 25, y - 18, 50, 18);
        g.endFill();
        g.beginFill(cHex);
        g.drawRect(x - 28, y - 18, 56, 5);
        g.endFill();
        g.beginFill(0x11111a);
        g.drawRect(x - 5, y - 30, 6, 12);
        g.drawRect(x + 5, y - 30, 6, 12);
        g.endFill();
        g.beginFill(0x22d3ee);
        g.drawRect(x - 4, y - 29, 2, 10);
        g.drawRect(x + 4, y - 29, 2, 10);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawDataVat(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x11111a);
        g.drawEllipse(x, y, 40, 10);
        g.endFill();
        g.beginFill(0x222233);
        g.drawRect(x - 40, y - 10, 80, 10);
        g.endFill();
        g.beginFill(0x22d3ee, 0.15);
        g.lineStyle(2, 0x22d3ee, 0.4);
        g.drawRect(x - 35, y - 80, 70, 70);
        g.endFill();
        g.lineStyle(0);
        g.beginFill(0x0ea5e9, 0.4);
        g.drawRect(x - 30, y - 75, 60, 65);
        g.endFill();
        g.beginFill(0x222233);
        g.drawRect(x - 40, y - 90, 80, 10);
        g.endFill();
        g.beginFill(0x11111a);
        g.drawEllipse(x, y - 90, 40, 10);
        g.endFill();
        c.addChild(g);
        const glow = new PIXI.Graphics();
        glow.beginFill(0x22d3ee, 0.3);
        glow.drawCircle(x, y - 45, 50);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawBrokenServer(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x222233);
        g.drawRect(x - 12, y - 28, 24, 28);
        g.endFill();
        g.beginFill(0x11111a);
        g.drawRect(x - 10, y - 26, 20, 24);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawCircle(x - 5, y - 20, 1);
        g.endFill();
        g.beginFill(0x555566);
        g.drawRect(x - 8, y - 10, 16, 2);
        g.rotation = 0.1;
        g.endFill();
        c.addChild(g);
        const glow = new PIXI.Graphics();
        glow.beginFill(0xef4444, 0.8);
        glow.drawCircle(x - 5, y - 20, 2);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawTombstone(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x555566);
        g.drawRoundedRect(x - 12, y - 25, 24, 25, 10);
        g.endFill();
        g.beginFill(0x33334a);
        g.drawRect(x - 14, y - 4, 28, 4);
        g.endFill();
        g.beginFill(0x222233);
        g.drawRect(x - 1, y - 18, 2, 8);
        g.drawRect(x - 3, y - 16, 6, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawCafeTable(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x33334a);
        g.drawRect(x - 2, y - 15, 4, 15);
        g.endFill();
        g.beginFill(0xf1f5f9);
        g.drawEllipse(x, y - 15, 20, 4);
        g.endFill();
        g.beginFill(0xd97706);
        g.drawRect(x - 18, y - 8, 6, 8);
        g.drawRect(x + 12, y - 8, 6, 8);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawBaristaCounter(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x - 30, y - 25, 60, 25);
        g.endFill();
        g.beginFill(0xb45309);
        g.drawRect(x - 32, y - 25, 64, 4);
        g.endFill();
        g.beginFill(0x94a3b8);
        g.drawRect(x - 20, y - 40, 15, 15);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawRect(x - 18, y - 35, 11, 10);
        g.endFill();
        g.beginFill(0xcbd5e1);
        g.drawRect(x - 15, y - 30, 2, 4);
        g.drawRect(x - 10, y - 30, 2, 4);
        g.endFill();
        g.beginFill(0x334155);
        g.drawPolygon([x + 10, y - 25, x + 25, y - 25, x + 22, y - 35, x + 13, y - 35]);
        g.endFill();
        const glow = new PIXI.Graphics();
        glow.beginFill(0x4ade80, 0.8);
        glow.drawRect(x + 14, y - 33, 7, 5);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawTree(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x4a2e1a);
        g.drawRect(x - 3, y - 30, 6, 30);
        g.endFill();
        g.beginFill(0x1b4332);
        g.drawCircle(x, y - 35, 15);
        g.drawCircle(x - 10, y - 25, 12);
        g.drawCircle(x + 10, y - 25, 12);
        g.endFill();
        g.beginFill(0x2d6a4f);
        g.drawCircle(x, y - 40, 12);
        g.drawCircle(x - 8, y - 32, 10);
        g.drawCircle(x + 8, y - 32, 10);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawArcadeCabinet(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x111111);
        g.drawPolygon([x - 10, y, x - 10, y - 30, x - 8, y - 35, x + 8, y - 35, x + 10, y - 30, x + 10, y]);
        g.endFill();
        g.beginFill(0x222222);
        g.drawRect(x - 8, y - 32, 16, 12);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawRect(x - 12, y - 20, 24, 4);
        g.endFill();
        const glow = new PIXI.Graphics();
        glow.beginFill(0x00ffff, 0.6);
        glow.drawRect(x - 6, y - 30, 12, 8);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawGlampingDome(c, x, y) {
        ...
        // Warm interior glow
        const glow = new PIXI.Graphics();
        glow.beginFill(0xfbbf24, 0.15);
        glow.drawCircle(x, y - 18, 16);
        glow.endFill();
        glow.beginFill(0xfbbf24, 0.3);
        glow.drawCircle(x, y - 16, 8);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        // Base platform
        g.beginFill(0x334155);
        g.drawRect(x - 24, y - 2, 48, 4);
        g.endFill();
        // Door slit
        g.beginFill(0xfbbf24, 0.4);
        g.drawRect(x - 3, y - 12, 6, 12);
        g.endFill();
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawFirePitLounge(c, x, y) {
        ...
        g.beginFill(0x1e293b);
        g.drawEllipse(x, y - 4, 35, 8);
        g.endFill();
        g.beginFill(0x334155);
        g.drawEllipse(x, y - 5, 30, 6);
        g.endFill();
        g.beginFill(0x0f172a);
        g.drawEllipse(x, y - 5, 22, 4);
        g.endFill();
        // Ember bed
        g.beginFill(0xef4444, 0.8);
        g.drawEllipse(x, y - 5, 12, 3);
        g.endFill();
        g.beginFill(0xfbbf24, 0.6);
        g.drawEllipse(x, y - 6, 8, 2);
        g.endFill();
        // Flame particles
        const glow = new PIXI.Graphics();
        glow.beginFill(0xfbbf24, 0.3);
        glow.drawEllipse(x, y - 12, 14, 10);
        glow.endFill();
        glow.beginFill(0xef4444, 0.15);
        glow.drawEllipse(x, y - 16, 20, 14);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        // Designer Adirondack chairs (4 around the pit)
        const chairCol = 0x78350f;
        [-30, -20, 20, 30].forEach((ox) => {
            g.beginFill(chairCol);
            g.drawRect(x + ox - 4, y - 10, 8, 8);
            g.endFill();
            g.beginFill(0x92400e);
            g.drawRect(x + ox - 3, y - 14, 6, 4);
            g.endFill();
        });
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawWhiskeyBar(c, x, y) {
        ...
        g.drawRect(x - 30, y - 20, 60, 20);
        g.endFill();
        g.beginFill(0x4a3520);
        g.drawRect(x - 32, y - 22, 64, 4);
        g.endFill();
        // Bar surface shine
        g.beginFill(0xfbbf24, 0.08);
        g.drawRect(x - 28, y - 20, 56, 2);
        g.endFill();
        // Backlit bottle shelves
        g.beginFill(0x1a1a2e);
        g.drawRect(x - 28, y - 40, 56, 18);
        g.endFill();
        const backlight = new PIXI.Graphics();
        backlight.beginFill(0xfbbf24, 0.2);
        backlight.drawRect(x - 26, y - 38, 52, 14);
        backlight.endFill();
        ...
        for (let i = 0; i < 5; i++) {
            const bx = x - 20 + i * 10;
            g.beginFill(bottleCols[i]);
            g.drawRect(bx, y - 38, 4, 12);
            g.endFill();
            g.beginFill(bottleCols[i], 0.7);
            g.drawRect(bx + 1, y - 42, 2, 4);
            g.endFill();
        }
        // Bar stools
        [-20, -8, 8, 20].forEach((ox) => {
            g.beginFill(0x111111);
            g.drawRect(x + ox - 2, y - 4, 4, 4);
            g.endFill();
            g.beginFill(0x1e293b);
            g.drawCircle(x + ox, y - 6, 4);
            g.endFill();
        });
    ...
    drawInfinityHotTub(c, x, y) {
        ...
        // Stone surround
        g.beginFill(0x334155);
        g.drawEllipse(x, y - 6, 32, 10);
        g.endFill();
        g.beginFill(0x1e293b);
        g.drawEllipse(x, y - 7, 28, 8);
        g.endFill();
        // Water
        g.beginFill(0x0ea5e9, 0.6);
        g.drawEllipse(x, y - 7, 24, 6);
        g.endFill();
        g.beginFill(0x38bdf8, 0.3);
        g.drawEllipse(x, y - 8, 18, 4);
        g.endFill();
        // Steam glow
        const glow = new PIXI.Graphics();
        glow.beginFill(0x38bdf8, 0.12);
        glow.drawEllipse(x, y - 18, 28, 16);
        glow.endFill();
        glow.beginFill(0xffffff, 0.06);
        glow.drawEllipse(x - 8, y - 22, 10, 8);
        glow.drawEllipse(x + 6, y - 20, 8, 6);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        // Rim lights
        g.beginFill(0x0ea5e9, 0.8);
        g.drawCircle(x - 26, y - 6, 2);
        g.drawCircle(x + 26, y - 6, 2);
    ...
    drawPuttingGreen(c, x, y) {
        ...
        // Manicured green
        g.beginFill(0x166534);
        g.drawEllipse(x, y - 4, 40, 8);
        g.endFill();
        g.beginFill(0x15803d);
        g.drawEllipse(x, y - 5, 36, 6);
        g.endFill();
        // Putting hole
        g.beginFill(0x0f172a);
        g.drawCircle(x + 15, y - 5, 3);
        g.endFill();
        // Flag pin
        g.beginFill(0x94a3b8);
        g.drawRect(x + 15, y - 28, 2, 23);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawPolygon([x + 17, y - 28, x + 27, y - 24, x + 17, y - 20]);
        g.endFill();
        // Golf ball
        g.beginFill(0xffffff);
        g.drawCircle(x - 10, y - 4, 2);
        g.endFill();
        // Sand trap
        g.beginFill(0xfbbf24, 0.3);
        g.drawEllipse(x - 25, y - 3, 10, 4);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawHelipad(c, x, y) {
        ...
        // Concrete pad
        g.beginFill(0x334155);
        g.drawRect(x - 30, y - 6, 60, 6);
        g.endFill();
        g.beginFill(0x475569);
        g.drawRect(x - 28, y - 5, 56, 4);
        g.endFill();
        // Circle marking
        g.lineStyle(2, 0xfacc15, 0.8);
        g.drawCircle(x, y - 3, 18);
        g.lineStyle(0);
        // H marking
        const hTxt = new PIXI.Text('H', {
            fontFamily: 'Arial',
            fontSize: 14,
            fill: 0xfacc15,
            fontWeight: 'bold',
        });
        hTxt.anchor.set(0.5, 0.5);
        hTxt.x = x;
        hTxt.y = y - 3;
        // Landing lights (corners)
        g.beginFill(0xfacc15, 0.9);
        g.drawCircle(x - 26, y - 1, 2);
        g.drawCircle(x + 26, y - 1, 2);
        g.drawCircle(x - 26, y - 5, 2);
        g.drawCircle(x + 26, y - 5, 2);
        g.endFill();
        const glow = new PIXI.Graphics();
        glow.beginFill(0xfacc15, 0.1);
        glow.drawRect(x - 30, y - 10, 60, 14);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
    ...
    drawStarlinkDish(c, x, y) {
        ...
        g.drawEllipse(x, y - 22, 12, 4);
        g.endFill();
        // Antenna pip
        g.beginFill(0x22d3ee);
        g.drawCircle(x, y - 28, 2);
        g.endFill();
        // Signal glow
        const glow = new PIXI.Graphics();
        glow.beginFill(0x22d3ee, 0.3);
        glow.drawCircle(x, y - 28, 4);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        // Blinking LEDs on base
        g.beginFill(0x4ade80);
        g.drawCircle(x - 4, y - 16, 1);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawCircle(x + 4, y - 16, 1);
        g.endFill();
    ...
    drawZenGardenProp(c, x, y) {
        ...
        g.eventMode = 'none';
        // Raked sand base
        g.beginFill(0xd6d3d1, 0.3);
        g.drawEllipse(x, y - 3, 35, 8);
        g.endFill();
        // Rake lines
        ...
        g.drawEllipse(x - 12, y - 4, 6, 4);
        g.endFill();
        g.beginFill(0x57534e);
        g.drawEllipse(x + 14, y - 3, 5, 3);
        g.endFill();
        g.beginFill(0x78716c);
        g.drawCircle(x + 4, y - 5, 3);
        g.endFill();
        // Bonsai tree
        g.beginFill(0x4a2e1a);
        g.drawRect(x + 20, y - 16, 3, 12);
        g.endFill();
        g.beginFill(0x166534);
        g.drawEllipse(x + 21, y - 20, 8, 6);
        g.endFill();
        g.beginFill(0x15803d);
        g.drawEllipse(x + 18, y - 16, 5, 4);
        g.drawEllipse(x + 25, y - 17, 5, 3);
        g.endFill();
        // Ceramic pot
        g.beginFill(0x78350f);
        g.drawRect(x + 17, y - 6, 8, 6);
        g.endFill();
        g.beginFill(0x92400e);
        g.drawRect(x + 16, y - 6, 10, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawLuxuryRedwood(c, x, y) {
        ...
        // Massive trunk
        g.beginFill(0x4a2e1a);
        g.drawRect(x - 5, y - 50, 10, 50);
        g.endFill();
        g.beginFill(0x3d2514);
        g.drawRect(x - 4, y - 50, 3, 50);
        g.endFill();
        // Deep canopy layers
        g.beginFill(0x064e3b);
        g.drawEllipse(x, y - 55, 22, 14);
        g.endFill();
        g.beginFill(0x065f46);
        g.drawEllipse(x - 5, y - 60, 16, 10);
        g.drawEllipse(x + 8, y - 58, 14, 9);
        g.endFill();
        g.beginFill(0x047857);
        g.drawEllipse(x, y - 65, 12, 8);
        g.endFill();
    ...
    drawRefreshmentStand(c, x, y) {
        ...
        g.drawRect(x - 10, y - 14, 4, 6);
        g.endFill();
        g.beginFill(0x22d3ee);
        g.drawRect(x - 2, y - 14, 4, 6);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawRect(x + 6, y - 14, 4, 6);
        g.endFill();
        // Wheels
    ...
    drawCoffeeMachine(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x - 12, y - 30, 24, 30);
        g.endFill();
        g.beginFill(0x334155);
        g.drawRect(x - 14, y - 32, 28, 4);
        g.endFill();
        // Steam vent
        g.beginFill(0x94a3b8, 0.3);
        g.drawRect(x - 2, y - 38, 4, 6);
        g.endFill();
        // Buttons
        g.beginFill(0x4ade80);
        g.drawCircle(x - 5, y - 20, 2);
        g.endFill();
        g.beginFill(0xef4444);
        g.drawCircle(x + 5, y - 20, 2);
        g.endFill();
        // Cup slot
        g.beginFill(0x0f172a);
        g.drawRect(x - 6, y - 12, 12, 10);
        g.endFill();
        // Cup
        g.beginFill(0xf8fafc);
        g.drawRect(x - 4, y - 10, 8, 8);
        g.endFill();
        g.beginFill(0x78350f);
        g.drawRect(x - 3, y - 9, 6, 5);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawMenuBoard(c, x, y) {
        ...
        g.eventMode = 'none';
        // Board
        g.beginFill(0x1c1917);
        g.drawRect(x - 25, y - 45, 50, 35);
        g.endFill();
        g.beginFill(0x78350f);
        g.lineStyle(2, 0x78350f);
        g.drawRect(x - 27, y - 47, 54, 39);
        g.endFill();
        g.lineStyle(0);
    ...
    drawScoreboard(c, x, y) {
        ...
        // Screen frame
        g.beginFill(0x0f172a);
        g.drawRect(x - 40, y - 40, 80, 30);
        g.endFill();
        g.beginFill(0x1e293b);
        g.drawRect(x - 38, y - 38, 76, 26);
        g.endFill();
        // Score bars
        g.beginFill(0xef4444, 0.6);
        g.drawRect(x - 34, y - 34, 30, 8);
        g.endFill();
        g.beginFill(0x38bdf8, 0.6);
        g.drawRect(x - 34, y - 22, 22, 8);
        g.endFill();
        // VS text area
        g.beginFill(0xfbbf24, 0.8);
        g.drawRect(x + 5, y - 30, 12, 12);
        g.endFill();
        // Mount
        g.beginFill(0x334155);
        g.drawRect(x - 2, y - 10, 4, 10);
        g.endFill();
        // Glow
        const glow = new PIXI.Graphics();
        glow.eventMode = 'none';
        glow.beginFill(0xef4444, 0.04);
        glow.drawRect(x - 50, y - 45, 100, 40);
        glow.endFill();
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawVendingMachine(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x1e293b);
        g.drawRect(x - 14, y - 40, 28, 40);
        g.endFill();
        g.beginFill(0x334155);
        g.drawRect(x - 12, y - 38, 24, 28);
        g.endFill();
        // Product display
        for (var vy = y - 35; vy < y - 12; vy += 8) {
            for (var vx2 = x - 9; vx2 < x + 9; vx2 += 7) {
                g.beginFill(Math.random() > 0.5 ? 0x22d3ee : 0x4ade80, 0.6);
                g.drawRect(vx2, vy, 5, 5);
                g.endFill();
            }
        }
        // Coin slot
        g.beginFill(0x0f172a);
        g.drawRect(x - 4, y - 8, 8, 4);
        g.endFill();
        // Brand light
        g.beginFill(0x38bdf8, 0.3);
        g.drawRect(x - 10, y - 38, 20, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawMirrorWall(c, x, y, w) {
        ...
        g.eventMode = 'none';
        const mw = w || 80;
        g.beginFill(0x94a3b8, 0.15);
        g.drawRect(x - mw / 2, y - 50, mw, 46);
        g.endFill();
        g.lineStyle(1, 0xffffff, 0.2);
        g.drawRect(x - mw / 2, y - 50, mw, 46);
        g.lineStyle(0);
    ...
    drawBarStool(c, x, y) {
        ...
        g.eventMode = 'none';
        g.beginFill(0x334155);
        g.drawRect(x - 1.5, y - 18, 3, 18);
        g.endFill();
        g.beginFill(0x475569);
        g.drawRect(x - 5, y, 10, 2);
        g.endFill();
        g.beginFill(0x78350f);
        g.drawEllipse(x, y - 18, 7, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawCafeCouch(c, x, y, col) {
        ...
        // Seat
        g.beginFill(cHex, 0.6);
        g.drawRoundedRect(x - 22, y - 12, 44, 12, 3);
        g.endFill();
        // Back
        g.beginFill(cHex, 0.4);
        g.drawRoundedRect(x - 22, y - 24, 44, 14, 3);
        g.endFill();
        // Arms
        g.beginFill(cHex, 0.5);
        g.drawRoundedRect(x - 26, y - 20, 6, 20, 2);
        g.drawRoundedRect(x + 20, y - 20, 6, 20, 2);
        g.endFill();
        // Cushions
        g.beginFill(0xfbbf24, 0.3);
        g.drawCircle(x - 8, y - 14, 4);
        g.drawCircle(x + 8, y - 14, 4);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawCafeBookshelf(c, x, y) {
        ...
        // Shelf frame
        g.beginFill(0x3d2914);
        g.drawRect(x - 18, y - 50, 36, 50);
        g.endFill();
        g.beginFill(0x2a1f0e);
        g.drawRect(x - 16, y - 48, 32, 46);
        g.endFill();
        // Shelves
        g.beginFill(0x4a3520);
        g.drawRect(x - 16, y - 36, 32, 2);
        g.drawRect(x - 16, y - 22, 32, 2);
        g.drawRect(x - 16, y - 8, 32, 2);
        g.endFill();
        // Books (random colors)
    ...
    drawPastryDisplay(c, x, y) {
        ...
        // Glass case
        g.beginFill(0x1e293b);
        g.drawRect(x - 20, y - 18, 40, 18);
        g.endFill();
        g.lineStyle(1, 0xffffff, 0.15);
        g.drawRect(x - 18, y - 30, 36, 14);
        g.lineStyle(0);
        g.beginFill(0x111118, 0.4);
        g.drawRect(x - 18, y - 30, 36, 14);
        g.endFill();
        // Pastries inside
        g.beginFill(0xd97706);
        g.drawEllipse(x - 8, y - 22, 5, 3);
        g.endFill();
        g.beginFill(0xca8a04);
        g.drawEllipse(x + 4, y - 23, 4, 3);
        g.endFill();
        g.beginFill(0xfbbf24);
        g.drawCircle(x + 12, y - 22, 3);
        g.endFill();
        g.beginFill(0x92400e);
        g.drawEllipse(x - 2, y - 20, 6, 2);
        g.endFill();
        // Top counter
        g.beginFill(0xf1f5f9);
        g.drawRect(x - 22, y - 18, 44, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawKitchenOven(c, x, y) {
        ...
        // Oven body
        g.beginFill(0x475569);
        g.drawRect(x - 16, y - 35, 32, 35);
        g.endFill();
        // Oven door
        g.beginFill(0x334155);
        g.drawRect(x - 12, y - 28, 24, 16);
        g.endFill();
        g.beginFill(0x1e293b);
        g.drawRect(x - 10, y - 26, 20, 12);
        g.endFill();
        // Window
        g.beginFill(0xf97316, 0.2);
        g.drawRect(x - 8, y - 24, 16, 8);
        g.endFill();
        // Handle
        g.beginFill(0xcbd5e1);
        g.drawRect(x - 10, y - 12, 20, 2);
        g.endFill();
        // Burner dials
        g.beginFill(0x111118);
        g.drawCircle(x - 8, y - 32, 2);
        g.drawCircle(x, y - 32, 2);
        g.drawCircle(x + 8, y - 32, 2);
        g.endFill();
        // Warm glow
        const glow = new PIXI.Graphics();
        glow.beginFill(0xf97316, 0.15);
        glow.drawRect(x - 8, y - 24, 16, 8);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawPrepStation(c, x, y) {
        ...
        // Cutting board
        g.beginFill(0x78350f);
        g.drawRect(x - 12, y - 22, 16, 2);
        g.endFill();
        // Knife
        g.beginFill(0xcbd5e1);
        g.drawRect(x + 6, y - 24, 1, 6);
        g.endFill();
        g.beginFill(0x4a2e1a);
        g.drawRect(x + 5, y - 19, 3, 4);
        g.endFill();
        // Ingredients
        g.beginFill(0xef4444);
        g.drawCircle(x - 8, y - 22, 2);
        g.endFill();
        g.beginFill(0x22c55e);
        g.drawCircle(x - 3, y - 23, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawOutdoorTable(c, x, y) {
        ...
        // Umbrella pole
        g.beginFill(0x888888);
        g.drawRect(x - 1, y - 45, 2, 30);
        g.endFill();
        // Umbrella canopy
        g.beginFill(0xef4444, 0.7);
        g.drawPolygon([x, y - 48, x - 24, y - 38, x + 24, y - 38]);
        g.endFill();
        g.beginFill(0xffffff, 0.15);
        g.drawPolygon([x, y - 48, x - 8, y - 38, x + 8, y - 38]);
        g.endFill();
        // Table
        g.beginFill(0xf1f5f9);
        g.drawEllipse(x, y - 15, 18, 4);
        g.endFill();
        g.beginFill(0x888888);
        g.drawRect(x - 1, y - 15, 2, 15);
        g.endFill();
        // Chairs
        g.beginFill(0x475569);
        g.drawRect(x - 16, y - 8, 6, 8);
        g.drawRect(x + 10, y - 8, 6, 8);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawYogaMat(c, x, y) {
        ...
        // Mat
        g.beginFill(col, 0.5);
        g.drawRoundedRect(x - 20, y - 3, 40, 3, 1);
        g.endFill();
        g.beginFill(col, 0.3);
        g.drawEllipse(x, y - 2, 20, 2);
        g.endFill();
        // Rolled edge
        g.beginFill(col, 0.7);
        g.drawEllipse(x + 20, y - 3, 2, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawExerciseBall(c, x, y) {
        ...
        g.endFill();
        g.beginFill(0xffffff, 0.15);
        g.drawEllipse(x - 3, y - 14, 4, 3);
        g.endFill();
        // Shadow
        g.beginFill(0x000000, 0.15);
        g.drawEllipse(x, y, 10, 3);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawPoolLane(c, x, y) {
        ...
        // Pool edge
        g.beginFill(0x1e293b);
        g.drawRect(x - 50, y - 4, 100, 4);
        g.endFill();
        g.beginFill(0xe2e8f0);
        g.drawRect(x - 52, y - 5, 104, 2);
        g.endFill();
        // Water
        g.beginFill(0x0284c7, 0.5);
        g.drawRect(x - 48, y - 3, 96, 3);
        g.endFill();
        // Lane dividers
        g.beginFill(0xfbbf24, 0.4);
        g.drawRect(x - 48, y - 3, 96, 1);
        g.endFill();
        // Water shimmer
        const glow = new PIXI.Graphics();
        glow.beginFill(0x38bdf8, 0.2);
        glow.drawRect(x - 48, y - 15, 96, 12);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawSteamRoom(c, x, y) {
        ...
        // Room box
        g.beginFill(0x3d2914);
        g.drawRect(x - 25, y - 35, 50, 35);
        g.endFill();
        g.beginFill(0x2a1f0e);
        g.drawRect(x - 22, y - 32, 44, 30);
        g.endFill();
        // Wooden bench
        g.beginFill(0x78350f);
        g.drawRect(x - 18, y - 12, 36, 4);
        g.endFill();
        g.beginFill(0x92400e);
        g.drawRect(x - 16, y - 8, 4, 8);
        g.drawRect(x + 12, y - 8, 4, 8);
        g.endFill();
        // Door
        g.beginFill(0x4a3520);
        g.drawRect(x - 4, y - 28, 8, 26);
        g.endFill();
        g.beginFill(0xcbd5e1);
        g.drawCircle(x + 2, y - 14, 1.5);
        g.endFill();
        // Steam wisps
    ...
    drawLockerRow(c, x, y) {
        ...
        for (let i = 0; i < 5; i++) {
            const lx = x + i * 12;
            g.beginFill(0x334155);
            g.drawRect(lx, y - 35, 10, 35);
            g.endFill();
            g.beginFill(0x475569);
            g.drawRect(lx + 1, y - 33, 8, 31);
            g.endFill();
            // Handle
            g.beginFill(0xcbd5e1);
            g.drawRect(lx + 6, y - 18, 2, 4);
            g.endFill();
            // Vent
    ...
    drawCommentaryDesk(c, x, y) {
        ...
        // Long desk
        g.beginFill(0x1e293b);
        g.drawRect(x - 40, y - 18, 80, 18);
        g.endFill();
        g.beginFill(0x334155);
        g.drawRect(x - 42, y - 18, 84, 4);
        g.endFill();
        // Monitors (3)
        for (let i = -1; i <= 1; i++) {
            g.beginFill(0x0f172a);
            g.drawRect(x + i * 26 - 10, y - 35, 20, 14);
            g.endFill();
            const glow = new PIXI.Graphics();
            glow.beginFill(0x38bdf8, 0.4);
            glow.drawRect(x + i * 26 - 8, y - 33, 16, 10);
            glow.endFill();
            glow.blendMode = PIXI.BLEND_MODES.ADD;
            c.addChild(glow);
            if (!this.indoorLights) this.indoorLights = [];
        ...
        // Microphones
        g.beginFill(0x555555);
        g.drawRect(x - 20, y - 24, 1, 6);
        g.drawRect(x + 20, y - 24, 1, 6);
        g.endFill();
        g.beginFill(0x222222);
        g.drawCircle(x - 20, y - 25, 2);
        g.drawCircle(x + 20, y - 25, 2);
        g.endFill();
        // Chairs
        g.beginFill(0x1e293b);
        g.drawRect(x - 18, y - 8, 8, 8);
        g.drawRect(x + 10, y - 8, 8, 8);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawTrophyCase(c, x, y) {
        ...
        // Glass case
        g.beginFill(0x1e293b);
        g.drawRect(x - 18, y - 45, 36, 45);
        g.endFill();
        g.lineStyle(1, 0xffffff, 0.1);
        g.drawRect(x - 16, y - 42, 32, 38);
        g.lineStyle(0);
        g.beginFill(0x111118, 0.3);
        g.drawRect(x - 16, y - 42, 32, 38);
        g.endFill();
        // Shelves
        g.beginFill(0x334155);
        g.drawRect(x - 16, y - 28, 32, 2);
        g.drawRect(x - 16, y - 14, 32, 2);
        g.endFill();
        // Trophies
        ...
        for (let s = 0; s < 3; s++) {
            ...
            g.drawCircle(x - 5, sy + 6, 2);
            g.drawCircle(x + 5, sy + 6, 2);
            g.endFill();
        }
        // Warm glow
        const glow = new PIXI.Graphics();
        glow.beginFill(0xfbbf24, 0.1);
        glow.drawRect(x - 16, y - 42, 32, 38);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawAudienceStands(c, x, y, w) {
        ...
        for (let row = 0; row < 3; row++) {
            ...
            const rw = hw - row * 12;
            g.beginFill(0x1e293b);
            g.drawRect(x - rw / 2, ry - 6, rw, 6);
            g.endFill();
            g.beginFill(0x334155);
            g.drawRect(x - rw / 2, ry - 8, rw, 3);
            g.endFill();
        }
        c.addChild(g);
    },
    ...
    drawJumbotron(c, x, y) {
        ...
        // Frame
        g.beginFill(0x0f172a);
        g.drawRect(x - 35, y - 45, 70, 35);
        g.endFill();
        g.beginFill(0x1e293b);
        g.drawRect(x - 37, y - 47, 74, 2);
        g.drawRect(x - 37, y - 10, 74, 2);
        g.endFill();
        // Screen content (ELO display)
        const glow = new PIXI.Graphics();
        glow.beginFill(0xef4444, 0.25);
        glow.drawRect(x - 30, y - 42, 28, 28);
        glow.endFill();
        glow.beginFill(0x3b82f6, 0.25);
        glow.drawRect(x + 2, y - 42, 28, 28);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        // VS text
        const vs = new PIXI.Text('VS', {
            fontFamily: 'JetBrains Mono',
            fontSize: 8,
            fill: 0xffffff,
            fontWeight: 'bold',
    ...
    drawLeaderboard(c, x, y) {
        ...
        // Board
        g.beginFill(0x0f172a);
        g.drawRect(x - 22, y - 50, 44, 46);
        g.endFill();
        g.beginFill(0x1e293b);
        g.drawRect(x - 24, y - 52, 48, 2);
        g.endFill();
        // Rows
        const rowCols = [0xfbbf24, 0xcbd5e1, 0xcd7f32, 0x64748b, 0x64748b];
        for (let i = 0; i < 5; i++) {
            g.beginFill(rowCols[i], 0.4);
            g.drawRect(x - 18, y - 46 + i * 8, 36, 6);
            g.endFill();
        }
        // Glow
        const glow = new PIXI.Graphics();
        glow.beginFill(0x22d3ee, 0.1);
        glow.drawRect(x - 22, y - 50, 44, 46);
        glow.endFill();
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
        if (!this.indoorLights) this.indoorLights = [];
    ...
    drawContributorWall(c, x, y) {
        ...
        // Title
        const title = new PIXI.Text('CONTRIBUTORS', {
            fontFamily: 'JetBrains Mono',
            fontSize: 6,
            fill: 0x8b949e,
        });
    ...
    drawHackathonDesk(c, x, y) {
        ...
        // Long shared desk
        g.beginFill(0x1e293b);
        g.drawRect(x - 30, y - 18, 60, 18);
        g.endFill();
        g.beginFill(0x334155);
        g.drawRect(x - 32, y - 18, 64, 3);
        g.endFill();
        // Multiple laptops
        for (let i = -1; i <= 1; i++) {
            const lx = x + i * 20;
            // Laptop base
            g.beginFill(0x475569);
            g.drawRect(lx - 7, y - 20, 14, 2);
            g.endFill();
            // Laptop screen
            g.beginFill(0x0f172a);
            g.drawRect(lx - 6, y - 30, 12, 10);
            g.endFill();
            const glow = new PIXI.Graphics();
            const sc = [0x22c55e, 0x06b6d4, 0xa855f7][i + 1];
            glow.beginFill(sc, 0.4);
            glow.drawRect(lx - 5, y - 29, 10, 8);
            glow.endFill();
            glow.blendMode = PIXI.BLEND_MODES.ADD;
            c.addChild(glow);
            if (!this.indoorLights) this.indoorLights = [];
        ...
        // Energy drinks
        g.beginFill(0xef4444);
        g.drawRect(x + 22, y - 24, 4, 6);
        g.endFill();
        g.beginFill(0x22c55e);
        g.drawRect(x - 26, y - 23, 4, 5);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawWhiteboard(c, x, y) {
        ...
        // Board
        g.beginFill(0xf1f5f9);
        g.drawRect(x - 30, y - 45, 60, 35);
        g.endFill();
        g.beginFill(0x94a3b8);
        g.drawRect(x - 32, y - 47, 64, 3);
        g.drawRect(x - 32, y - 10, 64, 3);
        g.endFill();
        ...
        g.drawCircle(x - 10, y - 16, 3);
        g.moveTo(x - 7, y - 16);
        g.lineTo(x + 5, y - 16);
        g.drawCircle(x + 8, y - 16, 3);
        g.lineStyle(0);
        // Markers in tray
        g.beginFill(0xef4444);
        g.drawRect(x - 25, y - 9, 8, 2);
        g.endFill();
        g.beginFill(0x3b82f6);
        g.drawRect(x - 15, y - 9, 8, 2);
        g.endFill();
        g.beginFill(0x22c55e);
        g.drawRect(x - 5, y - 9, 8, 2);
        g.endFill();
        c.addChild(g);
    },
    ...
    drawOpenServerRack(c, x, y) {
        ...
        g.beginFill(0x222233);
        g.drawRect(x - 14, y - 45, 28, 45);
        g.endFill();
        // Rails
        g.beginFill(0x475569);
        g.drawRect(x - 14, y - 45, 2, 45);
        g.drawRect(x + 12, y - 45, 2, 45);
        g.endFill();
        // Server units
        for (let i = 0; i < 5; i++) {
            const sy = y - 42 + i * 8;
            g.beginFill(0x1a1a2e);
            g.drawRect(x - 12, sy, 24, 6);
            g.endFill();
            // Status LEDs
            g.beginFill(0x4ade80);
            g.drawCircle(x - 8, sy + 3, 1);
            g.endFill();
            g.beginFill(Math.random() > 0.5 ? 0x4ade80 : 0xfbbf24);
            g.drawCircle(x - 4, sy + 3, 1);
            g.endFill();
        }
        // Activity glow
        const glow = new PIXI.Graphics();
        for (let i = 0; i < 5; i++) {
            glow.beginFill(0x4ade80, 0.15);
            glow.drawCircle(x - 8, y - 39 + i * 8, 2);
            glow.endFill();
        }
        glow.blendMode = PIXI.BLEND_MODES.ADD;
        c.addChild(g, glow);
    ...
    drawGardenPlanter(c, x, y) {
        ...
        g.endFill();
        g.beginFill(0x92400e);
        g.drawRect(x - 17, y - 10, 34, 3);
        g.endFill();
        // Soil
        g.beginFill(0x3d2914);
        g.drawRect(x - 13, y - 12, 26, 4);
        g.endFill();
        // Plants
        g.beginFill(0x166534);
        g.drawCircle(x - 6, y - 18, 5);
        g.drawCircle(x + 4, y - 16, 4);
        g.endFill();
        g.beginFill(0x22c55e);
        g.drawCircle(x - 4, y - 20, 3);
        g.drawCircle(x + 6, y - 18, 3);
        g.endFill();
        // Flowers
        g.beginFill(0xfbbf24);
        g.drawCircle(x - 8, y - 22, 2);
    ...
    drawHammock(c, x, y) {
        ...
        // Poles
        g.beginFill(0x4a2e1a);
        g.drawRect(x - 25, y - 30, 3, 30);
        g.drawRect(x + 22, y - 30, 3, 30);
        g.endFill();
