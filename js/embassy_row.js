...
const EmbassyRow = {
    BLDS: [
        ...
        },
        {
            ...
            milestone:
                'Jul 2, 2026: the UK and Germany agreed to deepen AI safety cooperation — AISI and German ministries expanding joint frontier-model evaluation and cyber-risk research.',
        ...
        },
        {
            ...
            milestone:
                'Feb 16–21, 2026: India hosted the AI Impact Summit — 88 countries adopted the non-binding New Delhi Declaration on AI Impact, with $200B+ in AI investment commitments announced.',
        ...
        },
        {
            ...
            milestone:
                "2026: Abu Dhabi's MGX is a Stargate co-owner and a check in OpenAI's $122B round and xAI's Series E — the Gulf as a top-tier AI investor.",
    ...
    drawCountryFlag(g, country, w, h) {
        ...
        if (c === 'us') {
            ...
            g.beginFill(0x012169);
            g.drawRect(0, 0, w, h);
            g.endFill();
            // White saltire (full diagonals)
            g.lineStyle(Math.max(1.6, h * 0.2), 0xffffff, 1);
            g.moveTo(0, 0);
            g.lineTo(w, h);
            g.moveTo(w, 0);
            g.lineTo(0, h);
            // Red saltire on top (thinner)
            g.lineStyle(Math.max(0.8, h * 0.1), 0xc8102e, 1);
            g.moveTo(0, 0);
            g.lineTo(w, h);
            g.moveTo(w, 0);
            g.lineTo(0, h);
            ...
            g.endFill();
            g.beginFill(0xffffff);
            g.drawRect(0, sH, w, sH);
            g.endFill();
            g.beginFill(0x138808);
            g.drawRect(0, sH * 2, w, sH);
            g.endFill();
            ...
            g.drawCircle(ckCx, ckCy, Math.max(0.6, ckR * 0.12));
            g.endFill();
        } else if (c === 'ae') {
            // Vertical red hoist (1/4 width) + 3 horizontal bands (green/white/black)
            const redW = w * 0.25;
            g.beginFill(0xce1126);
            g.drawRect(0, 0, redW, h);
            g.endFill();
            const fW = w - redW;
            const sH = h / 3;
            g.beginFill(0x00732f);
            g.drawRect(redW, 0, fW, sH);
            g.endFill();
            g.beginFill(0xffffff);
            g.drawRect(redW, sH, fW, sH);
            g.endFill();
            g.beginFill(0x000000);
            g.drawRect(redW, sH * 2, fW, sH);
            g.endFill();
        } else {
