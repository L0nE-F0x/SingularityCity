...
const Kardashev = {
    ...
    // ─── PILLAR WEIGHTS ───
    W_ENERGY: 0.3, // True to Kardashev's original energy thesis
    W_COMPUTE: 0.25, // The engine
    W_INTEL: 0.25, // The output
    W_RESEARCH: 0.1, // The pipeline
    W_INFRA: 0.1, // The supply chain
    ...
    // ─── MILESTONES ───
    MILESTONES: [
        { k: 0.72, label: 'Industrial Age', icon: '🏭', color: '#94a3b8' },
        { k: 0.75, label: 'Digital Age', icon: '💻', color: '#60a5fa' },
        { k: 0.78, label: 'AI Emergence', icon: '🧠', color: '#a78bfa' },
        { k: 0.82, label: 'AI Convergence', icon: '🔗', color: '#22d3ee' },
        { k: 0.87, label: 'Pre-Singularity', icon: '⚡', color: '#facc15' },
        { k: 0.92, label: 'Singularity Horizon', icon: '🌅', color: '#f97316' },
        { k: 0.96, label: 'Approaching Type I', icon: '🌟', color: '#fbbf24' },
        { k: 1.0, label: 'Type I Civilisation', icon: '🌍', color: '#4ade80' },
    ],
    ...
    calculate() {
        ...
        const launches =
            typeof SpaceData !== 'undefined' && SpaceData.launches ? SpaceData.launches.length : 0;
        ...
        this.score = 0.7 + (weighted / 100) * 0.3;
        this.score = Math.min(1.0, Math.round(this.score * 1000) / 1000);
        ...
        this.pillars = {
            energy: {
                ...
                color: '#fbbf24',
                details: grid
                    ? `${(grid.totalMW / 1000).toFixed(1)} GW mapped · ${grid.renewPct.toFixed(0)}% renewable`
                    : `${typeof PowerZone !== 'undefined' ? PowerZone.getTotalSupply() : 0} MW sim`,
            },
            compute: {
                ...
                emoji: '🖥️',
                color: '#22d3ee',
                details: `${totalGPU.toLocaleString()} GPUs · ${(totalDCPower / 1000).toFixed(1)} GW DC power`,
            },
            intel: {
                score: Math.round(intelScore),
                label: 'Intelligence',
                emoji: '🧠',
                color: '#a78bfa',
                details: `${activeModels.length} models · avg ${avgBench.toFixed(0)}% · top ELO ${topElo}`,
            },
            research: {
                ...
                emoji: '📡',
                color: '#4ade80',
                details: `${papers} papers · ${events} events · ${launches} launches`,
            },
            infra: {
                ...
                emoji: '🔧',
                color: '#f97316',
                details: `$${fundingB.toFixed(1)}B VC · ${scHealth.toFixed(0)}% supply health`,
            },
        };
    ...
    buildHUD() {
        ...
        if (window.isMobile) {
            ...
            el.style.cssText =
                'position:fixed;top:4px;left:260px;z-index:800;cursor:pointer;user-select:none;transition:opacity 0.3s ease;';
        }
    ...
    updateHUD() {
        ...
        const totalLen = 88;
        const fill = ((this.score - 0.7) / 0.3) * totalLen; // 0.700→0, 1.000→88
    ...
    showPanel() {
        ...
        nextMS = null;
        for (let i = 0; i < this.MILESTONES.length; i++) {
            if (this.MILESTONES[i].k > s) {
                nextMS = this.MILESTONES[i];
                break;
            }
        }

        let h =
            '<button class="ipanel-x" onclick="document.getElementById(\'kardashevOv\').classList.remove(\'open\')">✕</button>';

        // ─── Giant score display ───
        h += '<div style="text-align:center;padding:16px 0 8px">';
        h +=
            '<div style="font-size:8px;color:var(--t3);letter-spacing:3px;margin-bottom:4px">KARDASHEV SCALE</div>';
        h +=
            "<div style=\"font-size:36px;font-weight:bold;color:#fff;font-family:'JetBrains Mono',monospace;letter-spacing:2px;text-shadow:0 0 20px " +
            currentMS.color +
            '40">K ' +
            s.toFixed(3) +
            '</div>';
        ...
        h +=
            '<div style="width:' +
            pct +
            '%;height:100%;background:linear-gradient(90deg,#f59e0b,#22d3ee,#4ade80);border-radius:4px;transition:width 1s ease;box-shadow:0 0 8px ' +
            currentMS.color +
            '60"></div>';
        // Milestone markers
        this.MILESTONES.forEach((ms) => {
            const msPct = ((ms.k - 0.7) / 0.3) * 100;
            if (msPct > 0 && msPct < 100) {
                const reached = s >= ms.k;
                h +=
                    '<div style="position:absolute;top:-2px;left:' +
                    msPct +
                    '%;transform:translateX(-50%);width:3px;height:14px;background:' +
                    (reached ? ms.color : 'rgba(255,255,255,0.15)') +
                    ';border-radius:2px" title="K ' +
                    ms.k.toFixed(3) +
                    ' — ' +
                    ms.label +
                    '"></div>';
        ...
        // Labels at ends
        h +=
            '<div style="display:flex;justify-content:space-between;font-size:6px;color:var(--t3);margin-top:2px"><span>K 0.700</span><span>K 1.000 — Type I</span></div>';
        h += '</div>';
        ...
        if (nextMS) {
            ...
            h +=
                ...
                '0a;border:1px solid ' +
                nextMS.color +
                '33;border-radius:6px;padding:8px;margin-bottom:12px;display:flex;align-items:center;gap:8px">';
            h += '<span style="font-size:18px">' + nextMS.icon + '</span>';
            h +=
                '<div><div style="font-size:9px;font-weight:bold;color:#fff">Next: ' +
                nextMS.label +
                ' (K ' +
                nextMS.k.toFixed(3) +
                ')</div>';
        ...
        // ─── Five pillar bars ───
        h +=
            '<div style="font-size:8px;color:var(--t3);letter-spacing:2px;margin-bottom:6px">PILLAR BREAKDOWN</div>';
        ...
        pKeys.forEach((key, i) => {
            ...
            h +=
                '<div style="margin-bottom:8px;background:var(--cd);border:1px solid var(--bd);border-radius:6px;padding:8px">';
            h +=
                '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">';
            h +=
                '<div style="display:flex;align-items:center;gap:6px"><span style="font-size:12px">' +
                p.emoji +
                '</span><span style="font-size:9px;font-weight:bold;color:#fff">' +
                p.label +
                '</span><span style="font-size:7px;color:var(--t3)">(' +
                w +
                '% weight)</span></div>';
            h +=
            ...
            // Bar
            h +=
                '<div style="background:rgba(255,255,255,0.06);border-radius:3px;height:6px;overflow:hidden;margin-bottom:3px">';
            h +=
                '<div style="width:' +
                p.score +
                '%;height:100%;background:' +
                p.color +
                ';border-radius:3px;transition:width 0.5s ease"></div></div>';
            // Detail
        ...
        h +=
            '<div style="font-size:8px;color:var(--t3);letter-spacing:2px;margin:8px 0 6px">CONTRIBUTION MIX</div>';
        ...
        pKeys.forEach((key, i) => {
            ...
            h +=
                ...
                '%;background:' +
                p.color +
                ';display:flex;align-items:center;justify-content:center;min-width:10px" title="' +
                p.label +
                ' ' +
                contrib.toFixed(1) +
                '%">';
            h +=
                '<span style="font-size:5px;color:#000;font-weight:bold">' +
                (contrib >= 8 ? p.emoji : '') +
                '</span></div>';
        });
        h += '</div>';
        ...
        pKeys.forEach((key, i) => {
            ...
            if (!p) return;
            const contrib =
                totalWeighted > 0 ? (((p.score * weights[i]) / totalWeighted) * 100).toFixed(0) : 0;
            h +=
                '<span style="font-size:6px;color:var(--t3)"><span style="display:inline-block;width:6px;height:6px;border-radius:2px;background:' +
                p.color +
        ...
        // ─── Milestone timeline ───
        h +=
            '<div style="font-size:8px;color:var(--t3);letter-spacing:2px;margin:8px 0 6px">CIVILISATION MILESTONES</div>';
        this.MILESTONES.forEach((ms) => {
            const reached = s >= ms.k;
            h +=
                '<div style="display:flex;align-items:center;gap:8px;padding:3px 0;opacity:' +
                (reached ? '1' : '0.4') +
                '">';
            h += '<span style="font-size:12px">' + ms.icon + '</span>';
            h +=
                '<div style="flex:1"><span style="font-size:8px;font-weight:bold;color:' +
                (reached ? ms.color : 'var(--t3)') +
                '">' +
                ms.label +
                '</span></div>';
            h +=
                '<span style="font-size:7px;color:var(--t3);font-family:\'JetBrains Mono\',monospace">K ' +
                ms.k.toFixed(3) +
                '</span>';
        ...
        // Attribution
        h +=
            '<div style="text-align:center;font-size:6px;color:var(--t3);margin-top:12px;padding-top:8px;border-top:1px dashed var(--bd)">Score updates live from all city data streams · <a href="https://en.wikipedia.org/wiki/Kardashev_scale" target="_blank" style="color:#22d3ee;text-decoration:none">What is the Kardashev Scale?</a></div>';
