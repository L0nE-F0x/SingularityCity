...
const Holomap = {
    ...
    init() {
        ...
        this.scene.fog = new THREE.FogExp2('#04040e', 0.0001);
    ...
    show() {
        ...
        if (typeof SND !== 'undefined') {
            ...
            setTimeout(function () {
                SND.playTone(400, 'triangle', 0.4, 0.04, 1200);
            }, 150);
            setTimeout(function () {
                SND.setAmbient('holomap');
            }, 300);
        }
    ...
    hide() {
        ...
        if (typeof SND !== 'undefined') {
            ...
            setTimeout(function () {
                SND.playTone(600, 'triangle', 0.3, 0.03, 100);
            }, 100);
            setTimeout(function () {
    ...
    buildUniverse() {
        ...
        for (var i = 0; i < 3000; i++) {
            ...
            if (temp < 0.6) {
                bgCol1.push(0.6, 0.65, 1.0);
            } // cool blue-white
            else if (temp < 0.85) {
                bgCol1.push(1.0, 0.9, 0.7);
            } // warm gold
            else {
                bgCol1.push(1.0, 0.7, 0.85);
            } // soft pink
        }
        ...
        for (var j = 0; j < 500; j++) {
            ...
            var t2 = Math.random();
            if (t2 < 0.4) {
                bgCol2.push(0.8, 0.85, 1.0);
            } // blue-white
            else if (t2 < 0.7) {
                bgCol2.push(1.0, 1.0, 0.9);
            } // bright white
            else if (t2 < 0.9) {
                bgCol2.push(1.0, 0.8, 0.5);
            } // amber
            else {
                bgCol2.push(0.9, 0.6, 1.0);
            } // lavender
        }
        ...
        var dustPlane = new THREE.Sprite(
            new THREE.SpriteMaterial({
                map: glowTex,
                color: '#6644aa',
                transparent: true,
        ...
        Object.keys(labModelCounts).forEach(function (k) {
            if (labModelCounts[k] > maxModels) maxModels = labModelCounts[k];
        });
        Object.keys(labModelCounts).forEach(function (k) {
        ...
        var labKeys = Object.keys(labModelCounts).sort(function (a, b) {
            return labScores[b] - labScores[a];
        });
        ...
        this.nebulae.forEach(function (n) {
            ...
            for (var v = 0; v < cloudCount; v++) {
                ...
                cloud.position.set(
                    (Math.random() - 0.5) * cloudSpread * 0.6,
                    (Math.random() - 0.5) * cloudSpread * 0.6,
                    (Math.random() - 0.5) * cloudSpread * 0.6
                );
                n.group.add(cloud);
    ...
    buildStars() {
        ...
        this.meshToStarMap.clear();
        this.stars = [];
        document.querySelectorAll('.holo-star-label').forEach(function (el) {
            el.remove();
        });
        ...
        var nebKeySet = {};
        this.nebulae.forEach(function (n) {
            nebKeySet[n.key] = true;
        });
        var visibleModels = allModels.filter(function (m) {
            return m.lab && nebKeySet[m.lab];
        });

        // Find frontier per lab
        var labFrontiers = {};
        this.nebulae.forEach(function (n) {
            var labModels = visibleModels.filter(function (m) {
                return m.lab === n.key;
            });
            var frontier = null,
                maxScore = -1;
        ...
        visibleModels.forEach(function (m, mi) {
            var neb = self.nebulae.find(function (n) {
                return n.key === m.lab;
            });
            if (!neb) return;
            ...
            // Glow aura
            if (!retired) {
                var aura = new THREE.Sprite(
                    new THREE.SpriteMaterial({
                        map: glowTex,
                        color: cHex,
                        transparent: true,
                        opacity: isFrontier ? 0.6 : 0.3,
                        blending: THREE.AdditiveBlending,
                    })
                );
            ...
            if (!isFrontier && !retired && neb.modelCount < 30) {
                ...
                line = new THREE.Line(
                    lineGeo,
                    new THREE.LineBasicMaterial({ color: cHex, transparent: true, opacity: 0.1 })
                );
            ...
            if (isFrontier) {
                ...
                var lblHtml =
                    '<div class="holo-star-name" style="font-size:13px;font-weight:600;color:' +
                    cHex +
                    ';">' +
                    (m.name || m.id) +
                    '</div>';
    ...
    setupInput() {
        ...
        var updateRaycaster = function (clientX, clientY) {
            ...
            self.stars.forEach(function (s) {
                s.meshGroup.children.forEach(function (c) {
            ...
            if (intersects.length > 0) {
                ...
                if (self.hover && self.tooltip) {
                    ...
                    var stg = self.getStageForModel(m);
                    var ttH =
                        '<div class="htt-name" style="color:' +
                        self.hover.color +
                        '">' +
                        (m.name || m.id) +
                        '</div>';
                    ttH +=
                        '<div class="htt-lab" style="color:' + self.hover.color + '">' + lab.name + '</div>';
                    ttH +=
        ...
        window.addEventListener('mousemove', function (e) {
            if (self.active) updateRaycaster(e.clientX, e.clientY);
        });
        ...
        cv.addEventListener('touchend', function (e) {
            ...
            if (dx < 10 && dy < 10) {
                ...
                if (self.hover) {
                    ...
                    if (
                        self.infoPanel &&
                        self.infoPanel.classList.contains('on') &&
                        typeof SND !== 'undefined'
                    )
                        SND.playTone(600, 'sine', 0.06, 0.03, 300);
                    if (self.infoPanel) self.infoPanel.classList.remove('on');
                    self.selected = null;
                }
            }
    ...
    showStarInfo: function (s) {
        ...
        if (typeof SND !== 'undefined') {
            ...
            setTimeout(function () {
                SND.playTone(750, 'triangle', 0.08, 0.03, 1500);
            }, 60);
        }
        var m = s.m,
        ...
        h +=
            '<div class="holo-panel-top"><div class="holo-panel-name" style="color:' +
            cHex +
            '">' +
            (m.name || m.id) +
            '</div>';
        ...
        if (relDate)
            h +=
                '<div class="holo-stat"><span class="holo-stat-lbl">Released</span><span class="holo-stat-val">' +
                relDate +
                '</span></div>';
        ...
        h += '</div>';
        h +=
            '<div class="holo-panel-actions"><button class="holo-action-btn" onclick="Holomap.zoomToStar(\'' +
            m.id +
            '\')">🔭 Focus</button>';
    ...
    zoomToStar: function (id) {
        var target = this.stars.find(function (s) {
            return s.m.id === id;
        });
        if (target) {
            ...
            if (typeof SND !== 'undefined') {
                ...
                setTimeout(function () {
                    SND.playTone(600, 'sine', 0.2, 0.03, 1200);
                }, 100);
            }
    ...
    loop: function () {
        ...
        if (this.asiGroup) {
            this.asiGroup.children.forEach(function (c, i) {
                if (i > 1) {
                    c.rotation.x = Math.PI / 2 + Math.sin(self.tick * 0.01 + i) * 0.2;
        ...
        this.stars.forEach(function (s) {
            ...
            s.meshGroup.position.set(
                Math.cos(oa) * s.orbitR,
                Math.sin(oa + s.mi) * 15,
                Math.sin(oa) * s.orbitR
            );
        });
        ...
        if (this.hover || this.selected) {
            this.stars.forEach(function (s) {
                ...
                if (s.meshGroup.children[1] && !s.retired) {
                    s.meshGroup.children[1].material.opacity =
                        isSel || isHov ? 1.0 : s.isFrontier ? 0.9 : 0.55;
                    s.meshGroup.children[1].scale.setScalar(
                        s.mag * (isSel ? 25 : isHov ? 20 : s.isFrontier ? 16 : 10)
                    );
                }
            });
        }
        ...
        if (this.asiGroup && this.asiHtmlLabel) {
            this.asiGroup.getWorldPosition(tempV);
            tempV.project(this.camera);
            if (tempV.z < 1) {
                this.asiHtmlLabel.style.display = 'block';
                this.asiHtmlLabel.style.left = (tempV.x * 0.5 + 0.5) * this.W + 'px';
                this.asiHtmlLabel.style.top = (tempV.y * -0.5 + 0.5) * this.H + 'px';
        ...
        this.barrierLabels.forEach(function (b) {
            ...
            if (tempV.z < 1) {
                ...
                b.dom.style.left = (tempV.x * 0.5 + 0.5) * self.W + 'px';
                b.dom.style.top = (tempV.y * -0.5 + 0.5) * self.H + 'px';
        ...
        this.stars.forEach(function (s) {
            ...
            s.meshGroup.getWorldPosition(tempV);
            tempV.project(self.camera);
            if (tempV.z > 1) {
                s.htmlLabel.style.display = 'none';
                return;
            }
            s.htmlLabel.style.display = 'block';
            s.htmlLabel.style.left = (tempV.x * 0.5 + 0.5) * self.W + 'px';
            s.htmlLabel.style.top = (tempV.y * -0.5 + 0.5) * self.H - 25 + 'px';
        });

        // Nebula labels
        this.nebulae.forEach(function (n) {
            n.group.getWorldPosition(tempV);
            tempV.project(self.camera);
            if (tempV.z > 1) {
                n.htmlLabel.style.display = 'none';
                return;
            }
            n.htmlLabel.style.display = 'block';
            n.htmlLabel.style.left = (tempV.x * 0.5 + 0.5) * self.W + 'px';
            n.htmlLabel.style.top = (tempV.y * -0.5 + 0.5) * self.H - 60 + 'px';
        });
        ...
        if (this.tick % 30 === 0) {
            ...
            if (hudInfo) {
                var alive = this.stars.filter(function (s) {
                    return !s.retired;
                }).length;
                ...
                hudInfo.innerHTML =
                    '<span>' +
                    this.stars.length +
                    ' Models</span><span style="color:#4ade80">' +
                    alive +
                    ' Active</span><span>' +
                    this.nebulae.length +
                    ' Nebulae</span><span style="opacity:.5">Zoom ' +
                    (2500 / dist).toFixed(1) +
                    'x</span>';
            }
        }
