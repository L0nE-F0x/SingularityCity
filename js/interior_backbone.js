...
const InteriorBackbone = {
    ...
    build(bld, layer) {
        ...
        const layouts = {
            backbone_ixp: {
                floors: ['Cross-Connect Vault', 'Peering Hall', 'Meet-Me Room', 'NOC Desk'],
                roofLabel: 'INTERNET EXCHANGE',
                col: 0x22d3ee,
                npcs: ['Peering Eng', 'IX Operator'],
            },
            backbone_landing: {
                floors: ['Cable Vault', 'Amplifier Room', 'Monitoring'],
                roofLabel: 'CABLE LANDING',
                col: 0x3b82f6,
                npcs: ['Splice Tech', 'Cable Tech'],
            },
            backbone_ground: {
                floors: ['Antenna Hall', 'Tracking Room', 'Dish Control'],
                roofLabel: 'GROUND STATION',
                col: 0xa855f7,
                npcs: ['Sat Ops', 'RF Engineer'],
            },
            backbone_cdn: {
                floors: ['Cache Floor', 'Edge Compute', 'WAF Control', 'Ops Deck', 'Boardroom'],
                roofLabel: 'CDN / EDGE NODE',
                col: 0xf97316,
                npcs: ['CDN Eng', 'Cache Ops'],
            },
            backbone_noc: {
                floors: [
                    'Server Basement',
                ...
                roofLabel: 'NETWORK OPS CENTER',
                col: 0xef4444,
                npcs: ['NOC Lead', 'SRE', 'Analyst'],
            },
        };
        ...
        for (let f = -1; f < numFloors; f++) {
            ...
            // Floor slab
            rg.beginFill(0x0a1220);
            rg.drawRect(startX, fy + floorH - 6, bldW, 6);
            rg.endFill();
            rg.beginFill(0x1a2a40);
        ...
        this._onMove = (e) => {
            if (!InteriorBackbone.isDragging || !InteriorBackbone.scene || InteriorBackbone.scene.destroyed)
                return;
        ...
        if (fn.includes('war room') || fn.includes('monitoring')) {
            ...
            for (let mx = sx + 30; mx < sx + bw - 50; mx += 55) {
                ...
                for (let ly = 0; ly < 4; ly++) {
                    ...
                    g.drawRect(mx + 4, fy + 16 + ly * 5, 34, 1);
                    g.endFill();
                }
                // Status dot
            ...
            for (let rx = sx + 25; rx < sx + bw - 40; rx += 40) {
                ...
                g.beginFill(patchCol, 0.3);
                g.drawRect(rx + 26, fy + 16, 2, fh - 30);
                g.endFill();
            }
            ...
            for (let ci = 0; ci < 5; ci++) {
                ...
                g.drawRect(sx + 20, cy, bw - 40, 8);
                g.endFill();
                g.beginFill(cableColors[ci], 0.25);
                g.drawRect(sx + 22, cy + 2, bw - 44, 4);
                g.endFill();
            }
            // Splice trays
            for (let tx = sx + 60; tx < sx + bw - 80; tx += 120) {
                g.beginFill(0x1a2540);
                g.drawRect(tx, pY - 20, 80, 20);
                g.endFill();
                g.beginFill(0x222a40);
                g.drawRect(tx, pY - 22, 80, 3);
                g.endFill();
                // Fiber loops
                for (let fi = 0; fi < 6; fi++) {
                    g.beginFill(cableColors[fi % cableColors.length], 0.35);
                    g.drawCircle(tx + 10 + fi * 11, pY - 10, 3);
                    g.endFill();
                }
            }
        } else if (fn.includes('cache') || fn.includes('edge compute') || fn.includes('server')) {
            ...
            for (let dx = sx + 60; dx < sx + bw - 80; dx += 110) {
                // Dish base
                g.beginFill(0x334155);
                g.drawRect(dx + 15, pY - 10, 20, 10);
                g.endFill();
                // Dish arm
                g.beginFill(0x64748b);
                g.drawRect(dx + 22, fy + 20, 6, pY - 30 - fy);
                g.endFill();
                // Dish
                g.beginFill(0xcbd5e1, 0.4);
                g.drawEllipse(dx + 25, fy + 22, 30, 12);
                g.endFill();
                g.beginFill(0xe2e8f0, 0.3);
                g.drawEllipse(dx + 25, fy + 22, 26, 9);
                g.endFill();
                // Feed horn
                g.beginFill(0xf97316, 0.6);
                g.drawCircle(dx + 25, fy + 22, 3);
                g.endFill();
            }
            // Rack equipment
            for (let rx = sx + 30; rx < sx + bw - 50; rx += 80) {
                g.beginFill(0x111825);
                g.drawRect(rx, pY - 28, 24, 28);
                g.endFill();
                g.beginFill(0x4ade80, 0.4);
                g.drawRect(rx + 2, pY - 26, 2, 2);
                g.endFill();
                g.beginFill(0x22d3ee, 0.4);
                g.drawRect(rx + 6, pY - 26, 2, 2);
                g.endFill();
            }
        } else if (
            fn.includes('incident') ||
            fn.includes('executive') ||
            fn.includes('ops deck') ||
            fn.includes('boardroom')
        ) {
            // Conference/ops table
            g.beginFill(0x1e293b);
            g.drawRect(sx + bw / 2 - 80, pY - 16, 160, 16);
            g.endFill();
            g.beginFill(0x334155);
            g.drawRect(sx + bw / 2 - 80, pY - 18, 160, 3);
            g.endFill();
            // Chairs around table
            for (let ci = 0; ci < 6; ci++) {
                const cx = sx + bw / 2 - 70 + ci * 28;
                g.beginFill(0x475569);
                g.drawCircle(cx, pY - 6, 5);
                g.endFill();
            }
            // Wall screen
            g.beginFill(0x0a0a18);
            g.drawRect(sx + bw / 2 - 60, fy + 14, 120, 30);
            g.endFill();
            g.beginFill(col, 0.12);
            g.drawRect(sx + bw / 2 - 58, fy + 16, 116, 26);
            g.endFill();
            ...
            for (let ax = sx + 40; ax < sx + bw - 40; ax += 80) {
                g.beginFill(0x64748b);
                g.drawRect(ax, fy + 10, 4, fh - 20);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawRect(ax - 8, fy + 15, 20, 3);
                g.endFill();
                g.beginFill(0x94a3b8);
                g.drawRect(ax - 6, fy + 25, 16, 3);
                g.endFill();
                // Blinking light on top
                g.beginFill(0xef4444, 0.7);
                g.drawCircle(ax + 2, fy + 8, 2);
                g.endFill();
            }
        } else {
            // Generic ops floor
    ...
    drawNPC(c, x, y, role, col) {
        ...
        cont.on('pointertap', () => {
            if (typeof UI !== 'undefined' && UI.selectModel) {
                UI.selectModel({
                    id: 'bb_' + role.replace(/\s/g, '_').toLowerCase(),
                    name: role,
                    isNPC: true,
                    _trackType: 'npc',
                    role: role,
                    lab: 'backbone',
                    desc: role + '. Backbone Infrastructure.',
                });
            }
        });
        cont.on('pointerover', (e) => {
            if (typeof UI !== 'undefined' && UI.showTooltip)
                UI.showTooltip(e, role, 'Backbone Infrastructure');
        });
        cont.on('pointerout', () => {
                ...
                case 'working': {
                    ...
                    if (av.timer <= 0) {
                        ...
                        if (r < 0.3) {
                            ...
                            this.spawnBubble(
                                av,
                                BACKBONE_MSGS[Math.floor(Math.random() * BACKBONE_MSGS.length)]
                            );
                        } else {
